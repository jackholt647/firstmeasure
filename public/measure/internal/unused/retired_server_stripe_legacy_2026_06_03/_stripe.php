 ...
 *
 * SIGNUP MATCH DEAL (first-purchase bonus)
 * --------------------------------------------
 ...
 *
 * When eligible, stripeCreateCreditPurchaseSession stores the full credited amount in
 * metadata[credit_dollars] so fulfillment applies the right number.
 *
...
/**
 * Set to false to disable the signup credit match deal site-wide.
 * No other code changes needed — all eligibility checks read this constant.
 */
define('STRIPE_SIGNUP_MATCH_50_ENABLED', false);
 ...
 *
 * When $isSignupMatch is true, the configured bonus is applied:
 ...
 * @param bool   $isSignupMatch    Pass true to apply the signup match bonus
 *
 * @return array ['success', 'url', 'session'] or ['success' => false, 'error', ...]
 */
function stripeCreateCreditPurchaseSession($stripeSecretKey, $qty, $userEmail, $baseUrl, $isSignupMatch = false, $customerId = '', $orgId = '', $offerId = '', $bonusOverride = null, $offerTierId = '', $priceId = '') {
    $qty = max(1, min(50000, (int)$qty));

    $bonus       = ($bonusOverride !== null)
        ? max(0, (int)$bonusOverride)
        : ($isSignupMatch ? (int)round($qty * 0.50) : 0);
    $totalCredit = $qty + $bonus;

    // --- Dynamic line-item copy shown on Stripe's checkout page ---
    $matchPercent = ($qty > 0 && $bonus > 0) ? round(($bonus / $qty) * 100, 2) : 0;
    $matchPercentLabel = rtrim(rtrim(number_format($matchPercent, 2, '.', ''), '0'), '.');
    $productName = 'Roof Measurement Credits';
    if ($isSignupMatch && $bonus > 0) {
        $productName = 'Roof Measurement Credits With Bonus Match';
        $productDesc =
            "\${$qty} purchased + \${$bonus} first-purchase bonus ({$matchPercentLabel}% match) "
    ...
    $useInlinePriceData = $isSignupMatch && $bonus > 0;
    if (!$useInlinePriceData && trim((string)$priceId) !== '') {
        $fields['line_items[0][price]'] = trim((string)$priceId);
        $fields['line_items[0][quantity]'] = (string)$qty;
    } else {
        $fields['line_items[0][price_data][currency]'] = 'usd';
        $fields['line_items[0][price_data][unit_amount]'] = '100';
        $fields['line_items[0][price_data][product_data][name]'] = $productName;
        $fields['line_items[0][price_data][product_data][description]'] = $productDesc;
        $fields['line_items[0][quantity]'] = (string)$qty;
    }
    if ($offerId !== '') {
        $fields['metadata[offer_id]'] = (string)$offerId;
    }
    if ($offerTierId !== '') {
        $fields['metadata[offer_tier_id]'] = (string)$offerTierId;
    }
...
function stripeCreateSetupCheckoutSession($stripeSecretKey, $orgId, $customerId, $baseUrl) {
    ...
    $threshold = $o ? (int)($o['billing']['auto_topup']['threshold_dollars'] ?? 50) : 50;
    $topup     = $o ? (int)($o['billing']['auto_topup']['topup_dollars'] ?? 50) : 50;
...
function stripeFulfillFromSessionObject($sessionObj, $source = 'stripe_return') {
    ...
    $offerId   = trim((string)($meta['offer_id'] ?? ''));
    $offerTierId = trim((string)($meta['offer_tier_id'] ?? ''));
    $isSignupMatch = ($meta['is_signup_match'] ?? '0') === '1';
    $paidDollars = (int)($meta['paid_dollars'] ?? 0);
    ...
    $bonusBlockedReason = '';
    if ($isSignupMatch && $offerId === 'bonus_upfront_match_v1' && function_exists('orgOfferEligibilityByEmail')) {
        $eligibility = orgOfferEligibilityByEmail($userEmail, 'bonus_upfront_match_v1');
        $reason = (string)($eligibility['reason'] ?? '');
        if ($reason === 'already_claimed' || $reason === 'historical_claimed' || $reason === 'prior_purchase_exists') {
            $dollars = $paidDollars > 0 ? $paidDollars : max(1, (int)(($sessionObj['amount_total'] ?? 0) / 100));
            $isSignupMatch = false;
            $bonusBlockedReason = $reason;
        }
    }
    ...
    $add = creditsAddByEmail($userEmail, $dollars, 'stripe_checkout_paid', [
        ...
        'session_id'      => $sid,
        'offer_id'        => $offerId,
        'offer_tier_id'   => $offerTierId,
        'amount_total'    => $sessionObj['amount_total'] ?? null,
        'currency'        => $sessionObj['currency'] ?? null,
        'paid_dollars'    => $paidDollars > 0 ? $paidDollars : $dollars,
        'bonus_dollars'   => $isSignupMatch ? (int)($meta['bonus_dollars'] ?? 0) : 0,
        'is_signup_match' => $isSignupMatch,
        'bonus_blocked_reason' => $bonusBlockedReason,
...
// Stamp the deal as claimed now that payment is confirmed.
    ...
    if ($isSignupMatch && $dealOrgId) {
        $claimMeta = [
            ...
            'selected_tier_id' => $offerTierId,
            'source' => $source,
        ];
        if ($offerId === 'bonus_upfront_match_v1' && function_exists('orgOfferClaim')) {
            orgOfferClaim($dealOrgId, 'bonus_upfront_match_v1', $claimMeta);
        } else {
            orgMarkDealClaimed($dealOrgId, 'signup_match_50', $claimMeta);
            if (function_exists('orgOfferClaim')) {
                orgOfferClaim($dealOrgId, 'legacy_signup_match_v1', $claimMeta);
            }
        }
    ...
    if (!empty($add['org_id']) && function_exists('referralEvaluateRewardsForOrg')) {
        referralEvaluateRewardsForOrg($add['org_id']);
    }
...
function creditsAddToOrg($orgId, $amount, $reason, $meta = []) {
    ...
    $o['credits_balance'] = round((float)$o['credits_balance'] + $amount, 2);
    $o['credits_ledger'][] = [
        'ts'       => date('c'),
        'delta'    => $amount,
    ...
    return ['ok' => true, 'scope' => 'org', 'org_id' => $orgId, 'new_balance' => round((float)$o['credits_balance'], 2)];
}
...
function orgAutoTopupTry($orgId, $byEmailForCreditApply, $balanceAfterSpend, $context = []) {
    ...
    if (function_exists('referralEvaluateRewardsForOrg')) {
        ...
        if ($mode === 'setup') {
            ...
            try {
                ...
                if ($oPrime) {
                    ...
                    $threshold = (int)($oPrime['billing']['auto_topup']['threshold_dollars'] ?? 50);
                    $balance   = round((float)($oPrime['credits_balance'] ?? 0), 2);
...
function handleStripeActions($action) {
    ...
    if ($action === 'stripe_create_checkout') {
        ...
        $qty   = (int)($_POST['qty'] ?? 1);
        $requestedOfferId = trim((string)($_POST['offer_id'] ?? ''));

        if ($requestedOfferId !== '') {
            if (!function_exists('orgOfferDefinition') || !orgOfferDefinition($requestedOfferId)) {
                die(json_encode(['success' => false, 'error' => 'Unknown offer']));
            }
            if ($requestedOfferId !== 'bonus_upfront_match_v1') {
                die(json_encode(['success' => false, 'error' => 'This offer is not fulfilled through Stripe checkout']));
            }
        }

        $eligibility = ['eligible' => false, 'org_id' => null, 'reason' => 'no_offer_requested'];
        $isSignupMatch = false;
        $bonusQuote = null;
        if ($requestedOfferId === 'bonus_upfront_match_v1' && function_exists('orgOfferEligibilityByEmail')) {
            $eligibility = orgOfferEligibilityByEmail($email, 'bonus_upfront_match_v1');
            $isSignupMatch = !empty($eligibility['eligible']);
            if (!$isSignupMatch) {
                die(json_encode([
                    'success' => false,
                    'error' => 'Offer is not eligible for this organization',
                    'offer_id' => 'bonus_upfront_match_v1',
                    'reason' => $eligibility['reason'] ?? 'ineligible',
                    'org_id' => $eligibility['org_id'] ?? null,
                ]));
            }
            if (!function_exists('orgOfferBonusUpfrontMatchQuote')) {
                die(json_encode(['success' => false, 'error' => 'Offer pricing is unavailable on this server']));
            }
            $bonusQuote = orgOfferBonusUpfrontMatchQuote($qty);
            if (empty($bonusQuote['valid'])) {
                die(json_encode([
                    'success' => false,
                    'error' => 'This amount is not available for the bonus offer',
                    'offer_id' => 'bonus_upfront_match_v1',
                    'reason' => $bonusQuote['reason'] ?? 'invalid_amount',
                    'quote' => $bonusQuote,
                ]));
            }
        }
        ...
        $out = stripeCreateCreditPurchaseSession(
            $STRIPE_SECRET_KEY,
            $qty,
            $email,
            $BASE_URL,
            ...
            $isSignupMatch ? 'bonus_upfront_match_v1' : '',
            $bonusQuote['bonus_dollars'] ?? null,
            $bonusQuote['tier_id'] ?? '',
            $isSignupMatch ? ($GLOBALS['STRIPE_BONUS_PRICE_ID'] ?? '') : ($GLOBALS['STRIPE_PRICE_ID'] ?? '')
        );

        // Surface deal info to the frontend so it can show a banner / confirmation
        if ($isSignupMatch) {
            $out['deal_applied']   = 'bonus_upfront_match_v1';
            $out['offer_id']       = 'bonus_upfront_match_v1';
            $out['deal_org_id']    = $eligibility['org_id'];
            $out['bonus_dollars']  = (int)($bonusQuote['bonus_dollars'] ?? 0);
            $out['total_credited'] = (int)($bonusQuote['total_account_value'] ?? $qty);
            $out['offer_quote']    = $bonusQuote;
        }

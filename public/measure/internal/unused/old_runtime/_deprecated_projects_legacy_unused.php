 ...
 * Formerly the legacy internal projects module.
 *
 ...
 *
 * Keep only for historical reference while old migration/debug scripts exist.
 *
...
function google3dRemoveDir($dir) {
    ...
    foreach ($items as $item) {
        ...
        if (is_dir($path)) google3dRemoveDir($path);
        else @unlink($path);
    }
    return @rmdir($dir);
}
function google3dProjectRelativeBase($targetDir, $folder) {
    $norm = str_replace('\\', '/', $targetDir);
    if (strpos($norm, '/saves/') !== false) return 'saves/' . $folder . '/';
    if (strpos($norm, '/tutorials/master/') !== false) return 'tutorials/master/' . $folder . '/';
    $userSafe = basename(dirname(rtrim($targetDir, '/\\')));
    return 'tutorials/' . $userSafe . '/' . $folder . '/';
}
function google3dGeodeticToEcef($latDeg, $lonDeg, $heightMeters = 0.0) {
    $a = 6378137.0;
    $e2 = 6.69437999014e-3;
    $lat = deg2rad((float)$latDeg);
    $lon = deg2rad((float)$lonDeg);
    $sinLat = sin($lat);
    $cosLat = cos($lat);
    $sinLon = sin($lon);
    $cosLon = cos($lon);
    ...
    return [
        'x' => ($n + $heightMeters) * $cosLat * $cosLon,
        'y' => ($n + $heightMeters) * $cosLat * $sinLon,
        'z' => ($n * (1 - $e2) + $heightMeters) * $sinLat,
    ];
}
function google3dBoxToSphere($box) {
    return [
        'x' => (float)($box[0] ?? 0),
        'y' => (float)($box[1] ?? 0),
        'z' => (float)($box[2] ?? 0),
        'r' => hypot((float)($box[3] ?? 0), (float)($box[4] ?? 0), (float)($box[5] ?? 0)) +
               hypot((float)($box[6] ?? 0), (float)($box[7] ?? 0), (float)($box[8] ?? 0)) +
               hypot((float)($box[9] ?? 0), (float)($box[10] ?? 0), (float)($box[11] ?? 0)),
    ];
}
function google3dDistanceMeters($a, $b) {
    return hypot(
        ((float)$a['x']) - ((float)$b['x']),
        ((float)$a['y']) - ((float)$b['y']),
        ((float)$a['z']) - ((float)$b['z'])
...
function google3dIntersectsTargetTile($tile, $targetEcef, $radiusMeters) {
    if (empty($tile['boundingVolume']['box']) || !is_array($tile['boundingVolume']['box'])) return true;
    $sphere = google3dBoxToSphere($tile['boundingVolume']['box']);
    return google3dDistanceMeters($sphere, $targetEcef) <= ((float)$sphere['r'] + (float)$radiusMeters);
}
function google3dTileDistanceToTarget($tile, $targetEcef) {
    if (empty($tile['boundingVolume']['box']) || !is_array($tile['boundingVolume']['box'])) return INF;
    $sphere = google3dBoxToSphere($tile['boundingVolume']['box']);
    return google3dDistanceMeters($sphere, $targetEcef);
}
function google3dBuildUrl($input, $apiKey, $session = null) {
    $url = (string)$input;
    if (!preg_match('#^https?://#i', $url)) {
        if ($url === '' || $url[0] !== '/') $url = '/' . $url;
        $url = 'https://tile.googleapis.com' . $url;
    }
    $parts = parse_url($url);
    if (!$parts) return $url;
    $query = [];
    if (!empty($parts['query'])) parse_str($parts['query'], $query);
    if ($session && empty($query['session'])) $query['session'] = $session;
    if (($parts['host'] ?? '') === 'tile.googleapis.com' && empty($query['key'])) $query['key'] = $apiKey;
    $scheme = $parts['scheme'] ?? 'https';
    $host = $parts['host'] ?? 'tile.googleapis.com';
    $path = $parts['path'] ?? '';
    $queryString = http_build_query($query, '', '&', PHP_QUERY_RFC3986);
    return $scheme . '://' . $host . $path . ($queryString !== '' ? '?' . $queryString : '');
}
function google3dCleanUrlForHash($url) {
    $parts = parse_url($url);
...
function google3dFetchJson($url) {
    $raw = fetchUrl($url);
    if ($raw === false) return null;
    $data = json_decode($raw, true);
    return is_array($data) ? $data : null;
}
function google3dFetchBinary($url) {
    $headers = [];
    $ch = curl_init($url);
    curl_setopt_array($ch, [
...
function google3dExtFromUrl($url, $contentType) {
    $path = parse_url($url, PHP_URL_PATH);
    $ext = $path ? pathinfo($path, PATHINFO_EXTENSION) : '';
    if ($ext !== '') return '.' . strtolower($ext);
    $ct = strtolower((string)$contentType);
    if (strpos($ct, 'gltf') !== false || strpos($ct, 'glb') !== false) return '.glb';
    if (strpos($ct, 'json') !== false) return '.json';
    return '.bin';
}
function google3dCaptureProjectTiles($targetDir, $address, $lat, $lon, $radiusMeters = 100, $maxDepth = 40) {
    $apiKey = $GLOBALS['GOOGLE_API_KEY'] ?? '';
    if ($apiKey === '') return ['success' => false, 'error' => 'Missing Google API key.'];

    $captureDir = rtrim($targetDir, '/\\') . '/google_3d_tiles/';
    $tilesDir = $captureDir . 'tiles/';
    google3dRemoveDir($captureDir);
    @mkdir($tilesDir, 0777, true);

    $targetEcef = google3dGeodeticToEcef($lat, $lon, 0);
    $rootUrl = google3dBuildUrl('/v1/3dtiles/root.json', $apiKey, null);
    $rootTileset = google3dFetchJson($rootUrl);
    if (!is_array($rootTileset) || empty($rootTileset['root'])) {
        return ['success' => false, 'error' => 'Failed to load Google 3D root tileset.'];
    }

    $queue = [[ 'tile' => $rootTileset['root'], 'depth' => 0, 'session' => null ]];
    $jsonVisited = [google3dCleanUrlForHash($rootUrl) => true];
    $selectedLeafTiles = [];
    $visited = 0;

    while (!empty($queue)) {
        $entry = array_shift($queue);
        $tile = is_array($entry['tile'] ?? null) ? $entry['tile'] : [];
        $depth = (int)($entry['depth'] ?? 0);
        $session = $entry['session'] ?? null;
        $visited++;

        if (!google3dIntersectsTargetTile($tile, $targetEcef, $radiusMeters)) continue;

        $children = (!empty($tile['children']) && is_array($tile['children'])) ? $tile['children'] : [];
        $nextSession = $session;

        if (!empty($tile['content']['uri'])) {
            $previewUrl = google3dBuildUrl($tile['content']['uri'], $apiKey, $session);
            $previewQuery = [];
            parse_str((string)parse_url($previewUrl, PHP_URL_QUERY), $previewQuery);
            if (!empty($previewQuery['session'])) $nextSession = $previewQuery['session'];
        }

        if (!empty($tile['content']['uri'])) {
            $contentUrl = google3dBuildUrl($tile['content']['uri'], $apiKey, $session);
            if (preg_match('/\.json(?:$|\?)/i', $contentUrl)) {
                $marker = google3dCleanUrlForHash($contentUrl);
                if (empty($jsonVisited[$marker])) {
                    $jsonVisited[$marker] = true;
                    $externalTileset = google3dFetchJson($contentUrl);
                    if (is_array($externalTileset) && !empty($externalTileset['root'])) {
                        $queue[] = ['tile' => $externalTileset['root'], 'depth' => $depth + 1, 'session' => $nextSession];
                    }
                }
            } elseif (!(count($children) > 0 && $depth < $maxDepth)) {
                $selectedLeafTiles[] = [
                    'url' => $contentUrl,
                    'depth' => $depth,
                    'geometricError' => (float)($tile['geometricError'] ?? 0),
                    'distanceMeters' => google3dTileDistanceToTarget($tile, $targetEcef),
                ];
            }
        }

        if (count($children) > 0 && $depth < $maxDepth) {
            foreach ($children as $child) {
                if (is_array($child)) $queue[] = ['tile' => $child, 'depth' => $depth + 1, 'session' => $nextSession];
            }
        }
    }

    $unique = [];
    foreach ($selectedLeafTiles as $tile) {
        $key = google3dCleanUrlForHash($tile['url']);
        if (!isset($unique[$key])) $unique[$key] = $tile;
    }
    $uniqueLeaves = array_values($unique);
    usort($uniqueLeaves, function($a, $b) {
        $distanceCmp = (($a['distanceMeters'] ?? INF) <=> ($b['distanceMeters'] ?? INF));
        if ($distanceCmp !== 0) return $distanceCmp;
        return (($a['geometricError'] ?? INF) <=> ($b['geometricError'] ?? INF));
    });

    if (empty($uniqueLeaves)) {
        google3dRemoveDir($captureDir);
        return ['success' => false, 'error' => 'Google 3D traversal found no nearby leaf tiles for this property.'];
    }

    $manifestTiles = [];
    $totalBytes = 0;
    foreach ($uniqueLeaves as $index => $tile) {
        $binary = google3dFetchBinary($tile['url']);
        if (empty($binary['ok']) || !is_string($binary['body'])) continue;
        $hash = substr(sha1(google3dCleanUrlForHash($tile['url'])), 0, 16);
        $filename = str_pad((string)($index + 1), 4, '0', STR_PAD_LEFT) . '-' . $hash . google3dExtFromUrl($tile['url'], $binary['contentType']);
        $dest = $tilesDir . $filename;
        if (@file_put_contents($dest, $binary['body']) === false) continue;
        $bytes = filesize($dest) ?: strlen($binary['body']);
        $totalBytes += $bytes;
        $manifestTiles[] = [
            'file' => $filename,
            'sourcePath' => (string)(parse_url($tile['url'], PHP_URL_PATH) ?? ''),
            'geometricError' => (float)($tile['geometricError'] ?? 0),
            'distanceMeters' => (float)($tile['distanceMeters'] ?? 0),
            'depth' => (int)($tile['depth'] ?? 0),
            'bytes' => (int)$bytes,
        ];
    }

    if (empty($manifestTiles)) {
        google3dRemoveDir($captureDir);
        return ['success' => false, 'error' => 'Google 3D tile files could not be downloaded for this property.'];
    }
    ...
    $manifest = [
        ...
        'capture' => [
            ...
            'deepestLeafCount' => count($manifestTiles),
            'visitedNodeCount' => (int)$visited,
        ],
        'tiles' => $manifestTiles,
    ];

    if (@file_put_contents($captureDir . 'manifest.json', json_encode($manifest, JSON_PRETTY_PRINT)) === false) {
        google3dRemoveDir($captureDir);
        return ['success' => false, 'error' => 'Failed to write the Google 3D manifest file.'];
    }

    return [
        'success' => true,
        'tileCount' => count($manifestTiles),
        'captureRadiusMeters' => (float)$radiusMeters,
        'maxDepth' => (int)$maxDepth,
        'visitedNodeCount' => (int)$visited,
    ...
    if ($action === 'list_projects') {
        ...
        $buildProj = function($folder, $basePath, $relPrefix = null) use ($baseDir) {
            ...
            return [
                ...
                'project_type' => $m['project_type'] ?? 'residential',
                'include_gutter_measurements' => !empty($m['include_gutter_measurements']),
                'pins' => $m['pins'] ?? [],
...
//                 die(json_encode(['error' => 'Unauthorized']));
    ...
    // ------------------------------------------------
    // Returns all completed and terminally rejected projects with only the fields
    // stats.js needs, queried entirely from the SQL index — zero manifest file reads
    // for the indexed fields and a single manifest read for supplemental metadata.
    if ($action === 'stats_data') {
        ...
        $stmt = $db->prepare("
            SELECT id, st, ca, da, sa, ua, asn, fl, tm, org, cx
            FROM p
            WHERE st IN ('completed', 'rejected', 'rejected_no_coverage')
            ORDER BY COALESCE(da, ca, 0) DESC
        ");
        $res = $stmt->execute();
    ...
    if ($action === 'generate_google_3d_tiles') {
        if (!isset($_SESSION['user_email'])) { http_response_code(403); die(json_encode(['success' => false, 'error' => 'Not logged in'])); }
        $fid = $_POST['folder'] ?? '';
        if (!$fid && isset($_POST['address'])) $fid = getFolder($_POST['address']);
        $folder = preg_replace('/[^a-f0-9]/', '', $fid);
        if ($folder === '') die(json_encode(['success' => false, 'error' => 'Missing folder']));
        ...
        if (!is_array($manifest)) $manifest = [];
        $appMetaPath = $targetDir . 'app_metadata.json';
        $appMeta = file_exists($appMetaPath) ? json_decode(@file_get_contents($appMetaPath), true) : [];
        if (!is_array($appMeta)) $appMeta = [];

        $lat = isset($_POST['lat']) ? (float)$_POST['lat'] : (float)($manifest['lat'] ?? 0);
        $lng = isset($_POST['lng']) ? (float)$_POST['lng'] : (float)($manifest['lng'] ?? 0);
        $address = trim((string)($_POST['address'] ?? ($manifest['address'] ?? 'Project')));
        if (!is_finite($lat) || !is_finite($lng) || (abs($lat) < 0.000001 && abs($lng) < 0.000001)) {
            die(json_encode(['success' => false, 'error' => 'Project is missing valid coordinates for Google 3D capture.']));
        }

        $projectRadius = 20.0;
        if (isset($appMeta['radius_meters']) && is_numeric($appMeta['radius_meters'])) $projectRadius = (float)$appMeta['radius_meters'];
        elseif (isset($manifest['radius_meters']) && is_numeric($manifest['radius_meters'])) $projectRadius = (float)$manifest['radius_meters'];
        $captureRadius = isset($_POST['capture_radius_meters']) && is_numeric($_POST['capture_radius_meters'])
            ? (float)$_POST['capture_radius_meters']
            : max(75.0, min(160.0, round(max($projectRadius, 20.0) * 1.75)));
        $maxDepth = isset($_POST['max_depth']) && is_numeric($_POST['max_depth']) ? (int)$_POST['max_depth'] : 40;

        $capture = google3dCaptureProjectTiles($targetDir, $address, $lat, $lng, $captureRadius, $maxDepth);
        ...
        if (!isset($appMeta['google3dTiles']) || !is_array($appMeta['google3dTiles'])) $appMeta['google3dTiles'] = [];
        $manifestUrl = google3dProjectRelativeBase($targetDir, $folder) . 'google_3d_tiles/manifest.json';
        $appMeta['google3dTiles']['manifestUrl'] = $manifestUrl;
        $appMeta['google3dTiles']['generatedAt'] = gmdate('c');
        $appMeta['google3dTiles']['captureRadiusMeters'] = $captureRadius;
        $appMeta['google3dTiles']['tileCount'] = (int)($capture['tileCount'] ?? 0);
        $appMeta['google3dTiles']['maxDepth'] = (int)$maxDepth;
        @file_put_contents($appMetaPath, json_encode($appMeta, JSON_PRETTY_PRINT));

...
(function () {
  ...
  const workloadFields = [
    ['workload_base_minutes', 'Off-peak wait', 'min', 180, 1440, 1, 'Before the buffer; minimum 3 hours to preserve rush ordering.'],
    ['workload_peak_minutes', 'Peak wait', 'min', 180, 1440, 1, 'Before the buffer; the existing small daily variation is retained.'],
    ['workload_buffer_minutes', 'Delivery buffer', 'min', 0, 720, 1, 'Added to the workload estimate.'],
    ['turnaround_max_minutes', 'Maximum total turnaround', 'min', 180, 2880, 1, 'Includes the buffer. Set 480 for 8 hours; additional structures still add time.'],
    ['ramp_start_minute', 'Ramp-up starts', 'Pacific', 0, 1439, 1, 'Demand begins rising.'],
    ['peak_start_minute', 'Peak starts', 'Pacific', 1, 1439, 1, 'Start of the busy period.'],
    ['peak_end_minute', 'Peak ends', 'Pacific', 2, 1439, 1, 'Demand begins falling.'],
    ['ramp_end_minute', 'Wind-down ends', 'Pacific', 3, 1440, 1, 'Return to off-peak wait. 24:00 means midnight at the end of the day.']
  ];
  const exteriorFields = [
    ['exteriors_base_price','Full house standard / structure','$',0,1000,.01,'24-hour delivery. Includes roof and gutters.'],
    ['exteriors_same_day_fee','Full house under 6 hours / structure','$',0,1000,.01,'Added to the standard full-house price.'],
    ['exteriors_priority_fee','Full house under 3 hours / structure','$',0,1000,.01,'Added to the standard full-house price.']
  ];
  const allFields = [...fields, ...workloadFields, ...exteriorFields];
  const isTime = key => key.endsWith('_minute');
  function inputValue(key, value) {
    if (!isTime(key)) return Number(value);
    const match = /^(\d{2}):(\d{2})$/.exec(value);
    return match && Number(match[2]) < 60 && (Number(match[1]) < 24 || value === '24:00') ? Number(match[1]) * 60 + Number(match[2]) : NaN;
  }
  function displayValue(key, value) {
    return isTime(key) ? String(Math.floor(value / 60)).padStart(2, '0') + ':' + String(value % 60).padStart(2, '0') : value;
  }
  ...
  function values() { return Object.fromEntries(allFields.map(([key]) => [key, inputValue(key, root.querySelector(`[name="${key}"]`).value)])); }
  ...
  async function preview() {
    ...
    try {
      ...
      root.querySelector('[data-preview]').innerHTML = `<table><caption>Report price · no add-ons</caption><thead><tr><th>Wait</th><th>Standard</th><th>1–3 hour</th><th>Under 1 hour</th></tr></thead><tbody>${data.samples.map(row => `<tr><th>${esc(row.wait_minutes)} min</th>${row.residential.map(price => `<td>$${Number(price).toFixed(2)}</td>`).join('')}</tr>`).join('')}</tbody></table><p class="prices-note">Commercial / multifamily, per structure at full busy-fee threshold: ${data.samples[2].commercial.map(price => '$' + Number(price).toFixed(2)).join(' / ')}.</p>`;
      root.querySelector('[data-current]').textContent = 'Current estimated wait: ' + data.current.options[0].estimated_wait_minutes + ' min';
      root.querySelector('[data-timeline]').innerHTML = `<h3>Next 24 hours · Pacific time</h3><div class="prices-timeline">${data.timeline.map(row => `<div><small>${esc(row.pacific_label)}</small><strong>${esc(row.wait_minutes)} min</strong><small>1–3-hour price $${Number(row.rush_price).toFixed(2)}</small></div>`).join('')}</div>`;
    } catch (error) { if (sequence === previewSequence) {
      root.querySelector('[data-preview]').textContent = error.message;
      root.querySelector('[data-current]').textContent = '';
      root.querySelector('[data-timeline]').textContent = '';
    } }
  }
  function populate(config) { allFields.forEach(([key]) => { root.querySelector(`[name="${key}"]`).value = displayValue(key, config[key]); }); }
  function fieldMarkup(list) {
    return list.map(([key,label,unit,min,max,step,hint]) => `<label>${esc(label)}<div class="prices-input"><input required type="${isTime(key) ? 'text' : 'number'}" name="${key}" ${isTime(key) ? 'pattern="([01][0-9]|2[0-3]):[0-5][0-9]|24:00" placeholder="HH:MM"' : `min="${min}" max="${max}" step="${step}"`}><span>${unit}</span></div><small>${esc(hint)}</small></label>`).join('');
  }
  async function load() {

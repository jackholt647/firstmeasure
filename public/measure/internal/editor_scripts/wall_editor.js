...
window.exteriorSurfaceDisplay=function(group,mode=true){
 const translucent=mode===true||mode==='translucent',textured=mode==='textured';
 group.traverse(o=>{
...
window.wallPointPickVisible=function(group,p){
 ...
 return depth<=front.distance+epsilon;
};
window.wallLinePickVisible=function(group,a,b,e){
 const r=renderer.domElement.getBoundingClientRect(),ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((e.clientX-r.left)/r.width*2-1,1-(e.clientY-r.top)/r.height*2),camera);
 const point=new THREE.Vector3();ray.ray.distanceSqToSegment(a,b,null,point);return window.wallPointPickVisible(group,point);
};

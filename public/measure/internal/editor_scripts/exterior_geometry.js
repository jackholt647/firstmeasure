...
// Resolve evaluated edges in metric tolerance, including grid-rounded endpoints.
// A sample is never an editable anchor unless explicitly authored or a junction.
function curveEdgeRange(c,a,b,samples=curveSamples(c)){
 const locate=p=>{let best=null;for(let i=1;i<samples.length;i++){const x=samples[i-1],y=samples[i],v=sub(y,x),l2=dot(v,v);if(l2<GRID*GRID)continue;const t=Math.max(0,Math.min(1,dot(sub(p,x),v)/l2)),q={x:x.x+v.x*t,y:x.y+v.y*t,z:x.z+v.z*t},d=distance(p,q);if(d<=CONTACT&&(!best||d<best.d))best={d,t:x.curveT+(y.curveT-x.curveT)*t};}return best?.t;};
 const x=locate(a),y=locate(b);if(x===undefined||y===undefined||Math.abs(x-y)<1e-10)return null;
 const v=sub(b,a),l2=dot(v,v);if(l2<GRID*GRID)return null;
 if(samples.some(p=>p.curveT>Math.min(x,y)&&p.curveT<Math.max(x,y)&&distance(p,{x:a.x+v.x*dot(sub(p,a),v)/l2,y:a.y+v.y*dot(sub(p,a),v)/l2,z:a.z+v.z*dot(sub(p,a),v)/l2})>CONTACT))return null;
 const snap=t=>{const knots=c.type==='spline'?c.knots:[0,1];return knots.find(k=>distance(curvePoint(c,k),curvePoint(c,t))<=CONTACT)??t;};return [snap(x),snap(y)];
}
function annotateCurveGraph(sketch,curves=sketch.curves||[]){
 const nodes=new Map(sketch.nodes.map(n=>[n.id,n])),byId=new Map(curves.map(c=>[c.id,c])),samples=new Map(curves.map(c=>[c.id,curveSamples(c)]));
 for(const e of sketch.edges){if(e.curveId){if(!byId.has(e.curveId))throw Error('Curve edge is missing its analytic definition.');continue;}const a=nodes.get(e.a),b=nodes.get(e.b);if(!a||!b)continue;for(const c of curves){const range=curveEdgeRange(c,a,b,samples.get(c.id));if(range){e.curveId=c.id;e.curveRange=range;break;}}}
 const incident=new Map();for(const e of sketch.edges)for(const id of [e.a,e.b]){if(!incident.has(id))incident.set(id,[]);incident.get(id).push(e);}
 for(const n of sketch.nodes){const edges=incident.get(n.id)||[],curve=edges.find(e=>e.curveId),c=curve&&byId.get(curve.curveId);if(!c)continue;const anchor=n.userDraftPoint||n.curveCenter||(c.type==='spline'?c.controls:[curvePoint(c,0),curvePoint(c,1)]).some(p=>distance(n,p)<=CONTACT)||edges.length!==2||edges.some(e=>e.curveId!==curve.curveId);if(c.type==='spline'&&c.controls.some(p=>distance(n,p)<=CONTACT))n.curveControl=true;if(anchor)delete n.curveSample;else n.curveSample=true;}
 return sketch;
}

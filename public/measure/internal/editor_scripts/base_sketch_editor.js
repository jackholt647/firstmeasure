...
return {finishToolForSwitch,chamferSelection:()=>({points:selected.map(id=>copy(actual(node(id)))),edges:sketch().edges.filter(e=>lines.includes(e.id)).map(e=>[copy(actual(node(e.a))),copy(actual(node(e.b)))])}),cancelPointerGesture(){if(box||(drag&&!drag.height))cancel();},interaction:()=>curveTool?"Draw curve":axisPreview?"Base cut":armed?"Draw on base":drag?"Move base point":box?"Rectangle selection":null,selectEntities(ps,pairs,add=false,subtract=false){const priorPoints=selected.slice(),priorLines=lines.slice();const near=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y,a.z-b.z)<.001;selected=sketch().nodes.filter(n=>ps.some(p=>near(actual(n),p))).map(n=>n.id);lines=sketch().edges.filter(e=>pairs.some(([a,b])=>{return S.curveEdges(sketch()).filter(segment=>segment.id===e.id).some(segment=>window.WallSolidGeometry.sharedIntervals(segment.start,segment.end,[{points:[a,b]}]).length);})).map(e=>e.id);if(subtract){selected=priorPoints.filter(id=>!selected.includes(id));lines=priorLines.filter(id=>!lines.includes(id));}else if(add){selected=[...new Set([...priorPoints,...selected])];lines=[...new Set([...priorLines,...lines])];}preferredFace=null;report();},canPick:(e,v)=>!!closest(e,v)||!!referenceAt(e,v),singleLine:()=>lines.length===1?sketch().edges.filter(e=>e.id===lines[0]).map(e=>[copy(actual(node(e.a))),copy(actual(node(e.b)))])[0]:null,singlePoint:()=>selected.length===1&&node(selected[0])?copy(actual(node(selected[0]))):null,setFace,canHit:(e,v)=>!!v&&!!referenceAt(e,v),busy:()=>!!curveTool||!!axisPreview||!!drag||!!box||armed,down,move,up,keyDown,cancel,draw2D,draw3D,doubleClick(e,v){if(active()&&mode()==='point'){insert(e,v);return true;}return false;},clear(){cancel();preferredFace=null;selected=[];lines=[];}};
};
})();

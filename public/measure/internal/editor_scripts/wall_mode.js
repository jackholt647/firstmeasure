...
(function () {
    ...
    let gapHighlights=true,gapCacheWalls=null,gapCacheGround=null,gapCache=[];
    ...
    function finishEdit(){if(!pendingEdit)return;const before=pendingEdit;pendingEdit=null;const after=editSnapshot();if(JSON.stringify(before)===JSON.stringify(after))return;const last=editHistory.at(-1);if(nudgeKey&&last?.nudgeKey===nudgeKey&&JSON.stringify(last.after)===JSON.stringify(before))last.after=after;else editHistory.push({before,after,nudgeKey});if(editHistory.length>50)editHistory.shift();editFuture=[];}
    function undoEdit(redo=false){
        ...
        if(baseEditor?.busy()||wallEditor?.busy()){if(!redo){baseEditor?.leave();wallEditor?.clear();render();}return true;}
        const from=redo?editFuture:editHistory,to=redo?editHistory:editFuture,entry=from.pop();
        if(!entry){if(status)status.textContent=redo?'Nothing to redo':'Nothing to undo in this session';return true;}
        roofTrimEditor?.reset();baseEditor?.clearSelection();wallEditor?.clear();const value=JSON.parse(JSON.stringify(redo?entry.after:entry.before));if(entry.full){state=value.state;stage=value.stage;sourceContext=state.context;selected=null;}else for(const k of historyKeys){if(k in value)state[k]=value[k];else delete state[k];}to.push(entry);pendingEdit=null;baseTerrainKey='';persist();render();return true;
    }
    ...
    function generate(soffit,resetBase=false) {
        ...
        try {
            ...
            const finishDefaults=copy(state?.finishDefaults||{}),roofTrim=copy(state?.roofTrim||roofTrimOnly),ground=state?.ground,groundCandidates=state?.groundCandidates,base=resetBase?undefined:state?.base,wallCenters=state?.wallCenters!==false,displayMode=state?.displayMode,translucent=state?.translucent!==false,boundExtrusionToRoof=state?.boundExtrusionToRoof!==false;
            state={schemaVersion:1,engineVersion:ENGINE,...captured,roofSignature:signature,options,sources:r.sources,warnings:r.warnings,savedAt:Date.now(),ground,groundCandidates,base,wallCenters,translucent,displayMode,boundExtrusionToRoof,roofTrim,finishDefaults};
            initializeGround();
            ...
            if(before){editHistory.push({full:true,before,after:{state:copy(state),stage}});if(editHistory.length>50)editHistory.shift();}editFuture=[];return true;
    ...
    function currentGaps(){const walls=currentWalls(),ground=wallFloor();if(walls!==gapCacheWalls||ground!==gapCacheGround){gapCacheWalls=walls;gapCacheGround=ground;gapCache=window.WallGaps?.detect(walls,ground)||[];}return gapCache;}
    ...
    function render3DContent() {
        ...
        if(wallsVisible&&state&&stage>=2){
            ...
            const geo=G.topology(currentWalls().filter(w=>!wallEditor?.hasDraft(w.id)));
            for(const f of geo.faces){
                ...
                const points=f.triangles.flat().map(i=>geo.points[i]),bg=new THREE.BufferGeometry().setFromPoints(points.map(vector));bg.computeVertexNormals();
    ...
    function initialize() {
        ...
        #wall-advanced{position:relative} #wall-advanced>summary{list-style:none;cursor:pointer;padding:4px 8px;font-size:18px} #wall-advanced>summary::-webkit-details-marker{display:none} #wall-advanced>div{position:absolute;right:0;top:100%;width:260px;padding:12px;background:#242a30;border:1px solid #65707a;border-radius:7px;z-index:50;box-shadow:0 4px 14px #0006} #wall-panel #wall-advanced label{display:flex;justify-content:flex-start;gap:8px;align-items:center} #wall-panel #wall-advanced input{width:auto;flex:none} #wall-panel #wall-advanced p{font-size:12px;color:#b9c5cc;margin-top:8px}
        ...
        baseEditor=window.createBaseEditor?.({pickVisible:p=>state?.displayMode!=='textured'&&window.wallPointPickVisible(group3D,getVector3(toPixel(p))),pickLineVisible:(pair,e)=>state?.displayMode!=='textured'&&window.wallLinePickVisible(group3D,...pair.map(p=>getVector3(toPixel(p))),e),walls:currentWalls,state:baseState,ensure:()=>!!state||generate('auto'),enabled:()=>enabled,id:currentId,layer:()=>editingLayer,setLayer,
            ...
            recordHistory:before=>{if(before.wallEdits?.$base)delete before.base;recordEdit(before);},undo:()=>undoEdit(),canUndo:()=>editHistory.length>0,changed:()=>{groundChanged();finishEdit();baseEditor?.render();},redraw:()=>{render2D();render3D();}});
        ...
        wallEditor=window.createWallEditor?.({pickVisible:p=>state?.displayMode!=='textured'&&window.wallPointPickVisible(group3D,getVector3(toPixel(p))),pickLineVisible:(pair,e)=>state?.displayMode!=='textured'&&window.wallLinePickVisible(group3D,...pair.map(p=>getVector3(toPixel(p))),e),pasteHost:e=>{const hit=window.wallNearestSurface?.(group3D,e);if(hit?.layer==='base'){const f=(state.wallEdits?.$base||state.base)?.faces.find(f=>f.id===hit.object.userData.baseId);if(f)return {f,points:f.points};}return null;},selectBaseEntities:(points,pairs,add,subtract)=>{setLayer('base');baseEditor?.selectEntities(points,pairs,add,subtract);},message:text=>{document.getElementById('base-status').textContent=text||'Walls selected';},setLayer,state:()=>state,enabled:()=>enabled,layer:()=>editingLayer,visible:()=>wallsVisible,roofVisible:()=>roofVisible,walls:currentWalls,toPixel,
            position:(...args)=>groundEditor.position(...args),floorHeight:p=>GroundGeometry.height(wallFloor(),p),recordHistory:before=>recordEdit({wallEdits:before}),changed:()=>{finishEdit();persist();render();},redraw:()=>{render2D();render3D();}});
        ...
        document.getElementById('wall-auto-trim').onclick=()=>{if(!enabled)return;nudgeEpoch++;nudgeKey=null;wallEditor?.autoTrim?.();};
        ...
        for(const type of ['pointerdown','mousedown','dblclick','click'])window.addEventListener(type,e=>{
            ...
            if(!enabled)return;if(e.target.closest?.('#roof-trim-control,#wall-panel,#exterior-toolbar,#axis-gizmo-container,.enh-control-panel,.controls-3d-actions,.exterior-sticker-bar,.exterior-sticker-menu'))return;
            if(e.target.closest?.('#viewport,#three-view-wrapper') && e.button===0){
                ...
                if(type==='pointerdown'){nudgeEpoch++;nudgeKey=null;
                    ...
                    if(free&&e.target.closest?.('#three-view-wrapper')&&wallEditor?.startBox?.(e,()=>{pickPointer(e);wallEditor?.finishPointer?.(e);baseEditor?.finishPointer?.(e);})){e.stopImmediatePropagation();e.preventDefault();return;}
        ...
        const interactionText=document.createElement('span'),cancelButton=document.createElement('button');cancelButton.type='button';cancelButton.className='enh-btn';cancelButton.textContent='Cancel · Esc';cancelButton.onclick=cancelInteraction;interaction.append(interactionText,cancelButton);
        const updateInteraction=()=>{
            const toolbar=document.querySelector('#three-container .enh-control-panel');if(toolbar&&interaction.parentElement!==toolbar)toolbar.appendChild(interaction);
        ...
        window.addEventListener('wheel',e=>{
            if(!enabled||e.target.closest?.('#wall-panel,.enh-control-panel,.controls-3d-actions,.pane-swap-button,input,textarea,select'))return;
        ...
        window.WallFeatures?.mountUI((...args)=>{if(editingLayer!=='walls')setLayer('walls');window.SmartStickers?.exitPlacement();return wallEditor?.featureCommand(...args);},()=>wallEditor?.featureSelection(),()=>wallEditor?.busy(),{
            ...
            active:()=>wallEditor?.activeMaterial()||'default',

...
(function () {
    ...
    let resoffitMode=false,drivenSoffits=true;
...
function perf_finishEdit(){
    ...
    function initializeBase(useHeight=false){
        ...
        if(state&&!state.base&&window.BaseGeometry){
            ...
            // An overlap-adjusted exterior can sit outside the uniform inset
...
function perf_persist(touch=true) {
    ...
    function generate(soffit,resetBase=false) {
        ...
        try {
            ...
            const options={soffit,defaultSoffitInches:24,roofContacts:true,drivenSoffits:state?.drivenSoffits??state?.options?.drivenSoffits??drivenSoffits,ground:state?.options.ground??estimateGround(),tolerance:state?.options.tolerance??18*G.INCH};
            ...
            const finishDefaults=copy(state?.finishDefaults||{}),roofTrim=copy(state?.roofTrim||roofTrimOnly),ground=state?.ground,groundCandidates=state?.groundCandidates,base=resetBase?undefined:state?.base,wallCenters=state?.wallCenters!==false,lineCenters=state?.lineCenters!==false,displayMode=state?.displayMode,translucent=state?.translucent!==false,boundExtrusionToRoof=state?.boundExtrusionToRoof!==false,undoSelections=state?.undoSelections!==false,keepTrimStatic=state?.keepTrimStatic===true,wallTrimWidthInches=state?.wallTrimWidthInches===8?8:6;
            state={schemaVersion:1,engineVersion:ENGINE,...captured,roofSignature:signature,options,drivenSoffits:options.drivenSoffits,sources:r.sources,warnings:r.warnings,savedAt:Date.now(),ground,groundCandidates,base,wallCenters,lineCenters,translucent,displayMode,boundExtrusionToRoof,undoSelections,keepTrimStatic,wallTrimWidthInches,roofTrim,finishDefaults};
            initializeGround();
            initializeBase(true);
...
function perf_composeCurrentWalls(){const walls=!state||stage===1?[]:stage===2?(state.extruded||[]):stage===3?(state.deduplicated||[]):stage===4?(state.gapRepaired||[]):stage===5?(state.mergedWalls||[]):stage===6?(state.cleanedWalls||[]):(state.alignedWalls||[]);const applied=wallEditor?.apply(walls)||walls;if(!state||stage<2||!window.WallChimneys)return applied;const composed=WallChimneys.compose(applied,state);const final=[...composed.filter(w=>!w.chimney),...(wallEditor?.apply(composed.filter(w=>w.chimney))||composed.filter(w=>w.chimney))];const edits=state.wallEdits||{},protectedIds=[...Object.keys(edits).filter(k=>!k.startsWith('$')),...Object.values(edits.$drafts||{}).flatMap(d=>d.members||[])];return G.canonicalGeneratedWalls(stage>=7?WallChimneyCleanup.compose(final,state.chimneyCleanupReport,protectedIds):final,protectedIds);}
    ...
    function render2DContent(...args){if(!window.ExteriorPerf?.enabled)return perf_render2DContent.apply(this,args);return window.ExteriorPerf.measure('2D redraw',()=>perf_render2DContent.apply(this,args));}
function perf_render2DContent() {
...
function perf_render3DFrame() {
        ...
        try{
            ...
            sceneParts?.begin([surfaceMode(),state?.finishDefaults,state?.materialColors,wallEditor?.planeView?.(),[vector({x:0,y:0,z:0}),vector({x:1,y:0,z:0}),vector({x:0,y:1,z:0}),vector({x:0,y:0,z:1})].map(p=>[p.x,p.y,p.z])]);
            render3DContent();scene.add(group3D);window.ExteriorRendered?.update(group3D,{enabled:enabled&&surfaceMode()==='textured',scene,vector});sceneParts?.commit();window.invalidateScene3D?.();
            if(previous){previous.parent?.remove(previous);disposeObject3D(previous);}
        }
        catch(error){sceneParts?.rollback();if(group3D&&group3D!==previous){group3D.parent?.remove(group3D);disposeObject3D(group3D);}group3D=previous;lastScene=previousScene;throw error;}
    }
    function render3DContent() {
        ...
        if(wallsVisible&&state&&stage>=2){
            ...
            for(const f of geo.faces){
                ...
                const hostFace={points:f.pointIndices.map(i=>geo.points[i]),holes:(f.holes||[]).map(r=>r.map(i=>geo.points[i]))},parts=window.ExteriorModel.cutOpenings(hostFace,openings);
    ...
    function render() {
        ...
        for(const mode of ['all','moving','off'])document.getElementById('wall-lengths-'+mode).setAttribute('aria-pressed',String(mode===(state?.lineLengthMode||(state?.wallLengths===false?'off':'moving'))));document.getElementById('wall-feature-dimensions')?.setAttribute('aria-pressed',String(state?.featureDimensions!==false));
        document.getElementById('wall-roof-bounds').checked=state?.boundExtrusionToRoof!==false;
        document.getElementById('wall-static-trim').checked=state?.keepTrimStatic===true;
        document.getElementById('wall-driven-soffits').checked=state?.drivenSoffits??state?.options?.drivenSoffits??drivenSoffits;
        document.getElementById('wall-undo-selections').checked=state?.undoSelections!==false;
        const displayButton=document.getElementById('wall-translucency-toggle'),displayMode=surfaceMode();displayButton.textContent=surfaceLabel(displayMode);displayButton.title='Surface display: '+surfaceLabel(displayMode)+'. Click or press Tab to cycle '+surfaceModes().map(surfaceLabel).join(', ')+'.';displayButton.setAttribute('aria-label',displayButton.title);displayButton.setAttribute('aria-pressed',String(displayMode!=='translucent'));
        document.getElementById('wall-line-centers-toggle').setAttribute('aria-pressed',String(state?.lineCenters!==false));
    ...
    function initialize() {
        ...
        resoffitControl.innerHTML='<button id="wall-resoffit" aria-expanded="false" aria-pressed="false" title="Change the depth of selected roof-contact soffits">Resoffit</button><div id="wall-resoffit-menu" hidden><p>Select red-orange roof-contact lines. Click to add or remove; then Apply.</p><label>Soffit depth (ft)<input id="wall-resoffit-depth" type="number" min="0" max="16" step="any" value="2"></label><div id="wall-resoffit-presets"><button data-resoffit="0">0</button><button data-resoffit="0.5">½ ft</button><button data-resoffit="1">1 ft</button><button data-resoffit="1.5">1½ ft</button><button data-resoffit="2">2 ft</button></div><p id="wall-resoffit-count" role="status"></p><button id="wall-resoffit-apply">Apply</button><button id="wall-resoffit-done">Done</button><p id="wall-resoffit-result" role="status"></p></div>';
        ...
        function setResoffit(on){resoffitMode=!!on;wallEditor?.setResoffitMode?.(resoffitMode);resoffitMenu.hidden=!resoffitMode;resoffitButton.setAttribute('aria-expanded',String(resoffitMode));resoffitButton.setAttribute('aria-pressed',String(resoffitMode));if(on){document.getElementById('wall-resoffit-result').textContent='';setLayer('walls',true);document.getElementById('base-status').textContent='Select red-orange soffit lines, then choose a depth and Apply.';}}
        ...
        document.getElementById('wall-resoffit-apply').onclick=()=>{const input=document.getElementById('wall-resoffit-depth');if(!input.value.trim()||!input.checkValidity()){input.reportValidity();return;}const ok=wallEditor?.resoffit(Number(input.value)*.3048);document.getElementById('wall-resoffit-result').textContent=ok?'Selected soffits updated. '+document.getElementById('base-status').textContent:document.getElementById('base-status').textContent;};
        ...
        for(const mode of ['all','moving','off'])document.getElementById('wall-lengths-'+mode).onclick=()=>{if(!state||state.lineLengthMode===mode)return;state.lineLengthMode=mode;state.wallLengths=mode!=='off';persist();render();};document.getElementById('wall-feature-dimensions').onclick=()=>{if(!state)return;state.featureDimensions=state.featureDimensions===false;persist();render();};
        document.getElementById('wall-driven-soffits').onchange=e=>{drivenSoffits=e.target.checked;if(state){state.drivenSoffits=drivenSoffits;persist();}};
        document.getElementById('wall-static-trim').onchange=e=>{if(!state)return;wallEditor?.leave?.();state.keepTrimStatic=e.target.checked;persist();render();};
        ...
        function cycleSurfaceDisplay(){if(!state)return;const modes=surfaceModes(),mode=surfaceMode();state.displayMode=modes[(modes.indexOf(mode)+1)%modes.length];state.translucent=state.displayMode==='translucent';persist();render();}
        document.getElementById('wall-translucency-toggle').onclick=cycleSurfaceDisplay;
        document.getElementById('wall-line-centers-toggle').onclick=()=>{if(!state)return;state.lineCenters=state.lineCenters===false;persist();render();};
        ...
        function handleEditorKey(e){
            ...
            if(editingLayer==='base'&&['h','v'].includes(e.key.toLowerCase())&&!baseEditor?.busy()&&!e.ctrlKey&&!e.metaKey){const selected=baseEditor?.chamferSelection?.()?.points||[],p=baseEditor?.singlePoint(),points=selected.length?selected:p?[p]:[],preferred=baseEditor?.selectedFaceId();if(points.length){setLayer('walls',true);wallEditor?.cutFromPoints(points,e.key.toLowerCase(),preferred);e.preventDefault();e.stopImmediatePropagation();return true;}}
        ...
        window.ExteriorPerf?.mount(panel);
        setModeUI();render();
    }
    ...
    // Read-only camera experiment bridge; uses the same metric-to-scene mapping as walls.
    function aiGeometry(){
        ...
        return {min,max,center,faces:[...faces,...(state.roof.faces||[])].map(f=>({points:f.points,holes:f.holes||[]})),

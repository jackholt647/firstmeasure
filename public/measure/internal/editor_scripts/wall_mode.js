...
function perf_persist(touch=true) {
    ...
    function generate(soffit,resetBase=false) {
        ...
        try {
            ...
            state={schemaVersion:1,engineVersion:ENGINE,...captured,roofSignature:signature,options,sources:r.sources,warnings:r.warnings,savedAt:Date.now(),ground,groundCandidates,base,wallCenters,translucent,displayMode,boundExtrusionToRoof,undoSelections,keepTrimStatic,wallTrimWidthInches,roofTrim,finishDefaults};
            initializeGround();
            initializeBase(true);

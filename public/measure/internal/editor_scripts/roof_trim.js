...
const INCH=.0254,DEFAULT=6*INCH,key=(a,b)=>[K.pointKey(a),K.pointKey(b)].sort().join('|');
...
function perimeter(roof){
 ...
 const allowed=(roof?.connections||[]).filter(c=>['eave','rake'].includes(c.type)).map(c=>({a:roof.points?.[c.startIdx],b:roof.points?.[c.endIdx]})).filter(e=>e.a&&e.b);
 ...
 for(const edge of boundary){const axis=unit(sub(edge.b,edge.a)),length=Math.hypot(...Object.values(sub(edge.b,edge.a)));
  for(const c of allowed){const project=p=>dot(sub(p,edge.a),axis),on=p=>{const v=sub(p,edge.a),t=project(p);return Math.hypot(v.x-axis.x*t,v.y-axis.y*t,v.z-axis.z*t)<=K.CONTACT;};if(!on(c.a)||!on(c.b))continue;
   ...
   const point=t=>({x:edge.a.x+axis.x*t,y:edge.a.y+axis.y*t,z:edge.a.z+axis.z*t}),a=point(lo),b=point(hi);portions.set(key(a,b),{a,b});
  }
 }
 ...
 let changed=true;while(changed){changed=false;outer:for(let i=0;i<edges.length;i++)for(let j=i+1;j<edges.length;j++){
  const a=edges[i],b=edges[j];for(const [p,q]of [[a.a,a.b],[a.b,a.a]]){const r=K.pointKey(p)===K.pointKey(b.a)?b.b:K.pointKey(p)===K.pointKey(b.b)?b.a:null;if(!r)continue;const u=unit(sub(q,p)),v=unit(sub(r,p));if(dot(u,v)>-1+1e-9)continue;edges[i]={a:q,b:r};edges.splice(j,1);changed=true;break outer;}
 }}

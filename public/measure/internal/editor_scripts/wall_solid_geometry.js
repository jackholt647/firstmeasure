...
function transformGeometry(clip,index,change={}){
 ...
 const point=p=>{const q=K.local(frame,p),x=q.x*(change.scale?.x??1)*(flip==='x'?-1:1),y=q.y*(change.scale?.y??1)*(flip==='y'?-1:1);return {...p,...K.world(frame,{x:x*c-y*s+(delta.x||0),y:x*s+y*c+(delta.y||0),z:q.z*(change.scale?.z??1)})};};
...
function snapGeometryOnPlane(clip,index,targets,edges,screen,radius=10){
 const frame=clip.mounts[index].frame;let best=radius,delta=null;
 const offer=(p,q)=>{const d=sub(q,p);if(Math.abs(dot(d,frame.n))>K.CONTACT||Math.hypot(d.x,d.y,d.z)>.5)return;const a=screen(p),b=screen(q),distance=Math.hypot(a.x-b.x,a.y-b.y);if(a.visible!==false&&b.visible!==false&&distance<best){best=distance;delta={x:dot(d,frame.u),y:dot(d,frame.v)};}};
 ...
 return delta?transformGeometry(clip,index,{delta}):clip;
}
...
function pasteGeometry(clip,mountIndex,target,location,options={}){
 ...
 const contacts=clip.points.map((p,i)=>Math.abs(K.local(source,p).z)<=K.CONTACT?i:-1).filter(i=>i>=0),targets=options.points||rings(target).flat();
 if(options.screen&&options.snap!==false){let best=options.radius||10;for(const i of contacts)for(const targetPoint of targets){if(Math.abs(K.local(frame,targetPoint).z)>K.CONTACT)continue;const a=options.screen(points[i]),b=options.screen(targetPoint),pixels=Math.hypot(a.x-b.x,a.y-b.y);if(a.visible!==false&&b.visible!==false&&pixels<best&&Math.hypot(...Object.values(sub(targetPoint,points[i])))<.5){best=pixels;delta=sub(targetPoint,points[i]);snap={from:points[i],target:targetPoint};}}
...
for(const pair of clip.edges)if(pair.every(p=>Math.abs(K.local(source,p).z)<=K.CONTACT))for(let i=0;i<=20;i++)valid&&=inside(moved(mix3(pair[0],pair[1],i/20)));
 return {points,location:{x:location.x+delta.x,y:location.y+delta.y,z:location.z+delta.z},edges:clip.edges.map(pair=>pair.map(moved)),faces:clip.faces.map(f=>({...f,...K.mapCurveData(f,moved),points:f.points.map(moved),holes:f.holes.map(r=>r.map(moved)),retainedPoints:f.retainedPoints.map(moved),...(f.feature?.axis?{feature:{...f.feature,axis:sub(moved({x:source.origin.x+f.feature.axis.x,y:source.origin.y+f.feature.axis.y,z:source.origin.z+(f.feature.axis.z||0)}),moved(source.origin))}}:{})})),valid,snap};
}
...
function slideLines(faces,pairs,direction,amount){
 ...
 const result=faces.map(f=>{if(f.deleted)return f;
  ...
  const next={...f,points:replace(f.points),holes:(f.holes||[]).map(replace),retainedPoints:(f.retainedPoints||[]).map(p=>({...p,...(matching(p)?.to||{})}))};
...
function moveFabric(faces,id,amount){
 ...
 const moves=graph.vertices.filter(v=>v.faces.has(id)||v.anchors.some(p=>(source.retainedPoints||[]).some(q=>Math.hypot(q.x-p.x,q.y-p.y,q.z-p.z)<=K.CONTACT))).map(v=>v.point).map(p=>({from:p,to:{x:p.x+n.x*amount,y:p.y+n.y*amount,z:p.z+n.z*amount}}));
...
function moveSurface(faces,id,amount){
 ...
 const graph=K.topology(faces.filter(f=>!f.deleted),faces.flatMap(f=>f.retainedPoints||[]));
 for(const p of graph.vertices.filter(v=>v.faces.has(id)||v.anchors.some(p=>(original.retainedPoints||[]).some(q=>Math.hypot(q.x-p.x,q.y-p.y,q.z-p.z)<=K.CONTACT))).map(v=>v.point)){
...
function structuralIndex(faces){
 ...
 for(const f of faces.filter(f=>!f.deleted&&!f.drafted))for(const r of rings(f))for(let i=0;i<r.length;i++){
...
function structuralEdge(a,b,faces){return sharedIntervals(a,b,faces.filter(f=>!f.deleted&&!f.drafted)).length>0;}
function deleteSurfaceElements(edits,pointIds=[],edgeIds=[]){const points=new Set(pointIds),edges=new Set(edgeIds);edits.$removedSurfacePoints=[...new Set([...(edits.$removedSurfacePoints||[]),...points])];edits.$removedSurfaceEdges=[...new Set([...(edits.$removedSurfaceEdges||[]),...edges])];
 for(const f of edits.$surfaces||[]){if(f.drafted)continue;const selected=rings(f).flat().filter(p=>points.has(vertexKey(p))),defines=selected.some(p=>structuralPoint(p,[{...f,deleted:false,drafted:false}]));if(defines||rings(f).some(r=>r.some((p,i)=>edges.has(edgeKey(p,r[(i+1)%r.length])))))f.deleted=true;else if(selected.length){f.points=f.points.filter(p=>!points.has(vertexKey(p)));f.holes=(f.holes||[]).map(r=>r.filter(p=>!points.has(vertexKey(p))));}}
}
...
function removeFaceEdges(faces,segments){
 ...
 for(const [a,b] of segments){const boundary=faces.map((f,i)=>({f,i})).filter(({f})=>sharedIntervals(a,b,[f]).some(([lo,hi])=>hi-lo>1e-6));
  const incident=faces.map((f,i)=>({f,i})).filter(v=>boundary.some(b=>b.i===v.i)||(covers(a,b,v.f)&&boundary.some(b=>samePlane(b.f,v.f))));

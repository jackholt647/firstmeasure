...
window.createBaseEditor=function(host){
 ...
 function rebuildGrade(){
  ...
  const next=B.fitGrade(base(),state().ground),before=copy(base()),cuts=state().wallEdits?.$baseCuts;
  history.push({foundationBefore:before,baseCuts:cuts?copy(cuts):null});host.recordHistory?.({base:copy(before),wallEdits:copy(state().wallEdits||{})});state().base=next;if(state().wallEdits)delete state().wallEdits.$baseCuts;
  window.WallChimneys?.syncFoundation(state(),{force:true});select(null);message='Foundation rebuilt to grade';changed();
 }

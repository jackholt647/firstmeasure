...
function perf_snap(d,e,raw=null){
 ...
 function geometryCommand(mode,external){
  ...
  try{const curveOwner=curveGeometrySelection(points);if(curveOwner){const clip=curveOwner.clip;tool={kind:'geometryTransform',curveOwner:{key:curveOwner.key,base:curveOwner.base,ids:curveOwner.ids},snapTargets:curveOwner.snapTargets,before,prepared:copy(host.state().wallEdits),base:copy(host.state().wallEdits.$base||host.state().base),scene:[],original:copy(clip),clip,preview:copy(clip),mount:0,op:Date.now(),valid:true};geometryCommand(mode);return true;}
   const scene=moveScene().filter(f=>!f.snapOnly&&!f.deleted),base=host.state().wallEdits.$base||host.state().base;if(base&&(external||selectedBasePoints.length))scene.push(...base.faces.map(f=>({...f,baseId:f.id,id:'base:'+f.id})));
...
function perf_previewFeature(e){if(!F||viewOf(e)!=='3d')return;tool.preview=null;const ref=placementHost(e);if(!ref){host.redraw();return;}try{const frame=F.frame(ref.points),p=rayPoint({frame},e);if(!p)return;const preset={...F.defs.get(tool.type).sizes[tool.index]};if(tool.rotated)[preset.w,preset.h]=[preset.h,preset.w];const trim=(tool.trimInches||0)*.0254,door=tool.type==='door';if(trim){preset.w+=(door&&tool.rotated?trim:2*trim)/F.FT;preset.h+=(door&&!tool.rotated?trim:2*trim)/F.FT;if(door){if(tool.rotated)p.x-=trim/2;else p.y+=trim/2;}}const graph=sceneLines(),onPlane=p=>Math.abs(window.ExteriorGeometry.local(frame,p).z)<=window.ExteriorGeometry.CONTACT;
  ...
  const stickerScene=moveScene(),stickerTargets=W.stickerAlignmentTargets([...stickerScene,...stickerScene.flatMap(f=>F.trimFaces(f.points,f.feature).map(t=>({...t,feature:{type:'trim'}})))],frame);tool.alignmentTargets=stickerTargets;
  ...
  if(trim){const b=F.bounds(points);points.splice(0,points.length,...F.shape({left:b.left+trim,right:b.right-(door&&tool.rotated?0:trim),bottom:b.bottom+(door&&!tool.rotated?0:trim),top:b.top-trim}));}
 ...
 function placeFeature(e){previewFeature(e);if(!tool.preview)return true;const {ref,points,axis}=tool.preview,type=tool.type,index=tool.index,shape=F.defs.get(type).sizes[index].shape;const ok=transaction(()=>{const {d}=asDraft(ref),local=points.map(p=>toLocal(d,p));F.validate(local,d.sketch.outlines,d.faces.filter(f=>(f.feature||f.boundaryHole)&&!f.solidId&&!deleted(d,f)));W.importDraft(d,[local]);classify(d);const face=matchingRegion(d,local);if(!face)throw Error('The sticker crosses an existing divider.');face.feature=F.setTrim({type,preset:index,shape,axis},tool.trimInches||0,host.state()?.finishDefaults?.trimColor||'#f5f3ef');selectFeature(d,face);});if(ok&&!e.shiftKey)tool=null;host.redraw();return true;}
...
function perf_nudge(e){const step=(e.altKey ? .25 : e.shiftKey ? 6 : 1)*F.FT/12,dx=e.key==='ArrowRight'?step:e.key==='ArrowLeft'?-step:0,dy=e.key==='ArrowUp'?step:e.key==='ArrowDown'?-step:0;const points=selectedWorldPoints();if(geometrySelectionPoints().length>=3)return nudgeGeometry(dx,dy);if(points.length){if(points.length!==1){host.message('Select one point or a complete face to nudge.');return true;}return nudgePoint(e,step);}const selection=featureSelection();

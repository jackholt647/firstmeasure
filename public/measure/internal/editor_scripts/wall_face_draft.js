...
function perf_previewFeature(e){if(!F||viewOf(e)!=='3d')return;tool.preview=null;const ref=placementHost(e);if(!ref){host.redraw();return;}try{const frame=F.frame(ref.points),p=rayPoint({frame},e);if(!p)return;const preset={...F.defs.get(tool.type).sizes[tool.index]};if(tool.rotated)[preset.w,preset.h]=[preset.h,preset.w];const graph=stickerSnapGeometry(sceneLines(),moveScene()),onPlane=p=>Math.abs(window.ExteriorGeometry.local(frame,p).z)<=window.ExteriorGeometry.CONTACT;
  ...
  const stickerScene=moveScene().filter(f=>!f.trim&&f.feature?.type!=='trim'),stickerTargets=W.stickerAlignmentTargets(stickerScene,frame);tool.alignmentTargets=stickerTargets;
  const boundary=[ref.points.map(p=>W.inFrame(frame,p))],targets=[...boundary,...[...graph.lines.values()].filter(pair=>pair.every(onPlane)).map(pair=>pair.map(p=>W.inFrame(frame,p))),...graph.points.filter(onPlane).map(p=>[W.inFrame(frame,p)]),...stickerTargets.map(p=>[{...W.inFrame(frame,p),alignmentAxes:p.alignmentAxes,alignmentCenter:p.alignmentCenter}])],points=F.place(p,preset,targets,p=>host.screen(W.fromFrame(frame,p),'3d'),typeof isFreeMove!=='undefined'&&isFreeMove?0:12,boundary.map(points=>({points,holes:(ref.f?.holes||ref.solid?.holes||[]).map(r=>r.map(p=>W.inFrame(frame,ref.d?world(ref.d,p):p)))})));
...
function perf_nudge(e){const step=(e.altKey ? .25 : e.shiftKey ? 6 : 1)*F.FT/12,dx=e.key==='ArrowRight'?step:e.key==='ArrowLeft'?-step:0,dy=e.key==='ArrowUp'?step:e.key==='ArrowDown'?-step:0;const points=selectedWorldPoints();if(geometrySelectionPoints().length>=3)return nudgeGeometry(dx,dy);if(points.length){if(points.length!==1){host.message('Select one point or a complete face to nudge.');return true;}return nudgePoint(e,step);}const selection=featureSelection();
 ...
 function cycleStickerTrim(){
  const refs=selectedStickers().filter(({f})=>['window','door'].includes(f.feature?.type));if(!refs.length)return false;
  return transaction(()=>{for(const {f}of refs){const current=(f.feature.trim?.width||0)/.0254,index=F.trimSizes.findIndex(n=>Math.abs(n-current)<1e-6),width=F.trimSizes[(index+1)%F.trimSizes.length];f.feature=F.setTrim(f.feature,width,f.feature.trim?.color||host.state()?.finishDefaults?.trimColor||'#f5f3ef');}host.message('Sticker trim updated; T cycles off, 2, 3, 4 and 6 inches.');});
 }

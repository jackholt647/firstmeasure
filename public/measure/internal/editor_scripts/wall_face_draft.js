...
function perf_previewPaste(e){if(tool?.kind!=='paste'||!e||viewOf(e)!=='3d')return;const t=tool;t.preview=null;t.valid=false;
 ...
 function placePaste(e){previewPaste(e);const t=tool;if(!t?.valid||!t.preview)return true;try{const edits=host.state().wallEdits||={},preview=t.preview,prefix='paste-'+t.op;
   if(preview.faces.length&&preview.faces.every(f=>f.feature)){installStickers(t.ref,preview.faces);window.ExteriorModel.validateEdits(edits,t.before);host.commit(t.before);tool=null;clearGuides();host.message('Stickers placed. T cycles trim for the selected group.');host.redraw();return true;}

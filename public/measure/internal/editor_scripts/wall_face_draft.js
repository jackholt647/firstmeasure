...
window.createWallFaceDraft=function(host){
 ...
 function previewTrim(){const t=tool;host.state().wallEdits=copy(t.prepared);try{
  const result=window.WallTrim.partition(t.scene,t.pairs,t.variant,t.width,t.segments,t.finishColor);if(!result.replacements.length)throw Error('Select a straight line on a wall face for trim.');
 ...
 function selectTrimEdges(kind){
  ...
  const options={roof:state.roof,base:state.wallEdits.$base||state.base,ground:state.ground};
 ...
 function applyTrim(width=window.WallTrim.defaultWidth,finishColor,external=[]){
  if(tool&&!finishToolForSwitch())return false;
  const pairs=[...selectedTrimPairs(),...external];if(!pairs.length){host.message('Select wall or ground edges for trim.');return false;}
  const before=copy(host.state().wallEdits||{});
  try{const scene=moveScene().filter(f=>!f.snapOnly&&!f.deleted);tool={kind:'trim',before,prepared:copy(host.state().wallEdits),scene:copy(scene),pairs:copy(pairs),segments:copy(pairs),variant:0,width,finishColor};previewTrim();
   if(!tool.valid)throw Error('No trim could be applied to the selected edges.');
...
if(!d&&['n','q'].includes(k)&&selected())d=ensure(selected());if(!d)return false;
 ...
 function draw3D(group,vector){
  ...
  for(let d of Object.values(all())){retainSourcePoints(d);if(!d.faces.length&&!d.sketch.nodes.length&&!d.deletedFaces?.length){const w=walls().find(w=>d.members.includes(w.id));if(w)d=ensure(w);}if(!visibleDraft(d))continue;
   ...
   for(const source of d.faces.filter(f=>faceChosen({draft:draftKey(d),face:f.id})))for(const f of visibleDraftFaces(d,source)){if(!f.solidId&&!deleted(d,f))drawSelectionOutline(group,p=>vector(world(d,p)),f);}
 ...
 function featureDimensions(group,vector,points,feature,selected=false){if(!F||host.state()?.featureDimensions===false)return;const d=F.dimensions(points,feature),c={x:(d.bounds.left+d.bounds.right)/2,y:(d.bounds.bottom+d.bounds.top)/2,z:0},a=W.fromFrame(d.frame,{...c,x:d.bounds.left}),b=W.fromFrame(d.frame,{...c,x:d.bounds.right});window.wallLengthMarker?.(group,vector,{a,b,length:0,text:F.label(points,feature),selected,...(movingFace(points)?{centerY:1.65,horizontal:true}:{})});}

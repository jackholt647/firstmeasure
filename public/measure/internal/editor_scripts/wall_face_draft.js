...
window.createWallFaceDraft=function(host){
 ...
 const currentDraftId=()=>draftKey(current())||(selected()?id(selected()):null),visibleDraft=d=>!d.mergedInto&&(d.solidHost?solids().some(f=>f.id===d.solidHost&&!f.deleted):!!d.members?.length);
 ...
 function snap(d,e,raw=null){
  ...
  const result=(tool?.kind==='quad'&&W.quadDraftSnap(p,q=>quadPoints(d,q),nodes,edges,anchors,screen,radius))||W.draftSnap(p,nodes,edges,anchors,screen,radius);snapGuides=result.guides;showGuides(d,e);if(result.kind)host.message(result.kind);return result.point;
 }
 ...
 function deleteBoxSelection(){
  ...
  });if(!changed)return;draftSelection={};selectedBasePoints=[];picked=[];solidPoints=[];solidEdges=[];host.redraw();
 }
 ...
 function selectedWorldPoints(){
  const points=[],groups={...draftSelection},d=current();if(d)groups[draftKey(d)]=picked;
 ...
 function endBox(e){if(e&&e.button!==undefined&&e.button!==0)return;
  ...
  if(moved||(!b.lineClick&&!b.deferredPick))lineSelection=[];
  ...
  const inside=p=>{if(b.view==='3d'&&host.state()?.translucent===false&&host.pickVisible?.(p)===false)return false;const q=host.screen(p,b.view);return Number.isFinite(q.x)&&Number.isFinite(q.y)&&q.visible!==false&&q.x>=Math.min(b.x,b.ex)&&q.x<=Math.max(b.x,b.ex)&&q.y>=Math.min(b.y,b.ey)&&q.y<=Math.max(b.y,b.ey);};
  ...
  if(b.subtract){solidPoints=b.priorSolids.filter(id=>!wire().nodes.some(n=>n.id===id&&inside(n)));selectedBasePoints=b.priorBase.filter(id=>!basePoints().some(p=>W.vertexKey(p)===id&&inside(p)));}
 ...
 function selectRegion(d,e,w){
  if(w){activeDraftKey=null;host.select(w.id);}draftSelection={};picked=[];pickedLines=[];solidPoints=[];solidEdges=[];selectedSolid=null;const p=rayPoint(d,e),face=p&&d.faces.filter(f=>!deleted(d,f)&&!f.solidId&&G.contains(f,p)).sort((a,b)=>Number(b.opening)-Number(a.opening))[0];selectedRegion=face?{draft:w?id(w):draftKey(d),face:face.id}:null;if(face){preferredDraft=draftKey(d);preferredSolid=null;preferredRegion=face.id;}if(face)host.message(d.chimney?'Chimney wall selected · M adjusts its footprint':face.opening?'Opening selected':'Wall region selected');host.redraw();
 }
 ...
 function holdBoundary(e){if(host.state()?.displayMode==='textured'&&viewOf(e)==='3d')return false;
  let d=all()[preferredDraft];if(!d&&preferredSolid){const f=solids().find(f=>f.id===preferredSolid&&!f.deleted);if(f){const frame=W.faceFrame(f),local=p=>W.inFrame(frame,p);const near=f.points.some((p,i)=>{const a=host.screen(p,'3d'),b=host.screen(f.points[(i+1)%f.points.length],'3d'),dx=b.x-a.x,dy=b.y-a.y,l2=dx*dx+dy*dy,t=l2?Math.max(0,Math.min(1,((e.clientX-a.x)*dx+(e.clientY-a.y)*dy)/l2)):0;return Math.hypot(e.clientX-a.x-t*dx,e.clientY-a.y-t*dy)<12;});if(near&&viewOf(e)==='3d'&&(()=>{const p=rayPoint({frame},e);return p&&pickVisible(W.fromFrame(frame,p),e);})()){preferredDraft='solid:'+f.id;host.state().wallEdits.$drafts||={};d=all()[preferredDraft]||={frame,solidHost:f.id,members:[],faces:[{id:'surface-'+f.id,points:f.points.map(local)}]};S.ensure(d);for(const p of f.retainedPoints||[])if(!d.sketch.nodes.some(n=>Math.hypot(n.x-local(p).x,n.y-local(p).y)<.0001))S.add(d,local(p),.0001);for(const hole of f.holes||[])W.importDraft(d,[hole.map(local)]);f.drafted=true;}}}if(!d||!visibleDraft(d)||viewOf(e)!=='3d')return false;const hitPoint=rayPoint(d,e);if(hitPoint&&!pickVisible(world(d,hitPoint),e))return false;
 ...
 function pickSolid(e,pointsOnly=false){
  ...
  if(tool||!viewOf(e))return false;const graph=wire(),byId=id=>graph.nodes.find(n=>n.id===id);let nearest=null,distance=12;
  ...
  const r=renderer.domElement.getBoundingClientRect(),ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((e.clientX-r.left)/r.width*2-1,1-(e.clientY-r.top)/r.height*2),camera);for(const m of solidMeshes)m.updateMatrixWorld(true);const hit=ray.intersectObjects(solidMeshes)[0];if(!hit)return false;if(hit.object.userData.draftKey){const key=hit.object.userData.draftKey,d=all()[key];if(current()===d)return down(e,true);beginBox(e,d);box.click=()=>{const w=selected()||walls()[0];host.select(w?.id||null);activeDraftKey=key;selectedSolid=null;selectRegion(d,e);};return true;}const solidId=hit.object.userData.solidId,w=selected()||walls()[0];beginBox(e,current());box.click=()=>{draftSelection={};selectedRegion=null;selectedSolid=solidId;preferredSolid=solidId;preferredDraft=null;solidPoints=[];solidEdges=[];host.select(w?.id||null);picked=[];host.redraw();};return true;
 }
 ...
 function collectSceneLines(){
  const curveGroups=new Map(),graph=wire(),node=id=>graph.nodes.find(n=>n.id===id),segments=graph.edges.map(e=>[node(e.a),node(e.b)]),points=graph.nodes.filter(n=>!n.curveSample);
  ...
  const lines=new Map();for(const [a,b] of segments){const dx=b.x-a.x,dy=b.y-a.y,dz=b.z-a.z,l2=dx*dx+dy*dy+dz*dz;if(l2<1e-12)continue;const ts=[0,1];for(const p of points){const t=((p.x-a.x)*dx+(p.y-a.y)*dy+(p.z-a.z)*dz)/l2;if(t>1e-6&&t<1-1e-6&&Math.hypot(p.x-a.x-t*dx,p.y-a.y-t*dy,p.z-a.z-t*dz)<1e-5)ts.push(t);}ts.sort((a,b)=>a-b);const at=t=>({x:a.x+dx*t,y:a.y+dy*t,z:a.z+dz*t});for(let i=1;i<ts.length;i++)if(ts[i]-ts[i-1]>1e-6){const pair=[at(ts[i-1]),at(ts[i])];lines.set(W.edgeKey(...pair),pair);}}
 ...
 function pickLine3D(e){if(host.state()?.displayMode==='textured'&&viewOf(e)==='3d')return false;
 ...
 function finishToolForSwitch(){
  flushPreview();const t=tool;if(!t)return true;if(t.kind==='trim')return finishTrim();if(t.kind==='curve'){host.message('Finish the curve or press Escape before switching tools.');return false;}
 ...
 function geometrySelectionPoints(external){
  ...
  const d=current(),graph=wire(),points=selectedWorldPoints().concat(lineSelection.flatMap(l=>l.pair));const curved=solids().find(f=>f.id===selectedSolid&&f.curvedSurface?.logical);if(curved)points.push(...curved.points);
  for(const edge of graph.edges)if(solidEdges.includes(edge.id))points.push(...[edge.a,edge.b].map(id=>graph.nodes.find(n=>n.id===id)));
  if(d)for(const edge of draftSegments(d))if(pickedLines.includes(edge.id))points.push(world(d,edge.start),world(d,edge.end));
 ...
 function curveGeometrySelection(points){
  ...
  for(const owner of owners){const {d}=owner,toWorld=p=>owner.base?p:world(d,p),nodes=d.sketch?.nodes||[],ids=points.map(p=>nodes.find(n=>distance3(toWorld(n),p)<1e-5)?.id);if(!ids.length||ids.some(id=>!id))continue;
   const selected=new Set(ids),curves=(d.sketch.curves||[]).filter(c=>{const edges=d.sketch.edges.filter(e=>e.curveId===c.id);return edges.length&&edges.every(e=>selected.has(e.a)&&selected.has(e.b));});if(!curves.length)continue;
   ...
   const controls=nodes.filter(n=>selected.has(n.id));if(controls.some(n=>n.fixed))continue;const frame=owner.base?W.faceFrame(d.faces[0]):d.frame||W.faceFrame({points:d.sketch.outlines[0].map(toWorld)}),clip={points:controls.map(toWorld),edges:[],faces:[],curves:curves.map(c=>window.ExteriorGeometry.mapCurve(c,toWorld)),mounts:[{frame:W.clipboardFrame(geometryCenter({points:controls.map(toWorld)}),frame.n),count:controls.length}]};
 ...
 function applyCurveGeometry(t,candidate){const edits=copy(t.prepared),owner=t.curveOwner,d=owner.base?copy(t.base):edits.$drafts[owner.key],local=p=>owner.base?p:toLocal(d,p),curves=candidate.curves.map(c=>window.ExteriorGeometry.mapCurve(c,local));
  S.updateCurves(d,curves,owner.ids.map((id,i)=>({id,point:local(candidate.points[i])})));if(owner.base)edits.$base=d;else classify(d);window.ExteriorModel.validateEdits(edits,t.before);return edits;
 }
 ...
 function geometryStatus(){const t=tool;host.message(({m:'Move on plane',r:'Rotate',f:'Flip '+(t.flipAxis==='y'?'vertical':t.flipAxis==='x'?'horizontal':'original'),y:'Resize '+(t.resizeAxis||'all').toUpperCase()}[t.editMode]||'Move')+' · Plane '+(t.mount+1)+' / '+t.clip.mounts.length+' · '+(t.editMode==='f'?'T cycles vertical / horizontal / original on this plane; X/V chooses flip. ':t.editMode==='r'?'Move around the center; snapping catches 45° increments. ':'')+'M / R / T / Y switches tool; repeat M/R/Y cycles plane; click places; Escape restores.');}
 function geometryCommand(mode,external){
  if(mode==='t')mode='f';
  if(tool?.kind==='paste'||tool?.kind==='geometryTransform'){
   const t=tool;if(t.invalid){host.message('Adjust this preview or press Escape before switching.');return true;}
   const cycle=t.editMode===mode;
   if(cycle&&mode==='f'){t.flipAxis=t.flipAxis==='y'?'x':t.flipAxis==='x'?null:'y';t.startEvent=mouse;previewGeometry(mouse,true);return true;}
   if(t.kind==='geometryTransform')t.clip=copy(t.preview||t.clip);
   if(cycle)t.mount=(t.mount+1)%t.clip.mounts.length;
   t.rotationCenter=t.kind==='paste'&&t.preview?geometryCenter(t.preview):geometryCenter(t.clip);t.moveAnchor=t.kind==='paste'&&mode==='m'&&!cycle&&t.anchor?{...t.anchor,start:mouse&&rayPoint({frame:W.faceFrame(t.anchor.target)},mouse)}:null;if(mode==='y'&&t.editMode!=='y')t.resizeAxis='all';t.editMode=mode;t.stage=copy(t.clip);t.flipAxis='y';t.startEvent=mouse;
   t.inputFrame=t.kind==='paste'?t.anchor&&W.clipboardFrame(t.anchor.location,t.anchor.normal):{...t.clip.mounts[t.mount].frame,origin:geometryCenter(t.clip)};
   ...
   geometryStatus();showResizeAxes();return true;
  }
  if(tool)return false;
  if(external?.event)mouse=external.event;const points=geometrySelectionPoints(external);if(points.length<3)return false;
  const before=copy(host.state().wallEdits||{});
  try{const curveOwner=curveGeometrySelection(points);if(curveOwner){const clip=curveOwner.clip;tool={kind:'geometryTransform',curveOwner:{key:curveOwner.key,base:curveOwner.base,ids:curveOwner.ids},snapTargets:curveOwner.snapTargets,before,prepared:copy(host.state().wallEdits),base:copy(host.state().wallEdits.$base||host.state().base),scene:[],original:copy(clip),clip,preview:copy(clip),mount:0,op:Date.now(),valid:true};geometryCommand(mode);return true;}
   const scene=moveScene().filter(f=>!f.snapOnly&&!f.deleted),base=host.state().wallEdits.$base||host.state().base;if(base)scene.push(...base.faces.map(f=>({...f,baseId:f.id,id:'base:'+f.id})));
   const snapScene=sceneLines(),clip=W.copyGeometry(scene,points,[...snapScene.lines.values()]),selected=p=>clip.points.some(q=>distance3(p,q)<=window.ExteriorGeometry.CONTACT);const snapTargets={points:snapScene.points.filter(p=>!selected(p)),edges:[...snapScene.lines.values()].filter(pair=>pair.every(p=>!selected(p)))};
   tool={kind:'geometryTransform',snapTargets,before,prepared:copy(host.state().wallEdits),base:copy(base),scene:copy(scene),original:copy(clip),clip,preview:copy(clip),mount:0,op:Date.now(),valid:true};
   geometryCommand(mode);return true;
  }catch(error){host.state().wallEdits=before;host.message(error.message);return true;}
 }
 function previewGeometry(e,keepAxis=false){
  ...
  const q=rayPoint({frame:t.inputFrame},e),start=t.startPoint;if(!q||!start)return;
  const change={freezeMounts:t.kind==='paste'};
  if(t.editMode==='m')change.delta={x:q.x-start.x,y:q.y-start.y};
  if(t.editMode==='r'){
  ...
  let preview;if(t.editMode==='y'){let factor=Math.exp((e.clientX-t.startEvent.clientX-(e.clientY-t.startEvent.clientY))/160);factor=Math.max(.01,Math.min(100,factor));if(!(typeof isFreeMove!=='undefined'&&isFreeMove)){let targets=t.snapTargets,screen=p=>host.screen(p,'3d');if(t.kind==='paste'&&t.anchor){const src=t.stage.mounts[t.mount].frame,dst=W.clipboardFrame(t.anchor.location,t.anchor.normal),toSource=p=>window.ExteriorGeometry.world(src,window.ExteriorGeometry.local(dst,p)),toWorld=p=>window.ExteriorGeometry.world(dst,window.ExteriorGeometry.local(src,p)),scene=sceneLines();targets={points:scene.points.map(toSource),edges:[...scene.lines.values()].map(r=>r.map(toSource))};screen=p=>host.screen(toWorld(p),'3d');}if(targets)factor=W.snapGeometryScale(t.stage,t.mount,factor,t.resizeAxis||'all',targets.points,targets.edges,screen);}t.scaleFactor=factor;preview=W.geometryScale(t.stage,t.mount,factor,t.resizeAxis||'all');}else preview=W.transformGeometry(t.stage,t.mount,change);if(t.kind==='geometryTransform'&&t.editMode==='m'&&!(typeof isFreeMove!=='undefined'&&isFreeMove))preview=W.snapGeometryOnPlane(preview,t.mount,t.snapTargets.points,t.snapTargets.edges,p=>host.screen(p,'3d'));
  if(t.kind==='geometryTransform'&&t.editMode==='m'){
  ...
  const build=candidate=>{if(t.curveOwner)return applyCurveGeometry(t,candidate);const edits=copy(t.prepared),result=W.transformSelection(t.scene,t.original,candidate);applyLineResult(t,result,edits);window.ExteriorModel.validateEdits(edits,t.before);return edits;};
  try{let edits;try{edits=build(preview);}catch(error){
   if(t.editMode!=='m')throw error;
 ...
 function placeGeometry(){const t=tool;if(t?.kind!=='geometryTransform'||t.invalid)return true;try{if(t.changed)host.commit(t.before);else host.state().wallEdits=copy(t.before);tool=null;draftSelection={};picked=[];pickedLines=[];lineSelection=[];selectedBasePoints=[];selectedRegion=null;selectedSolid=null;solidPoints=t.preview.points.map(W.vertexKey);solidEdges=[];if(t.curveOwner){if(t.curveOwner.base){selectedBasePoints=solidPoints;solidPoints=[];}else{activeDraftKey=t.curveOwner.key;picked=t.curveOwner.ids.slice();draftSelection={[activeDraftKey]:picked.slice()};solidPoints=[];}}clearGuides();host.message('Geometry placed.');host.redraw();}catch(error){host.message(error.message);}return true;}
 ...
 function showResizeAxes(){const t=tool;if(t?.editMode!=='y'||typeof document==='undefined'||!document.body?.appendChild)return;
  ...
  for(const [axis,x,y]of [['all',8,78],['x',108,61],['y',56,8],['z',16,25]]){const button=document.createElement('button');button.textContent=axis.toUpperCase();button.dataset.axis=axis;button.title={all:'Resize all axes',x:'Width',y:'Height',z:'Depth from mounting face'}[axis];button.style.cssText='position:absolute;left:'+x+'px;top:'+y+'px;border:1px solid #8ba9b9;border-radius:5px;color:white;min-width:28px;height:27px;cursor:pointer;';button.onclick=e=>{e.preventDefault();e.stopPropagation();resizeAxis(axis);};panel.appendChild(button);}
  for(const event of ['pointerdown','mousedown','dblclick'])panel.addEventListener(event,e=>e.stopPropagation());panel.addEventListener('wheel',e=>{if(!e.ctrlKey)return;e.preventDefault();e.stopPropagation();const axes=['all','x','y','z'];resizeAxis(axes[(axes.indexOf(t.resizeAxis||'all')+(e.deltaY>0?1:3))%4]);},{passive:false});document.body.appendChild(panel);t.resizeHud=panel;showResizeAxes();
 ...
 function geometryKey(e){const k=e.key.toLowerCase(),t=tool;if(k==='f'&&!e.ctrlKey&&!e.metaKey){if(!e.repeat&&typeof isFreeMove!=='undefined'){isFreeMove=!isFreeMove;previewGeometry(mouse,true);}return true;}if(!e.ctrlKey&&!e.metaKey&&['m','r','t','y'].includes(k)){if(!e.repeat)geometryCommand(k);return true;}if(k==='escape'||k==='z'&&(e.ctrlKey||e.metaKey)){host.state().wallEdits=copy(t.before);tool=null;clearGuides();host.redraw();return true;}if(k==='enter')return t.kind==='paste'?placePaste(mouse):placeGeometry();if(t.editMode==='f'&&['x','v'].includes(k)){t.flipAxis=k==='v'?'y':'x';previewGeometry(mouse,true);return true;}return !['shift','control','alt','meta'].includes(k);}
 function clipboardCommand(kind,external){
  if(kind==='copy'){
   if(tool&&!finishToolForSwitch())return true;const selected=external?.points||selectedWorldPoints(),before=copy(host.state().wallEdits||{});
   try{const scene=moveScene().filter(f=>!f.snapOnly),base=host.state().wallEdits.$base||host.state().base;if(base)scene.push(...base.faces.map(f=>({...f,id:'base:'+f.id})));window.exteriorGeometryClipboard=W.copyGeometry(scene,selected,[...sceneLines().lines.values()]);const c=window.exteriorGeometryClipboard;host.message('Copied '+c.points.length+' points, '+c.edges.length+' lines and '+c.faces.length+' faces.');}catch(e){host.message(e.message);}finally{host.state().wallEdits=before;host.redraw();}return true;
  }
 ...
 function placePaste(e){previewPaste(e);const t=tool;if(!t?.valid||!t.preview)return true;try{const edits=host.state().wallEdits||={},preview=t.preview,prefix='paste-'+t.op;
   const faces=preview.faces.map((f,i)=>({...f,id:prefix+'-'+i}));edits.$surfaces=[...(edits.$surfaces||[]),...faces];
   ...
   window.ExteriorModel.validateEdits(edits,t.before);host.commit(t.before);tool=null;draftSelection={};picked=[];lineSelection=[];solidPoints=preview.points.map(W.vertexKey);selectedBasePoints=[];selectedSolid=null;selectedRegion=null;clearGuides();host.message('Geometry pasted.');host.redraw();
 ...
 function chamferCommand(external=null,rounded=false){
  if(tool&&!finishToolForSwitch())return true;
  const before=copy(host.state().wallEdits||{}),d=current(),graph=wire(),byId=id=>graph.nodes.find(n=>n.id===id);
 ...
 function drawChamfer(group,vector){
  ...
  for(const [index,m]of (tool.metrics||[]).entries()){
   ...
   m.shoulders.forEach((p,i)=>{const row=document.createElement('div');row.style.color=colors[i%colors.length];row.textContent=String.fromCharCode(65+i)+'  '+(m.angles[i]??0).toFixed(1)+'°  ·  '+(m.distances[i]/.3048).toFixed(2)+' ft';label.appendChild(row);});
   const widthRow=document.createElement('div');widthRow.textContent=(tool.pointMode?'Max width  ':'Width  ')+(chamferWidth(m)/.3048).toFixed(2)+' ft';widthRow.style.fontWeight='600';label.appendChild(widthRow);
   const middle=document.createElement('div');middle.textContent='Center  '+(centerDistance/.3048).toFixed(2)+' ft';label.appendChild(middle);
 ...
 function stepWheel(e){if(!e.ctrlKey)return false;if(['paste','geometryTransform'].includes(tool?.kind)&&tool.editMode==='y'&&viewOf(e)){const axes=['all','x','y','z'];resizeAxis(axes[(axes.indexOf(tool.resizeAxis||'all')+(e.deltaY>0?1:3))%4]);return true;}if(tool?.kind==='paste'&&tool.editMode==='m'&&viewOf(e)){tool.mount=(tool.mount+(e.deltaY>0?1:-1)+tool.clip.mounts.length)%tool.clip.mounts.length;if(mouse)previewPaste(mouse);return true;}if(chamferWheel(e))return true;if(tool?.kind!=='step'||!viewOf(e))return false;tool.sizeScale=window.WallSteps.wheelScale(tool.sizeScale,e);previewStep();return true;}
 ...
 function beginEntity(mode,external=null){
  ...
  if(!source&&!pair)return false;if(mode==='move'&&pair&&beginLineMove(true,{pair,event:mouse}))return true;
 ...
 function beginLineMove(sharedOnly=false,external=null){
  ...
  const before=copy(host.state().wallEdits||{}),base=copy(before.$base||host.state().base),scene=moveScene().filter(f=>!f.snapOnly&&!f.deleted);
  if(base)scene.push(...base.faces.map(f=>({...copy(f),id:'base:'+f.id,baseId:f.id})));
  ...
  tool={kind:'lineMove',before,stageBefore:copy(host.state().wallEdits),base,scene,sceneDrafts:copy(all()),snapGeometry:sceneLines(),heightPoints:heightPoints(),pairs:copy(pairs),originalPairs:copy(pairs),initialMouse:mouse,candidates:candidates.map(f=>({id:f.id,frame:W.faceFrame(f)})),planeIndex:0,amount:0,changed:false,op:Date.now()};
  setLinePlane();return true;
 }
 function setLinePlane(){const t=tool,frame=t.candidates[t.planeIndex].frame,[a,b]=t.pairs[0],u={x:b.x-a.x,y:b.y-a.y,z:b.z-a.z},length=Math.hypot(u.x,u.y,u.z),n=frame.n;
  t.direction={x:(n.y*u.z-n.z*u.y)/length,y:(n.z*u.x-n.x*u.z)/length,z:(n.x*u.y-n.y*u.x)/length};t.plane={frame};t.start=rayPoint(t.plane,mouse);t.baseAmount=0;t.amount=0;t.numeric=null;
  host.message('Move line - Face '+(t.planeIndex+1)+' / '+t.candidates.length+' - M changes face; either direction stays on this plane. Click to place; Escape cancels.');host.redraw();
 }
 ...
 function previewLineMove(e){
  const t=tool,p=rayPoint(t.plane,e);if(!p)return;if(!t.start){t.start=p;return;}
  ...
  if(t.navigation){t.navigation=false;t.start=p;t.baseAmount=t.amount||0;return;}
  let amount=t.numeric??((t.baseAmount||0)+(p.x-t.start.x)*du+(p.y-t.start.y)*dv);t.pathSnap=null;
  if(t.numeric==null&&!(typeof isFreeMove!=='undefined'&&isFreeMove)){t.pathSnap=W.motionSnap(t.pairs.flat(),t.pairs,t.direction,amount,t.scene,{heightPoints:t.heightPoints,points:t.snapGeometry.points,edges:[...t.snapGeometry.lines.values()],excludeMoving:true,screen:p=>host.screen(p,'3d'),radius:12,minPixelsPerMeter:33.33});if(t.pathSnap)amount=t.pathSnap.amount;}
  try{const result=W.slideLines(t.scene,t.pairs,t.direction,amount),edits=copy(t.stageBefore||t.before);edits.$drafts=copy(t.sceneDrafts);
 ...
 function applyLineResult(t,result,edits){
  if(edits.$loose){const move=p=>({...p,...(result.moves.find(m=>distance3(m.from,p)<1e-5)?.to||{})});edits.$loose={points:edits.$loose.points.map(move),edges:edits.$loose.edges.map(pair=>pair.map(move))};}
   ...
   for(const [key,d]of Object.entries(edits.$drafts||{}))if(!result.faces.some(f=>f.draftKey===key&&result.affected.includes(f.id)))for(const n of d.sketch.nodes){const p=world(d,n),move=result.moves.find(m=>Math.hypot(m.from.x-p.x,m.from.y-p.y,m.from.z-p.z)<1e-5);if(move){const q=d.frame?W.inFrame(d.frame,move.to):{x:(move.to.x-d.origin.x)*d.u.x+(move.to.y-d.origin.y)*d.u.y,y:move.to.z,z:0};Object.assign(n,q);}}
 ...
 function draftSegments(d){if(renderSegments?.has(d))return renderSegments.get(d);const result=computeDraftSegments(d);renderSegments?.set(d,result);return result;}
 ...
 function applyFabricMove(scene,id,amount){
  const result=W.moveFabric(scene.filter(f=>!f.snapOnly),id,amount),edits=host.state().wallEdits;
  ...
  return edits.$surfaces.find(f=>f.id===id);
 }
 function applySolidMove(scene,id,amount){
  const result=W.moveSurface(scene.filter(f=>!f.snapOnly),id,amount),edits=host.state().wallEdits;edits.$surfaces=[...solids().filter(f=>f.drafted),...result.faces.filter(f=>!f.draft)];
 ...
 function startQuad(){if(tool&&!finishToolForSwitch())return true;
  const before=copy(host.state().wallEdits||{});try{
   let d=current();if(!picked.length){const groups=Object.entries(draftSelection).filter(([,ids])=>ids.length);if(groups.length===1&&!solidPoints.length){activeDraftKey=groups[0][0];d=all()[activeDraftKey];picked=groups[0][1].slice();}}
 ...
 function quadPoints(d,p){const nodes=picked.map(id=>d.sketch.nodes.find(n=>n.id===id));if(nodes.length===2){const [a,b]=nodes,dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy),h=((p.x-a.x)*-dy+(p.y-a.y)*dx)/(len*len);return [a,b,{x:b.x-dy*h,y:b.y+dx*h,z:0},{x:a.x-dy*h,y:a.y+dx*h,z:0}];}
  const a=nodes[0];if(!a)return [];if(!tool.diagonal)return [a,{x:p.x,y:a.y,z:0},p,{x:a.x,y:p.y,z:0}];const b=tool.diagonal,c={x:(a.x+b.x)/2,y:(a.y+b.y)/2},radius=Math.hypot(a.x-b.x,a.y-b.y)/2,l=Math.hypot(p.x-c.x,p.y-c.y)||1,v={x:(p.x-c.x)*radius/l,y:(p.y-c.y)*radius/l};return [a,{x:c.x+v.x,y:c.y+v.y,z:0},b,{x:c.x-v.x,y:c.y-v.y,z:0}];
 }
 ...
 function downUnsafe(e,skipSolid=false){
  ...
  if(tool){if(['move','scale','rotate'].includes(tool.kind)){host.commit(tool.before);tool=null;clearGuides();host.redraw();return true;}
   const p=snap(d,e);if(p){const placed=transaction(()=>{const next=add(d,p);for(const from of picked)if(from!==next)S.connect(d,[from,next]);picked=[next];});if(placed){tool=null;clearGuides();}host.redraw();}return true;
  }
 ...
 function structuralFaces(includeConsumed=false){
  ...
  for(const d of Object.values(all()))if(visibleDraft(d)&&!d.mergedInto){
   ...
   for(const points of B.boundary(filled)){const holes=B.boundary(W.subtract({points},filled.map(points=>({points}))));faces.push(engineFace(d,{points,holes},{retainedPoints:[]}));}
  }
 ...
 function removeDraftPoints(d,ids){healCoplanarPointDeletion(d.sketch.nodes.filter(n=>ids.includes(n.id)).map(n=>world(d,n)));
 ...
 function solidDraft(f){if(f.curvedSurface?.logical)throw Error('Select the curved surface to move, rotate or resize it; its curved boundaries remain linked.');const key='solid:'+f.id;host.state().wallEdits.$drafts||={};let d=all()[key];if(!d){const frame=F.frame(f.points);d={frame,joinedChimneys:f.joinedChimneys&&copy(f.joinedChimneys),chimney:f.chimney&&copy(f.chimney),solidHost:f.id,members:[],faces:[{id:'surface-'+f.id,points:f.points.map(p=>W.inFrame(frame,p)),...(f.material?{material:f.material}:{}),finishColor:f.finishColor,trim:f.trim,...window.ExteriorGeometry.mapCurveData(f,p=>W.inFrame(frame,p)),...(f.feature?{feature:copy(f.feature)}:{})}]};S.ensure(d);all()[key]=d;for(const r of f.holes||[])W.importDraft(d,[r.map(p=>W.inFrame(frame,p))]);// Retained anchors from an earlier extrusion can outlive their old face height.
 ...
 function selectedFinishFace(){if(selectedWorldPoints().length)return null;return selectedSolid?solids().find(f=>f.id===selectedSolid&&!f.deleted):selectedRegion?all()[selectedRegion.draft]?.faces.find(f=>f.id===selectedRegion.face&&!f.boundaryHole):null;}
 ...
 function applySelectedFinish(changes){const face=selectedFinishFace();if(!face||face.feature)return false;const ok=transaction(()=>updateFaceFinish(face,changes));if(ok){tool=null;host.message('Selected face finish updated.');}return true;}
 ...
 function materialColor(f){return host.state()?.materialColors!==false&&(f.finishColor||window.ExteriorMaterials?.[f.material]?.color);}
 ...
 function selectFeature(d,f){activeDraftKey=draftKey(d);preferredDraft=activeDraftKey;preferredSolid=null;preferredRegion=f.id;selectedRegion={draft:activeDraftKey,face:f.id};selectedSolid=null;picked=[];pickedLines=[];solidPoints=[];solidEdges=[];lineSelection=[];draftSelection={};}
 ...
 function editRegion(d,f,points){F.validate(points,d.sketch.outlines,d.faces.filter(o=>o!==f&&(o.feature||o.boundaryHole)&&!o.solidId&&!deleted(d,o)));B.validate({points});const meta=f.feature&&copy(f.feature),old=copy(d.faces);
  ...
  restoreMovedRegions(d,old);classify(d);const next=matchingRegion(d,points);if(!next)throw Error('This edit would cross a divider or split the feature.');if(meta)next.feature=meta;selectFeature(d,next);return next;
 }
 ...
 function featureCommand(type,index,place=false){if(!F)return false;if(type==='none'){if(tool&&!finishToolForSwitch())return true;const selection=featureSelection();if(!selection)return false;return transaction(()=>{delete (selection.f||selection.solid).feature;});}const def=F.defs.get(type);if(!def)return false;
  if(tool?.kind==='feature'){const same=tool.type===type;tool.type=type;tool.index=Number.isInteger(index)?index:same?(tool.index+1)%def.sizes.length:0;if(mouse)previewFeature(mouse);return true;}
  if(tool&&!finishToolForSwitch())return true;const selection=!place&&featureSelection();if(selection){return transaction(()=>{if(index!==null&&(selection.feature?.type===type||Number.isInteger(index))){resizeFeature(selection,type,Number.isInteger(index)?index:((selection.feature?.preset??-1)+1)%def.sizes.length);}else{const f=selection.f||selection.solid;f.feature={type,preset:null,shape:f.feature?.shape||'custom',axis:f.feature?.axis||F.orientedFrame(selection.points).u};host.message(def.name+' - current dimensions kept; press '+(def.key?.toUpperCase()||'the library button')+' with Ctrl again for common sizes.');}});}
  tool={kind:'feature',type,index:Number.isInteger(index)?index:0,preview:null};host.message('Place '+def.name+' on a face. Use the library to cycle sizes; R rotates; Shift-click keeps placing.');if(mouse)previewFeature(mouse);host.redraw();return true;
 }
 function placementHost(e,includeFeatures=false){if(host.featureHost)return host.featureHost(e);if(solidMeshes.length&&typeof THREE!=='undefined'){const r=renderer.domElement.getBoundingClientRect(),ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((e.clientX-r.left)/r.width*2-1,1-(e.clientY-r.top)/r.height*2),camera);for(const m of solidMeshes)m.updateMatrixWorld(true);const hit=ray.intersectObjects(solidMeshes)[0];if(hit){if(hit.object.userData.draftKey){const d=all()[hit.object.userData.draftKey],p=rayPoint(d,e),f=d.faces.find(f=>!f.solidId&&(includeFeatures||!f.feature)&&!deleted(d,f)&&G.contains(f,p));if(f)return {d,f,points:f.points.map(p=>world(d,p))};}const f=solids().find(f=>f.id===hit.object.userData.solidId);if(f&&(includeFeatures||!f.feature))return {solid:f,points:f.points};}}
  const w=host.hit?.(e);if(w){const d=ensure(w),p=rayPoint(d,e),f=d.faces.find(f=>(includeFeatures||!f.feature)&&!f.solidId&&!deleted(d,f)&&G.contains(f,p));if(f)return {d,f,points:f.points.map(p=>world(d,p))};}return null;
 }
 function previewFeature(e){if(!F||viewOf(e)!=='3d')return;tool.preview=null;const ref=placementHost(e);if(!ref){host.redraw();return;}try{const frame=F.frame(ref.points),p=rayPoint({frame},e);if(!p)return;const preset={...F.defs.get(tool.type).sizes[tool.index]};if(tool.rotated)[preset.w,preset.h]=[preset.h,preset.w];const boundary=[ref.points.map(p=>W.inFrame(frame,p))],points=F.place(p,preset,boundary,p=>host.screen(W.fromFrame(frame,p),'3d'));
  F.validate(points,boundary,(ref.f?.holes||ref.solid?.holes||[]).map(r=>({points:r.map(p=>W.inFrame(frame,ref.d?world(ref.d,p):p))})));const worldPoints=points.map(p=>W.fromFrame(frame,p));tool.preview={ref,points:worldPoints,axis:tool.rotated?frame.v:frame.u};host.message(F.defs.get(tool.type).name+' '+F.label(worldPoints)+' - click to place');}catch(error){host.message(error.message);}host.redraw();}
 function placeFeature(e){previewFeature(e);if(!tool.preview)return true;const {ref,points,axis}=tool.preview,type=tool.type,index=tool.index,shape=F.defs.get(type).sizes[index].shape;const ok=transaction(()=>{const {d}=asDraft(ref),local=points.map(p=>toLocal(d,p));F.validate(local,d.sketch.outlines,d.faces.filter(f=>(f.feature||f.boundaryHole)&&!f.solidId&&!deleted(d,f)));W.importDraft(d,[local]);classify(d);const face=matchingRegion(d,local);if(!face)throw Error('The sticker crosses an existing divider.');face.feature={type,preset:index,shape,axis};selectFeature(d,face);});if(ok&&!e.shiftKey)tool=null;host.redraw();return true;}
 ...
 function cycleFaceMove(e){const t=tool;if(e.repeat)return;host.state().wallEdits=copy(t.before);host.state().wallEdits.$drafts=copy(t.sceneDrafts);selectedSolid=t.priorSolid||null;activeDraftKey=t.priorDraft||null;selectedRegion=copy(t.priorRegion);t.moveMode=(t.moveMode+1)%4;t.numeric=null;t.changed=false;t.amount=0;t.baseAmount=0;t.previewCap=null;t.pathSnap=null;t.contained=null;t.belowBase=null;t.roofSnap=null;t.mergeSeams=[];t.invalid=false;t.startX=mouse.clientX;t.startY=mouse.clientY;t.axis=extrudeAxis(t.face);t.moveFrame=F.viewFrame(t.face.points,p=>host.screen(p,'3d'));t.planeStart=rayPoint({frame:t.moveFrame},mouse);moveModeStatus();host.redraw();}
 function previewFaceSlide(e){const t=tool; t.pathSnap=null;const frame=t.moveFrame,raw=rayPoint({frame},e);if(!raw||!t.planeStart){t.invalid=true;host.message('Turn toward the face to move along it.');return;}let dx=t.moveMode===2?0:raw.x-t.planeStart.x,dy=t.moveMode===1?0:raw.y-t.planeStart.y;if(t.numeric!=null){if(t.moveMode===1)dx=t.numeric;else dy=t.numeric;}t.amount=t.moveMode===1?dx:dy;
  if(t.numeric==null&&!(typeof isFreeMove!=='undefined'&&isFreeMove)){const axes=t.moveMode===1?['u']:t.moveMode===2?['v']:['u','v'];for(const axis of axes){const amount=axis==='u'?dx:dy,snap=W.motionSnap(t.face.points,t.face.points.map((p,i)=>[p,t.face.points[(i+1)%t.face.points.length]]),frame[axis],amount,t.scene,{points:t.snapGeometry.points,edges:[...t.snapGeometry.lines.values()],heightPoints:t.heightPoints,excludeMoving:true,screen:p=>host.screen(p,'3d'),radius:10,minPixelsPerMeter:33.33});if(snap){if(axis==='u')dx=snap.amount;else dy=snap.amount;t.pathSnap=snap;}}}
 ...
 function finishAxisCut(){if(tool?.kind!=='axisCut')return false;if(!tool.valid)return false;const t=tool;host.commit(t.before);tool=null;host.redraw();return true;}
 ...
 function buildAxisCut(p,k,preferredBase=null){
  ...
  try{for(const w of cutWalls){const face={points:[...w.bottom,...w.top.slice().reverse()]},fr=W.faceFrame(face);if(!fr)continue;const local=W.inFrame(fr,p);if(Math.abs((p.x-fr.origin.x)*fr.n.x+(p.y-fr.origin.y)*fr.n.y+(p.z-fr.origin.z)*fr.n.z)<.002&&G.contains({points:face.points.map(q=>W.inFrame(fr,q))},local))ensure(w);}}finally{renderWalls=previousWalls;}
  for(const f of solids().filter(f=>!f.deleted&&!f.drafted)){const fr=W.faceFrame(f);if(fr&&Math.abs(fr.n.z)<1e-5&&Math.abs((p.x-fr.origin.x)*fr.n.x+(p.y-fr.origin.y)*fr.n.y+(p.z-fr.origin.z)*fr.n.z)<.002&&G.contains({points:f.points.map(q=>W.inFrame(fr,q)),holes:(f.holes||[]).map(r=>r.map(q=>W.inFrame(fr,q)))},W.inFrame(fr,p)))solidDraft(f);}
  for(const [key,d]of Object.entries(all()))for(const f of d.faces.filter(f=>!f.solidId&&!deleted(d,f))){const face={points:f.points.map(q=>world(d,q)),holes:(f.holes||[]).map(r=>r.map(q=>world(d,q)))},n=W.normal(face.points);if(!n||Math.abs(n.z)>1e-5||Math.abs((p.x-face.points[0].x)*n.x+(p.y-face.points[0].y)*n.y+(p.z-face.points[0].z)*n.z)>.002||!G.contains(f,toLocal(d,p)))continue;const nodes=d.sketch.nodes.map(q=>world(d,q)),edges=d.sketch.edges.map(e=>[e.a,e.b].map(id=>world(d,d.sketch.nodes.find(n=>n.id===id)))),targets=A.wall(face,p,k,edges,nodes);if(targets.length)candidates.push({key,face:f.id,targets});}
  if(k==='h'&&base){S.ensure(base);for(const f of base.faces){const fr=W.faceFrame(f);if(!fr||Math.abs((p.x-fr.origin.x)*fr.n.x+(p.y-fr.origin.y)*fr.n.y+(p.z-fr.origin.z)*fr.n.z)>.002)continue;const local=q=>W.inFrame(fr,q),face={points:f.points.map(local)},origin=A.boundaryPoint(face,local(p));if(!G.contains(face,origin))continue;let direction;try{direction=A.perpendicular(face,origin);}catch{continue;}const nodes=base.sketch.nodes.map(n=>{const q=f.points.find(q=>q.nodeId===n.id);return q?{...n,z:q.z}:n;}).filter(n=>Math.abs((n.x-fr.origin.x)*fr.n.x+(n.y-fr.origin.y)*fr.n.y+(n.z-fr.origin.z)*fr.n.z)<.002),byId=id=>nodes.find(n=>n.id===id),edges=base.sketch.edges.map(e=>[byId(e.a),byId(e.b)]).filter(e=>e.every(Boolean)),targets=A.targets(face,origin,direction,edges.map(e=>e.map(local)),nodes.map(local),.001).map(q=>W.fromFrame(fr,q));if(targets.length)candidates.push({base:true,face:f.id,start:W.fromFrame(fr,origin),targets});}}
  candidates.sort((a,b)=>Number(b.base?b.face===preferredBase:b.key===preferredDraft&&b.face===preferredRegion)-Number(a.base?a.face===preferredBase:a.key===preferredDraft&&a.face===preferredRegion)||(k==='v'?Math.max(...b.targets.map(q=>q.z))-Math.max(...a.targets.map(q=>q.z)):0));
 ...
 function previewAxisCut(){const t=tool;host.state().wallEdits=copy(t.preparedEdits);t.lines=[];try{for(const c of t.variants[t.index]){if(c.base){const base=host.state().wallEdits.$base||copy(t.base),start=S.add(base,c.start||t.start,.001);for(const p of c.targets){const end=S.add(base,p,.001);if(end!==start)S.connect(base,[start,end]);t.lines.push([c.start||t.start,p]);}host.state().wallEdits.$base=base;}else{const d=all()[c.key],old=copy(d.faces),start=add(d,toLocal(d,t.start));for(const p of c.targets){const end=add(d,toLocal(d,p));if(end!==start)S.connect(d,[start,end]);t.lines.push([t.start,p]);}restoreMovedRegions(d,old);classify(d);}}t.valid=true;const choices=t.variants[t.index];host.message((t.axis==='v'?'Vertical':'Horizontal')+' cut: '+(choices.length>1?'all adjoining faces':choices[0].base?'base face':'wall face')+' · '+t.axis.toUpperCase()+' cycles; click to place; Escape cancels.');}catch(error){host.state().wallEdits=copy(t.before);t.valid=false;tool=null;host.message('Cut not created: '+error.message);}host.redraw();}
 function nudgePoint(e,step){
  const points=selectedWorldPoints(),source=points.length===1?points[0]:null;
  ...
  for(const pair of sceneLines().lines.values())if(onSegment(source,...pair))for(const end of pair){const length=distance3(source,end);if(length<1e-5)continue;const b=host.screen(end,'3d'),pixels=Math.hypot(b.x-a.x,b.y-a.y);if(pixels<1e-5)continue;const score=((b.x-a.x)*sx+(b.y-a.y)*sy)/pixels;if(score>.2)candidates.push({end,length,score});}
  candidates.sort((a,b)=>b.score-a.score||a.length-b.length);const c=candidates[0];if(!c){host.message('No connected line runs in that screen direction.');return true;}
  ...
  const ok=transaction(()=>{const scene=moveScene().filter(f=>!f.snapOnly&&!f.deleted),base=copy(host.state().wallEdits.$base||host.state().base);if(base)for(const f of base.faces)scene.push({...copy(f),id:'base:'+f.id,baseId:f.id});
   const result=W.slideLines(scene,[[source,source]],direction,amount),edits=copy(host.state().wallEdits);applyLineResult({base,op:Date.now()},result,edits);host.state().wallEdits=edits;
  });
  if(ok){selectPlacedEntity({source,preview:[point]},false);host.redraw();}return true;
 }
 function nudge(e){const step=(e.altKey ? .25 : e.shiftKey ? 6 : 1)*F.FT/12,dx=e.key==='ArrowRight'?step:e.key==='ArrowLeft'?-step:0,dy=e.key==='ArrowUp'?step:e.key==='ArrowDown'?-step:0;const points=selectedWorldPoints();if(points.length){if(points.length!==1){host.message('Select one point to nudge along its connected line.');return true;}return nudgePoint(e,step);}const selection=featureSelection();
  if(lineSelection.length){const pairs=copy(lineSelection);return transaction(()=>{
    ...
    const frame=F.viewFrame(face.points.map(p=>world(d,p)),p=>host.screen(p,'3d')),delta={x:frame.u.x*dx+frame.v.x*dy,y:frame.u.y*dx+frame.v.y*dy,z:frame.u.z*dx+frame.v.z*dy},before=copy(d.faces),moves=new Map();
    for(const n of d.sketch.nodes){const p=world(d,n);if(!pairs.some(l=>onSegment(p,...l.pair)))continue;const q=toLocal(d,{x:p.x+delta.x,y:p.y+delta.y,z:p.z+delta.z});if(!d.sketch.outlines.some(points=>G.contains({points},q)))throw Error('Keep the line inside its supporting face.');moves.set(n.id,q);}
    if(!moves.size)throw Error('The selected line no longer belongs to this face. Select it again.');
    // Standalone line endpoints and retained split points need not be face vertices.
    for(const f of before.filter(f=>live(d,f)))if(f.points.some(p=>moves.has(p.nodeId))&&!F.containsShape(f.points.map(p=>moves.get(p.nodeId)||p),d.sketch.outlines))throw Error('Keep the line inside its supporting face.');
    for(const n of d.sketch.nodes)if(moves.has(n.id))Object.assign(n,moves.get(n.id));
    S.resolve(d);restoreMovedRegions(d,before);classify(d);for(const f of d.faces.filter(f=>f.feature))F.validate(f.points,d.sketch.outlines,d.faces.filter(o=>o!==f&&o.feature));
 ...
 function previewTrim(){const t=tool;host.state().wallEdits=copy(t.prepared);try{
  const result=window.WallTrim.partition(t.scene,t.pairs,t.variant,t.width,t.segments);if(!result.replacements.length)throw Error('Select a straight line on a wall face for trim.');
 ...
 function autoTrim(){
  ...
  try{
   ...
   tool={kind:'trim',before,prepared:copy(state.wallEdits),scene:copy(scene),pairs,segments:pairs,variant:0,width:window.WallTrim.defaultWidth};previewTrim();
 ...
 function trimCommand(){
  ...
  const d=current(),graph=wire(),byId=id=>graph.nodes.find(n=>n.id===id),pairs=[...lineSelection.map(l=>l.pair),...solidEdges.map(id=>graph.edges.find(e=>e.id===id)).filter(Boolean).map(e=>[byId(e.a),byId(e.b)]),...(d?pickedLines.map(id=>d.sketch.edges.find(e=>e.id===id)).filter(e=>e&&!e.curveId).map(e=>[world(d,d.sketch.nodes.find(n=>n.id===e.a)),world(d,d.sketch.nodes.find(n=>n.id===e.b))]):[])];if(!pairs.length)return false;
...
if(!d&&['n','q'].includes(k)&&selected())d=ensure(selected());if(!d)return false;
 ...
 function movePointer(e){if(tool?.kind==='curveMove'){const t=tool,p=rayPoint(current(),e);if(!p)return;host.state().wallEdits=copy(t.before);try{S.move(all()[t.draftKey],t.ids,{x:p.x-t.start.x,y:p.y-t.start.y});t.changed=true;}catch(error){host.message(error.message);}host.redraw();return;}
 ...
 function flushPreview(){const pending=pendingPreview;pendingPreview=null;if(previewFrame!==null){cancelAnimationFrame(previewFrame);previewFrame=null;}if(pending&&pending.tool===tool)runPointer(pending.event);}
 ...
 window.addEventListener('pointermove',e=>{
  if(tool&&['extrude','entityExtrude','lineMove'].includes(tool.kind)&&viewOf(e)&&!(e.buttons&6)){
   e.preventDefault();e.stopImmediatePropagation();pendingPreview={tool,event:e};if(previewFrame===null)previewFrame=requestAnimationFrame(flushPreview);
 ...
 function visibleDraftFaces(d,f){
  ...
  if(!host.state()?.chimneys?.items?.length)return [f];
  const face=engineFace(d,f);
  return window.WallChimneys.visibleParts(face,host.state()).map(part=>({...f,points:part.points.map(p=>toLocal(d,p)),holes:(part.holes||[]).map(r=>r.map(p=>toLocal(d,p)))}));
 }
 function draw3D(group,vector){
  ...
  for(let d of Object.values(all())){retainSourcePoints(d);if(!d.faces.length&&!d.sketch.nodes.length&&!d.deletedFaces?.length){const w=walls().find(w=>d.members.includes(w.id));if(w)d=ensure(w);}if(!visibleDraft(d))continue;
   ...
   for(const source of d.faces)for(const f of visibleDraftFaces(d,source)){if(f.boundaryHole||f.solidId||deleted(d,f))continue;const chosen=!selectedWorldPoints().length&&current()===d&&selectedRegion?.face===f.id,opening=inner.includes(source),holes=f.holes?.length?f.holes:opening?[]:inner.filter(h=>h!==source&&containsRegion(f,h)).map(h=>h.points),polys=[f.points,...holes],points=polys.flat(),tri=window.ExteriorGeometry.triangles(polys[0],holes).triangles;
    const geo=new THREE.BufferGeometry().setFromPoints(points.map(p=>vector(world(d,p))));geo.setIndex(tri.flat());const mesh=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:f.feature&&F?.defs.get(f.feature.type)?.color||materialColor(f)|| (chosen?'#fff0b0':d.chimney?window.WallChimneys.COLOR:opening?'#ff962f':'#ffd84d'),side:THREE.DoubleSide,transparent:true,opacity:chosen ? .85 : opening ? .65 : .3,depthWrite:false}));mesh.userData.pickLayer='walls';mesh.userData.exteriorSelected=chosen;window.ExteriorFinishes?.prepare(mesh,{...f,points:f.points.map(p=>world(d,p))},points.map(p=>world(d,p)));group.add(mesh);if(f.feature)featureDimensions(group,vector,f.points.map(p=>world(d,p)),f.feature);else if(!f.curvedSurface&&host.state()?.wallCenters!==false)window.wallCenterMarker?.(group,vector,world(d,B.center(f)));if(d.frame||f.feature||host.state()?.chimneys?.items?.length||!d.members.some(id=>walls().some(w=>w.id===id))){mesh.userData.draftKey=draftKey(d);solidMeshes.push(mesh);}
   }
   ...
   if(current()===d&&selectedRegion){const f=d.faces.find(f=>f.id===selectedRegion.face);if(f&&!f.solidId&&!deleted(d,f))for(const ring of [f.points,...(f.holes||[])]){const geo=new THREE.BufferGeometry().setFromPoints([...ring,ring[0]].map(p=>vector(world(d,p))));group.add(new THREE.Line(geo,new THREE.LineBasicMaterial({color:'#fff',depthTest:false})));}}
   for(const n of draftNodes(d)){const geo=new THREE.BufferGeometry().setFromPoints([vector(world(d,n))]);group.add(new THREE.Points(geo,new THREE.PointsMaterial({color:isPicked(d,n.id)?'#fff':structuralDraftPoint(d,n)?'#e7ad52':'#6ce4ed',size:isPicked(d,n.id)?12:8,sizeAttenuation:false,depthTest:false})));}
 ...
 function previewLine(group,vector,a,b,color,depthTest=false){
  const intervals=W.sharedIntervals(a,b,(tool?.mergeSeams||[]).map(points=>({points}))),at=t=>({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t,z:a.z+(b.z-a.z)*t});let start=0;
  const draw=(lo,hi,dashed)=>{if(hi-lo<1e-7)return;const geo=new THREE.BufferGeometry().setFromPoints([at(lo),at(hi)].map(vector)),material=dashed?new THREE.LineDashedMaterial({color:'#65e6ff',dashSize:.12,gapSize:.08,depthTest:false}):new THREE.LineBasicMaterial({color,depthTest}),line=new THREE.Line(geo,material);if(dashed)line.computeLineDistances();group.add(line);};
 ...
 function featureDimensions(group,vector,points,feature){if(!F||host.state()?.featureDimensions===false)return;const d=F.dimensions(points,feature),c={x:(d.bounds.left+d.bounds.right)/2,y:(d.bounds.bottom+d.bounds.top)/2,z:0},a=W.fromFrame(d.frame,{...c,x:d.bounds.left}),b=W.fromFrame(d.frame,{...c,x:d.bounds.right});window.wallLengthMarker?.(group,vector,{a,b,length:0,text:F.label(points,feature),...(movingFace(points)?{centerY:1.65,horizontal:true}:{})});}
 ...
 function drawLengths(group,vector){
  ...
  else if(tool.kind==='lineMove'&&tool.changed){const seen=new Map();for(const line of lineSelection)for(const edge of W.attachedLengths({points:line.pair},structuralFaces()))seen.set(W.edgeKey(edge.a,edge.b),edge);edges=[...seen.values()];}
 ...
 function drawSolids(group,vector){
  ...
  if(tool?.kind==='feature'&&tool.preview){const points=tool.preview.points,frame=F.frame(points),geo=new THREE.BufferGeometry().setFromPoints(points.map(vector));geo.setIndex(THREE.ShapeUtils.triangulateShape(points.map(p=>{const q=W.inFrame(frame,p);return new THREE.Vector2(q.x,q.y);}),[]).flat());group.add(new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:F.defs.get(tool.type).color,side:THREE.DoubleSide,transparent:true,opacity:.7,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2})));for(let i=0;i<points.length;i++)previewLine(group,vector,points[i],points[(i+1)%points.length],'#fff');featureDimensions(group,vector,points,{axis:tool.preview.axis});}
  ...
  for(const source of solids())for(const f of (window.WallChimneys?.visibleParts(source,host.state())||[source])){if(f.deleted||f.drafted)continue;if(f.curvedSurface?.logical){const data=window.ExteriorGeometry.surfaceMesh(f),geo=new THREE.BufferGeometry().setFromPoints(data.positions.map(vector));geo.setIndex(data.triangles.flat());if(THREE.Float32BufferAttribute&&geo.setAttribute)geo.setAttribute('uv',new THREE.Float32BufferAttribute(data.uv.flatMap(p=>[p.x,p.y]),2));geo.computeVertexNormals?.();const mesh=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:materialColor(f)||(f.id===selectedSolid?'#fff0b0':'#ffd84d'),side:THREE.DoubleSide,transparent:true,opacity:.5,depthWrite:false}));mesh.userData.solidId=f.id;mesh.userData.exteriorSelected=f.id===selectedSolid;mesh.userData.curvedSurface=true;window.ExteriorFinishes?.prepare(mesh,f,data.positions,data.uv);group.add(mesh);solidMeshes.push(mesh);continue;}const frame=W.faceFrame(f);if(!frame)continue;const {origin,u,v}=frame,local=p=>new THREE.Vector2((p.x-origin.x)*u.x+(p.y-origin.y)*u.y+(p.z-origin.z)*u.z,(p.x-origin.x)*v.x+(p.y-origin.y)*v.y+(p.z-origin.z)*v.z),polys=[f.points,...(f.holes||[])];const geo=new THREE.BufferGeometry().setFromPoints(polys.flat().map(vector));geo.setIndex(window.ExteriorGeometry.triangles(polys[0].map(local),polys.slice(1).map(h=>h.map(local))).triangles.flat());const mesh=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:f.feature&&F?.defs.get(f.feature.type)?.color||materialColor(f)||(f.id===selectedSolid?'#fff0b0':f.chimney?window.WallChimneys.COLOR:'#ffd84d'),side:THREE.DoubleSide,transparent:true,opacity:.5,depthWrite:false}));mesh.userData.solidId=f.id;mesh.userData.exteriorSelected=f.id===selectedSolid;window.ExteriorFinishes?.prepare(mesh,f,polys.flat());group.add(mesh);solidMeshes.push(mesh);if(f.feature)featureDimensions(group,vector,f.points,f.feature);else if(!f.curvedSurface&&host.state()?.wallCenters!==false)window.wallCenterMarker?.(group,vector,{x:f.points.reduce((s,p)=>s+p.x,0)/f.points.length,y:f.points.reduce((s,p)=>s+p.y,0)/f.points.length,z:f.points.reduce((s,p)=>s+p.z,0)/f.points.length});}
  const graph=wire(),byId=id=>graph.nodes.find(n=>n.id===id);for(const e of graph.edges){previewLine(group,vector,byId(e.a),byId(e.b),solidEdges.includes(e.id)?'#fff':structuralEdge(byId(e.a),byId(e.b),supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed',true);}for(const n of graph.nodes.filter(n=>!n.curveSample)){const geo=new THREE.BufferGeometry().setFromPoints([vector(n)]);group.add(new THREE.Points(geo,new THREE.PointsMaterial({color:solidPoints.includes(n.id)?'#fff':structuralPoint(n,supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed',size:solidPoints.includes(n.id)?12:8,sizeAttenuation:false,depthTest:false})));}
 }
 ...
 return {pointSelection:()=>selectedWorldPoints().map(p=>({...p})),createSelectedFace,extrudeFace,geometryCommand,clipboardCommand,mergeAll,autoTrim,pickVisiblePoint,finishToolForSwitch,chamferCommand,trimCommand,materialCommand,colorCommand,activeMaterial:()=>tool?.kind==='material'?tool.type:selectedFinishFace()?.material||null,finishAxisCut,interaction:()=>tool?({geometryTransform:'Transform geometry',paste:'Paste geometry',extrude:tool.mode==='move'?'Move face':'Extrude face',entityExtrude:tool.mode==='move'?'Move point / line':'Extrude point / line',chamfer:tool.rounded?'Fillet':'Chamfer',material:'Paint material',axisCut:'H/V cut',lineMove:'Move line',step:'Auto-step',curve:'Draw curve',draw:'Draw line',quad:'Draw rectangle',feature:'Place feature'}[tool.kind]||tool.kind):box?'Rectangle selection':null,cancelPointerGesture(){marqueeCompleted=false;box=null;boxEl?.remove?.();boxEl=null;},drawEntity,beginEntity,distanceInput,drawBaseSelection,hasBaseSelection:()=>selectedBasePoints.length>0||!!tool?.external,consumeSelectionClick:()=>marqueeCompleted,startBox(e,click){if(tool||box)return false;beginBox(e,current());box.deferredPick=true;box.click=click;return true;},finishPointer:endBox,cutFromPoint,pickPoint:e=>pickSolid(e,true),pickLine3D,stepWheel,stepCommand,featureCommand,featureSelection,featureContext(e){if(tool?.kind==='feature'){featureCommand(tool.type);return 'cycle';}return featureSelection()?'face':false;},mergeWallContained,draw2D(rot,svg,inv){F?.refreshUI?.();if(tool?.kind==='curve'){const d=current(),t=tool,ps=t.samples||[t.start,t.pointer||t.start];for(const g of t.guides||[])svg('polyline',{points:g.points.map(p=>{const q=host.toPixel(world(d,p));return q.x+','+q.y;}).join(' '),fill:'none',stroke:g.color,'stroke-width':inv,'stroke-dasharray':(5*inv)+' '+(4*inv),'data-curve-guide':g.role,'pointer-events':'none'},rot);svg('polyline',{points:ps.map(p=>{const q=host.toPixel(world(d,p));return q.x+','+q.y;}).join(' '),fill:'none',stroke:t.valid===false?'#ff6666':'#FFD700','stroke-width':2*inv,'pointer-events':'none'},rot);const marker=host.toPixel(world(d,t.center||t.pointer||t.start));svg('circle',{cx:marker.x,cy:marker.y,r:8*inv,fill:'none',stroke:'#72ffb0','stroke-width':1.5*inv,'pointer-events':'none'},rot);if(t.center){const a=host.toPixel(world(d,t.start)),b=host.toPixel(world(d,t.center));svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:'#72ffb0','stroke-dasharray':'5 5','stroke-width':inv},rot);}}renderWalls=host.walls();renderSegments=new Map();try{supportSnapshot=structuralFaces();supportIndex=W.structuralIndex(supportSnapshot);for(const d of Object.values(all())){for(const edge of draftSegments(d)){const a=host.toPixel(world(d,edge.start)),b=host.toPixel(world(d,edge.end));svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:current()===d&&pickedLines.includes(edge.id)?'#fff':structuralDraftEdge(d,edge)?'#e7ad52':'#6ce4ed','stroke-width':2*inv,'pointer-events':'none'},rot);}for(const n of draftNodes(d)){const p=host.toPixel(world(d,n));svg('circle',{cx:p.x,cy:p.y,r:(isPicked(d,n.id)?6:4)*inv,fill:isPicked(d,n.id)?'#fff':structuralDraftPoint(d,n)?'#e7ad52':'#6ce4ed','pointer-events':'none'},rot);}}const graph=wire(),byId=id=>graph.nodes.find(n=>n.id===id);for(const e of graph.edges){const a=host.toPixel(byId(e.a)),b=host.toPixel(byId(e.b));svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:solidEdges.includes(e.id)?'#fff':structuralEdge(byId(e.a),byId(e.b),supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed','stroke-width':2*inv,'pointer-events':'none'},rot);}for(const n of graph.nodes.filter(n=>!n.curveSample)){const p=host.toPixel(n);svg('circle',{cx:p.x,cy:p.y,r:(solidPoints.includes(n.id)?6:4)*inv,fill:solidPoints.includes(n.id)?'#fff':structuralPoint(n,supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed','pointer-events':'none'},rot);}}finally{supportSnapshot=null;supportIndex=null;renderWalls=null;renderSegments=null;}},holdBoundary,beginFace,pickSolid,reflow,doubleClick,down,key,draw3D(group,vector){drawBaseSelection(group,vector);renderWalls=host.walls();renderSegments=new Map();const output=window.wallGeometryBatch?.(group)||group;try{supportSnapshot=structuralFaces();supportIndex=W.structuralIndex(supportSnapshot);draw3D(output,vector);drawSolids(output,vector);drawLengths(output,vector);drawChamfer(output,vector);if(tool?.kind==='curve'){const t=tool,d=current(),ps=t.samples||[t.start,t.pointer||t.start];window.wallCurveGuides?.(output,vector,(t.guides||[]).map(g=>({...g,points:g.points.map(p=>world(d,p))})));for(let i=1;i<ps.length;i++)previewLine(output,vector,world(d,ps[i-1]),world(d,ps[i]),t.valid===false?'#ff6666':'#FFD700');if(t.center)previewLine(output,vector,world(d,t.start),world(d,t.center),'#72ffb0');window.wallCurveCenterMarker?.(output,vector,world(d,t.center||t.pointer||t.start));}for(const selection of lineSelection)for(const pair of selection.pairs||[selection.pair]){const line={...selection,pair};if(window.wallSelectedLine)window.wallSelectedLine(output,vector,line.pair);else previewLine(output,vector,...line.pair,'#fff',false);}output.flush?.();}finally{output.disposePending?.();supportSnapshot=null;supportIndex=null;renderWalls=null;renderSegments=null;}},discardEmpty:id=>{for(const [key,d]of Object.entries(all()))if(d.members.includes(id)&&!d.faces.some(f=>f.material||f.finishColor)&&!(d.deletedFaces||[]).length&&!d.sketch.edges.some(e=>!e.fixed)&&d.sketch.nodes.length===d.sketch.outlines.flat().length)delete all()[key];},modified:id=>Object.values(all()).some(d=>d.members.includes(id)&&(d.sketch.edges.some(e=>!e.fixed)||d.sketch.nodes.length>d.sketch.outlines.flat().length)),has:id=>Object.values(all()).some(d=>d.members.includes(id)&&(d.faces.length||d.sketch.nodes.length||d.deletedFaces?.length)),canBox:()=>walls().length>0||solids().length>0||Object.values(all()).some(visibleDraft),busy:()=>!!tool||!!box,clear(){preferredDraft=null;preferredSolid=null;preferredRegion=null;solidMeshes=[];quadFinish=null;selectedBasePoints=[];lineSelection=[];clearGuides();box=null;boxEl?.remove?.();boxEl=null;if(tool?.before)host.state().wallEdits=tool.before;tool=null;const stepButton=typeof document!=='undefined'&&document.getElementById('wall-step');if(stepButton)stepButton.textContent='Auto-step (S)';activeDraftKey=null;draftSelection={};picked=[];pickedLines=[];solidPoints=[];solidEdges=[];selectedSolid=null;selectedRegion=null;}};
};
})();

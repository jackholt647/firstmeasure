...
window.createWallFaceDraft=function(host){
 ...
 function planeScene(){
  if(workingPlane.scene)return workingPlane.scene;
  const K=window.ExteriorGeometry,frame=workingPlane.frame,on=p=>Math.abs(K.local(frame,p).z)<=K.CONTACT;
  ...
  return workingPlane.scene={points,lines};
 }
 ...
 function startPlaneRotation(points){
  ...
  workingPlane={selection:[],drawing:false,display:'faint',hover:null,rotation:{origin,axis,points:copy(points),single:points.length===1,axisIndex:0,angle:0,numeric:null,radius:Math.max(1,points.length===2?distance3(...points)*.6:1.5)}};
 ...
 function setupPlaneRotation(){
  ...
  r.zero=project(Math.abs(r.axis.x)<.9?{x:1,y:0,z:0}:{x:0,y:1,z:0});r.tangent=planeCross(r.axis,r.zero);r.snaps=[];
  ...
  const add=(p,label)=>{const v=project(p);if(v)r.snaps.push({angle:Math.atan2(planeDot(v,r.tangent),planeDot(v,r.zero))*180/Math.PI,label});};
  ...
  const graph=sceneLines();for(const [a,b]of graph.lines.values())if(r.points.some(p=>onSegment(p,a,b)))add({x:b.x-a.x,y:b.y-a.y,z:b.z-a.z},'Connected line');
 ...
 function updatePlaneRotation(angle){
  ...
  p.frame={origin:copy(r.origin),u:copy(r.axis),v,n:planeCross(r.axis,v)};p.seed=[{x:-r.radius,y:-r.radius,z:0},{x:r.radius,y:r.radius,z:0}];p.scene=null;
  host.message('Rotate plane: '+angle.toFixed(1)+' degrees'+(r.snap?' - '+r.snap:'')+'; type an angle; click to draw'+(r.single?'; P cycles vertical (Y) / X / Z axes':'')+'; Escape cancels.');
 }
 ...
 function togglePlane(source=null){
  if(workingPlane?.rotation){const r=workingPlane.rotation;if(r.single){r.axisIndex=(r.axisIndex+1)%3;r.axis=[{x:0,y:0,z:1},{x:1,y:0,z:0},{x:0,y:1,z:0}][r.axisIndex];setupPlaneRotation();}return true;}
  ...
  const inferred=source?.axisPoints||(!source&&(lineSelection.length===1?lineSelection[0].pair:selectedWorldPoints()));if(inferred?.length&&inferred.length<=2)return startPlaneRotation(inferred);
 ...
 function planePoint(e){
  ...
  const result=(workingPlane.action?.kind==='quad'&&W.quadDraftSnap(p,q=>planeQuad(q),scene.points.map(local),scene.lines.map(pair=>pair.map(local)),workingPlane.selection.map(local),q=>host.screen(K.world(d.frame,q),viewOf(e)),typeof snapRadius!=='undefined'?snapRadius:20))||W.draftSnap(p,scene.points.map(local),scene.lines.map(pair=>pair.map(local)),workingPlane.selection.map(local),q=>host.screen(K.world(d.frame,q),viewOf(e)),typeof snapRadius!=='undefined'?snapRadius:20);
  snapGuides=result.guides;showGuides(d,e);return {...result.point,z:0};
 }
 function planeDown(e){
  if(!workingPlane||viewOf(e)!=='3d'||e.button!==0)return false;
  mouse=e;if(workingPlane.rotation){delete workingPlane.rotation;host.message('Plane ready - draw points and lines; P exits.');host.redraw();return true;}if(workingPlane.action)return placePlaneAction(e);
  ...
  box.click=()=>{let best=null,dist=12;for(const p of planeScene().points){const q=host.screen(p,'3d'),d=Math.hypot(q.x-e.clientX,q.y-e.clientY);if(q.visible!==false&&d<dist){best=p;dist=d;}}
   const prior=workingPlane.selection,others=prior.filter(p=>!best||W.vertexKey(p)!==W.vertexKey(best));
   workingPlane.selection=e.ctrlKey||e.metaKey?others:e.shiftKey?[...others,...(best?[best]:[])]:best?[best]:[];host.redraw();};return true;
 }
 function planeEdit(point,pairs=[],curve=null){
  ...
  const ok=transaction(()=>{
   ...
   plane.scene=null;const scene=planeScene(),nodes=[],lookup=new Map(),node=p=>{const local=K.local(frame,p),key=K.pointKey(local);if(!lookup.has(key)){lookup.set(key,'plane-node-'+nodes.length);nodes.push({...local,id:lookup.get(key)});}return lookup.get(key);};
   const edges=scene.lines.map(([a,b],i)=>({id:'plane-edge-'+i,a:node(a),b:node(b)}));
   const graph=K.partition({nodes,edges}),local=p=>K.local(frame,p),world=p=>K.world(frame,p);
   const owners=moveScene().filter(f=>!f.deleted&&!f.snapOnly&&!f.curvedSurface&&[f.points,...(f.holes||[])].flat().every(p=>Math.abs(local(p).z)<=K.CONTACT)),shape=f=>({points:f.points.map(local),holes:(f.holes||[]).map(r=>r.map(local))}),covered=owners.map(shape);
   const base=edits.$base||host.state().base;for(const f of base?.faces||[])if(!f.deleted&&f.points.every(p=>Math.abs(local(p).z)<=K.CONTACT)){covered.push(shape(f));owners.push({...f,planeBaseId:f.id});}
   const target=host.state().wallEdits;target.$surfaces||=[];let next=0;
   const curves=Object.values(target.$drafts||{}).filter(d=>d.frame).flatMap(d=>(d.sketch?.curves||[]).map(c=>K.mapCurve(c,p=>K.world(d.frame,p)))).filter(c=>K.curveSamples(c).every(p=>Math.abs(local(p).z)<=K.CONTACT));
 ...
 function planeKey(e){
  if(workingPlane.rotation){if(e.key.toLowerCase()==='escape'){workingPlane=null;host.redraw();}return true;}
  if(e.repeat)return true;const plane=workingPlane,k=e.key.toLowerCase(),K=window.ExteriorGeometry;
  ...
  else if(k==='c'&&!plane.action){const ps=plane.selection;planeEdit(null,ps.slice(1).map((p,i)=>[ps[i],p]));}
 ...
 function endBox(e){if(e&&e.button!==undefined&&e.button!==0)return;
  ...
  if(b.plane&&workingPlane){if(!moved){b.click?.();return;}marqueeCompleted=true;const selected=planeScene().points.filter(p=>{const q=host.screen(p,'3d');return q.visible!==false&&q.x>=Math.min(b.x,e?.clientX??b.ex)&&q.x<=Math.max(b.x,e?.clientX??b.ex)&&q.y>=Math.min(b.y,e?.clientY??b.ey)&&q.y<=Math.max(b.y,e?.clientY??b.ey);}),keys=new Set(selected.map(W.vertexKey));
   workingPlane.selection=b.subtract?b.priorPlane.filter(p=>!keys.has(W.vertexKey(p))):[...new Map([...(b.add?b.priorPlane:[]),...selected].map(p=>[W.vertexKey(p),p])).values()];host.redraw();e?.stopImmediatePropagation?.();return;}
 ...
 function pickLine3D(e){const dividerOnly=host.state()?.displayMode==='textured'&&viewOf(e)==='3d',dividers=dividerOnly?divisionSeams():[];
 ...
 function finishToolForSwitch(){
  flushPreview();const t=tool;if(!t)return true;if(t.kind==='divide'){finishDivision();return !tool;}if(t.kind==='trim')return finishTrim();if(t.kind==='curve'){host.message('Finish the curve or press Escape before switching tools.');return false;}
 ...
 function geometryCommand(mode,external){
  ...
  if(tool?.kind==='paste'||tool?.kind==='geometryTransform'){
   ...
   if(cycle&&mode==='m'&&t.stickerMove)t.moveAxis=((t.moveAxis||0)+1)%4;else if(cycle)t.mount=(t.mount+1)%t.clip.mounts.length;
  ...
  try{const curveOwner=curveGeometrySelection(points);if(curveOwner){const clip=curveOwner.clip;tool={kind:'geometryTransform',curveOwner:{key:curveOwner.key,base:curveOwner.base,ids:curveOwner.ids},snapTargets:curveOwner.snapTargets,before,prepared:copy(host.state().wallEdits),base:copy(host.state().wallEdits.$base||host.state().base),scene:[],original:copy(clip),clip,preview:copy(clip),mount:0,op:Date.now(),valid:true};geometryCommand(mode);return true;}
   ...
   tool={kind:'geometryTransform',stickerMove:clip.faces.length>0&&clip.faces.every(f=>f.feature),snapTargets,before,prepared:copy(host.state().wallEdits),base:copy(base),scene:copy(scene),original:copy(clip),clip,preview:copy(clip),mount:0,op:Date.now(),valid:true};
 ...
 function previewGeometry(e,keepAxis=false,translation=null){
 ...
 function clipboardCommand(kind,external){
  if(kind==='copy'){
   if(tool&&!finishToolForSwitch())return true;const selected=external?.points||[...selectedWorldPoints(),...selectedStickerPoints()],before=copy(host.state().wallEdits||{});
 ...
 function previewPaste(e){if(tool?.kind!=='paste'||!e||viewOf(e)!=='3d')return;const t=tool;t.preview=null;t.valid=false;
  try{let anchor=t.editMode!=='m'&&t.anchor;
   ...
   t.preview=W.pasteGeometry(t.clip,t.mount,target,location,{normal,bound:t.editMode==='m',screen:p=>host.screen(p,'3d'),points:[...target.points,...sceneLines().points],stickerFaces:t.clip.faces.length&&t.clip.faces.every(f=>f.feature)?moveScene():[],snap:t.editMode==='m'&&!anchor.preservePosition&&!(typeof isFreeMove!=='undefined'&&isFreeMove)});t.valid=t.preview.valid;t.ref=ref;if(t.preview.valid&&t.preview.location)t.anchor.location=t.preview.location;
 ...
 function beginEntity(mode,external=null){
  ...
  if(!source&&!pair)return false;if(mode==='move'&&pair&&beginLineMove(false,{pair,event:mouse}))return true;
 ...
 function setLinePlane(){const t=tool,frame=t.candidates[t.planeIndex].frame,[a,b]=t.pairs[0],u={x:b.x-a.x,y:b.y-a.y,z:b.z-a.z},length=Math.hypot(u.x,u.y,u.z),n=frame.n;
  ...
  t.dimension=null;if(feature){const values=feature.points.map(p=>(p.x-a.x)*t.direction.x+(p.y-a.y)*t.direction.y+(p.z-a.z)*t.direction.z),lo=Math.min(...values),hi=Math.max(...values);if(Math.abs(lo)<1e-5||Math.abs(hi)<1e-5){const opposite=Math.abs(lo)>Math.abs(hi)?lo:hi,axis=F.dimensions(feature.points,feature.feature).frame.u;t.dimension={opposite,sign:opposite>0?-1:1,label:Math.abs(t.direction.x*axis.x+t.direction.y*axis.y+t.direction.z*axis.z)>.5?'Width':'Height'};}}
 ...
 function previewLineMove(e){
  ...
  try{const result=W.slideLines(copy(t.scene),t.pairs,t.direction,amount),edits=copy(t.stageBefore||t.before);edits.$drafts=copy(t.sceneDrafts);
 ...
 let renderDividers=null;
 function divisionSeams(){return renderDividers||F.divisionSegments(divisionScene());}
 function isDivisionPair(a,b,seams=divisionSeams()){return W.sharedIntervals(a,b,seams.map(s=>({points:s.pair}))).reduce((sum,[lo,hi])=>sum+hi-lo,0)>.99999;}
 ...
 function previewDivision(e){
  ...
  try{t.result=F.divideSticker(t.faces,t.orientation,amount,{frame:t.layout.frame,groupId:t.groupId,operationId:t.op});host.message('Divide '+(h?'from top: ':'from left: ')+(amount/F.FT).toFixed(2)+"\u2032 / "+((size-amount)/F.FT).toFixed(2)+"\u2032. D changes direction; type a distance; click to place.");}
 ...
 function startMultiExtrusion(){
  ...
  try{
   ...
   tool.engine=window.ExteriorModel.createExtrusions({members,scene,base,roof:tool.roof});host.state().wallEdits=copy(before);
 ...
 function previewMultiExtrusion(e){
  ...
  try{
   ...
   if(t.roof!==host.roof?.()){t.roof=host.roof?.();t.engine=window.ExteriorModel.createExtrusions({members:t.members,scene:t.scene,base:t.base,roof:t.roof});}
 ...
 function applySelectedFeature(type){
  ...
  return transaction(()=>{for(const {d,f} of faces){if(type==='none')delete f.feature;else f.feature={type,preset:null,shape:f.feature?.shape||'custom',axis:f.feature?.axis||F.orientedFrame(d?f.points.map(p=>world(d,p)):f.points).u};}host.message(faces.length+' selected face'+(faces.length===1?'':'s')+' updated; each size kept.');});
 }
 function featureCommand(type,index,place=false){if(!F)return false;if(type==='none'){if(tool&&!finishToolForSwitch())return true;if(selectedFaces().length)return applySelectedFeature(type);const selection=featureSelection();if(!selection)return false;return transaction(()=>{delete (selection.f||selection.solid).feature;});}const def=F.defs.get(type);if(!def)return false;
  if(tool?.kind==='feature'){const same=tool.type===type;tool.type=type;tool.index=Number.isInteger(index)?index:same?F.nextPreset(type,tool.index):(def.defaultPreset??0);if(mouse)previewFeature(mouse);return true;}
 ...
 function previewFeature(e){if(!F||viewOf(e)!=='3d')return;tool.preview=null;const ref=placementHost(e);if(!ref){host.redraw();return;}try{const frame=F.frame(ref.points),p=rayPoint({frame},e);if(!p)return;const preset={...F.defs.get(tool.type).sizes[tool.index]};if(tool.rotated)[preset.w,preset.h]=[preset.h,preset.w];const graph=sceneLines(),onPlane=p=>Math.abs(window.ExteriorGeometry.local(frame,p).z)<=window.ExteriorGeometry.CONTACT;
  // Single-point guides align either sticker edge to intentional anchors,
  // including vertices inserted along a straight wall/base boundary.
  const stickerTargets=W.stickerAlignmentTargets(moveScene(),frame);tool.alignmentTargets=stickerTargets;
  const boundary=[ref.points.map(p=>W.inFrame(frame,p))],targets=[...boundary,...[...graph.lines.values()].filter(pair=>pair.every(onPlane)).map(pair=>pair.map(p=>W.inFrame(frame,p))),...graph.points.filter(onPlane).map(p=>[W.inFrame(frame,p)]),...stickerTargets.map(p=>[{...W.inFrame(frame,p),alignmentAxes:p.alignmentAxes}])],points=F.place(p,preset,targets,p=>host.screen(W.fromFrame(frame,p),'3d'),typeof isFreeMove!=='undefined'&&isFreeMove?0:12,boundary.map(points=>({points,holes:(ref.f?.holes||ref.solid?.holes||[]).map(r=>r.map(p=>W.inFrame(frame,ref.d?world(ref.d,p):p)))})));
 ...
 function buildAxisCut(p,k,preferredBase=null){
  ...
  candidates.sort((a,b)=>Number(!!b.feature)-Number(!!a.feature)||Number(b.base?b.face===preferredBase:b.key===preferredDraft&&b.face===preferredRegion)-Number(a.base?a.face===preferredBase:a.key===preferredDraft&&a.face===preferredRegion)||(k==='v'?Math.max(...b.targets.map(q=>q.z))-Math.max(...a.targets.map(q=>q.z)):0));
 ...
 function nudge(e){const step=(e.altKey ? .25 : e.shiftKey ? 6 : 1)*F.FT/12,dx=e.key==='ArrowRight'?step:e.key==='ArrowLeft'?-step:0,dy=e.key==='ArrowUp'?step:e.key==='ArrowDown'?-step:0;const points=selectedWorldPoints();if(geometrySelectionPoints().length>=3)return nudgeGeometry(dx,dy);if(points.length){if(points.length!==1){host.message('Select one point or a complete face to nudge.');return true;}return nudgePoint(e,step);}const selection=featureSelection();
 ...
 function nudgeGeometry(dx,dy){
  ...
  const t=tool,frame=t.clip.mounts[t.mount].frame,view=F.viewFrame(t.clip.faces[0]?.points||t.clip.points,p=>host.screen(p,'3d'));
 ...
 function selectTrimEdges(kind,materials){
  ...
  const pairs=kind==='ground'?window.WallTrim.groundCandidates(visible,options):window.WallTrim.candidates(visible,options);
  ...
  lineSelection=[...new Map([...selectedTrimPairs(),...segments].map(pair=>[W.edgeKey(...pair),{id:W.edgeKey(...pair),pair:copy(pair)}])).values()];
...
if(!d&&['n','q'].includes(k)&&selected())d=ensure(selected());if(!d)return false;
 ...
 window.addEventListener('pointermove',e=>{
  if(tool&&['extrude','multiExtrude','entityExtrude','lineMove'].includes(tool.kind)&&viewOf(e)&&!(e.buttons&6)){
 ...
 function draw3D(group,vector){
  ...
  for(let d of Object.values(all())){retainSourcePoints(d);if(!d.faces.length&&!d.sketch.nodes.length&&!d.deletedFaces?.length){const w=walls().find(w=>d.members.includes(w.id));if(w)d=ensure(w);}if(!visibleDraft(d))continue;
   ...
   for(const source of d.faces)for(const f of visibleDraftFaces(d,source)){if(f.boundaryHole||f.solidId||deleted(d,f))continue;const chosen=faceChosen({draft:draftKey(d),face:source.id}),opening=inner.includes(source),holes=f.holes?.length?f.holes:opening?[]:inner.filter(h=>h!==source&&containsRegion(f,h)).map(h=>h.points),polys=[f.points,...holes],points=polys.flat(),tri=window.ExteriorGeometry.triangles(polys[0],holes).triangles;
 ...
 function previewLine(group,vector,a,b,color,depthTest=false){
  const intervals=W.sharedIntervals(a,b,[...(tool?.mergeSeams||[]).map(points=>({points})),...divisionSeams().map(s=>({points:s.pair}))]),at=t=>({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t,z:a.z+(b.z-a.z)*t});let start=0;
 ...
 function drawStickerPreview(group,vector,points,feature,valid=true){const frame=F.frame(points),geo=new THREE.BufferGeometry().setFromPoints(points.map(vector));geo.setIndex(THREE.ShapeUtils.triangulateShape(points.map(p=>{const q=W.inFrame(frame,p);return new THREE.Vector2(q.x,q.y);}),[]).flat());group.add(new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:valid?F.defs.get(feature.type).color:'#ff7575',side:THREE.DoubleSide,transparent:true,opacity:.7,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2})));for(let i=0;i<points.length;i++)previewLine(group,vector,points[i],points[(i+1)%points.length],'#fff');featureDimensions(group,vector,points,feature);}
 function featureDimensions(group,vector,points,feature,selected=false){if(!F||host.state()?.featureDimensions===false||tool?.kind==='divide'&&tool.faces.some(f=>f.points.length===points.length&&f.points.every(p=>points.some(q=>distance3(p,q)<1e-5))))return;const d=F.dimensions(points,feature),c={x:(d.bounds.left+d.bounds.right)/2,y:(d.bounds.bottom+d.bounds.top)/2,z:0},a=W.fromFrame(d.frame,{...c,x:d.bounds.left}),b=W.fromFrame(d.frame,{...c,x:d.bounds.right});window.wallLengthMarker?.(group,vector,{a,b,length:0,text:F.label(points,feature),selected,...(movingFace(points)?{centerY:1.65,horizontal:true}:{})});}
 ...
 function drawSolids(group,vector){
  ...
  if(tool?.kind==='feature'&&tool.preview)drawStickerPreview(group,vector,tool.preview.points,{type:tool.type,axis:tool.preview.axis});
 ...
 return {featurePlacement:()=>tool?.kind==='feature'?{type:tool.type,index:tool.index,session:tool.session}:null,selectionSnapshot:()=>copy({preferredDraft,preferredSolid,preferredRegion,activeDraftKey,draftSelection,picked,pickedLines,selectedSolid,selectedRegion,solidPoints,solidEdges,selectedBasePoints,lineSelection,faceSelection,workingPlane:workingPlane&&{frame:workingPlane.frame,material:workingPlane.material,finishColor:workingPlane.finishColor,selection:workingPlane.selection,display:workingPlane.display,seed:workingPlane.seed}}),restoreSelection(value){const s=copy(value||{});preferredDraft=s.preferredDraft||null;preferredSolid=s.preferredSolid||null;preferredRegion=s.preferredRegion||null;activeDraftKey=s.activeDraftKey||null;draftSelection=s.draftSelection||{};picked=s.picked||[];pickedLines=s.pickedLines||[];selectedSolid=s.selectedSolid||null;selectedRegion=s.selectedRegion||null;solidPoints=s.solidPoints||[];solidEdges=s.solidEdges||[];selectedBasePoints=s.selectedBasePoints||[];lineSelection=s.lineSelection||[];faceSelection=s.faceSelection||[];workingPlane=s.workingPlane?{...s.workingPlane,drawing:false,hover:null}:null;},togglePlane,planeView:()=>workingPlane&&({frame:workingPlane.frame,display:workingPlane.display,rotating:!!workingPlane.rotation}),setPlaneDisplay(value){if(workingPlane&&['normal','faint','hidden'].includes(value)){workingPlane.display=value;host.redraw();}},planeActive:()=>!!workingPlane,planeDown,pointSelection:()=>(workingPlane?workingPlane.selection:selectedWorldPoints()).map(p=>({...p})),createSelectedFace,extrudeFace,geometryCommand,clipboardCommand,mergeAll,autoTrim,trimMaterials,selectTrimEdges,applyTrim,pickVisiblePoint,finishToolForSwitch,chamferCommand,trimCommand,materialCommand,colorCommand,activeMaterial:()=>tool?.kind==='material'?tool.type:selectedFinishFace()?.material||null,finishAxisCut,interaction:()=>workingPlane?workingPlane.rotation?'Rotate drawing plane':'Drawing plane':tool?({divide:'Divide window / door',multiExtrude:'Extrude selected faces',geometryTransform:'Transform geometry',paste:'Paste geometry',extrude:tool.mode==='move'?'Move face':'Extrude face',entityExtrude:tool.mode==='move'?'Move point / line':'Extrude point / line',chamfer:tool.rounded?'Fillet':'Chamfer',material:'Paint material',axisCut:'H/V cut',lineMove:'Move line',step:'Auto-step',curve:'Draw curve',draw:'Draw line',quad:'Draw rectangle',feature:'Place feature'}[tool.kind]||tool.kind):box?.moved?'Rectangle selection':null,cancelPointerGesture(){marqueeCompleted=false;box=null;boxEl?.remove?.();boxEl=null;},drawEntity,beginEntity,distanceInput,drawBaseSelection,hasBaseSelection:()=>selectedBasePoints.length>0||!!tool?.external,consumeSelectionClick:()=>marqueeCompleted,startBox(e,click){if(tool||box)return false;beginBox(e,current());box.deferredPick=true;box.click=click;return true;},finishPointer:endBox,cutFromPoint,pickPoint:e=>pickSolid(e,true),pickLine3D,stepWheel,stepCommand,featureCommand,featureSelection,featureContext(e){if(tool?.kind==='feature'){featureCommand(tool.type);return 'cycle';}return featureSelection()?'face':false;},mergeWallContained,draw2D(rot,svg,inv){renderDividers=F.divisionSegments(divisionScene());F?.refreshUI?.();if(tool?.kind==='curve'){const d=current(),t=tool,ps=t.samples||[t.start,t.pointer||t.start];for(const g of t.guides||[])svg('polyline',{points:g.points.map(p=>{const q=host.toPixel(world(d,p));return q.x+','+q.y;}).join(' '),fill:'none',stroke:g.color,'stroke-width':inv,'stroke-dasharray':(5*inv)+' '+(4*inv),'data-curve-guide':g.role,'pointer-events':'none'},rot);svg('polyline',{points:ps.map(p=>{const q=host.toPixel(world(d,p));return q.x+','+q.y;}).join(' '),fill:'none',stroke:t.valid===false?'#ff6666':'#FFD700','stroke-width':2*inv,'pointer-events':'none'},rot);const marker=host.toPixel(world(d,t.center||t.pointer||t.start));svg('circle',{cx:marker.x,cy:marker.y,r:8*inv,fill:'none',stroke:'#72ffb0','stroke-width':1.5*inv,'pointer-events':'none'},rot);if(t.center){const a=host.toPixel(world(d,t.start)),b=host.toPixel(world(d,t.center));svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:'#72ffb0','stroke-dasharray':'5 5','stroke-width':inv},rot);}}renderWalls=host.walls();renderSegments=new Map();try{supportSnapshot=structuralFaces();supportIndex=W.structuralIndex(supportSnapshot);for(const d of Object.values(all())){for(const edge of draftSegments(d)){const a=host.toPixel(world(d,edge.start)),b=host.toPixel(world(d,edge.end));svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:current()===d&&pickedLines.includes(edge.id)?'#fff':structuralDraftEdge(d,edge)?'#e7ad52':'#6ce4ed','stroke-dasharray':isDivisionPair(world(d,edge.start),world(d,edge.end))?(5*inv)+' '+(4*inv):undefined,'stroke-width':2*inv,'pointer-events':'none'},rot);}for(const n of draftNodes(d)){const p=host.toPixel(world(d,n));svg('circle',{cx:p.x,cy:p.y,r:(isPicked(d,n.id)?6:4)*inv,fill:isPicked(d,n.id)?'#fff':structuralDraftPoint(d,n)?'#e7ad52':'#6ce4ed','pointer-events':'none'},rot);}}const graph=wire(),nodesById=new Map(graph.nodes.map(n=>[n.id,n])),byId=id=>nodesById.get(id);for(const e of graph.edges){const a=host.toPixel(byId(e.a)),b=host.toPixel(byId(e.b));svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:solidEdges.includes(e.id)?'#fff':structuralEdge(byId(e.a),byId(e.b),supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed','stroke-dasharray':isDivisionPair(byId(e.a),byId(e.b))?(5*inv)+' '+(4*inv):undefined,'stroke-width':2*inv,'pointer-events':'none'},rot);}for(const n of graph.nodes.filter(n=>!n.curveSample)){const p=host.toPixel(n);svg('circle',{cx:p.x,cy:p.y,r:(solidPoints.includes(n.id)?6:4)*inv,fill:solidPoints.includes(n.id)?'#fff':structuralPoint(n,supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed','pointer-events':'none'},rot);}}finally{renderDividers=null;supportSnapshot=null;supportIndex=null;renderWalls=null;renderSegments=null;}},holdBoundary,beginFace,pickSolid,reflow,doubleClick,down,key,draw3D(group,vector){renderDividers=F.divisionSegments(divisionScene());renderOpenings=window.ExteriorModel.indexOpenings(window.ExteriorModel.collect(host.state()).filter(f=>f.feature));drawBaseSelection(group,vector);renderWalls=host.walls();renderSegments=new Map();const output=window.wallGeometryBatch?.(group)||group;try{supportSnapshot=structuralFaces();supportIndex=W.structuralIndex(supportSnapshot);draw3D(output,vector);drawSolids(output,vector);drawLengths(output,vector);drawDivision(output,vector);drawChamfer(output,vector);if(tool?.kind==='curve'){const t=tool,d=current(),ps=t.samples||[t.start,t.pointer||t.start];window.wallCurveGuides?.(output,vector,(t.guides||[]).map(g=>({...g,points:g.points.map(p=>world(d,p))})));for(let i=1;i<ps.length;i++)previewLine(output,vector,world(d,ps[i-1]),world(d,ps[i]),t.valid===false?'#ff6666':'#FFD700');if(t.center)previewLine(output,vector,world(d,t.start),world(d,t.center),'#72ffb0');window.wallCurveCenterMarker?.(output,vector,world(d,t.center||t.pointer||t.start));}for(const selection of lineSelection)for(const pair of selection.pairs||[selection.pair]){const line={...selection,pair};if(window.wallSelectedLine&&!isDivisionPair(...line.pair))window.wallSelectedLine(output,vector,line.pair);else previewLine(output,vector,...line.pair,'#fff',false);}output.flush?.();}finally{output.disposePending?.();renderDividers=null;renderOpenings=[];supportSnapshot=null;supportIndex=null;renderWalls=null;renderSegments=null;}},discardEmpty:id=>{for(const [key,d]of Object.entries(all()))if(d.members.includes(id)&&!d.faces.some(f=>f.material||f.finishColor)&&!(d.deletedFaces||[]).length&&!d.sketch.edges.some(e=>!e.fixed)&&d.sketch.nodes.length===d.sketch.outlines.flat().length)delete all()[key];},modified:id=>Object.values(all()).some(d=>d.members.includes(id)&&(d.sketch.edges.some(e=>!e.fixed)||d.sketch.nodes.length>d.sketch.outlines.flat().length)),has:id=>Object.values(all()).some(d=>d.members.includes(id)&&(d.faces.length||d.sketch.nodes.length||d.deletedFaces?.length)),canBox:()=>walls().length>0||solids().length>0||Object.values(all()).some(visibleDraft),busy:()=>!!tool||!!box||!!workingPlane?.rotation,clear(){workingPlane=null;faceSelection=[];preferredDraft=null;preferredSolid=null;preferredRegion=null;solidMeshes=[];quadFinish=null;selectedBasePoints=[];lineSelection=[];clearGuides();box=null;boxEl?.remove?.();boxEl=null;if(tool?.before)host.state().wallEdits=tool.before;tool=null;const stepButton=typeof document!=='undefined'&&document.getElementById('wall-step');if(stepButton)stepButton.textContent='Auto-step (S)';activeDraftKey=null;draftSelection={};picked=[];pickedLines=[];solidPoints=[];solidEdges=[];selectedSolid=null;selectedRegion=null;}};
};
})();

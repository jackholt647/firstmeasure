...
window.createWallFaceDraft=function(host){
 ...
 function togglePlane(source=null){
  ...
  const inferred=source?.axisPoints||(!source&&geometrySelectionPoints());if(inferred?.length){const fit=selectionPlane(inferred);if(!fit){host.message('Selected points and lines must all lie on one plane.');return true;}if(fit.axis)return startPlaneRotation(fit.axis);workingPlane={frame:fit.frame,selection:copy(inferred),drawing:false,display:'faint',hover:null,seed:inferred.map(p=>window.ExteriorGeometry.local(fit.frame,p))};refreshPlane();host.message('Plane constrained to selected geometry. M: move; R: rotate; T: flip; Y: resize; P: exit.');host.redraw();return true;}
 ...
 function planeDown(e){
  ...
  box.click=()=>{let best=null,dist=12;for(const p of planeScene().points){const q=host.screen(p,'3d'),d=Math.hypot(q.x-e.clientX,q.y-e.clientY);if(q.visible!==false&&d<dist){best=p;dist=d;}}
   ...
   if(lineMode()||!best){let pair=null,distance=10;for(const edge of planeScene().lines){const [a,b]=edge.map(p=>host.screen(p,'3d')),dx=b.x-a.x,dy=b.y-a.y,l=dx*dx+dy*dy;if(l<1e-8)continue;const t=Math.max(0,Math.min(1,((e.clientX-a.x)*dx+(e.clientY-a.y)*dy)/l)),d=Math.hypot(e.clientX-a.x-t*dx,e.clientY-a.y-t*dy);if(d<distance){pair=edge;distance=d;}}if(pair){chosen=pair;workingPlane.selectedLines=[pair];}}
...
function perf_previewGeometry(e,keepAxis=false,translation=null){
 ...
 function clipboardCommand(kind,external){
  if(kind==='copy'){
   if(tool&&!finishToolForSwitch())return true;const selected=external?.points||(workingPlane?workingPlane.selection:[...selectedWorldPoints(),...selectedStickerPoints()]),before=copy(host.state().wallEdits||{});
   ...
   window.exteriorGeometryClipboard=stickerFaces?W.copyGeometry(stickerFaces,selectedStickerPoints()):W.copyGeometry(scene,selected,[...sceneLines().lines.values()]);const c=window.exteriorGeometryClipboard;if(workingPlane){constrainPlaneClip(c);const curves=curveGeometrySelection(selected);if(curves)c.curves=copy(curves.clip.curves);}host.message('Copied '+c.points.length+' points, '+c.edges.length+' lines and '+c.faces.length+' faces.');}catch(e){host.message(e.message);}finally{host.state().wallEdits=before;host.redraw();}return true;
  }

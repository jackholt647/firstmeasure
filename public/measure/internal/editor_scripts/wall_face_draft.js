...
function perf_snap(d,e,raw=null){
 ...
 function endBox(e){if(e&&e.button!==undefined&&e.button!==0)return;
  if(!box||e?.button!==undefined&&e.button!==0)return;if(e&&viewOf(e))mouse=e;const b=box;const pointOnly=!selectedSolid&&!selectedRegion&&!lineSelection.length&&!pickedLines.length&&!solidEdges.length&&!selectedBasePoints.length&&!workingPlane?.selectedLines?.length&&!workingPlane?.selectedFaces?.length;box=null;boxEl?.remove?.();boxEl=null;
  ...
  if(b.plane&&workingPlane){if(!moved){b.click?.();return;}marqueeCompleted=true;const selected=planeScene().points.filter(p=>{const q=host.screen(p,'3d');return q.visible!==false&&q.x>=Math.min(b.x,e?.clientX??b.ex)&&q.x<=Math.max(b.x,e?.clientX??b.ex)&&q.y>=Math.min(b.y,e?.clientY??b.ey)&&q.y<=Math.max(b.y,e?.clientY??b.ey);}),keys=new Set(selected.map(W.vertexKey));
   workingPlane.selectedLines=[];workingPlane.selectedFaces=[];workingPlane.selection=b.subtract?b.priorPlane.filter(p=>!keys.has(W.vertexKey(p))):[...new Map([...(b.add?b.priorPlane:[]),...selected].map(p=>[W.vertexKey(p),p])).values()];if(!pointOnly||!host.redrawSelection?.(workingPlane.selection))host.redraw();e?.stopImmediatePropagation?.();return;}
  ...
  let materialized=false;const priorWalls=renderWalls;try{renderWalls=host.walls();if(!(b.add&&!b.subtract&&(lineSelection.length||pickedLines.length||solidEdges.length))&&host.wallsVisible?.()!==false)for(const w of renderWalls){if(!all()[id(w)])materialized=true;ensure(w);}}finally{renderWalls=priorWalls;}
  const select=()=>withSelectionGeometry(()=>{
  ...
  picked=draftSelection[draftKey(current())]||[];const points=selectedWorldPoints();if(!pointOnly||materialized||selectedBasePoints.length||b.view!=='3d'||!host.redrawSelection?.(points))host.redraw();const count=points.length;host.message(count+(count===1?' point selected':' points selected'));e?.stopImmediatePropagation?.();
  });
  return host.withSelectionPicking?host.withSelectionPicking(select):select();
 ...
 function doubleClick(e,w){
  ...
  if(viewOf(e)==='3d'&&placeLinePoint(e,w))return true;
 ...
 // Use exactly the visible-line hit tolerance used by selection. In perspective
 // screen interpolation is not world interpolation, so solve on the projected
 // segment instead of placing a point at the wrong depth.
 function placeLinePoint(e,w){
  if(w)return false;
  const hit=withSelectionGeometry(()=>pickLineInScene(e,true));if(!hit)return false;
  // Preserve inference and curve snapping when the actual face hit owns
  // this edge; standalone lines do not need a drawing plane.
  const owns=face=>W.sharedIntervals(...hit.pair,[face]).some(([a,b])=>b-a>.999);
  const face=placementHost(e,true);if(face&&owns({points:face.points}))return false;
  const [a,b]=hit.pair,at=t=>({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t,z:a.z+(b.z-a.z)*t}),sa=host.screen(a,'3d'),sb=host.screen(b,'3d'),dx=sb.x-sa.x,dy=sb.y-sa.y,l2=dx*dx+dy*dy;
  let lo=0,hi=1,t=hit.t;for(let i=0;i<40;i++){const q=host.screen(at(t),'3d'),u=((q.x-sa.x)*dx+(q.y-sa.y)*dy)/l2;if(Math.abs(u-hit.t)<1e-12)break;if(u<hit.t)lo=t;else hi=t;t=(lo+hi)/2;}
  let point=at(t);
  const snaps=[a,b,...(host.state().lineCenters!==false?[at(.5)]:[])].map(p=>({p,q:host.screen(p,'3d')})).filter(({q})=>Math.hypot(q.x-e.clientX,q.y-e.clientY)<8).sort((a,b)=>Math.hypot(a.q.x-e.clientX,a.q.y-e.clientY)-Math.hypot(b.q.x-e.clientX,b.q.y-e.clientY));
  if(snaps.length)point=copy(snaps[0].p);
  box=null;boxEl?.remove?.();boxEl=null;
  transaction(()=>{
   let d=Object.values(all()).find(d=>visibleDraft(d)&&draftSegments(d).some(edge=>hit.pair.every(p=>onSegment(p,world(d,edge.start),world(d,edge.end)))));
   if(!d){const w=walls().find(w=>W.sharedIntervals(...hit.pair,[{points:[...w.bottom,w.top[1],w.top[0]]}]).some(([a,b])=>b-a>.999));if(w)d=ensure(w);}
   draftSelection={};picked=[];pickedLines=[];solidPoints=[];solidEdges=[];selectedBasePoints=[];lineSelection=[];selectedRegion=null;selectedSolid=null;preferredSolid=null;preferredRegion=null;
   if(d){activeDraftKey=draftKey(d);preferredDraft=activeDraftKey;host.select(walls().find(w=>d.members.includes(w.id))?.id||null);picked=[add(d,toLocal(d,point))];}
   else{const edits=host.state().wallEdits,loose=edits.$loose||={points:[],edges:[]};if(!loose.points.some(p=>W.vertexKey(p)===W.vertexKey(point)))loose.points.push(point);edits.$removedSurfacePoints=(edits.$removedSurfacePoints||[]).filter(id=>id!==W.vertexKey(point));activeDraftKey=null;preferredDraft=null;host.select(null);solidPoints=[W.vertexKey(point)];}
  });return true;
 }
 function pickLine3D(e){return withSelectionGeometry(()=>pickLineInScene(e));}
 function pickLineInScene(e,hitOnly=false){let contacts;const soffits=()=>contacts ||= renderDerived('soffit-edges',()=>soffitEdges());const isSoffit=pair=>soffits().some(c=>W.sharedIntervals(...pair,[{points:c.pair}]).some(([a,b])=>b-a>.001));const dividerOnly=['textured','rendered','match-textured'].includes(host.state()?.displayMode)&&viewOf(e)==='3d',dividers=dividerOnly?indexDividers(divisionSeams()):null;
...
function perf_movePointer(e){if(workingPlane&&!tool&&!box){mouse=e;if(workingPlane.rotation){if(viewOf(e)==='3d'&&!(e.buttons&6))previewPlaneRotation(e);return;}if(viewOf(e)==='3d'&&!(e.buttons&6)&&(workingPlane.drawing||workingPlane.action)){if(workingPlane.action?.kind==='curve')previewPlaneCurve(e);else workingPlane.hover=planePoint(e);host.redraw();}else{workingPlane.hover=null;clearGuides();host.redraw();}return;}if(tool?.kind==='curveMove'){const t=tool,p=rayPoint(current(),e);if(!p)return;host.state().wallEdits=copy(t.before);try{S.move(all()[t.draftKey],t.ids,{x:p.x-t.start.x,y:p.y-t.start.y});t.changed=true;}catch(error){host.message(error.message);}host.redraw();return;}
 ...
 function draw3D(group,vector){
  ...
  for(let d of Object.values(all())){retainSourcePoints(d);if(!d.faces.length&&!d.sketch.nodes.length&&!d.deletedFaces?.length){const w=walls().find(w=>d.members.includes(w.id));if(w)d=ensure(w);}if(!visibleDraft(d))continue;
   ...
   for(const n of draftNodes(d))selectablePoint(group,vector,world(d,n),structuralDraftPoint(d,n)?'#e7ad52':'#6ce4ed',8,isPicked(d,n.id),12);
 ...
 function selectablePoint(group,vector,p,color,size,selected,selectedSize){if(selectionMarkers){selectionMarkers.push({p,key:W.vertexKey(p),color,size,selected,selectedSize});return;}previewPoint(group,vector,p,selected?'#fff':color,selected?selectedSize:size);}
 ...
 function drawSolids(group,vector){
  ...
  const graph=wire(),nodesById=new Map(graph.nodes.map(n=>[n.id,n])),byId=id=>nodesById.get(id);for(const e of graph.edges){previewLine(group,vector,byId(e.a),byId(e.b),solidEdges.includes(e.id)?'#fff':structuralEdge(byId(e.a),byId(e.b),supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed',true);}for(const n of graph.nodes.filter(n=>!n.curveSample))selectablePoint(group,vector,n,structuralPoint(n,supportSnapshot||structuralFaces())?'#e7ad52':'#6ce4ed',8,solidPoints.includes(n.id),12);
 }

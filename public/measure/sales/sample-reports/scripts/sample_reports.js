(function () {
  ...
  async function api(payload) {
    const base = String((cfg().endpoints && cfg().endpoints.sample_reports) || '/v1/internal/sample-reports').replace(/\/+$/, '');
    const action = String(payload?.action || '');
    const actor = cfg().user || {};
    const actorQuery = new URLSearchParams();
    if (actor.email) actorQuery.set('actor_email', actor.email);
    if (actor.name) actorQuery.set('actor_name', actor.name);
    if (actor.role) actorQuery.set('actor_role', actor.role);
    const withActor = (url) => `${url}${url.includes('?') ? '&' : '?'}${actorQuery.toString()}`;

    if (action === 'list_sample_projects') {
      const params = new URLSearchParams();
      ['search', 'project_type', 'report_state', 'page', 'limit'].forEach((key) => {
        if (payload[key] !== undefined && payload[key] !== null) params.set(key, payload[key]);
      });
      actorQuery.forEach((value, key) => params.set(key, value));
      const response = await fetch(`${base}/projects?${params.toString()}`, { cache: 'no-store' });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.success === false) throw new Error(data.error || data.message || 'Could not load sample projects.');
      return data;
    }

    if (action === 'load_sample_project_bundle') {
      const folder = encodeURIComponent(String(payload.folder || payload.project_id || payload.id || ''));
      const response = await fetch(withActor(`${base}/projects/${folder}/bundle`), { cache: 'no-store' });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.success === false) throw new Error(data.error || data.message || 'Could not load this sample project.');
      return data;
    }

    if (action === 'toggle_sample_favorite') {
      const response = await fetch(withActor(`${base}/favorites`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload || {})
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.success === false) throw new Error(data.error || data.message || 'Could not save favorite.');
      return data;
    }

    throw new Error(`Unsupported sample reports action: ${action}`);
  }

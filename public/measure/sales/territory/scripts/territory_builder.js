...
(function () {
  ...
  function ensureMarkup(){
    ...
    wrap.style.display=cfg().standalone_territory ? "block" : "none";
    wrap.style.height="100%";
    wrap.style.minHeight="0";
    wrap.innerHTML='<div id="territoryRoot" style="width:100%;height:100%;"></div>';
    ...
    <div id="F">
      <header id="Fbar">
        ...
        <nav id="Fsteps">
          ...
          <span class="Fstep-arrow">→</span>
          <button class="Fstep" data-step="4"><span class="Fstep-num">4</span> Import Territories</button>
          <span class="Fstep-arrow">·</span>
        ...
        <section id="step4" class="Fpane">
          <div id="S4LeadLists">
            <div class="S4lead-header">
              <div>
                <div class="S2title">STEP 4 - Import Territories</div>
                <div class="S1price-note">Turn a saved territory list into Sales Leads using the same preview and duplicate handling as CSV import.</div>
              </div>
            ...
            <div class="S4lead-grid">
              <div class="S4lead-sidebar">
                ...
                <div class="S2group">
                  <label class="S2label">New leads</label>
                  <select id="s4NewAction">
                ...
                <div class="S2group">
                  <label class="S2label">Changed duplicates</label>
                  <select id="s4DuplicateAction">
                ...
                <div class="S2group">
                  <label class="S2label">Identical leads</label>
                  <select id="s4UnchangedAction">
                    <option value="skip">Skip identical</option>
                    <option value="touch">Update import date</option>
                  </select>
                </div>

                <button class="Fbtn Fbtn-ghost" id="s4LeadPreview" style="width:100%">Preview Import</button>
                <button class="Fbtn Fbtn-accent" id="s4LeadCreate" style="width:100%;margin-top:8px" disabled>Import Territory</button>

                <div id="s4LeadSummary" class="S4lead-summary">
                  <div class="S4lead-empty">Choose a saved territory list, preview the import, then commit it into Sales Leads.</div>
                </div>
              </div>
              ...
              <div class="S4lead-main">
                <div class="S4lead-main-header">
                  ...
                  <div class="S2hint">Fetch business info first if you want imported leads to include phone, website, and address details.</div>
                </div>
  ...
  async function saveFilterList(){
    ...
    alert(`Saved ${name}. Use Step 4 to import this territory list into Sales Leads.`);
  }
  ...
  async function loadLeadListPreview(selectedName=""){
    ...
    if(!listName){
      s4LeadPreview = null;
      updateTerritoryImportButton();
      return null;
    }
    const groupMode=document.getElementById("s4LeadGroupMode")?.value || "by_state";
    const res=await api("preview_sales_lead_import",{list_name:listName,group_mode:groupMode});
    s4LeadPreview = res && res.success ? res : null;
    updateTerritoryImportButton();
    return s4LeadPreview;
  }

  function updateTerritoryImportButton(){
    const btn=document.getElementById("s4LeadCreate");
    const selectedName=document.getElementById("s4LeadListSelect")?.value || "";
    if(btn) btn.disabled=!(s4LeadPreview && s4LeadPreview.import_id && s4LeadPreview.list_name===selectedName);
  }
  ...
  function renderLeadListCards(){
    ...
    filterLists.forEach(item=>{
      ...
      const importLabel=preview
        ? `${Number(preview.summary?.total || preview.row_count || 0).toLocaleString()} rows`
        : "Not previewed";
      ...
      card.innerHTML=`
        ...
        <div class="S4lead-card-meta">
          ...
          <div class="S4lead-card-meta-item">
            <div class="S4lead-card-meta-label">Preview</div>
            <div class="S4lead-card-meta-value">${esc(importLabel)}</div>
          </div>
          <div class="S4lead-card-meta-item">
        ...
        <div class="S4lead-card-actions">
          ...
          <button class="Fbtn Fbtn-accent" data-s4-create="${esc(item.list_name)}">Preview</button>
        </div>`;
    ...
    wrap.querySelectorAll("[data-s4-create]").forEach(btn=>{
      btn.onclick=async()=>{
        ...
        if(sel) sel.value=name;
        await renderLeadListGenerator(name);
        await previewTerritoryImport(name);
      };
    });
  }
  ...
  async function renderLeadListGenerator(selectedName=""){
    ...
    if(!item){
      s4LeadPreview = null;
      summary.innerHTML='<div class="S4lead-empty">Choose a saved territory list, preview the import, then commit it into Sales Leads.</div>';
      updateTerritoryImportButton();
      renderLeadListCards();
      return;
    }
    const doneCount=item.done_count||0;
    const groupMode=document.getElementById("s4LeadGroupMode")?.value || "by_state";
    const preview=(s4LeadPreview && s4LeadPreview.list_name===name) ? s4LeadPreview : null;
    const groups=preview?.groups || [];
    const importSummary=preview?.summary || null;
      ...
      ? groups.map(group=>{
          ...
          return `
            <div class="S4lead-summary-item">
              ...
              <div class="S4lead-summary-note" style="margin-top:6px">Imported with region ${esc(group.region || "UNASSIGNED")}</div>
            </div>`;
        }).join("")
      : '<div class="S4lead-empty">Preview this territory to see new, changed, identical, and invalid rows.</div>';
    const duplicateSummaryHtml=importSummary ? `
        <div class="S4lead-summary-item">
          <div class="S4lead-summary-label">New</div>
          <div class="S4lead-summary-value">${Number(importSummary.new_count||0).toLocaleString()}</div>
        </div>
        <div class="S4lead-summary-item">
          <div class="S4lead-summary-label">Changed</div>
          <div class="S4lead-summary-value">${Number(importSummary.duplicate_count||0).toLocaleString()}</div>
        </div>
        <div class="S4lead-summary-item">
          <div class="S4lead-summary-label">Identical</div>
          <div class="S4lead-summary-value">${Number(importSummary.unchanged_count||0).toLocaleString()}</div>
        </div>
        <div class="S4lead-summary-item">
          <div class="S4lead-summary-label">Invalid</div>
          <div class="S4lead-summary-value">${Number(importSummary.invalid_count||0).toLocaleString()}</div>
        </div>`
      : '<div class="S4lead-empty">No import preview yet.</div>';
    summary.innerHTML=`
      <div class="S4lead-card-title">${esc(item.list_name)}</div>
      <div class="S4lead-summary-grid">
        ...
        <div class="S4lead-summary-item">
          <div class="S4lead-summary-label">Previewed Rows</div>
          <div class="S4lead-summary-value">${Number(importSummary?.total || preview?.row_count || 0).toLocaleString()}</div>
        </div>
        <div class="S4lead-summary-item">
      ...
      <div class="S4lead-summary-grid">${duplicateSummaryHtml}</div>
      <div class="S4lead-summary-grid">${groupSummaryHtml}</div>
      <div class="S4lead-summary-note">Step 4 imports these businesses into the Sales Leads grid. New, changed, and identical rows follow the action choices above.</div>`;
    updateTerritoryImportButton();
    renderLeadListCards();
  }

  async function previewTerritoryImport(forcedName=""){
    ...
    const summary=document.getElementById("s4LeadSummary");
    if(summary) summary.innerHTML='<div class="S4lead-empty">Previewing territory import...</div>';
    try{
      const res=await loadLeadListPreview(listName);
      if(!res || !res.success){
        alert(res?.error||"Could not preview territory import");
        return;
      }
      await renderLeadListGenerator(listName);
    }catch(error){
      s4LeadPreview=null;
      updateTerritoryImportButton();
      if(summary) summary.innerHTML=`<div class="S4lead-empty">Preview failed: ${esc(error?.message || "Request failed")}</div>`;
      alert(error?.message || "Could not preview territory import");
    }
  }
  ...
  async function createCRMListsFromSelectedTerritoryList(forcedName=""){
    const listName=forcedName || document.getElementById("s4LeadListSelect")?.value || "";
    if(!listName){alert("Choose a saved territory list first");return}
    if(!s4LeadPreview || s4LeadPreview.list_name!==listName || !s4LeadPreview.import_id){
      await previewTerritoryImport(listName);
    }
    if(!s4LeadPreview || !s4LeadPreview.import_id) return;
    const btn=document.getElementById("s4LeadCreate");
    const summary=document.getElementById("s4LeadSummary");
    if(btn) btn.disabled=true;
    if(summary) summary.insertAdjacentHTML("afterbegin", '<div class="S4lead-empty">Importing territory into Sales Leads...</div>');
    let res;
    try{
      res=await api("commit_sales_lead_import",{
        import_id:s4LeadPreview.import_id,
        new_action:document.getElementById("s4NewAction")?.value || "create",
        duplicate_action:document.getElementById("s4DuplicateAction")?.value || "update",
        unchanged_action:document.getElementById("s4UnchangedAction")?.value || "skip"
      });
    }catch(error){
      if(summary) summary.insertAdjacentHTML("afterbegin", `<div class="S4lead-empty">Import failed: ${esc(error?.message || "Request failed")}</div>`);
      alert(error?.message || "Could not import territory");
      updateTerritoryImportButton();
      return;
    }
    if(!res.success && !res.ok){
      alert(res.error||"Could not import territory");
      updateTerritoryImportButton();
      return;
    }
    ...
    updateTerritoryImportButton();
    await renderLeadListGenerator(listName);
    alert(`Imported ${listName}. Created ${Number(counts.created||0).toLocaleString()}, updated ${Number(counts.updated||0).toLocaleString()}, identical synced ${Number(counts.unchanged||0).toLocaleString()}, skipped ${Number(counts.skipped||0).toLocaleString()}, invalid ${Number(counts.invalid||0).toLocaleString()}.`);
  }
  ...
  function hookAll(){
    ...
    document.getElementById("s4NewAction").addEventListener("change",updateTerritoryImportButton);
    document.getElementById("s4DuplicateAction").addEventListener("change",updateTerritoryImportButton);
    document.getElementById("s4UnchangedAction").addEventListener("change",updateTerritoryImportButton);
    document.getElementById("s4LeadPreview").addEventListener("click",()=>previewTerritoryImport());
  ...
  if (cfg().standalone_territory) {
    const bootStandalone = () => {
      if (territoryBooted) return;
      const view = document.getElementById("view-territory-builder");

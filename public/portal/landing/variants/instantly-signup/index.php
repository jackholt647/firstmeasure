...
$landingAttributionVariant = $landingVariantMode ?: (string)($_GET['variant'] ?? basename(__DIR__));
...
$faqs = [
  [
    ...
    'answer' => 'All major manufacturers and material types are supported. You can configure your preferred products in your company settings.',
  ],
  [
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .lp-hero::before {
      ...
      height: 360px;
      z-index: -2;
      background: url("<?= htmlspecialchars($mediaBase, ENT_QUOTES) ?>/neighborhood_header.png") center/cover no-repeat;
      opacity: 0.14;
    }
    .lp-hero::after {
      content: "";
  ...
  <div class="lp-shell" id="top">
      ...
      <section class="lp-signup-band" id="signup" data-signup-section>
        <div class="lp-container lp-signup-band-inner">
          <div class="lp-signup-band-copy">
            <h2>Order Your First Roof Report Today</h2>
          ...
          <div class="lp-signup-band-widget">
            <aside class="lp-signup-card" aria-label="Create your FirstMate account">
              ...
              <p><?= $isReferralLanding ? 'Create your account now to order your first report.' : 'Create your account now.' ?></p>
      ...
      <section class="lp-section alt" id="faqs">
        <div class="lp-container">
          ...
          <div class="lp-faq-cta">
            <button type="button" data-open-signup>Start Ordering Reports</button>
          </div>
        </div>
      </section>
    ...
    <footer class="lp-footer">
      <div class="lp-container lp-footer-inner">
        <img src="<?= htmlspecialchars($mediaBase, ENT_QUOTES) ?>/logo_red.png" alt="First Mate" class="lp-footer-logo">
  ...
  <script>
    (function(){
      ...
      window.addEventListener('wheel', event => {
        ...
        const inZone = isInFlipZone();
        const canMove = canFlip(event.deltaY);
        const isCoolingDown = Date.now() < lockedUntil;
        if (inZone && (canMove || isCoolingDown)) {
          event.preventDefault();
          flipByDelta(event.deltaY);
        }
      }, { passive: false });
      ...
      window.addEventListener('touchmove', event => {
        ...
        const inZone = isInFlipZone();
        const canMove = canFlip(deltaY);
        const isCoolingDown = Date.now() < lockedUntil;
        if (inZone && (canMove || isCoolingDown)) {
          event.preventDefault();
          if (flipByDelta(deltaY)) touchStartY = event.touches[0].clientY;
        }
      }, { passive: false });
      */

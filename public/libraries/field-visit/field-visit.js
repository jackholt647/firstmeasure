...
(function(){
  ...
  function mount(root, context = {}, options = {}){
    ...
    root.innerHTML = `<div class="fm-visit-state"><div><div class="fm-visit-spinner"></div><strong>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_3f024db829343a","Loading this visit") ?? "Loading this visit")}</strong></div></div>`;
    ...
    const notificationContent = (step) => {
      ...
      return `<div class="fm-visit-card fm-visit-notice fm-visit-page-card">${String(title)}<div class="fm-visit-notice-row"><span><strong>${String(esc(customer))}</strong><br><small>${String(esc(phone || 'No mobile number on file'))}</small></span>${String(phone ? `<a href="tel:${esc(phone.replace(/[^0-9+]/g,''))}"><i class="fa-solid fa-phone"></i>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_34ac7f9c427ea4"," Call") ?? " Call")}</a>` : '')}</div><div class="fm-visit-eta"><label>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_389a628cbe1e55","Estimated arrival ") ?? "Estimated arrival ")}<small data-eta-source>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_91b42e93f819bd","Using the scheduled route estimate") ?? "Using the scheduled route estimate")}</small></label><label>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_db4ed7996e84c9","Minutes") ?? "Minutes")}<input type="number" min="5" max="240" step="5" value="${String(eta)}" data-visit-eta></label></div><div class="fm-visit-message" data-visit-message data-message-template="${String(esc(template))}">${String(esc(preview))}</div><div class="fm-visit-actions"><button class="fm-visit-action" type="button" data-visit-notify>${String(esc(step.action?.label || 'Send SMS & start trip'))}</button><button class="fm-visit-secondary" type="button" data-visit-skip>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_92e33d4ffc2022","Skip SMS") ?? "Skip SMS")}</button></div></div>`;
    };
    ...
    const followUpContent = (step) => {
      ...
      return `<div class="fm-visit-hero"><i class="fa-solid fa-phone"></i><h3>${String(esc(step.title))}</h3><p>${String(esc(step.description))}</p></div><div class="fm-visit-card"><span class="fm-visit-card-label">${(globalThis.PlatformLanguage?.htmlText("field-visit","m_76340b00b0d14a","Assigned next step") ?? "Assigned next step")}</span><strong style="display:block;margin-top:7px">${String(esc(followUp?.title || (globalThis.PlatformLanguage?.text("field-visit","m_2e6670ed312621","Customer follow-up") ?? "Customer follow-up")))}</strong><div class="fm-visit-followup-status"><i class="fa-solid fa-list-check"></i><span>${String(esc(followUp ? `${clean(followUp.status || 'ready').replaceAll('_',' ')}${due ? ` · Due ${due}` : ''}` : 'Added to your assigned to-do list'))}</span></div></div>`;
    };
    const statusContent = (step) => `<div class="fm-visit-hero"><i class="fa-solid ${clean(step.kind) === 'complete' ? 'fa-flag-checkered' : clean(step.action?.transition) === 'arrived' ? 'fa-location-dot' : 'fa-route'}"></i><h3>${esc(step.title)}</h3><p>${esc(step.description)}</p>${step.completed ? `<span class="fm-visit-complete"><i class="fa-solid fa-check"></i>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_70a2f1f3873399"," Completed") ?? " Completed")}</span>` : step.action ? `<button class="fm-visit-action" type="button" data-visit-action>${esc(step.action.label || 'Continue')}</button>` : ''}</div>`;
  ...
  if (runtime?.registerApp) runtime.registerApp({
    ...
    mount:(context = {}) => {
      ...
      const render = (value) => {
        ...
        root.innerHTML = `<main class="fm-visit-body"><div class="fm-customer-grid"><section class="fm-visit-card fm-customer-card"><span class="fm-visit-card-label">${(globalThis.PlatformLanguage?.htmlText("field-visit","m_4204cc53f171b3","Customer contacts") ?? "Customer contacts")}</span>${String(contactCards || `<p>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_8513532cca77d9","No customer contact information is available.") ?? "No customer contact information is available.")}</p>`)}</section><section class="fm-visit-card fm-customer-card"><span class="fm-visit-card-label">${(globalThis.PlatformLanguage?.htmlText("field-visit","m_3dbf4d87db5138","Property & project") ?? "Property & project")}</span><div class="fm-customer-detail"><i class="fa-solid fa-house"></i><span><strong>${String(esc(project.title || (globalThis.PlatformLanguage?.text("field-visit","m_aaebd7ccba0b30","Project") ?? "Project")))}</strong><br>${String(esc(clean(project.status || project.stage).replaceAll('_',' ')))}</span></div>${String(address ? `<a class="fm-customer-detail" href="https://maps.google.com/?q=${encodeURIComponent(address)}" target="_blank" rel="noopener"><i class="fa-solid fa-location-dot"></i><span>${esc(address)}<br><strong>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_5fad3819c6e85c","Open directions") ?? "Open directions")}</strong></span></a>` : '')}</section><section class="fm-visit-card fm-customer-card wide"><span class="fm-visit-card-label">${(globalThis.PlatformLanguage?.htmlText("field-visit","m_5a654ad9b6d2e3","Appointment") ?? "Appointment")}</span><div class="fm-customer-detail"><i class="fa-solid fa-calendar"></i><span><strong>${String(esc(whenLabel))}</strong>${String(upcoming.notes ? `<br>${esc(upcoming.notes)}` : '')}</span></div></section><section class="fm-visit-card fm-customer-card wide"><span class="fm-visit-card-label">${(globalThis.PlatformLanguage?.htmlText("field-visit","m_d6113dda50d96c","Project notes") ?? "Project notes")}</span><p class="fm-customer-note">${String(esc(project.notes || 'No project notes have been added.'))}</p></section></div></main>`;
      };
      root.innerHTML = `<div class="fm-visit-state"><div><div class="fm-visit-spinner"></div><strong>${(globalThis.PlatformLanguage?.htmlText("field-visit","m_bf1271de7156aa","Loading customer information") ?? "Loading customer information")}</strong></div></div>`;

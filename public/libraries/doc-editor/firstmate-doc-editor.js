...
(function (root, factory) {
  ...
  const EDITOR_CSS_V7 = "\n/* ---------- v7: anchored document comments ---------- */\n" +
    ...
    ".fmde-suggestion-card{border-color:rgba(124,58,237,.28)}.fmde-suggestion-card:before{border-color:rgba(124,58,237,.28)}.fmde-suggestion-card .fmde-comment-avatar{background:var(--fmde-accent)}\n" +
    ...
    ".fmde-comment-actions [data-accept-suggestion]{background:var(--fmde-accent);color:#fff}.fmde-comment-actions [data-reject-suggestion]{color:#5f6368}\n";
  ...
  const EDITOR_CSS_V9 = "\n/* ---------- v9: Docs-parity tranche A ---------- */\n" +
    ...
    ".fmde-bookmark-flag{position:absolute;z-index:30;width:16px;height:16px;padding:0;border:0;background:transparent;color:var(--fmde-accent);cursor:pointer;opacity:.85}.fmde-bookmark-flag:hover{opacity:1}.fmde-bookmark-flag svg{width:14px;height:14px;fill:currentColor;stroke:currentColor}\n" +
    ...
    "::highlight(fmde-spell){background:#fce8e6}::highlight(fmde-grammar){background:color-mix(in srgb, var(--fmde-accent) 10%, transparent)}\n" +
  ...
  function mount(container, opts) {
    ...
    function editorBrandingPalette() {
      ...
      try {
        ...
        inherited = computed.getPropertyValue("--primary").trim() ||
          computed.getPropertyValue("--primary-readable").trim() ||
          computed.getPropertyValue("--fm-primary").trim();
      } catch (e) { /* detached/test documents use the explicit branding context */ }
      const candidates = [
        inherited, colors.primary, branding.primary, colors.brand, branding.brand,
        colors.accent, branding.accent, overrides["--fm-primary"],
        "#d93025"
      ];
      let primary = null;
    ...
    function showColorPalette(anchor, current, options) {
      ...
      const picker = el("input", { type: "color", value: /^#[0-9a-f]{6}$/i.test(String(current)) ? current : editorBrandingPalette().primary, "aria-label": "Custom color" });
    ...
    const onCompanyThemeUpdated = function () { if (!state.destroyed) applyEditorBranding(dom.root); };
    root.addEventListener?.("fm:theme:updated", onCompanyThemeUpdated);
    ...
    function destroy() {
      ...
      root.removeEventListener?.("fm:theme:updated", onCompanyThemeUpdated);
      closeFindPanel();
      closeSpellingPanel();

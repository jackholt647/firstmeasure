...
(function(root){
  ...
  function buildPopover(instance){
    ...
    popover.innerHTML = `<header class="fm-insight-head"><span class="fm-insight-head-icon fm-insight-logo" aria-hidden="true"></span><div class="fm-insight-heading"><span class="fm-insight-kicker">${(globalThis.PlatformLanguage?.htmlText("insights","m_89b8c4efe6661d","Insight") ?? "Insight")}</span><h3 class="fm-insight-title">${String(esc(definition.title))}</h3></div><button class="fm-insight-close" type="button" aria-label="${(globalThis.PlatformLanguage?.htmlText("insights","m_e8d6e1ba2cb635","Close insight") ?? "Close insight")}">${(globalThis.PlatformLanguage?.htmlText("insights","m_dd0c616953d455","&times;") ?? "&times;")}</button></header><div class="fm-insight-scroll"><div class="fm-insight-conversation"></div></div>`;
  ...
  function syncComposer(view){
    ...
    form.innerHTML = `<textarea class="fm-insight-input" rows="1" maxlength="4000" placeholder="${(globalThis.PlatformLanguage?.htmlText("insights","m_2f4c19ca03f27f","Ask about this insight…") ?? "Ask about this insight…")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("insights","m_3cfe2fbbe87ff9","Ask the Insights agent") ?? "Ask the Insights agent")}"></textarea><button class="fm-insight-send" type="submit" aria-label="${(globalThis.PlatformLanguage?.htmlText("insights","m_c23a056552a09f","Send") ?? "Send")}"><i class="fas fa-arrow-up" aria-hidden="true"></i></button>`;
  ...
  function appendMessage(view, message){
    ...
    if (message.pending) {
      ...
      element.innerHTML = `<span class="fm-insight-thinking"><span class="fm-insight-thinking-mark"><span class="fm-insight-logo" aria-hidden="true"></span></span><span class="fm-insight-thinking-dots" aria-label="${(globalThis.PlatformLanguage?.htmlText("insights","m_c65c4642411503","Agent is thinking") ?? "Agent is thinking")}"><i></i><i></i><i></i></span></span>`;
  ...
  function mountSettings(target, config = {}){
    ...
    configure(config);
    host.innerHTML = `<div class="fm-insights-settings"><section class="fm-insights-settings-hero"><div><h3>${(globalThis.PlatformLanguage?.htmlText("insights","m_5430e6902b900e","Insights") ?? "Insights")}</h3><p>${(globalThis.PlatformLanguage?.htmlText("insights","m_164d2262739a4e","Place clear, contextual recommendations beside the settings and tools that need them. Insights stay out of the way until opened and automatically choose the clearest direction on screen.") ?? "Place clear, contextual recommendations beside the settings and tools that need them. Insights stay out of the way until opened and automatically choose the clearest direction on screen.")}</p></div><div class="fm-insights-preview-orbit" data-insights-preview></div></section><div class="fm-insights-settings-grid"><label class="fm-insights-setting"><span class="fm-insights-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("insights","m_cf99b36a994097","Show Insights") ?? "Show Insights")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("insights","m_a48f727d2a1dde","Display recommendation icons throughout FirstMate. Turning this off hides every insight without changing its content.") ?? "Display recommendation icons throughout FirstMate. Turning this off hides every insight without changing its content.")}</span></span><span class="fm-insights-switch"><input type="checkbox" data-insights-enabled><span></span></span></label><label class="fm-insights-setting"><span class="fm-insights-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("insights","m_8edb1954aa90fe","Add the Insights agent") ?? "Add the Insights agent")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("insights","m_49d657042fa263","Add a compact reply box so your team can ask follow-up questions inside each insight.") ?? "Add a compact reply box so your team can ask follow-up questions inside each insight.")}</span></span><span class="fm-insights-switch"><input type="checkbox" data-insights-agent><span></span></span></label></div><div class="fm-insights-settings-status" role="status" data-insights-status></div></div>`;

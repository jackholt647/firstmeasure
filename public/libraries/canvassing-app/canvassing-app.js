...
(function(){
  ...
  class CanvassingApp {
    ...
    renderShell(){
      const managerButton = this.canManage() ? `<button class="${String(this.state.view === 'manager' ? 'active' : '')}" data-fmc-tab="manager"><i class="fas fa-chart-simple"></i> <span>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_32263fb2e2d35f","Manager") ?? "Manager")}</span></button>` : '';
      this.root.innerHTML = `
    ...
    openSheet(pin = null, coords = null){
      ...
      sheet.innerHTML = `
        <label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_1352cafa75b8da","Status") ?? "Status")}</label>
        ...
        <label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_53d803cdbe9ab1","Address") ?? "Address")}</label><input data-fmc-field="address" value="${String(escapeHtml(pin?.address || ''))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_53d803cdbe9ab1","Address") ?? "Address")}">
        <div class="fmc-row">
          <div><label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_8cf345002184e5","Name") ?? "Name")}</label><input data-fmc-field="name" value="${String(escapeHtml(contact.name || ''))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_8cf345002184e5","Name") ?? "Name")}"></div>
          <div><label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_ed04c65845180f","Phone") ?? "Phone")}</label><input data-fmc-field="phone" value="${String(escapeHtml(contact.phone || ''))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_ed04c65845180f","Phone") ?? "Phone")}"></div>
        </div>
        <label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_de7d6168ae1ad6","Notes") ?? "Notes")}</label><textarea data-fmc-field="notes" placeholder="${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_de7d6168ae1ad6","Notes") ?? "Notes")}">${String(escapeHtml(pin?.notes || ''))}</textarea>
        <div class="fmc-row" style="margin-top:10px">
          <button data-fmc-action="close-sheet">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_3742924668fb10","Close") ?? "Close")}</button>
          <button class="primary" data-fmc-action="save-pin">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_5bab3e72de1ebf","Save") ?? "Save")}</button>
        </div>
        ${String(pin ? `<div class="fmc-row" style="margin-top:8px"><button class="primary" data-fmc-action="promote-pin">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_c97dbb6c0270fb","Create Lead") ?? "Create Lead")}</button></div>` : '')}
      `;
    ...
    async renderManager(){
      ...
      content.innerHTML = `
        <section class="fmc-panel">
          <h2>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_eb8da783249abd","Canvassing Manager") ?? "Canvassing Manager")}</h2>
          <div class="fmc-muted">${String(escapeHtml(this.session?.organization?.name || ''))}</div>
          <div class="fmc-statgrid" style="margin-top:12px">
            <div class="fmc-stat"><div class="num">${String(total)}</div><div class="lbl">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_576cd53c8d929f","Pins") ?? "Pins")}</div></div>
            <div class="fmc-stat"><div class="num">${String(todayCount)}</div><div class="lbl">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_23929ba4ba84dd","Today") ?? "Today")}</div></div>
            <div class="fmc-stat"><div class="num">${String(leads)}</div><div class="lbl">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_bc5721b3795c59","Leads") ?? "Leads")}</div></div>
          </div>
        </section>
        <section class="fmc-panel">
          <h2>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_19344c5b0a26ae","Add Canvasser") ?? "Add Canvasser")}</h2>
          <form data-fmc-form="add-user">
            <div class="fmc-row">
              <div><label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_8cf345002184e5","Name") ?? "Name")}</label><input data-fmc-field="new-user-name" required></div>
              <div><label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_5d2b9327181e33","Email") ?? "Email")}</label><input data-fmc-field="new-user-email" type="email" required></div>
            </div>
            <div class="fmc-row">
              <div><label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_17b51e98936157","Role") ?? "Role")}</label><select data-fmc-field="new-user-role"><option value="canvasser">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_88c2073b2326a6","Canvasser") ?? "Canvasser")}</option><option value="canvassing_manager">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_eb8da783249abd","Canvassing Manager") ?? "Canvassing Manager")}</option></select></div>
              <div><label>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_745f82f81c8230","Temporary Password") ?? "Temporary Password")}</label><input data-fmc-field="new-user-password" placeholder="${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_a3f9a6b065b2d8","Optional") ?? "Optional")}"></div>
            </div>
            <button class="primary" type="submit" style="margin-top:10px;width:100%">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_53a6f3f99f98de","Add User") ?? "Add User")}</button>
            <div class="fmc-muted" style="margin-top:8px">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_9be93d6051475b","If no temporary password is entered, the user is stored as invited for now. Email delivery/accept-invite can be wired later.") ?? "If no temporary password is entered, the user is stored as invited for now. Email delivery/accept-invite can be wired later.")}</div>
          </form>
        </section>
        <section class="fmc-panel">
          <h2>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_0891b6d3299aa3","Canvassing Users") ?? "Canvassing Users")}</h2>
          <div data-fmc-users>${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_cf106559e52254","Loading...") ?? "Loading...")}</div>
        </section>
      `;
    ...
    async loadUsers(){
      ...
      `).join('') || `<div class="fmc-muted">${(globalThis.PlatformLanguage?.htmlText("canvassing-app","m_a19a5f28971e25","No canvassing users yet.") ?? "No canvassing users yet.")}</div>`;
    }

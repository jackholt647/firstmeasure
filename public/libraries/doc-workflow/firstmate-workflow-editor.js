...
`;
  ...
  function fieldDragPreview(kind) {
    ...
    const control = choiceField || booleanField
      ? `<div class="fmwfe-field-drag-options"><span>${booleanField ? 'Yes' : 'Option 1'}</span><span>${booleanField ? 'No' : 'Option 2'}</span>${booleanField ? '' : `<span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_356cd50e508aeb","Option 3") ?? "Option 3")}</span>`}</div>`
  ...
  function mount(container, options = {}) {
    ...
    const fieldsTab = {
      ...
      render(host, api) {
        host.innerHTML = `<div class="fmwfe-fields-head"><strong>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_3fb0ccaf330e5c","Fields") ?? "Fields")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_af14b8b6b3f8da","Drag a field section onto the page, then arrange fields and visual elements together.") ?? "Drag a field section onto the page, then arrange fields and visual elements together.")}</span></div>
          <button type="button" class="fmwfe-section-card" data-fmwfe-section><i class="fas fa-layer-group"></i><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_2128b09a02a1b0","Field section") ?? "Field section")}<small>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_f7014706241e4f","Resizable conditional group") ?? "Resizable conditional group")}</small></span></button>
    ...
    function mountFieldActions() {
      ...
      fieldActions.innerHTML = `<button type="button" data-fmwfe-action="duplicate" title="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_3caa7b4c2c6387","Duplicate below") ?? "Duplicate below")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_3caa7b4c2c6387","Duplicate below") ?? "Duplicate below")}"><i class="fas fa-copy"></i></button><button type="button" data-fmwfe-action="condition" title="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_5972d014cecc83","Edit display condition") ?? "Edit display condition")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_5972d014cecc83","Edit display condition") ?? "Edit display condition")}"><i class="fas fa-code-branch"></i></button><button type="button" data-fmwfe-action="settings" title="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_6cec3410f41f6d","Field settings") ?? "Field settings")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_6cec3410f41f6d","Field settings") ?? "Field settings")}"><i class="fas fa-sliders"></i></button>`;
    ...
    function choiceRowsHtml(options) {
      return arrayValue(options).map((raw, index) => {
        ...
        return `<div class="fmwfe-choice-row" data-fmwfe-choice="${String(index)}">
          <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_9fd79f4276d659","Label") ?? "Label")}</span><input data-fmwfe-choice-label value="${String(esc(firstText(option.label, option.value, `Option ${index + 1}`)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_25e13201551ca4","Choice label") ?? "Choice label")}"></label>
          <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_bdbba2cffe168c","Output value") ?? "Output value")}</span><input data-fmwfe-choice-value value="${String(esc(firstText(option.value)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_5264d36c59e39d","choice_value") ?? "choice_value")}"></label>
          <button type="button" data-fmwfe-choice-remove title="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_ed53c026108fd8","Remove choice") ?? "Remove choice")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_ed53c026108fd8","Remove choice") ?? "Remove choice")}"><i class="fas fa-trash"></i></button>
        </div>`;
      }).join('');
    }
    ...
    function inspectorHtml(entity) {
      ...
      if (type === 'page') return `<div class="fmwfe-inspector"><h4>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_1d2e0fe66e5761","Page settings") ?? "Page settings")}</h4>
        <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_29dbd3d8b69f55","Title") ?? "Title")}</span><input data-fmwfe-title value="${String(esc(firstText(step.title)))}"></label>
        <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_785881bc8d1731","Page id") ?? "Page id")}</span><input data-fmwfe-page-id value="${String(esc(firstText(step.id)))}"></label>
        <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_386f0bf7477d03","Show when") ?? "Show when")}</span><input data-fmwfe-when value="${String(esc(firstText(step.when)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_e3158515ef4f1b","{{ params.answer == 'yes' }}") ?? "{{ params.answer == 'yes' }}")}"></label>
        <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_efa5eb7c34d75f","Audience") ?? "Audience")}</span><input data-fmwfe-audience value="${String(esc(arrayValue(step.audience).join(', ')))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_960591e2ed87fb","internal, customer") ?? "internal, customer")}"></label></div>`;
      if (type === 'section') {
        const meta = objectValue(objectValue(node.props).workflow_section);
        return `<div class="fmwfe-inspector"><h4>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_956b329890836d","Section flow") ?? "Section flow")}</h4><label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_8cf345002184e5","Name") ?? "Name")}</span><input data-fmwfe-title value="${String(esc(firstText(meta.title, node.name)))}"></label>
          <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_386f0bf7477d03","Show when") ?? "Show when")}</span><input data-fmwfe-when value="${String(esc(firstText(meta.when)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_2975b72b690b92","Optional condition") ?? "Optional condition")}"></label>
          <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_db0640bcb0df97","Entrance") ?? "Entrance")}</span><select data-fmwfe-transition>${String(['grow','fade','slide','none'].map((value) => `<option ${firstText(objectValue(meta.transition).type, 'grow') === value ? 'selected' : ''}>${value}</option>`).join(''))}</select></label></div>`;
      }
      if (type === 'field') {
        const cfg = objectValue(objectValue(node.props).config);
        return `<div class="fmwfe-inspector"><h4>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_f424dcdf79099f","Field Settings") ?? "Field Settings")}</h4>
          <div class="fmwfe-inspector-group"><strong>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_a20b1dbdbc6449","Field") ?? "Field")}</strong><label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_29dbd3d8b69f55","Title") ?? "Title")}</span><input data-fmwfe-title value="${String(esc(firstText(cfg.label, node.name)))}"></label>
            <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_aa136ecb65672f","Description") ?? "Description")}</span><textarea data-fmwfe-description placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_4c36d4672bb9ed","Helpful context shown below the title") ?? "Helpful context shown below the title")}">${String(esc(firstText(cfg.description)))}</textarea></label>
            <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_8802aaa05aaa4a","Variable type") ?? "Variable type")}</span><select data-fmwfe-kind>${String(kinds.map((kind) => `<option value="${esc(kind.id)}" ${kind.id === cfg.kind ? 'selected' : ''}>${esc(kind.label)}</option>`).join(''))}</select></label>
            <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_0ee9789116b880","Output variable") ?? "Output variable")}</span><input data-fmwfe-writes value="${String(esc(firstText(cfg.writes)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_2cd9214f1f578e","params.field_name") ?? "params.field_name")}"></label>
            <label class="check"><input type="checkbox" data-fmwfe-required ${String(cfg.required ? 'checked' : '')}>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_2b9a93fc89378f"," Required") ?? " Required")}</label></div>
          ${String(FIELD_PRESENTATIONS.has(cfg.kind) ? `<div class="fmwfe-inspector-group"><strong>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_627546e4279386","Choices") ?? "Choices")}</strong><label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_f309f6468d8a53","Layout") ?? "Layout")}</span><select data-fmwfe-choice-style>${['tiles','radio','dropdown'].map((value) => `<option ${firstText(cfg.choice_style, 'tiles') === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><div class="fmwfe-choice-list">${choiceRowsHtml(cfg.options)}</div><button type="button" class="fmwfe-add-choice" data-fmwfe-choice-add><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_9843402d556eeb"," Add choice") ?? " Add choice")}</button></div>` : '')}
          <div class="fmwfe-inspector-group"><strong>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_3525af194bf016","Visibility & motion") ?? "Visibility & motion")}</strong><label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_386f0bf7477d03","Show when") ?? "Show when")}</span><input data-fmwfe-when value="${String(esc(firstText(cfg.when)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_2975b72b690b92","Optional condition") ?? "Optional condition")}"></label>
            <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_db0640bcb0df97","Entrance") ?? "Entrance")}</span><select data-fmwfe-transition>${String(['fade','grow','slide','none'].map((value) => `<option ${firstText(cfg.transition, 'fade') === value ? 'selected' : ''}>${value}</option>`).join(''))}</select></label></div></div>`;
      }
      const visibility = objectValue(objectValue(node.props).workflow_visibility);
      return `<div class="fmwfe-inspector"><h4>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_eb6d93dcf4a28a","Element flow") ?? "Element flow")}</h4><div class="fmwfe-muted" style="padding:0">${((v0) => globalThis.PlatformLanguage?.htmlText("doc-workflow","m_ac1207164a103f",`${v0} remains a normal visual element. These settings only control when it enters the workflow.`,{v0}) ?? `${v0} remains a normal visual element. These settings only control when it enters the workflow.`)(esc(firstText(node.name, node.type)))}</div>
        <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_386f0bf7477d03","Show when") ?? "Show when")}</span><input data-fmwfe-when value="${String(esc(firstText(visibility.when)))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_2975b72b690b92","Optional condition") ?? "Optional condition")}"></label>
        <label><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_db0640bcb0df97","Entrance") ?? "Entrance")}</span><select data-fmwfe-transition>${String(['fade','grow','slide','none'].map((value) => `<option ${firstText(visibility.transition, 'fade') === value ? 'selected' : ''}>${value}</option>`).join(''))}</select></label></div>`;
    }
    ...
    function renderFlowHost(host) {
      ...
      syncCurrent();
      host.innerHTML = `<div class="fmwfe-flow"><div class="fmwfe-flow-head"><div><strong>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_ae23b158b6c400","Flow structure") ?? "Flow structure")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_e4323cd0b4bfd7","Pages, dependencies, and entrance order") ?? "Pages, dependencies, and entrance order")}</span></div><i class="fas fa-diagram-project"></i></div><div class="fmwfe-tree">${String(steps().map((step, pageIndex) => {
        const design = ensureStepDesign(step);
        const sections = arrayValue(objectValue(design.root).children).filter((node) => node.type === 'frame');
        return `<div><button data-fmwfe-page="${esc(step.id)}" class="${step.id === activeStepId && !selectedId ? 'active' : ''}"><i class="fas fa-file"></i><span>${esc(firstText(step.title, `Page ${pageIndex + 1}`))}</span>${step.when ? `<b>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_11cf7fd40040a9","IF") ?? "IF")}</b>` : ''}</button>${sections.map((section) => {
    ...
    function renderPreviewNav(mode) {
      ...
      previewNav.innerHTML = `<button type="button" data-fmwfe-prev ${String(index === 0 ? 'disabled' : '')}><i class="fas fa-arrow-left"></i>${(globalThis.PlatformLanguage?.htmlText("doc-workflow","m_9c5b830c019950"," Previous") ?? " Previous")}</button><span>${((v1,v2) => globalThis.PlatformLanguage?.htmlText("doc-workflow","m_38042c9b59d2e5",`Page ${v1} of ${v2} · preview answers reset when you return to the builder`,{v1,v2}) ?? `Page ${v1} of ${v2} · preview answers reset when you return to the builder`)(index + 1,steps().length)}</span><button type="button" class="primary" data-fmwfe-next>${String(index === steps().length - 1 ? '<i class="fas fa-check"></i> Submit' : 'Next <i class="fas fa-arrow-right"></i>')}</button>`;

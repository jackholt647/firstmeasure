...
(function(){
  ...
  function mount(context = {}){
    ...
    function renderLibrary(query = ''){
      ...
      root.innerHTML = `<div class="fm-workflow-picker"><div class="fm-workflow-picker-head"><button type="button" data-library-close aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_121372231b5699","Back") ?? "Back")}"><i class="fa-solid fa-arrow-left"></i></button><h3>${(globalThis.PlatformLanguage?.htmlText("signatures","m_2ebfdc1730afd2","Add Work") ?? "Add Work")}</h3></div><input class="fm-workflow-search" data-library-search type="search" value="${String(esc(query))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("signatures","m_721025547915c1","Search job types and workflows") ?? "Search job types and workflows")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_531c49ff18ea3b","Search job types") ?? "Search job types")}"><div class="fm-workflow-library">${String(matches.length ? matches.map((entry, index) => `${(index === 0 || matches[index - 1]?.category !== entry.category) ? `<h4 class="fm-library-heading">${esc(entry.frequent ? 'Frequently used' : entry.category)}</h4>` : ''}<button type="button" class="fm-library-item" data-library-scope="${esc(entry.id)}"><i class="fa-solid ${esc(entry.icon || 'fa-list-check')}"></i><span><strong>${esc(entry.title)}</strong><small>${esc(entry.description)}</small></span><i class="fa-solid fa-chevron-right"></i></button>`).join('') : `<div class="fm-sig-empty"><div><i class="fa-solid fa-magnifying-glass"></i><strong>${(globalThis.PlatformLanguage?.htmlText("signatures","m_18f4bab14c1217","No available workflows match that search.") ?? "No available workflows match that search.")}</strong></div></div>`)}</div></div>`;
    ...
    function waitingChips(item){
      ...
      if (status === 'canceled') return [`<span class="fm-chip fm-chip-muted"><i class="fa-solid fa-ban"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_43290aaa17806c","Canceled") ?? "Canceled")}</span>`];
      if (status === 'declined') return [`<span class="fm-chip fm-chip-danger"><i class="fa-solid fa-circle-xmark"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_bf5390659e3677","Declined") ?? "Declined")}</span>`];
      if (item.kind !== 'document') return [];
      if (status === 'completed') return [`<span class="fm-chip fm-chip-success"><i class="fa-solid fa-circle-check"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_8cb6b086a0e69c","Done") ?? "Done")}</span>`];
      ...
      if (item.sent === false && status === 'ready') chips.unshift(`<span class="fm-chip fm-chip-info"><i class="fa-solid fa-paper-plane"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_9fcfa20f91b853","Not sent") ?? "Not sent")}</span>`);
      return chips;
    }
    ...
    function openCancelSheet(item, { onCanceled } = {}){
      const sheet = openSheet(`
        <h4>${((v0) => globalThis.PlatformLanguage?.htmlText("signatures","m_3199eb11d1297f",`Cancel “${v0}”?`,{v0}) ?? `Cancel “${v0}”?`)(esc(item.title || 'this work'))}</h4>
        <small>${(globalThis.PlatformLanguage?.htmlText("signatures","m_e570dab38a72a6","The customer's link stops working immediately. The item stays in this list as canceled.") ?? "The customer's link stops working immediately. The item stays in this list as canceled.")}</small>
        <div class="fm-cancel-form">
          <label>${(globalThis.PlatformLanguage?.htmlText("signatures","m_fa4eab36ffa898","Reason ") ?? "Reason ")}<span style="font-weight:600;color:#98a2b3">${(globalThis.PlatformLanguage?.htmlText("signatures","m_82710819dd8da8","(optional)") ?? "(optional)")}</span></label>
          <textarea data-cancel-reason placeholder="${(globalThis.PlatformLanguage?.htmlText("signatures","m_2c2da875072dfa","Why is this being canceled?") ?? "Why is this being canceled?")}"></textarea>
          <label class="fm-cancel-choice"><input type="radio" name="fm-cancel-visibility" value="visible" checked><span><strong>${(globalThis.PlatformLanguage?.htmlText("signatures","m_58ca9f8d152659","Keep visible to the customer") ?? "Keep visible to the customer")}</strong><small>${(globalThis.PlatformLanguage?.htmlText("signatures","m_609fe38989726c","The customer portal shows it as canceled.") ?? "The customer portal shows it as canceled.")}</small></span></label>
          <label class="fm-cancel-choice"><input type="radio" name="fm-cancel-visibility" value="hidden"><span><strong>${(globalThis.PlatformLanguage?.htmlText("signatures","m_0ca52809b79e3b","Hide from the customer") ?? "Hide from the customer")}</strong><small>${(globalThis.PlatformLanguage?.htmlText("signatures","m_3f7d5d03469573","It disappears from the customer portal entirely.") ?? "It disappears from the customer portal entirely.")}</small></span></label>
          <div class="fm-cancel-actions"><button type="button" data-cancel-dismiss>${(globalThis.PlatformLanguage?.htmlText("signatures","m_226649e4dd2138","Keep it") ?? "Keep it")}</button><button type="button" class="is-danger" data-cancel-confirm>${(globalThis.PlatformLanguage?.htmlText("signatures","m_20ee63ef8d3df1","Cancel work") ?? "Cancel work")}</button></div>
        </div>`);
    ...
    function openDeliverySheet(added){
      ...
      const sheet = openSheet(`
        <h4>${((v0) => globalThis.PlatformLanguage?.htmlText("signatures","m_637eb6c5af9bc1",`“${v0}” is ready`,{v0}) ?? `“${v0}” is ready`)(esc(added.title || 'New work'))}</h4>
        <small>${(globalThis.PlatformLanguage?.htmlText("signatures","m_b8b40bb59c4daa","How should the customer complete it?") ?? "How should the customer complete it?")}</small>
        <button type="button" class="fm-work-action" data-delivery-device><i class="fa-solid fa-mobile-alt"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_ecd62f5abc6beb","Open on this device now") ?? "Open on this device now")}</button>
        <button type="button" class="fm-work-action" data-delivery-portal><i class="fa-solid fa-paper-plane"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_9402bccd71676d","Send to the customer portal") ?? "Send to the customer portal")}</button>
        <button type="button" class="fm-work-action" data-delivery-later><i class="fa-solid fa-clock"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_fdcf5a3ea78fe1","Decide later") ?? "Decide later")}</button>
        ${String(replaced.length ? `<small style="margin-top:8px">${(globalThis.PlatformLanguage?.htmlText("signatures","m_ef71068fcf4003","Does this replace earlier work?") ?? "Does this replace earlier work?")}</small>${replaced.map((item, index) => `<button type="button" class="fm-work-action is-danger" data-delivery-replace="${index}"><i class="fa-solid fa-ban"></i>${((v1) => globalThis.PlatformLanguage?.htmlText("signatures","m_68c760d3b53c89",`Cancel “${v1}”…`,{v1}) ?? `Cancel “${v1}”…`)(esc(item.title || 'earlier work'))}</button>`).join('')}` : '')}`);
    ...
    function openManageSheet(item, { includeOpen = true, onCanceled } = {}){
      ...
      const sheet = openSheet(`
        <h4>${String(esc(item.title || (globalThis.PlatformLanguage?.text("signatures","m_9acd909e2fe732","Project work") ?? "Project work")))}</h4>
        ${String(includeOpen ? `<button type="button" class="fm-work-action" data-manage-open><i class="fa-solid fa-arrow-up-right-from-square"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_c25cc66b28cc9d","Open") ?? "Open")}</button>` : '')}
        ${String(active ? `<button type="button" class="fm-work-action" data-manage-portal><i class="fa-solid fa-paper-plane"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_fa63ef6069048a","Send to customer portal") ?? "Send to customer portal")}</button>` : '')}
        ${String(active ? `<button type="button" class="fm-work-action is-danger" data-manage-cancel><i class="fa-solid fa-ban"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_788d9f90e20689","Cancel this work…") ?? "Cancel this work…")}</button>` : '')}
        <button type="button" class="fm-work-action" data-manage-close><i class="fa-solid fa-xmark"></i>${(globalThis.PlatformLanguage?.htmlText("signatures","m_3742924668fb10","Close") ?? "Close")}</button>`);
    ...
    async function loadDocument(id, phase = ''){
      ...
      try {
        ...
        if (!destroyed) root.innerHTML = `<div class="fm-sig-head"><button type="button" data-signature-back aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_121372231b5699","Back") ?? "Back")}"><i class="fa-solid fa-arrow-left"></i></button><h2>${(globalThis.PlatformLanguage?.htmlText("signatures","m_c711e170be0c5a","Customer agreement") ?? "Customer agreement")}</h2></div><div class="fm-sig-empty"><strong>${String(esc(error?.message || 'Could not open this agreement.'))}</strong></div>`;
    ...
    function renderWorkflow(){
      ...
      root.innerHTML = `<div class="fm-sig-head"><button type="button" data-signature-back aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_121372231b5699","Back") ?? "Back")}"><i class="fa-solid fa-arrow-left"></i></button><h2>${String(esc(doc.title || (globalThis.PlatformLanguage?.text("signatures","m_7c0b81b4d913ff","On-site quote") ?? "On-site quote")))}</h2><button type="button" data-detail-menu aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_a0c3b163769e12","Manage this work") ?? "Manage this work")}" style="margin-left:auto"><i class="fa-solid fa-ellipsis-vertical"></i></button></div><div class="fm-field-workflow" data-field-workflow></div>`;
      root.querySelector('[data-signature-back]')?.addEventListener('click', loadList);
    ...
    function renderDocument(){
      ...
      root.classList.remove('is-workflow');
      const doc = object(detail?.document);
      root.innerHTML = `<div class="fm-sig-head"><button type="button" data-signature-back aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_121372231b5699","Back") ?? "Back")}"><i class="fa-solid fa-arrow-left"></i></button><h2>${String(esc(doc.title || (globalThis.PlatformLanguage?.text("signatures","m_c711e170be0c5a","Customer agreement") ?? "Customer agreement")))}</h2><button type="button" data-detail-menu aria-label="${(globalThis.PlatformLanguage?.htmlText("signatures","m_a0c3b163769e12","Manage this work") ?? "Manage this work")}" style="margin-left:auto"><i class="fa-solid fa-ellipsis-vertical"></i></button></div><div class="fm-doc-handoff"><i class="fa-solid fa-circle-check"></i><span>${String(detail?.workflow ? 'Workflow prepared. Complete it on this device or send it to the customer portal.' : 'Complete this document on this device or send it to the customer portal.')}</span><span class="fm-doc-actions">${String(detail?.workflow ? `<button type="button" class="fm-doc-edit" data-edit-workflow>${(globalThis.PlatformLanguage?.htmlText("signatures","m_5b9378df7220c1","Edit") ?? "Edit")}</button>` : '')}<button type="button" class="fm-doc-portal" data-send-portal>${(globalThis.PlatformLanguage?.htmlText("signatures","m_649dd47ec9c618","Send to portal") ?? "Send to portal")}</button></span></div><div class="fm-sig-stage" data-signature-stage></div>`;
      root.querySelector('[data-signature-back]')?.addEventListener('click', loadList);
      root.querySelector('[data-detail-menu]')?.addEventListener('click', () => openManageSheet(detailItem(doc), { includeOpen:false, onCanceled: loadList }));

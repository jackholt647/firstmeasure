  ...
  function createApp(context = {}){
    ...
    function profitabilityHtml(){
      ...
      return `<section class="fn-metrics">
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_b20395aa99636a","Projected active profit ") ?? "Projected active profit ")}<i class="fas fa-chart-line"></i></span><strong class="${String(projectedProfit >= 0 ? 'fn-positive' : 'fn-negative')}">${String(money(projectedProfit))}</strong><small>${((v2,v3) => globalThis.PlatformLanguage?.text("financials","m_abba3eaded07d0",`${v2} active project${v3}`,{v2,v3}) ?? `${v2} active project${v3}`)(activeCount,activeCount === 1 ? '' : 's')}</small></div>
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_129da9e878009d","Profit to date ") ?? "Profit to date ")}<i class="fas fa-coins"></i></span><strong class="${String(profitToDate >= 0 ? 'fn-positive' : 'fn-negative')}">${String(money(profitToDate))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_7be7089e8807f3","Collected revenue less paid project costs") ?? "Collected revenue less paid project costs")}</small></div>
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_f8e2f8481620fe","Owed by period end ") ?? "Owed by period end ")}<i class="fas fa-file-invoice-dollar"></i></span><strong>${String(money(owed))}</strong><small>${((v7) => globalThis.PlatformLanguage?.text("financials","m_e146b7e444b6f1",`Scheduled customer obligations due by ${v7}`,{v7}) ?? `Scheduled customer obligations due by ${v7}`)(esc(dateLabel(selectedRange.endInclusive)))}</small></div>
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_add2f7f9427cda","Revenue in period ") ?? "Revenue in period ")}<i class="fas fa-arrow-trend-up"></i></span><strong>${String(money(revenueInRange))}</strong><small>${((v9) => globalThis.PlatformLanguage?.text("financials","m_44577439ce4ab7",`Actual customer money clearing in this ${v9}`,{v9}) ?? `Actual customer money clearing in this ${v9}`)(esc(state.grain))}</small></div>
        </section>
        <section class="fn-card"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_7f51d2b5cbf408","Active project profitability") ?? "Active project profitability")}</h2><p>${((v10) => globalThis.PlatformLanguage?.text("financials","m_02f87093df8bfd",`Projected totals alongside the financial position at the end of the selected ${v10}.`,{v10}) ?? `Projected totals alongside the financial position at the end of the selected ${v10}.`)(esc(state.grain))}</p></div><span class="fn-count">${((v11) => globalThis.PlatformLanguage?.text("financials","m_32a87082fe85c2",`${v11} active`,{v11}) ?? `${v11} active`)(activeCount)}</span></div>
          ${String(activeRows.length ? `<div class="fn-table-wrap"><table class="fn-table"><thead><tr><th>Project</th><th>Schedule</th><th>Status</th><th class="fn-right">Projected revenue</th><th class="fn-right">Projected profit</th><th class="fn-right">Profit by period end</th><th class="fn-right">Customer owed</th><th class="fn-right">Margin</th></tr></thead><tbody>${activeRows.map((row) => {
          ...
          }).join('')}</tbody></table></div>${state.data?.hasMore ? '<div class="fn-page-actions"><button class="fn-btn" data-fn-action="load-more" '+(state.loading ? 'disabled' : '')+'><i class="fas fa-plus"></i> Load more projects</button></div>' : ''}` : '<div class="fn-empty"><i class="fas fa-folder-open"></i><strong>No active financial projects yet</strong><span>Signed project revenue and costs will appear here.</span></div>')}
        </section>
        <section class="fn-card"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_ee752842d445f7","Revenue and profitability by cycle") ?? "Revenue and profitability by cycle")}</h2><p>${((v13) => globalThis.PlatformLanguage?.text("financials","m_81c887bb713805",`Previous ${v13} cycles are based on cleared customer payments and actual outgoing payments.`,{v13}) ?? `Previous ${v13} cycles are based on cleared customer payments and actual outgoing payments.`)(esc(state.grain))}</p></div></div><div class="fn-history-grid">${String(cycles.map((cycle) => `<article class="fn-cycle ${cycle.current ? 'current' : ''}"><span class="fn-cycle-label">${esc(cycle.label)}${cycle.current ? ' · Current' : ''}</span><strong class="fn-cycle-profit ${cycle.profit >= 0 ? 'fn-positive' : 'fn-negative'}">${money(cycle.profit, true)}</strong><div class="fn-cycle-row"><span>Revenue</span><b>${money(cycle.revenue, true)}</b></div><div class="fn-cycle-row"><span>Expenses</span><b>${money(cycle.expenses, true)}</b></div><div class="fn-cycle-bar"><span style="width:${Math.round(Math.abs(cycle.profit) / maxProfit * 100)}%;background:${cycle.profit >= 0 ? '#12b76a' : '#f04438'}"></span></div></article>`).join(''))}</div></section>`;
    }
    ...
    function reconcileRowHtml(payment, cleared){
      ...
      return `<tr>
        ...
        <td>${projectId ? `<button type="button" class="fn-link" data-fn-open-project="${esc(projectId)}">${esc(clean(payment.project_title) || projectId)}</button>` : `<span class="fn-muted">${(globalThis.PlatformLanguage?.text("financials","m_500872d3c049f6","Company") ?? "Company")}</span>`}</td>
        ...
        <td>${cleared ? esc(dateLabel(payment.cleared_at, { year:false })) : `<span class="fn-muted">${(globalThis.PlatformLanguage?.text("financials","m_df2a233dc044ae","Awaiting bank") ?? "Awaiting bank")}</span>`}</td>
        <td class="fn-right">${cleared
          ? `<button type="button" class="fn-btn" data-fn-unclear-payment="${String(esc(id))}" ${String(state.loading ? 'disabled' : '')}><i class="fas fa-ban"></i>${(globalThis.PlatformLanguage?.text("financials","m_121bd1ce330c8e"," Unclear") ?? " Unclear")}</button>`
          : `<button type="button" class="fn-btn primary" data-fn-clear-payment="${String(esc(id))}" ${String(state.loading ? 'disabled' : '')}><i class="fas fa-check-double"></i>${(globalThis.PlatformLanguage?.text("financials","m_040ce86d2436e1"," Mark cleared") ?? " Mark cleared")}</button>`}</td>
      </tr>`;
    }
    ...
    function reconcileHtml(){
      ...
      return `<section class="fn-metrics">
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_66cfe4d15dbd80","Awaiting the bank ") ?? "Awaiting the bank ")}<i class="fas fa-hourglass-half"></i></span><strong>${String(number(totals.uncleared_count))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_abdd734e2a6c2f","Settled payments not yet confirmed against a statement") ?? "Settled payments not yet confirmed against a statement")}</small></div>
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_1e774361086e9c","Uncleared in ") ?? "Uncleared in ")}<i class="fas fa-arrow-down"></i></span><strong class="fn-positive">${String(money(totals.uncleared_inbound_cents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_7492b6603e6c7e","Recorded customer money not yet confirmed") ?? "Recorded customer money not yet confirmed")}</small></div>
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_4741f245997513","Uncleared out ") ?? "Uncleared out ")}<i class="fas fa-arrow-up"></i></span><strong class="fn-negative">${String(money(totals.uncleared_outbound_cents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_33123704a14dd7","Refunds and disbursements not yet confirmed") ?? "Refunds and disbursements not yet confirmed")}</small></div>
          <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_a63049b96293f9","Uncleared net ") ?? "Uncleared net ")}<i class="fas fa-scale-balanced"></i></span><strong class="${String(unclearedNet >= 0 ? 'fn-positive' : 'fn-negative')}">${String(money(unclearedNet))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_50d6a2d62c5016","Cash flow shifts by this much once confirmed") ?? "Cash flow shifts by this much once confirmed")}</small></div>
        </section>
        <section class="fn-card"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_fde479c103476d","To reconcile") ?? "To reconcile")}</h2><p>${(globalThis.PlatformLanguage?.text("financials","m_256721a32d46c6","Confirm each recorded settlement against your bank statement. Cleared payments upgrade the cash-flow forecast from estimated to actual.") ?? "Confirm each recorded settlement against your bank statement. Cleared payments upgrade the cash-flow forecast from estimated to actual.")}</p></div><span class="fn-count">${String(uncleared.length)}</span></div>
          ${String(uncleared.length ? `<div class="fn-table-wrap"><table class="fn-table"><thead><tr><th>Date</th><th>Project</th><th>Payment</th><th class="fn-right">Amount</th><th>Cleared</th><th></th></tr></thead><tbody>${uncleared.map((payment) => reconcileRowHtml(payment, false)).join('')}</tbody></table></div>` : '<div class="fn-empty"><i class="fas fa-check-double"></i><strong>Everything is reconciled</strong><span>New settled payments will appear here until they are confirmed at the bank.</span></div>')}
        </section>
        <section class="fn-card"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_d82b01c94f9f33","Recently cleared") ?? "Recently cleared")}</h2><p>${((v7,v8,v9) => globalThis.PlatformLanguage?.text("financials","m_4244cf5c731306",`${v7} confirmed · ${v8} in · ${v9} out`,{v7,v8,v9}) ?? `${v7} confirmed · ${v8} in · ${v9} out`)(number(totals.cleared_count),money(totals.cleared_inbound_cents),money(totals.cleared_outbound_cents))}</p></div></div>
          ${String(cleared.length ? `<div class="fn-table-wrap"><table class="fn-table"><thead><tr><th>Date</th><th>Project</th><th>Payment</th><th class="fn-right">Amount</th><th>Cleared</th><th></th></tr></thead><tbody>${cleared.slice(0, 25).map((payment) => reconcileRowHtml(payment, true)).join('')}</tbody></table></div>` : '<div class="fn-empty compact"><i class="fas fa-building-columns"></i><strong>Nothing cleared yet</strong><span>Payments you confirm will collect here.</span></div>')}
        </section>`;
    }
    ...
    function payoutRowHtml(payout){
      ...
      return `<tr data-fn-payout="${esc(payout.id)}" tabindex="0">
        ...
        <td>${arrived ? esc(dateLabel(arrived)) : `<span class="fn-muted">${(globalThis.PlatformLanguage?.text("financials","m_8e048627901567","Not arrived") ?? "Not arrived")}</span>`}</td>
    ...
    function payoutDetailModalHtml(){
      ...
      const body = detail.loading
        ? `<div class="fn-empty compact"><i class="fas fa-circle-notch fn-spin"></i><strong>${(globalThis.PlatformLanguage?.text("financials","m_abe6a9e399dd32","Loading payout") ?? "Loading payout")}</strong></div>`
        : detail.error
          ? `<div class="fn-empty compact"><i class="fas fa-triangle-exclamation"></i><strong>${(globalThis.PlatformLanguage?.text("financials","m_ec28130f32485c","Payout could not load") ?? "Payout could not load")}</strong><span>${String(esc(detail.error?.message || clean(detail.error) || 'Unknown error'))}</span></div>`
          : `<div class="fn-modal-stats">
              <div class="fn-modal-stat"><span>${(globalThis.PlatformLanguage?.text("financials","m_c87aa4a42612df","Amount to bank") ?? "Amount to bank")}</span><strong>${String(moneyFine(payout.amount_cents))}</strong></div>
              <div class="fn-modal-stat"><span>${(globalThis.PlatformLanguage?.text("financials","m_fcc3723f722f18","Processing fees") ?? "Processing fees")}</span><strong>${String(moneyFine(payout.fee_cents))}</strong></div>
              <div class="fn-modal-stat"><span>${(globalThis.PlatformLanguage?.text("financials","m_c6aef2a3790870","Expected arrival") ?? "Expected arrival")}</span><strong>${String(clean(payout.expected_arrival_at) ? esc(dateLabel(payout.expected_arrival_at)) : '—')}</strong></div>
              <div class="fn-modal-stat"><span>${(globalThis.PlatformLanguage?.text("financials","m_b7ee7c97c74f9b","Arrived") ?? "Arrived")}</span><strong>${String(clean(payout.arrived_at) ? esc(dateLabel(payout.arrived_at)) : 'Not yet')}</strong></div>
            </div>
            ${String(transactions.length ? `<div class="fn-table-wrap"><table class="fn-table"><thead><tr><th>Date</th><th>Project</th><th>Invoice</th><th class="fn-right">Gross</th><th class="fn-right">Fee</th><th class="fn-right">Net</th></tr></thead><tbody>${transactions.map((transaction) => {
              ...
              return `<tr>
                ...
                <td>${projectId ? `<button type="button" class="fn-link" data-fn-open-project="${esc(projectId)}">${esc(clean(transaction.project_title) || projectId)}</button>` : '<span class="fn-muted">Company</span>'}</td>
            ...
            }).join('')}</tbody></table></div>` : '<div class="fn-empty compact"><i class="fas fa-receipt"></i><strong>No member transactions</strong><span>This payout has no linked payment records.</span></div>')}`;
      return `<div class="fn-shade" data-fn-payout-shade><div class="fn-modal" role="dialog" aria-modal="true" aria-label="${(globalThis.PlatformLanguage?.text("financials","m_335102d80b2040","Payout detail") ?? "Payout detail")}">
        <div class="fn-modal-head"><div><strong>${((v0) => globalThis.PlatformLanguage?.text("financials","m_c32410e74d6868",`Payout ${v0}`,{v0}) ?? `Payout ${v0}`)(esc(clean(payout.provider_payout_id) || clean(detail.payoutId)))}</strong><span class="fn-modal-sub">${String(detail.loading ? 'Loading…' : `${array(detail.transactions).length} transaction${array(detail.transactions).length === 1 ? '' : 's'} · ${esc(clean(payout.currency) || 'USD')}`)}</span></div>
        <div style="display:flex;align-items:center;gap:10px">${String(detail.loading ? '' : payoutBadge(payout.status))}<button type="button" data-fn-close-payout aria-label="${(globalThis.PlatformLanguage?.text("financials","m_43963f1b979b52","Close payout detail") ?? "Close payout detail")}"><i class="fas fa-xmark"></i></button></div></div>
    ...
    function payoutsHtml(){
      ...
      return `<section class="fn-metrics payouts">
          <div class="fn-metric" data-fn-tile="balance_pending"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_4044a709c2c433","Pending sweep ") ?? "Pending sweep ")}<i class="fas fa-wallet"></i></span><strong>${String(moneyFine(summary.balance_pending_cents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_3f21d8055c5d00","Captured payments awaiting the next batch") ?? "Captured payments awaiting the next batch")}</small></div>
          <div class="fn-metric" data-fn-tile="in_transit"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_9ff03ce19be18f","In transit ") ?? "In transit ")}<i class="fas fa-building-columns"></i></span><strong class="fn-available">${String(moneyFine(summary.in_transit_cents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_bbe4da53a62b2d","Batched and on the way to your bank") ?? "Batched and on the way to your bank")}</small></div>
          <div class="fn-metric" data-fn-tile="paid_out_30d"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_a22666a4be492d","Paid out (30d) ") ?? "Paid out (30d) ")}<i class="fas fa-circle-check"></i></span><strong class="fn-positive">${String(moneyFine(summary.paid_out_30d_cents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_799e9b77e1dc9a","Arrived at the bank in the last 30 days") ?? "Arrived at the bank in the last 30 days")}</small></div>
          <div class="fn-metric" data-fn-tile="fees_30d"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_ceabd802db7bfd","Fees (30d) ") ?? "Fees (30d) ")}<i class="fas fa-percent"></i></span><strong class="${String(number(summary.fees_30d_cents) ? 'fn-negative' : '')}">${String(moneyFine(summary.fees_30d_cents))}</strong><small>${String(number(summary.gross_30d_cents) ? `Effective rate ${effectiveRate.toFixed(2)}% on ${moneyFine(summary.gross_30d_cents)}` : 'No processed volume yet')}</small></div>
          <div class="fn-metric" data-fn-tile="next_payout"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_e3ec213ac69265","Next payout ") ?? "Next payout ")}<i class="fas fa-calendar-day"></i></span><strong class="fn-expected">${String(clean(nextPayout.payout_id) ? moneyFine(nextPayout.amount_cents) : '—')}</strong><small>${String(clean(nextPayout.payout_id) ? (clean(nextPayout.expected_arrival_at) ? `Expected ${esc(dateLabel(nextPayout.expected_arrival_at))}` : 'Arrival date pending') : 'No payout scheduled')}</small></div>
          <div class="fn-metric" data-fn-tile="open_disputes"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_b7febf91468fd6","Open disputes ") ?? "Open disputes ")}<i class="fas fa-scale-balanced"></i></span><strong class="${String(openDisputes ? 'fn-negative' : '')}">${String(openDisputes)}</strong><small>${String(openDisputes ? 'Respond before the deadline to contest' : 'No open chargebacks')}</small></div>
        </section>
        <section class="fn-card"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_e06874e8b912d9","Payouts") ?? "Payouts")}</h2><p>${(globalThis.PlatformLanguage?.text("financials","m_2e930010f0998a","Batches swept from your processing balance to the bank, newest first.") ?? "Batches swept from your processing balance to the bank, newest first.")}</p></div><div class="fn-chips" role="group" aria-label="${(globalThis.PlatformLanguage?.text("financials","m_5bcdc053c26110","Payout status filter") ?? "Payout status filter")}">${String(PAYOUT_FILTERS.map((key) => `<button type="button" class="fn-chip ${state.payoutFilter === key ? 'active' : ''}" data-fn-payout-filter="${esc(key)}">${key === 'all' ? 'All' : esc(key)}<small>${countFor(key)}</small></button>`).join(''))}</div></div>
          ${String(filtered.length ? `<div class="fn-table-wrap"><table class="fn-table"><thead><tr><th>Payout</th><th>Status</th><th class="fn-right">Amount</th><th class="fn-right">Fees</th><th>Expected arrival</th><th>Arrived</th><th class="fn-right">Transactions</th></tr></thead><tbody>${filtered.map(payoutRowHtml).join('')}</tbody></table></div>`
          : payouts.length ? '<div class="fn-empty"><i class="fas fa-filter"></i><strong>No payouts match this filter</strong><span>Try another status.</span></div>'
          : '<div class="fn-empty"><i class="fas fa-money-bill-transfer"></i><strong>No payouts yet</strong><span>Payouts will appear once payment processing is live.</span></div>')}
        </section>
        <section class="fn-card"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_9b7a2edd995dfe","Disputes") ?? "Disputes")}</h2><p>${(globalThis.PlatformLanguage?.text("financials","m_96113a4810ac77","Chargebacks and payment disputes reported by the processor.") ?? "Chargebacks and payment disputes reported by the processor.")}</p></div><span class="fn-count">${String(disputes.length)}</span></div>
          ${String(disputes.length ? `<div class="fn-table-wrap"><table class="fn-table"><thead><tr><th>Status</th><th class="fn-right">Amount</th><th>Reason</th><th>Opened</th><th>Payment</th><th>Project</th><th></th></tr></thead><tbody>${disputes.map((dispute) => {
            ...
            return `<tr data-fn-dispute="${esc(dispute.id)}">
              ...
              <td class="fn-right">${closed ? '<span class="fn-muted">—</span>' : `<button type="button" class="fn-chip" data-fn-dispute-respond="${esc(dispute.id)}" title="Dispute evidence is submitted on the processor's secure merchant portal"><i class="fas fa-arrow-up-right-from-square"></i> Respond at Forward</button>`}</td>
            </tr>`;
          }).join('')}</tbody></table></div>` : '<div class="fn-empty compact"><i class="fas fa-scale-balanced"></i><strong>No disputes</strong><span>Chargebacks reported by the processor will appear here.</span></div>')}
    ...
    function chartHtml(series){
      if (!series.length) return `<div class="fn-chart-empty">${(globalThis.PlatformLanguage?.text("financials","m_b5bf39bc056d36","No days are available in this period.") ?? "No days are available in this period.")}</div>`;
      const width = 1080;
      const height = 360;
    ...
    function transactionRailHtml(transactions, selectedRange){
      ...
      return `<aside class="fn-card fn-transaction-rail"><div class="fn-card-head"><div><h2>${(globalThis.PlatformLanguage?.text("financials","m_ad68f8741d9002","Transactions") ?? "Transactions")}</h2><p>${(globalThis.PlatformLanguage?.text("financials","m_bc494668697e73","Cleared, expected, and reserved movement.") ?? "Cleared, expected, and reserved movement.")}</p></div><span class="fn-count">${String(number(state.data?.transactionCount || visible.length))}</span></div><div class="fn-transaction-list">${String(visible.length ? visible.map((item) => {
      ...
      }).join('') : '<div class="fn-empty compact"><i class="fas fa-receipt"></i><strong>No transactions in this period</strong><span>Undated commitments will appear here too.</span></div>')}${String(state.data?.hasMore ? `<div class="fn-page-actions"><button class="fn-btn" data-fn-action="load-more" ${state.loading ? 'disabled' : ''}>Load more</button></div>` : '')}</div></aside>`;
    }
    ...
    function cashflowHtml(transactions){
      ...
      return `<section class="fn-metrics cash">
        <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_0f5102aad156d3","Available cash ") ?? "Available cash ")}<i class="fas fa-building-columns"></i></span><strong class="fn-available">${String(money(end.available))}</strong><small>${((v1) => globalThis.PlatformLanguage?.text("financials","m_bba8f2c0880969",`Cleared cash${v1}`,{v1}) ?? `Cleared cash${v1}`)(state.openingBalance ? ' plus opening balance' : ' · net movement from zero')}</small></div>
        <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_07d652ce236d27","Expected available ") ?? "Expected available ")}<i class="fas fa-chart-line"></i></span><strong class="fn-expected">${String(money(end.expected))}</strong><small>${((v3,v4) => globalThis.PlatformLanguage?.text("financials","m_41cf05cf456858",`${v3} in · ${v4} out this period`,{v3,v4}) ?? `${v3} in · ${v4} out this period`)(money(expectedIn, true),money(expectedOut, true))}</small></div>
        <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_a2f15a2d87d4e3","Safe to spend ") ?? "Safe to spend ")}<i class="fas fa-shield-heart"></i></span><strong class="${String(end.safe >= 0 ? 'fn-positive' : 'fn-negative')}">${String(money(end.safe))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_0ba2b05af9d11a","Expected cash less unpaid commitments") ?? "Expected cash less unpaid commitments")}</small></div>
        <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_17c29efbe23853","Pending incoming ") ?? "Pending incoming ")}<i class="fas fa-hourglass-half"></i></span><strong>${String(money(end.pendingIncomingCents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_b411f5963a3d0f","Collected but not yet available") ?? "Collected but not yet available")}</small></div>
        <div class="fn-metric"><span class="fn-metric-label">${(globalThis.PlatformLanguage?.text("financials","m_067e43c05b2612","Undated exposure ") ?? "Undated exposure ")}<i class="fas fa-calendar-xmark"></i></span><strong class="${String(end.undatedExposureCents ? 'fn-negative' : '')}">${String(money(end.undatedExposureCents))}</strong><small>${(globalThis.PlatformLanguage?.text("financials","m_7fb2c0748a593b","Reserved without a clearing date") ?? "Reserved without a clearing date")}</small></div>
      </section>
      <div class="fn-cash-workspace">${String(transactionRailHtml(transactions, selectedRange))}<section class="fn-card fn-chart-card"><div class="fn-card-head fn-cash-chart-head"><div><h2>${String(state.openingBalance ? 'Forecast cash position' : 'Net cash movement')}</h2><p>${(globalThis.PlatformLanguage?.text("financials","m_47fe2d7add39b4","Hover a line node or transaction to trace its effect. Safe to spend reserves future and undated outgoing commitments.") ?? "Hover a line node or transaction to trace its effect. Safe to spend reserves future and undated outgoing commitments.")}</p></div><div class="fn-chart-head-actions"><div class="fn-legend"><span><i style="background:#2e90fa"></i>${(globalThis.PlatformLanguage?.text("financials","m_f326bdcd77881a","Available") ?? "Available")}</span><span><i style="background:#7f56d9"></i>${(globalThis.PlatformLanguage?.text("financials","m_ad21d839382e4a","Expected available") ?? "Expected available")}</span><span><i style="background:#12b76a"></i>${(globalThis.PlatformLanguage?.text("financials","m_e4218fdc8067e4","Safe to spend") ?? "Safe to spend")}</span></div><div class="fn-cash-settings"><label class="fn-clearing">${(globalThis.PlatformLanguage?.text("financials","m_d2583a98d76017","Opening $") ?? "Opening $")}<input type="number" step="0.01" value="${String((state.openingBalance / 100).toFixed(2))}" data-fn-opening-balance aria-label="${(globalThis.PlatformLanguage?.text("financials","m_7ccaf7f30436e3","Opening cash balance in dollars") ?? "Opening cash balance in dollars")}"></label><label class="fn-clearing">${(globalThis.PlatformLanguage?.text("financials","m_bae1f3af80c464","Clearing ") ?? "Clearing ")}<input type="number" min="0" max="336" step="1" value="${String(state.clearingHours)}" data-fn-clearing aria-label="${(globalThis.PlatformLanguage?.text("financials","m_4e589dfcef5c24","Customer payment clearing time in hours") ?? "Customer payment clearing time in hours")}">${(globalThis.PlatformLanguage?.text("financials","m_f9b04bf72b9b3a"," hours") ?? " hours")}</label></div></div></div><div class="fn-chart-wrap">${String(chartHtml(series))}</div></section><div class="fn-tooltip" data-fn-tooltip role="tooltip"></div><div class="fn-tx-popover" data-fn-transaction-tooltip role="tooltip"></div></div>`;
    }
    ...
    function showNode(node){
      ...
      tooltip.innerHTML = `<strong>${String(esc(dateLabel(item.date)))} · ${String(esc(labels[line] || line))}</strong><div class="fn-tooltip-total"><span>${(globalThis.PlatformLanguage?.text("financials","m_78ff1aaef0c389","Position") ?? "Position")}</span><b>${String(money(item[line]))}</b></div><div class="fn-tooltip-total"><span>${(globalThis.PlatformLanguage?.text("financials","m_9964885fff7421","Daily change") ?? "Daily change")}</span><b>${String(delta >= 0 ? '+' : '−')}${String(money(Math.abs(delta)))}</b></div><div class="fn-tooltip-exposure"><span><i class="fas fa-calendar-days"></i>${(globalThis.PlatformLanguage?.text("financials","m_854e8e12743170","Dated future exposure ") ?? "Dated future exposure ")}<b>${String(money(item.datedExposureCents))}</b></span><span class="${String(item.undatedExposureCents ? 'warn' : '')}"><i class="fas fa-calendar-xmark"></i>${(globalThis.PlatformLanguage?.text("financials","m_644de3ea02521d","Undated exposure reserved ") ?? "Undated exposure reserved ")}<b>${String(money(item.undatedExposureCents))}</b></span><span><i class="fas fa-hourglass-half"></i>${(globalThis.PlatformLanguage?.text("financials","m_17c29efbe23853","Pending incoming ") ?? "Pending incoming ")}<b>${String(money(item.pendingIncomingCents))}</b></span></div>${String(dayEvents.length ? `<div class="fn-tooltip-list">${dayEvents.slice(0, 6).map((event) => `<div class="fn-tooltip-item"><span>${esc(event.label)}</span><b>${event.amount >= 0 ? '+' : '−'}${money(Math.abs(event.amount))}</b></div>`).join('')}</div>` : '<div class="fn-tooltip-list">No new movement on this day.</div>')}`;
    ...
    function showTransactionDetail(row, transaction){
      ...
      popover.innerHTML = `<strong>${String(esc(transaction.label))}</strong><span>${String(esc(transaction.projectTitle || (transaction.kind === 'payroll' ? 'Company payroll' : 'Company')))}</span><div class="fn-tx-popover-grid"><span>${(globalThis.PlatformLanguage?.text("financials","m_5469184513c8bc","Amount ") ?? "Amount ")}<b>${String(transaction.amount >= 0 ? '+' : '−')}${String(money(Math.abs(transaction.amount)))}</b></span><span>${(globalThis.PlatformLanguage?.text("financials","m_5b1cdca9bcee11","Status ") ?? "Status ")}<b>${String(esc(transactionStateLabel(transaction)))}</b></span><span>${(globalThis.PlatformLanguage?.text("financials","m_21043cdf9f50a0","Known / accrued ") ?? "Known / accrued ")}<b>${String(transaction.knownAt ? esc(dateLabel(transaction.knownAt)) : 'Unknown')}</b></span><span>${String(esc(clearLabel))} <b>${String(at ? esc(dateLabel(at)) : 'Undated')}</b></span></div>`;

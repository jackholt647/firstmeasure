...
(function(){
  ...
  function workforceLabel(kind, form = 'singular'){
    ...
    return cleanText(configured?.[form] || configured?.singular || (kind === 'organization_connection' ? 'Partner' : 'Team'));
  }
  ...
  function panelHtml(){
    return `<div class="mt-app" data-materials-root><div class="mt-empty">${(globalThis.PlatformLanguage?.text("materials","m_9d3e82ecfd10ec","Scope") ?? "Scope")}</div></div>`;
  }
  ...
  function renderOrderDeliveryCalendar(dialog = materialRoot()?.querySelector?.('[data-mt-order-dialog]')){
    ...
    if (!focusList || !Scheduling || !ScheduleView?.renderProjectRangeScheduler) {
      target.innerHTML = `<div class="mt-order-calendar-empty"><i class="fas fa-calendar-xmark"></i><strong>${(globalThis.PlatformLanguage?.text("materials","m_34f144e14d85d9","Calendar unavailable") ?? "Calendar unavailable")}</strong><span>${(globalThis.PlatformLanguage?.text("materials","m_ce68b66a8b5f3b","Select a material list or reload the project calendar.") ?? "Select a material list or reload the project calendar.")}</span></div>`;
      return;
    }
    const selected = selectedOrderLists();
    ...
    root.innerHTML = `
      <div class="mt-top">
        <div class="mt-title">
          ...
          <div><strong>${(globalThis.PlatformLanguage?.text("materials","m_9d3e82ecfd10ec","Scope") ?? "Scope")}</strong><span>${String(escapeHtml(list ? `${list.title || activeTerms.list} active · ${visibleCount} of ${state.lists.length} visible` : (state.project?.address || state.project?.title || 'No scope lists')))}</span></div>
        </div>
        <div class="mt-actions">
          <button type="button" class="mt-icon-btn" data-mt-refresh title="${(globalThis.PlatformLanguage?.text("materials","m_78973ce0cf3403","Refresh") ?? "Refresh")}"><i class="fas fa-rotate"></i></button>
          ${String(activeType === 'material' ? `<button type="button" class="mt-btn${state.pricebookOpen ? ' primary' : ''}" data-mt-open-pricebook ${!list ? 'disabled' : ''}><i class="fas fa-book"></i> Price Book</button>` : '')}
          ...
          ${String(canSchedule ? `<button type="button" class="mt-btn" data-mt-schedule-list="${escapeHtml(list?.id || '')}" ${state.saving || state.schedulingListId ? 'disabled' : ''}><i class="fas ${state.schedulingListId === cleanText(list?.id) ? 'fa-rotate mt-spin' : 'fa-calendar-day'}"></i> Schedule</button>` : '')}
      ...
      <div class="mt-body">
        <main class="mt-main">
          ${String(state.loading ? `<div class="mt-card"><p>Loading scope...</p></div>` : '')}
          ...
          <div class="mt-footer">
            <div class="mt-summary-grid">
              <div class="mt-stat"><span>${(globalThis.PlatformLanguage?.text("materials","m_929d3bd2149645","Projected") ?? "Projected")}</span><strong>${String(money(totals.projected))}</strong></div>
              <div class="mt-stat"><span>${(globalThis.PlatformLanguage?.text("materials","m_f05be3d739b87e","Quoted") ?? "Quoted")}</span><strong>${String(money(totals.quoted))}</strong></div>
              <div class="mt-stat"><span>${(globalThis.PlatformLanguage?.text("materials","m_956173c8527121","Paid") ?? "Paid")}</span><strong>${String(money(totals.paid))}</strong></div>
            </div>
          </div>
        </main>
      </div>
  ...
  function renderLaborProjectionPanel(list){
    ...
    if (state.expenseError) return `<section class="mt-labor-estimate"><div class="mt-labor-estimate-head"><div><strong>${(globalThis.PlatformLanguage?.text("materials","m_64da48200b02da","Estimated labor expense") ?? "Estimated labor expense")}</strong><span>${(globalThis.PlatformLanguage?.text("materials","m_ce86fe78082565","Unavailable") ?? "Unavailable")}</span></div></div><div class="mt-empty">${((v0) => globalThis.PlatformLanguage?.text("materials","m_894fbe7ce53443",`${v0} Refresh the project before relying on this estimate.`,{v0}) ?? `${v0} Refresh the project before relying on this estimate.`)(escapeHtml(state.expenseError))}</div></section>`;
    return `<section class="mt-labor-estimate">
      <div class="mt-labor-estimate-head"><div><strong>${(globalThis.PlatformLanguage?.text("materials","m_64da48200b02da","Estimated labor expense") ?? "Estimated labor expense")}</strong><span>${String(escapeHtml(resourceName || (state.expenseLoading ? 'Updating estimate...' : `Assign a ${workResourceLabel()} to calculate team member rates`)))}</span></div><strong>${String(money(total / 100))}</strong></div>
  ...
  function renderMaterialSection(section, items = sectionItems(section.key)){
    ...
    return `
      <section class="mt-section${String(needsData ? ' needs-data' : '')}" data-mt-section-key="${String(escapeHtml(section.key))}">
        <div class="mt-section-head"><h3><i class="fas ${String(escapeHtml(section.icon))}"></i>${String(escapeHtml(section.title))}</h3><div class="mt-section-head-actions"><span class="${String(needsData ? 'mt-warn' : '')}">${String(needsData ? 'Needs measurements' : `${items.length} items`)}</span>${String(material ? `<button type="button" class="mt-units-toggle${state.unitsMode === 'order' ? ' order' : ''}" data-mt-units-toggle data-fm-tooltip="${state.unitsMode === 'order' ? 'Showing order units (bundles, rolls, pieces). Click for measured amounts.' : 'Showing measured amounts. Click for order units (bundles, rolls, pieces).'}" aria-pressed="${state.unitsMode === 'order' ? 'true' : 'false'}"><i class="fas ${state.unitsMode === 'order' ? 'fa-boxes-stacked' : 'fa-ruler-combined'}"></i>${state.unitsMode === 'order' ? 'Order units' : 'Measured'}</button>` : '')}<button type="button" class="mt-section-add" data-mt-add-section="${String(escapeHtml(section.key))}" title="${((v8,v9) => globalThis.PlatformLanguage?.text("materials","m_c63af829948dff",`Add ${v8} to ${v9}`,{v8,v9}) ?? `Add ${v8} to ${v9}`)(escapeHtml(terms.singular.toLowerCase()),escapeHtml(section.title))}" aria-label="${((v10,v11) => globalThis.PlatformLanguage?.text("materials","m_d8e783b101ecb6",`Add ${v10} to ${v11}`,{v10,v11}) ?? `Add ${v10} to ${v11}`)(escapeHtml(terms.singular.toLowerCase()),escapeHtml(section.title))}" ${String(state.saving ? 'disabled' : '')}><i class="fas fa-plus"></i></button></div></div>
        <table class="mt-table">
          ...
          <thead><tr><th>${String(escapeHtml(terms.singular))}</th>${String(material ? '<th>Color</th>' : '')}${String(labor ? '<th>Pay type</th>' : '')}<th>${(globalThis.PlatformLanguage?.text("materials","m_1a29aea570fbc4","Qty") ?? "Qty")}</th><th>${(globalThis.PlatformLanguage?.text("materials","m_4b91b73dae1ff3","Unit") ?? "Unit")}</th><th>${String(type === 'labor' ? 'Rate' : 'Unit $')}</th><th></th></tr></thead>
          <tbody>${String(items.length ? items.map(renderLineRow).join('') : `<tr><td colspan="${material ? 6 : labor ? 6 : 5}"><div class="mt-empty">No ${escapeHtml(terms.plural.toLowerCase())} in this category.</div></td></tr>`)}</tbody>
        </table>
      </section>
    `;
  }
  ...
  function renderLineRow(item){
    ...
    return `
      <tr class="mt-list-row ${String(incomplete ? 'incomplete' : 'complete')}" data-mt-item-row="${String(escapeHtml(item.id))}" data-mt-list-id="${String(escapeHtml(listId))}" style="--list-color:${String(escapeHtml(listColor))}">
        ...
        <td><button type="button" class="mt-icon-btn" data-mt-remove="${String(escapeHtml(item.id))}" data-mt-list-id="${String(escapeHtml(listId))}" title="${(globalThis.PlatformLanguage?.text("materials","m_f643f568915438","Remove") ?? "Remove")}" ${String(state.saving ? 'disabled' : '')}><i class="fas fa-trash"></i></button></td>
      </tr>
    `;
  }
  ...
  function topBarHtml(){
    ...
    return `<div class="mt-top">
      <div class="mt-title"><i class="fas fa-clipboard-list"></i><div><strong>${(globalThis.PlatformLanguage?.text("materials","m_9d3e82ecfd10ec","Scope") ?? "Scope")}</strong></div></div>
      <div class="mt-actions">
        <button type="button" class="mt-icon-btn" data-mt-refresh title="${(globalThis.PlatformLanguage?.text("materials","m_78973ce0cf3403","Refresh") ?? "Refresh")}"><i class="fas fa-rotate"></i></button>
        ${String(activeType === 'material' ? `<button type="button" class="mt-btn${state.pricebookOpen ? ' primary' : ''}" data-mt-open-pricebook ${!list ? 'disabled' : ''}><i class="fas fa-book"></i> Price Book</button>` : '')}
        <button type="button" class="mt-btn" data-mt-add-custom ${String(!list ? 'disabled' : '')}><i class="fas fa-plus"></i> ${String(escapeHtml(activeTerms.singular))}</button>
    ...
    return `
      <div class="mt-pricebook-panel" data-mt-pricebook-panel>
        <div class="mt-pricebook-head">
          <strong><i class="fas fa-book"></i>${(globalThis.PlatformLanguage?.text("materials","m_cf3630161575f6"," Price Book") ?? " Price Book")}</strong>
          <div class="mt-actions">
            <button type="button" class="mt-icon-btn" data-mt-expand-pricebook title="${(globalThis.PlatformLanguage?.text("materials","m_df1458500a02a8","Open full price book") ?? "Open full price book")}"><i class="fas fa-up-right-and-down-left-from-center"></i></button>
            <button type="button" class="mt-icon-btn" data-mt-close-pricebook title="${(globalThis.PlatformLanguage?.text("materials","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button>
          </div>
        </div>
        <div class="mt-pricebook-body">
          <input class="mt-search" data-mt-pricebook-search value="${String(escapeHtml(state.pricebookSearch))}" placeholder="${(globalThis.PlatformLanguage?.text("materials","m_d87f548c7ada6e","Search price book") ?? "Search price book")}">
  ...
  function renderOrders(){
    return `
      <div class="mt-card" style="margin-top:12px">
        <h3><i class="fas fa-truck"></i>${(globalThis.PlatformLanguage?.text("materials","m_226e67393d8997"," Orders") ?? " Orders")}</h3>
        <div class="mt-orders">
  ...
  function orderRequestDetailsHtml(row){
    return row.details.map((entry) => `<span class="mt-order-detail"><strong>${escapeHtml(entry.label)}:</strong> ${escapeHtml(entry.value)}</span>`).join('') || `<span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_caa672dddbf42e","&mdash;") ?? "&mdash;")}</span>`;
  }
  ...
  function orderRequestHtml(lists = selectedOrderLists()){
    ...
    const sections = lists.map((list) => `<p><strong>${String(escapeHtml(list.title || 'Materials'))}</strong><br><em>${String(escapeHtml(orderDeliveryInstruction(list)))}</em></p><table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%"><thead><tr><th style="${String(cell)}">${(globalThis.PlatformLanguage?.text("materials","m_613d6b4084975e","Material") ?? "Material")}</th><th style="${String(cell)}">${(globalThis.PlatformLanguage?.text("materials","m_b3ecc234f63212","Details") ?? "Details")}</th><th style="${String(cell)}">${(globalThis.PlatformLanguage?.text("materials","m_9c689ddee2f502","Quantity") ?? "Quantity")}</th><th style="${String(cell)}">${(globalThis.PlatformLanguage?.text("materials","m_4b91b73dae1ff3","Unit") ?? "Unit")}</th></tr></thead><tbody>${String(orderRequestRows(list).map((row) => `<tr><td style="${cell}">${escapeHtml(row.name)}</td><td style="${cell}">${row.details.map((entry) => `<strong>${escapeHtml(entry.label)}:</strong> ${escapeHtml(entry.value)}`).join('<br>') || '&mdash;'}</td><td style="${cell}">${escapeHtml(row.quantity)}</td><td style="${cell}">${escapeHtml(row.unit)}</td></tr>`).join(''))}</tbody></table>`).join('<br>');
    return `<p><strong>${String(escapeHtml(orderRequestSubject(lists)))}</strong></p><p>${(globalThis.PlatformLanguage?.text("materials","m_2634bd92bed7f7","Hello,") ?? "Hello,")}</p><p>${(globalThis.PlatformLanguage?.text("materials","m_2dc5eebd0941a4","Please place the following material order for delivery to ") ?? "Please place the following material order for delivery to ")}<strong>${String(escapeHtml(address))}</strong>.</p><p><strong>${(globalThis.PlatformLanguage?.text("materials","m_0e22a1d03e954f","Delivery instructions") ?? "Delivery instructions")}</strong></p>${String(instructions)}${String(sections)}<p>${(globalThis.PlatformLanguage?.text("materials","m_09326393374cc9","Please confirm product availability, quantities, variant and color details, and each requested delivery appointment before processing the order.") ?? "Please confirm product availability, quantities, variant and color details, and each requested delivery appointment before processing the order.")}</p><p>${(globalThis.PlatformLanguage?.text("materials","m_e495d4a9827c67","Thank you.") ?? "Thank you.")}</p>`;
  }
  ...
  function orderRequestPreviewHtml(lists = selectedOrderLists()){
    ...
    const sections = lists.map((list) => `<section class="mt-request-group"><p><strong>${String(escapeHtml(list.title || 'Materials'))}</strong> <em>${((v1) => globalThis.PlatformLanguage?.text("materials","m_da1df6b7a47ee1",`(${v1} items)`,{v1}) ?? `(${v1} items)`)(escapeHtml(orderRequestRows(list).length))}</em><br><em>${String(escapeHtml(orderDeliveryInstruction(list)))}</em></p><table class="mt-order-table"><thead><tr><th>${(globalThis.PlatformLanguage?.text("materials","m_613d6b4084975e","Material") ?? "Material")}</th><th>${(globalThis.PlatformLanguage?.text("materials","m_b3ecc234f63212","Details") ?? "Details")}</th><th>${(globalThis.PlatformLanguage?.text("materials","m_1a29aea570fbc4","Qty") ?? "Qty")}</th><th>${(globalThis.PlatformLanguage?.text("materials","m_4b91b73dae1ff3","Unit") ?? "Unit")}</th></tr></thead><tbody>${String(orderRequestRows(list).map((row) => `<tr><td>${escapeHtml(row.name)}</td><td>${orderRequestDetailsHtml(row)}</td><td>${escapeHtml(row.quantity)}</td><td>${escapeHtml(row.unit)}</td></tr>`).join(''))}</tbody></table></section>`).join('');
    return `<p>${(globalThis.PlatformLanguage?.text("materials","m_2634bd92bed7f7","Hello,") ?? "Hello,")}</p><p>${(globalThis.PlatformLanguage?.text("materials","m_2dc5eebd0941a4","Please place the following material order for delivery to ") ?? "Please place the following material order for delivery to ")}<strong>${String(escapeHtml(address))}</strong>.</p><p><strong>${(globalThis.PlatformLanguage?.text("materials","m_0e22a1d03e954f","Delivery instructions") ?? "Delivery instructions")}</strong></p><ul class="mt-email-instruction-list">${String(instructions)}</ul>${String(sections)}<p>${(globalThis.PlatformLanguage?.text("materials","m_09326393374cc9","Please confirm product availability, quantities, variant and color details, and each requested delivery appointment before processing the order.") ?? "Please confirm product availability, quantities, variant and color details, and each requested delivery appointment before processing the order.")}</p><p>${(globalThis.PlatformLanguage?.text("materials","m_e495d4a9827c67","Thank you.") ?? "Thank you.")}</p>`;
  }
  ...
  function renderOrderListPicker(allLists, selectedLists){
    ...
    return `<section class="mt-order-list-picker"><div class="mt-order-section-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_d903a6133c49d2","Included in this order") ?? "Included in this order")}</span><strong>${((v0,v1) => globalThis.PlatformLanguage?.text("materials","m_a5a370c47df87a",`${v0} material list${v1}`,{v0,v1}) ?? `${v0} material list${v1}`)(escapeHtml(selectedLists.length),selectedLists.length === 1 ? '' : 's')}</strong></div><span>${(globalThis.PlatformLanguage?.text("materials","m_39d567f6484730","Click a tile to include or remove it") ?? "Click a tile to include or remove it")}</span></div><div class="mt-order-list-options" role="group" aria-label="${(globalThis.PlatformLanguage?.text("materials","m_3422a94241c9cd","Material lists to order") ?? "Material lists to order")}">${String(tiles)}</div></section>`;
  }
  ...
  function renderOrderScheduleTargets(selectedLists){
    ...
    return `<section class="mt-order-schedule-targets"><div class="mt-order-section-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_27872ce794104c","Delivery being edited") ?? "Delivery being edited")}</span><strong>${(globalThis.PlatformLanguage?.text("materials","m_f5839bfd412709","Select one calendar item") ?? "Select one calendar item")}</strong></div><span>${(globalThis.PlatformLanguage?.text("materials","m_d919b5541b3cfc","Does not change the order lists above") ?? "Does not change the order lists above")}</span></div><div class="mt-order-target-options" role="group" aria-label="${(globalThis.PlatformLanguage?.text("materials","m_27872ce794104c","Delivery being edited") ?? "Delivery being edited")}">${String(selectedLists.map((list, index) => {
      const id = cleanText(list.id);
      const focused = focusId === id;
      return `<button type="button" class="mt-order-target ${focused ? 'active' : ''}" data-mt-order-schedule-focus="${escapeHtml(id)}" aria-pressed="${focused ? 'true' : 'false'}"><span class="mt-request-list-dot" style="--list-color:${escapeHtml(materialListColor(list, Math.max(index, state.lists.indexOf(list))))}"></span><span><strong>${escapeHtml(list.title || 'Material list')}</strong><small>${escapeHtml(orderDeliveryLabel(list))}</small></span><em>${focused ? 'Editing' : 'Edit'}</em></button>`;
    }).join(''))}</div></section>`;
  }
  ...
  function renderOrderTimingControls(selectedLists){
    ...
    if (state.orderCalendarOpen && scheduled) {
      return `<section class="mt-order-delivery-choice scheduling"><div class="mt-order-section-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_bfe13760089f83","Delivery timing") ?? "Delivery timing")}</span><strong>${(globalThis.PlatformLanguage?.text("materials","m_be783654eba71d","Editing delivery dates") ?? "Editing delivery dates")}</strong></div><span class="mt-required-pill ${String(datesComplete ? 'ready' : '')}">${String(datesComplete ? 'Ready to confirm' : `${scheduledCount} of ${selectedLists.length}`)}</span></div>${String(dateSummary)}<button type="button" class="mt-order-text-action" data-mt-order-timing="without_dates"><i class="fas fa-calendar-xmark"></i>${(globalThis.PlatformLanguage?.text("materials","m_34d8f8348422d8"," Order without dates instead") ?? " Order without dates instead")}</button></section>`;
    }
    if (scheduled && datesComplete && state.orderDatesConfirmed) {
      return `<section class="mt-order-delivery-choice confirmed"><div class="mt-order-section-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_bfe13760089f83","Delivery timing") ?? "Delivery timing")}</span><strong>${(globalThis.PlatformLanguage?.text("materials","m_c832049612431e","Using scheduled delivery dates") ?? "Using scheduled delivery dates")}</strong></div><span class="mt-required-pill ready"><i class="fas fa-check"></i>${(globalThis.PlatformLanguage?.text("materials","m_a280e9fb3b2819"," Ready") ?? " Ready")}</span></div>${String(dateSummary)}<div class="mt-order-timing-actions"><button type="button" data-mt-order-open-calendar><i class="fas fa-pen-to-square"></i><span><strong>${(globalThis.PlatformLanguage?.text("materials","m_76c8711c1d6f59","Edit delivery dates") ?? "Edit delivery dates")}</strong><small>${(globalThis.PlatformLanguage?.text("materials","m_4ecc9339296a69","Open the project calendar") ?? "Open the project calendar")}</small></span></button><button type="button" data-mt-order-timing="without_dates"><i class="fas fa-calendar-xmark"></i><span><strong>${(globalThis.PlatformLanguage?.text("materials","m_dda45bd2654af7","Order without dates") ?? "Order without dates")}</strong><small>${(globalThis.PlatformLanguage?.text("materials","m_a4ce40461c05cf","Ignore the scheduled dates for this order") ?? "Ignore the scheduled dates for this order")}</small></span></button></div></section>`;
    }
    if (scheduled) {
      return `<section class="mt-order-delivery-choice"><div class="mt-order-section-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_bfe13760089f83","Delivery timing") ?? "Delivery timing")}</span><strong>${String(datesComplete ? 'Confirm the delivery dates' : 'Some deliveries still need dates')}</strong></div><span class="mt-required-pill">${(globalThis.PlatformLanguage?.text("materials","m_db97f048cd99aa","Required") ?? "Required")}</span></div>${String(dateSummary)}<div class="mt-order-timing-actions"><button type="button" data-mt-order-open-calendar><i class="fas fa-calendar-check"></i><span><strong>${String(datesComplete ? 'Review and confirm dates' : 'Schedule missing deliveries')}</strong><small>${(globalThis.PlatformLanguage?.text("materials","m_4ecc9339296a69","Open the project calendar") ?? "Open the project calendar")}</small></span></button><button type="button" data-mt-order-timing="without_dates"><i class="fas fa-calendar-xmark"></i><span><strong>${(globalThis.PlatformLanguage?.text("materials","m_dda45bd2654af7","Order without dates") ?? "Order without dates")}</strong><small>${(globalThis.PlatformLanguage?.text("materials","m_825ae356dc7e5d","Arrange timing with the supplier later") ?? "Arrange timing with the supplier later")}</small></span></button></div></section>`;
    }
    const scheduledDatesNote = scheduledCount
      ? `<div class="mt-order-no-date-note"><i class="fas fa-calendar-check"></i><span>${((v0) => globalThis.PlatformLanguage?.text("materials","m_d85134d46b1895",`${v0} Their dates will simply be left out of this supplier email.`,{v0}) ?? `${v0} Their dates will simply be left out of this supplier email.`)(scheduledCount === selectedLists.length ? 'These deliveries remain scheduled on the project.' : `${scheduledCount} of ${selectedLists.length} deliveries remain scheduled on the project.`)}</span></div>`
      : `<div class="mt-order-no-date-note"><i class="fas fa-envelope-open-text"></i><span>${(globalThis.PlatformLanguage?.text("materials","m_c6a2b7b62977b1","The email will ask the supplier to contact you to arrange each delivery.") ?? "The email will ask the supplier to contact you to arrange each delivery.")}</span></div>`;
    ...
    return `<section class="mt-order-delivery-choice"><div class="mt-order-section-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_bfe13760089f83","Delivery timing") ?? "Delivery timing")}</span><strong>${String(withoutDates ? 'Ordering without delivery dates' : 'Choose one option')}</strong></div><span class="mt-required-pill ${String(withoutDates ? 'ready' : '')}">${String(withoutDates ? 'Selected' : 'Required')}</span></div>${String(scheduledCount ? dateSummary : '')}<div class="mt-order-timing-choices"><button type="button" data-mt-order-timing="scheduled"><i class="fas fa-calendar-check"></i><span><strong>${String(datedOptionLabel)}</strong><small>${String(datedOptionHint)}</small></span></button><button type="button" class="${String(withoutDates ? 'active' : '')}" data-mt-order-timing="without_dates"><i class="fas fa-calendar-xmark"></i><span><strong>${(globalThis.PlatformLanguage?.text("materials","m_dda45bd2654af7","Order without dates") ?? "Order without dates")}</strong><small>${(globalThis.PlatformLanguage?.text("materials","m_5189a46ae92108","Leave dates out of this supplier email") ?? "Leave dates out of this supplier email")}</small></span></button></div>${String(withoutDates ? scheduledDatesNote : '')}${String(withoutDates && scheduledCount ? '<button type="button" class="mt-order-text-action" data-mt-order-open-calendar><i class="fas fa-pen-to-square"></i> Edit scheduled deliveries</button>' : '')}</section>`;
  }
  ...
  function renderOrderDialog(){
    ...
    const detailsPanel = `<div class="mt-order-details">${String(listPicker)}${String(timingControls)}${String(scheduleTargets)}<div class="mt-order-summary"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_aaebd7ccba0b30","Project") ?? "Project")}</span><strong>${String(escapeHtml(state.project?.address || state.project?.title || ''))}</strong></div></div><div class="mt-form-grid"><label class="wide"><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_5a81714733da92","Order title") ?? "Order title")}</span><input class="mt-input" name="title" value="${String(escapeHtml(state.orderDraft.title || (lists.length === 1 ? lists[0]?.title : 'Combined material order') || 'Material order'))}"></label><label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_ee089efa867b5d","Supplier") ?? "Supplier")}</span><input class="mt-input" name="vendor" value="${String(escapeHtml(state.orderDraft.vendor))}" placeholder="${(globalThis.PlatformLanguage?.text("materials","m_ea1009fc19cb4c","Supplier name") ?? "Supplier name")}"></label><label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_0850099cb964d6","Supplier email") ?? "Supplier email")}</span><input class="mt-input" type="email" name="vendor_email" value="${String(escapeHtml(state.orderDraft.vendor_email))}" placeholder="${(globalThis.PlatformLanguage?.text("materials","m_6718ec5ad32aa0","orders@supplier.com") ?? "orders@supplier.com")}"></label><label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_a0ff1e9d49d84c","Quoted total") ?? "Quoted total")}</span><input class="mt-input" type="number" min="0" step="0.01" name="quoted_price" value="${String(escapeHtml(state.orderDraft.quoted_price))}"></label><label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_d1c2c0c41314fa","Paid total") ?? "Paid total")}</span><input class="mt-input" type="number" min="0" step="0.01" name="paid_price" value="${String(escapeHtml(state.orderDraft.paid_price))}"></label></div><div class="mt-order-lock-note ${String(pendingLists.length ? '' : 'ordered')}"><i class="fas ${String(pendingLists.length ? 'fa-circle-info' : 'fa-lock')}"></i><span>${String(pendingLists.length ? 'Copying this request does not place the order. Selected lists are recorded separately while sharing this supplier email.' : 'Every selected list has already been marked as ordered. You can still copy the combined request.')}</span></div></div>`;
    const emailPanel = `<div class="mt-email-preview"><div class="mt-email-preview-head"><strong><i class="fas fa-envelope"></i> ${String(escapeHtml(orderRequestSubject(lists)))}</strong><button type="button" class="mt-email-copy" data-mt-copy-order><i class="fas fa-copy"></i>${(globalThis.PlatformLanguage?.text("materials","m_f1733e455032ff"," Copy email & table") ?? " Copy email & table")}</button></div><div class="mt-email-body">${String(orderRequestPreviewHtml(lists))}</div></div>`;
    const focusList = orderScheduleFocusList();
    const datesReady = orderScheduleDatesComplete();
    const calendarPanel = `<section class="mt-order-calendar-panel"><div class="mt-order-calendar-head"><div><span>${(globalThis.PlatformLanguage?.text("materials","m_c0683b1d95229a","Project calendar") ?? "Project calendar")}</span><strong>${((v0) => globalThis.PlatformLanguage?.text("materials","m_20094abf6017fb",`${v0} delivery`,{v0}) ?? `${v0} delivery`)(escapeHtml(focusList?.title || 'Material'))}</strong><small>${(globalThis.PlatformLanguage?.text("materials","m_2cb817ed73e87b","Existing project events are shown for context. Click or drag on the calendar to place this delivery.") ?? "Existing project events are shown for context. Click or drag on the calendar to place this delivery.")}</small></div><div class="mt-order-calendar-actions"><button type="button" data-mt-order-review-email><i class="fas fa-envelope"></i>${(globalThis.PlatformLanguage?.text("materials","m_3b770970fb24ef"," Review email") ?? " Review email")}</button><button type="button" class="primary" data-mt-order-confirm-dates ${String(datesReady ? '' : 'disabled')}><i class="fas fa-check"></i>${(globalThis.PlatformLanguage?.text("materials","m_81a9a36dc1c9ba"," Confirm dates") ?? " Confirm dates")}</button></div></div><div class="mt-order-calendar" data-mt-order-calendar></div></section>`;
    ...
    const calendarFooter = `<button type="button" class="mt-btn" data-mt-order-review-email><i class="fas fa-envelope"></i>${(globalThis.PlatformLanguage?.text("materials","m_3b770970fb24ef"," Review email") ?? " Review email")}</button><button type="button" class="mt-btn ${String(datesReady ? 'primary' : '')}" data-mt-order-confirm-dates ${String(datesReady ? '' : 'disabled')}><i class="fas fa-check"></i>${(globalThis.PlatformLanguage?.text("materials","m_81a9a36dc1c9ba"," Confirm dates") ?? " Confirm dates")}</button>`;
    const orderFooter = `${state.orderTimingChoice === 'scheduled' ? `<button type="button" class="mt-btn" data-mt-order-open-calendar><i class="fas fa-pen-to-square"></i>${(globalThis.PlatformLanguage?.text("materials","m_164172c27ea2a4"," Edit delivery dates") ?? " Edit delivery dates")}</button>` : ''}<button type="submit" class="mt-btn ${canSubmit ? 'primary' : ''}" ${canSubmit ? '' : 'disabled'}><i class="fas ${canSubmit ? 'fa-check' : 'fa-circle-exclamation'}"></i> ${escapeHtml(submitLabel)}</button>`;
    return `<dialog class="mt-modal" data-mt-order-dialog><div class="mt-modal-backdrop-hit" data-mt-close-order-backdrop aria-hidden="true"></div><form method="dialog" data-mt-order-form class="mt-modal-shell mt-order-modal"><div class="mt-modal-head"><div class="mt-modal-title"><i class="fas fa-cart-shopping"></i><div><strong>${(globalThis.PlatformLanguage?.text("materials","m_36be482213df36","Order materials") ?? "Order materials")}</strong><span>${((v0,v1) => globalThis.PlatformLanguage?.text("materials","m_39b15ac7849e51",`${v0} list${v1} selected · build one supplier-ready request`,{v0,v1}) ?? `${v0} list${v1} selected · build one supplier-ready request`)(escapeHtml(lists.length),lists.length === 1 ? '' : 's')}</span></div></div><button type="button" class="mt-modal-close" data-mt-close-order aria-label="${(globalThis.PlatformLanguage?.text("materials","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button></div><div class="mt-order-layout"><nav class="mt-order-providers" aria-label="${(globalThis.PlatformLanguage?.text("materials","m_831dd5e3d9d0b5","Order method") ?? "Order method")}" role="tablist">${String(orderSources.map((source) => `<button type="button" role="tab" aria-selected="${provider === source.id ? 'true' : 'false'}" class="mt-provider ${provider === source.id ? 'active' : ''}" data-mt-order-provider="${escapeHtml(source.id)}"><i class="fas ${escapeHtml(orderSourceIcon(source))}"></i><span><strong>${escapeHtml(source.name)}</strong><span>${source.kind === 'manual' && source.status === 'active' ? 'Email or call supplier' : source.status === 'disabled' ? 'Unavailable' : 'Coming Soon'}</span></span></button>`).join(''))}</nav>${String(manualProvider ? detailsContent : unavailableContent(activeSource))}</div><div class="mt-modal-foot"><span class="mt-small">${((v4,v5,v6,v7,v8) => globalThis.PlatformLanguage?.text("materials","m_2004834d3931db",`${v4} line item${v5} across ${v6} list${v7} · ${v8}`,{v4,v5,v6,v7,v8}) ?? `${v4} line item${v5} across ${v6} list${v7} · ${v8}`)(escapeHtml(rows.length),rows.length === 1 ? '' : 's',escapeHtml(lists.length),lists.length === 1 ? '' : 's',escapeHtml(deliverySummary))}</span><div class="mt-actions">${String(state.orderCalendarOpen ? calendarFooter : orderFooter)}</div></div></form></dialog>`;
  }
    ...
    return `
      <dialog class="mt-modal" data-mt-new-list-dialog>
        <form method="dialog" data-mt-new-list-form class="mt-modal-shell mt-new-list-modal">
          <div class="mt-modal-head"><div class="mt-modal-title"><i class="fas fa-layer-group"></i><div><strong>${(globalThis.PlatformLanguage?.text("materials","m_c08e5572d68b86","New scope list") ?? "New scope list")}</strong><span>${(globalThis.PlatformLanguage?.text("materials","m_e17a3d60158caf","Create a project-specific resource grouping") ?? "Create a project-specific resource grouping")}</span></div></div><button type="button" class="mt-modal-close" data-mt-close-new-list aria-label="${(globalThis.PlatformLanguage?.text("materials","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button></div>
          <div class="mt-new-list-body">
            <label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_cc9396f2523c62","Resource type") ?? "Resource type")}</span><select class="mt-input" name="resource_type"><option value="material">${(globalThis.PlatformLanguage?.text("materials","m_691187e28aba8e","Materials") ?? "Materials")}</option><option value="labor">${(globalThis.PlatformLanguage?.text("materials","m_7acfa5ed3b7739","Labor") ?? "Labor")}</option><option value="equipment">${(globalThis.PlatformLanguage?.text("materials","m_2813f320a63b94","Equipment") ?? "Equipment")}</option></select></label>
            <label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_48a52dec920f8b","List name") ?? "List name")}</span><input class="mt-input" name="title" required maxlength="120" placeholder="${(globalThis.PlatformLanguage?.text("materials","m_da697bd7def41b","Example: Flashing delivery") ?? "Example: Flashing delivery")}"></label>
            <label><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_c7b224210d489f","List color") ?? "List color")}</span><span class="mt-color-field"><input type="color" name="color" value="${String(escapeHtml(color))}"><input class="mt-input" value="${String(escapeHtml(color))}" data-mt-new-list-color-text aria-label="${(globalThis.PlatformLanguage?.text("materials","m_1aea29df534ede","List color value") ?? "List color value")}"></span></label>
            <p class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_f96a5913aa2f19","This color identifies the list throughout Scope and Scheduling. Scheduling is optional for equipment.") ?? "This color identifies the list throughout Scope and Scheduling. Scheduling is optional for equipment.")}</p>
          </div>
          <div class="mt-modal-foot"><span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_23c8f416472bf8","Scope defaults can create these automatically.") ?? "Scope defaults can create these automatically.")}</span><div class="mt-actions"><button type="button" class="mt-btn" data-mt-close-new-list>${(globalThis.PlatformLanguage?.text("materials","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button type="submit" class="mt-btn">${(globalThis.PlatformLanguage?.text("materials","m_38d4da1be10d46","Create List") ?? "Create List")}</button><button type="submit" class="mt-btn primary" data-mt-new-list-schedule><i class="fas fa-calendar-plus"></i>${(globalThis.PlatformLanguage?.text("materials","m_ef51e1150bf5be"," Create & Schedule") ?? " Create & Schedule")}</button></div></div>
        </form>
      </dialog>
    `;
  }
    ...
    if (!list) {
      state.leftRoot.innerHTML = `
        <div class="r-step-shell" style="grid-template-rows:1fr"><div class="r-step-inner"><div class="r-step-body">
          <label id="rProposalLabel">${(globalThis.PlatformLanguage?.text("materials","m_9d3e82ecfd10ec","Scope") ?? "Scope")}</label>
  ...
  function scopeNotesHtml(){
    ...
    if (!api) return `<div class="mt-empty">${(globalThis.PlatformLanguage?.text("materials","m_9bf0612c80cedb","Notes are unavailable.") ?? "Notes are unavailable.")}</div>`;
    ...
    return `<div class="mt-project-notes">
      <div class="mt-note-compose">
        <textarea data-mt-note-text placeholder="${(globalThis.PlatformLanguage?.text("materials","m_7546d22adf561a","Add a project note… Use @name or @email to tag someone.") ?? "Add a project note… Use @name or @email to tag someone.")}"></textarea>
        <div data-mt-audio-mount></div>
        <div class="mt-note-compose-foot"><span class="mt-small"><i class="fas fa-eye"></i> ${String(escapeHtml(api.visibilityLabel(state.noteVisibility)))}</span><span><input type="file" data-mt-note-upload-input accept="image/*,video/*,audio/*,.pdf,.doc,.docx" hidden><button type="button" class="mt-note-visibility" data-mt-note-visibility aria-label="${(globalThis.PlatformLanguage?.text("materials","m_f9c51272aecc10","Note visibility") ?? "Note visibility")}"><i class="fas fa-eye"></i></button> <button type="button" class="mt-note-visibility" data-mt-note-upload aria-label="${(globalThis.PlatformLanguage?.text("materials","m_1a061030ca2e35","Upload media") ?? "Upload media")}"><i class="fas fa-paperclip"></i></button> <button type="button" class="mt-note-visibility" data-mt-note-audio aria-label="${(globalThis.PlatformLanguage?.text("materials","m_dd978d5c5b3bd9","Record audio note") ?? "Record audio note")}"><i class="fas fa-microphone"></i></button> <button type="button" class="mt-note-save" data-mt-note-save>${String(state.editingNoteId ? 'Save note' : 'Add note')}</button></span></div>
        <div class="mt-note-visibility-pop" data-mt-note-visibility-pop ${String(state.noteVisibilityOpen ? '' : 'hidden')}><button type="button" data-mt-note-group="everybody"><i class="fas fa-${String(all ? 'check-circle' : 'circle')}"></i>${(globalThis.PlatformLanguage?.text("materials","m_d068fb68fb07d2","Everybody") ?? "Everybody")}</button>${String(api.GROUPS.map((group) => `<button type="button" data-mt-note-group="${group}"><i class="fas fa-${state.noteVisibility.includes(group) ? 'check-circle' : 'circle'}"></i>${escapeHtml(group[0].toUpperCase() + group.slice(1))}</button>`).join(''))}<button type="button" data-mt-note-group="only_tagged"><i class="fas fa-${String(state.noteVisibility.length ? 'circle' : 'check-circle')}"></i>${(globalThis.PlatformLanguage?.text("materials","m_6909fe060ad4e1","Only Tagged") ?? "Only Tagged")}</button></div>
      </div>
  ...
  function leftHtml(){
    ...
    return `
      <div class="mt-left">
        <div class="mt-left-head"><strong>${(globalThis.PlatformLanguage?.text("materials","m_9d3e82ecfd10ec","Scope") ?? "Scope")}</strong>${String(state.activeList ? `<em class="mt-status ${statusClass(state.activeList.status)}">${escapeHtml(resourceTerms(resourceType(state.activeList), state.activeList).plural)}</em>` : '')}</div>
        <div class="mt-left-body">
          <section class="mt-left-group ${String(listsOpen ? '' : 'collapsed')}">
            <button type="button" class="mt-left-group-head" data-mt-left-toggle="lists" aria-expanded="${String(listsOpen ? 'true' : 'false')}"><strong>${(globalThis.PlatformLanguage?.text("materials","m_aee5caa035aefc","Lists") ?? "Lists")}</strong><span>${String(state.lists.length)} <i class="fas fa-chevron-${String(listsOpen ? 'up' : 'down')}"></i></span></button>
            <div class="mt-left-group-body">
              ...
              }).join('') || '<div class="mt-empty">This scope does not define any resource lists.</div>')}
              ...
              <button type="button" class="mt-list-add" data-mt-open-new-list><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.text("materials","m_b8547d1ae4d15f"," New scope list") ?? " New scope list")}</button>
            </div>
          </section>
          <section class="mt-left-group ${String(measurementsOpen ? '' : 'collapsed')}">
            <button type="button" class="mt-left-group-head" data-mt-left-toggle="measurements" aria-expanded="${String(measurementsOpen ? 'true' : 'false')}"><strong>${(globalThis.PlatformLanguage?.text("materials","m_0ce54f0aa2d758","Scope measurements") ?? "Scope measurements")}</strong><span>${String(measurementRows.length)} <i class="fas fa-chevron-${String(measurementsOpen ? 'up' : 'down')}"></i></span></button>
            <div class="mt-left-group-body">
              ${String(measurementRows.length ? `<div class="mt-scope-measures">${measurementRows.map((row) => `<div class="mt-scope-measure"><span>${escapeHtml(row.label)}</span><strong>${escapeHtml(row.value)}</strong></div>`).join('')}</div><div class="mt-small"><i class="fas fa-lock"></i> Saved from the project scope</div>` : '<div class="mt-empty">No measurements were saved with this scope.</div>')}
            </div>
          </section>
          ${String(customFieldRows.length ? `<section class="mt-left-group ${customFieldsOpen ? '' : 'collapsed'}">
            <button type="button" class="mt-left-group-head" data-mt-left-toggle="custom_fields" aria-expanded="${customFieldsOpen ? 'true' : 'false'}"><strong>Custom fields</strong><span>${customFieldRows.length} <i class="fas fa-chevron-${customFieldsOpen ? 'up' : 'down'}"></i></span></button>
            <div class="mt-left-group-body">
              ...
              <div class="mt-small"><i class="fas fa-table-list"></i> Project custom fields selected for Scope</div>
            </div>
          </section>` : '')}
          <section class="mt-left-group ${String(notesOpen ? '' : 'collapsed')}">
            <button type="button" class="mt-left-group-head" data-mt-left-toggle="notes" aria-expanded="${String(notesOpen ? 'true' : 'false')}"><strong>${(globalThis.PlatformLanguage?.text("materials","m_d1e91b9e7610fa","Project Notes") ?? "Project Notes")}</strong><span>${String(noteCount)} <i class="fas fa-chevron-${String(notesOpen ? 'up' : 'down')}"></i></span></button>
  ...
  function renderListActionControls(list, type, ordered){
    ...
    return `<div class="mt-list-secondary">
      ${schedule ? `<button type="button" data-mt-list-schedule="${String(escapeHtml(id))}" ${String(state.schedulingListId ? 'disabled' : '')}><i class="fas fa-calendar-day"></i>${(globalThis.PlatformLanguage?.text("materials","m_80ce49ac8b6049"," Schedule") ?? " Schedule")}</button>` : ''}
  ...
  function renderLaborListControls(list){
    ...
    return `<div class="mt-list-secondary mt-labor-controls">
      <label title="${(globalThis.PlatformLanguage?.text("materials","m_af5a4817bd5b60","Assigned work resource") ?? "Assigned work resource")}"><span>${(globalThis.PlatformLanguage?.text("materials","m_2f9d72baaebef0","Assigned") ?? "Assigned")}</span><select aria-label="${String(escapeHtml(workResourceLabel()))}" data-mt-labor-crew="${String(escapeHtml(id))}" ${String(state.saving ? 'disabled' : '')}><option value="">${(globalThis.PlatformLanguage?.text("materials","m_8a993ed9b573b4","Unassigned") ?? "Unassigned")}</option>${String(availableResources.map((resource) => `<option value="${escapeHtml(resource.id)}" ${cleanText(resource.id) === resourceId ? 'selected' : ''}>${escapeHtml(resource.name || resource.label || resource.id)}</option>`).join(''))}</select></label>
      <label title="${(globalThis.PlatformLanguage?.text("materials","m_ea57418325cafc","Project pay") ?? "Project pay")}"><span>${(globalThis.PlatformLanguage?.text("materials","m_f4f42f8c853a8d","Pay") ?? "Pay")}</span><select aria-label="${(globalThis.PlatformLanguage?.text("materials","m_ea57418325cafc","Project pay") ?? "Project pay")}" data-mt-labor-pay="${String(escapeHtml(id))}" ${String(state.saving ? 'disabled' : '')}><option value="crew_default" ${String(mode === 'crew_default' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_01fe62d31733b2","Resource default") ?? "Resource default")}</option><option value="hourly" ${String(mode === 'hourly' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_6a122e6ae08ae2","Hourly") ?? "Hourly")}</option><option value="piece_rate" ${String(mode === 'piece_rate' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_237cf67332e1e8","Piece rate") ?? "Piece rate")}</option><option value="hybrid" ${String(mode === 'hybrid' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_2ba7610473702e","Hourly + piece") ?? "Hourly + piece")}</option><option value="none" ${String(mode === 'none' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_15c0c3889ab7d1","No project pay") ?? "No project pay")}</option></select></label>
      ${String(usesHours ? `<label title="Estimated project hours per person"><span>Hours</span><input aria-label="Estimated hours per person" data-mt-labor-hours="${escapeHtml(id)}" type="number" min="0" step="0.25" value="${escapeHtml(estimatedHours || '')}" placeholder="0" ${state.saving ? 'disabled' : ''}></label>` : '')}
      <label title="${(globalThis.PlatformLanguage?.text("materials","m_fc17203896ecce","How salary compensation should be represented in this project expense") ?? "How salary compensation should be represented in this project expense")}"><span>${(globalThis.PlatformLanguage?.text("materials","m_40c128013ab7d0","Salary") ?? "Salary")}</span><select aria-label="${(globalThis.PlatformLanguage?.text("materials","m_d880e01e767290","Salary expense treatment") ?? "Salary expense treatment")}" data-mt-labor-salary="${String(escapeHtml(id))}" ${String(state.saving ? 'disabled' : '')}><option value="ignore" ${String(salaryMode === 'ignore' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_b26fb21a853ebc","Ignore") ?? "Ignore")}</option><option value="hourly" ${String(salaryMode === 'hourly' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("materials","m_5b8c03e32d38d3","Convert to hourly") ?? "Convert to hourly")}</option></select></label>
      ${String((resourceScheduleEnabled(list) || canRegenerate) ? `<span class="mt-labor-actions">${canRegenerate ? `<button type="button" data-mt-regenerate-labor="${escapeHtml(id)}" ${state.saving || regenerating ? 'disabled' : ''}><i class="fas ${regenerating ? 'fa-rotate mt-spin' : 'fa-arrows-rotate'}"></i> ${regenerating ? 'Regenerating' : 'Regenerate labor'}</button>` : ''}${resourceScheduleEnabled(list) ? `<button type="button" data-mt-list-schedule="${escapeHtml(id)}" ${state.schedulingListId ? 'disabled' : ''}><i class="fas fa-calendar-day"></i> Schedule</button>` : ''}</span>` : '')}
    </div>`;
  }
  ...
  function renderDetailsSection(){
    return `
      <div class="mt-detail-card">
        <div class="mt-prompt">${(globalThis.PlatformLanguage?.text("materials","m_b3ecc234f63212","Details") ?? "Details")}</div>
        <div class="mt-choice-row">
          <span class="mt-small">${(globalThis.PlatformLanguage?.text("materials","m_996992330dcc5c","Flashing color") ?? "Flashing color")}</span>

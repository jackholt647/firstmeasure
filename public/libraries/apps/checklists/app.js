  ...
  function mountChecklists(context = {}){
    ...
    const itemHtml = (checklist, item) => {
      ...
      const control = rating
        ...
        : '';
      const check = rating ? '' : `<button type="button" class="fmcl-check" data-fmcl-toggle aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_892c2703c8b0d7","Complete item") ?? "Complete item")}"><i class="fas fa-check"></i></button>`;
      ...
      const editorBody = drawer.preview
        ? `<div class="fmcl-md-preview fmcl-markdown">${clean(noteDraft) ? renderMarkdown(noteDraft) : `<div class="fmcl-md-empty">${(globalThis.PlatformLanguage?.htmlText("checklists","m_62ba08942f1514","Nothing to preview yet.") ?? "Nothing to preview yet.")}</div>`}</div>`
      ...
      const drawerHtml = `<div class="fmcl-drawer ${String(drawer.open ? 'open' : '')}"><div><div class="fmcl-drawer-inner">
        <div class="fmcl-note-editor">
          <div class="fmcl-md-toolbar" role="toolbar" aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_d78d0fabcb7532","Note formatting") ?? "Note formatting")}"><div class="fmcl-md-tools" ${String(drawer.preview ? 'hidden' : '')}>${String(markdownTools)}</div><div class="fmcl-md-modes"><button class="fmcl-md-mode ${String(drawer.preview ? '' : 'on')}" type="button" data-fmcl-md-view="write">${(globalThis.PlatformLanguage?.htmlText("checklists","m_c760a381a3956a","Write") ?? "Write")}</button><button class="fmcl-md-mode ${String(drawer.preview ? 'on' : '')}" type="button" data-fmcl-md-view="preview">${(globalThis.PlatformLanguage?.htmlText("checklists","m_afff48796c3165","Preview") ?? "Preview")}</button></div></div>
          ${String(editorBody)}
        </div>
        ${String(drawer.hint ? `<div class="fmcl-drawer-hint">${(globalThis.PlatformLanguage?.htmlText("checklists","m_16bae870307854","A note is required before this item can be marked bad.") ?? "A note is required before this item can be marked bad.")}</div>` : '')}
        <div class="fmcl-drawer-actions"><span class="fmcl-md-help"><i class="fab fa-markdown"></i>${(globalThis.PlatformLanguage?.htmlText("checklists","m_e2760453b83d6f"," Markdown supported") ?? " Markdown supported")}</span><button class="fmcl-btn ghost" type="button" data-fmcl-note-cancel>${(globalThis.PlatformLanguage?.htmlText("checklists","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button class="fmcl-btn primary" type="button" data-fmcl-note-save>${String(drawer.pendingRating ? 'Save rating' : 'Save note')}</button></div>
      </div></div></div>`;
      ...
      const titleHtml = drawer.editingTitle
        ? `<div class="fmcl-inline-title"><input data-fmcl-title-input value="${String(esc(drawer.titleDraft || item.title))}" aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_6ba9f4c970b98a","Checklist item title") ?? "Checklist item title")}"><button type="button" data-fmcl-title-save aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_355f235735b2c7","Save item") ?? "Save item")}"><i class="fas fa-check"></i></button><button type="button" data-fmcl-title-cancel aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_7bb3394461ca4a","Cancel item edit") ?? "Cancel item edit")}"><i class="fas fa-xmark"></i></button></div>`
        : `<div class="fmcl-item-title"><strong>${String(esc(first(item.title, 'Checklist item')))}</strong><button class="fmcl-title-edit-trigger" type="button" data-fmcl-title-edit aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_a53b7016b47ef5","Edit item") ?? "Edit item")}"><i class="fas fa-pen"></i></button></div>`;
      const evidenceDrawer = `<div class="fmcl-evidence-drawer ${String(drawer.evidenceOpen ? 'open' : '')}" data-fmcl-evidence-drawer>
        <div class="fmcl-evidence-title"><div><strong>${(globalThis.PlatformLanguage?.htmlText("checklists","m_22d6acf47e2b46","Evidence and requirements") ?? "Evidence and requirements")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_5bb1ac146b4ed0","Choose accepted evidence types and attach proof from the same place.") ?? "Choose accepted evidence types and attach proof from the same place.")}</span></div><button class="fmcl-icon-btn" type="button" data-fmcl-evidence-close aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_c6a90fd1f43087","Close evidence settings") ?? "Close evidence settings")}"><i class="fas fa-xmark"></i></button></div>
        <div class="fmcl-requirement-grid">
          <div class="fmcl-requirement-row"><div class="fmcl-requirement-main"><span><strong>${(globalThis.PlatformLanguage?.htmlText("checklists","m_67ee91a7366dbe","Require media") ?? "Require media")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_170d8a175669ea","Customer or field user must attach a photo or video.") ?? "Customer or field user must attach a photo or video.")}</span></span><button class="fmcl-switch ${String(mediaRequirement ? 'on' : '')}" type="button" data-fmcl-requirement-toggle="media" aria-pressed="${String(!!mediaRequirement)}"></button></div>${String(mediaRequirement ? `<div class="fmcl-media-kinds"><label class="fmcl-check-option"><input type="checkbox" data-fmcl-media-kind="photo" ${mediaAllowed.includes('photo') ? 'checked' : ''}>${(globalThis.PlatformLanguage?.htmlText("checklists","m_118dadd0134a17"," Photos") ?? " Photos")}</label><label class="fmcl-check-option"><input type="checkbox" data-fmcl-media-kind="video" ${mediaAllowed.includes('video') ? 'checked' : ''}>${(globalThis.PlatformLanguage?.htmlText("checklists","m_541ac1617a8425"," Videos") ?? " Videos")}</label><span class="fmcl-sub" style="margin-left:auto">${((v2) => globalThis.PlatformLanguage?.htmlText("checklists","m_dc154253e89b9b",`${v2}/1 attached`,{v2}) ?? `${v2}/1 attached`)(countFor(mediaRequirement))}</span></div>` : '')}</div>
          <div class="fmcl-requirement-row"><div class="fmcl-requirement-main"><span><strong>${(globalThis.PlatformLanguage?.htmlText("checklists","m_3da85b9b7dac86","Require document") ?? "Require document")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_d45bc06e1c924d","PDF, Word, spreadsheet, or another document.") ?? "PDF, Word, spreadsheet, or another document.")}</span></span><button class="fmcl-switch ${String(documentRequirement ? 'on' : '')}" type="button" data-fmcl-requirement-toggle="document" aria-pressed="${String(!!documentRequirement)}"></button></div>${String(documentRequirement ? `<span class="fmcl-sub">${((v0) => globalThis.PlatformLanguage?.htmlText("checklists","m_e03dcbe5245216",`${v0}/1 attached`,{v0}) ?? `${v0}/1 attached`)(countFor(documentRequirement))}</span>` : '')}</div>
          <div class="fmcl-requirement-row"><div class="fmcl-requirement-main"><span><strong>${(globalThis.PlatformLanguage?.htmlText("checklists","m_d2e13e894b6086","Require audio") ?? "Require audio")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_7a5dae00b3de2b","A recorded voice explanation or uploaded audio file.") ?? "A recorded voice explanation or uploaded audio file.")}</span></span><button class="fmcl-switch ${String(audioRequirement ? 'on' : '')}" type="button" data-fmcl-requirement-toggle="audio" aria-pressed="${String(!!audioRequirement)}"></button></div>${String(audioRequirement ? `<span class="fmcl-sub">${((v0) => globalThis.PlatformLanguage?.htmlText("checklists","m_e03dcbe5245216",`${v0}/1 attached`,{v0}) ?? `${v0}/1 attached`)(countFor(audioRequirement))}</span>` : '')}</div>
        </div>
        <div class="fmcl-evidence-actions"><button class="fmcl-btn" type="button" data-fmcl-attach-evidence><i class="fas fa-paperclip"></i>${(globalThis.PlatformLanguage?.htmlText("checklists","m_175426b21cddb4"," Attach evidence") ?? " Attach evidence")}</button>${String(audioRequirement ? `<button class="fmcl-btn" type="button" data-fmcl-record-evidence><i class="fas fa-microphone"></i>${(globalThis.PlatformLanguage?.htmlText("checklists","m_8ff6951a5684a0"," Record audio") ?? " Record audio")}</button>` : '')}<input type="file" hidden data-fmcl-evidence-input><div class="fmcl-evidence-recorder" data-fmcl-evidence-recorder></div></div>
        ${String(attachments.length ? `<div class="fmcl-evidence-files">${attachments.map((attachment) => `<span class="fmcl-file-chip"><i class="fas fa-paperclip"></i> ${esc(first(attachment.file_name, attachment.kind, 'Evidence'))}</span>`).join('')}</div>` : `<span class="fmcl-sub">${(globalThis.PlatformLanguage?.htmlText("checklists","m_94594916e042ac","No evidence attached yet.") ?? "No evidence attached yet.")}</span>`)}
      </div>`;
      return `<div class="${String(classes)}" data-fmcl-item="${String(esc(checklist.id))}::${String(esc(item.id))}"><div class="fmcl-item-main">${String(check)}<div class="fmcl-item-copy">${String(titleHtml)}${String(itemSubtitle(item) ? `<span>${esc(itemSubtitle(item))}</span>` : '')}</div>${String(control)}${String(noteButton)}<span class="fmcl-item-tools"><button class="fmcl-icon-btn fmcl-item-action fmcl-evidence-button ${String(drawer.evidenceOpen ? 'active' : '')} ${String(attachments.length ? 'has-files' : '')}" type="button" data-fmcl-evidence-open aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_22d6acf47e2b46","Evidence and requirements") ?? "Evidence and requirements")}" title="${(globalThis.PlatformLanguage?.htmlText("checklists","m_22d6acf47e2b46","Evidence and requirements") ?? "Evidence and requirements")}"><i class="fas fa-paperclip"></i>${String(requirements.length ? '<span class="fmcl-evidence-dot"></span>' : '')}</button><button class="fmcl-icon-btn fmcl-item-action danger" type="button" data-fmcl-item-delete aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_aaf8dbee05f91a","Delete item") ?? "Delete item")}" title="${(globalThis.PlatformLanguage?.htmlText("checklists","m_aaf8dbee05f91a","Delete item") ?? "Delete item")}"><i class="fas fa-trash"></i></button></span></div>${String(evidenceDrawer)}${String(noteLine)}${String(drawerHtml)}</div>`;
    };
    ...
    const checklistHtml = (checklist, detail = false, order = 0, deleted = false) => {
      ...
      const eligibilityChips = eligibility.tokens.map((token) => `<span class="fmcl-type-chip">${String(esc(tokenLabel(token)))}<button type="button" data-fmcl-eligibility-remove="${String(esc(token))}" aria-label="${((v2) => globalThis.PlatformLanguage?.htmlText("checklists","m_b44c243626b37c",`Remove ${v2}`,{v2}) ?? `Remove ${v2}`)(esc(tokenLabel(token)))}"><i class="fas fa-xmark"></i></button></span>`).join('');
      ...
      return `<section class="fmcl-card ${String(detail ? 'detail' : '')} ${String(collapsed ? 'collapsed' : '')} ${String(deleted ? 'deleted' : '')}" data-fmcl-list="${String(esc(checklist.id))}" data-fmcl-order="${String(order)}" data-fmcl-deleted="${String(deleted)}">
        <div class="fmcl-cl-head"><button class="fmcl-cl-icon" type="button" data-fmcl-collapse aria-expanded="${String(!collapsed)}" aria-label="${((v7) => globalThis.PlatformLanguage?.htmlText("checklists","m_ce48bcc0fd78ab",`${v7} checklist`,{v7}) ?? `${v7} checklist`)(collapsed ? 'Expand' : 'Collapse')}" title="${String(collapsed ? 'Expand checklist' : 'Collapse checklist')}"><i class="fas ${String(collapsed ? 'fa-chevron-down' : 'fa-chevron-up')}"></i></button><div class="fmcl-cl-copy"><input class="fmcl-list-title ${String(untitled ? 'untitled' : '')}" data-fmcl-rename value="${String(untitled ? '' : esc(checklist.title))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("checklists","m_9054974500fee9","Untitled…") ?? "Untitled…")}" aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_3bf1f1282b3808","Checklist title") ?? "Checklist title")}" title="${String(deleted ? 'Deleted checklist' : 'Click to rename')}" ${String(deleted ? 'readonly' : '')}>${String(deleted ? `<span class="fmcl-deleted-tag"><i class="fas fa-trash"></i>${((v0) => globalThis.PlatformLanguage?.htmlText("checklists","m_5f680e6eb92fdb",` Deleted ${v0}`,{v0}) ?? ` Deleted ${v0}`)(esc(shortDate(checklist.deleted_at)))}</span>` : '')}</div><span class="fmcl-cl-tools"><button class="fmcl-icon-btn voice-update" type="button" data-fmcl-voice-update aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_b794aea060fc8f","Update checklist by voice") ?? "Update checklist by voice")}" title="${(globalThis.PlatformLanguage?.htmlText("checklists","m_8f89b708111629","Update by voice") ?? "Update by voice")}"><i class="fas fa-microphone"></i></button></span></div>
        <div class="fmcl-card-body"><div class="fmcl-card-body-inner">
          <div class="fmcl-card-actions">
            <span class="fmcl-menu-wrap"><button class="fmcl-config-btn ${String(assignmentUi.open ? 'open' : '')}" type="button" data-fmcl-assign><i class="fas fa-user-group"></i>${((v16) => globalThis.PlatformLanguage?.htmlText("checklists","m_467cf3556084e7",` Completes: ${v16} `,{v16}) ?? ` Completes: ${v16} `)(esc(assignmentLabel))}<i class="fas fa-chevron-down"></i></button><div class="fmcl-assignment ${String(assignmentUi.open ? 'open' : '')}" data-fmcl-assignment>
              <div class="fmcl-assignment-top"><div class="fmcl-field-editing"><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_b983a2145eb15f","Allow field editing") ?? "Allow field editing")}</span><button class="fmcl-switch ${String(checklist.crew_editable ? 'on' : '')}" type="button" data-fmcl-editable-v2 aria-pressed="${String(!!checklist.crew_editable)}"></button></div></div>
              <div class="fmcl-assignment-stack">
                <div class="fmcl-assignment-field"><span class="fmcl-compact-label">${(globalThis.PlatformLanguage?.htmlText("checklists","m_a3b601c625c54a","Eligible type") ?? "Eligible type")}</span><div class="fmcl-eligibility">${String(eligibilityChips || `<span class="fmcl-type-chip">${(globalThis.PlatformLanguage?.htmlText("checklists","m_ba4c0181dbbab5","Everyone") ?? "Everyone")}</span>`)}${String(eligibility.customRules.length ? `<span class="fmcl-type-chip" title="${(globalThis.PlatformLanguage?.htmlText("checklists","m_ef04a14ddff020","Additional filters are inherited from the scope definition.") ?? "Additional filters are inherited from the scope definition.")}">${(globalThis.PlatformLanguage?.htmlText("checklists","m_14b5d0674601b3","Scope filters") ?? "Scope filters")}</span>` : '')}<div class="fmcl-expand-picker ${String(assignmentUi.eligibilityPickerOpen ? 'open' : '')}" data-fmcl-picker="eligibility"><button class="fmcl-expand-trigger" type="button" data-fmcl-picker-toggle="eligibility" aria-expanded="${String(assignmentUi.eligibilityPickerOpen)}"><i class="fas fa-plus"></i><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_7ce48edf7d0424","Add eligible type") ?? "Add eligible type")}</span></button><div class="fmcl-picker-options">${String(assignmentUi.loading ? `<span class="fmcl-picker-empty">${(globalThis.PlatformLanguage?.htmlText("checklists","m_60869a53c82d79","Loading types…") ?? "Loading types…")}</span>` : `<button type="button" data-fmcl-eligibility-option="everyone">${(globalThis.PlatformLanguage?.htmlText("checklists","m_ba4c0181dbbab5","Everyone") ?? "Everyone")}</button>${eligibilityOptions.map((option) => `<button type="button" data-fmcl-eligibility-option="${esc(option.token)}">${esc(option.label)}</button>`).join('')}`)}</div></div></div></div>
                <div class="fmcl-assignment-field"><span class="fmcl-compact-label">${(globalThis.PlatformLanguage?.htmlText("checklists","m_6167d42c586703","Assign person") ?? "Assign person")}</span><div class="fmcl-eligibility">${String(assigneeChips || '')}<div class="fmcl-expand-picker ${String(assignmentUi.personPickerOpen ? 'open' : '')}" data-fmcl-picker="person"><button class="fmcl-expand-trigger" type="button" data-fmcl-picker-toggle="person" aria-expanded="${String(assignmentUi.personPickerOpen)}"><i class="fas fa-plus"></i><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_6167d42c586703","Assign person") ?? "Assign person")}</span></button><div class="fmcl-picker-options">${String(assignmentUi.loading ? `<span class="fmcl-picker-empty">${(globalThis.PlatformLanguage?.htmlText("checklists","m_e35d44ebe92c7e","Loading people…") ?? "Loading people…")}</span>` : subjectChoices.length ? subjectChoices.map((subject) => `<button type="button" data-fmcl-person-option="${esc(subject.value)}">${esc(subject.label)}</button>`).join('') : `<span class="fmcl-picker-empty">${(globalThis.PlatformLanguage?.htmlText("checklists","m_0559904744409b","No eligible people found") ?? "No eligible people found")}</span>`)}</div></div></div></div>
              </div>
              ${String(assignmentUi.error ? `<div class="fmcl-drawer-hint">${esc(assignmentUi.error)}</div>` : '')}
            </div></span>
            <span class="fmcl-menu-wrap"><button class="fmcl-config-btn customer ${String(sharedWithCustomer ? 'on' : '')} ${String(customerPanel.open ? 'open' : '')}" type="button" data-fmcl-customer><i class="fas fa-user-shield"></i> ${String(esc(customerLabel))} <i class="fas fa-chevron-down"></i></button><div class="fmcl-customer ${String(customerPanel.open ? 'open' : '')}" data-fmcl-customer-panel><div class="fmcl-compact-field"><span class="fmcl-compact-label">${(globalThis.PlatformLanguage?.htmlText("checklists","m_df7c812069f56a","Customer can") ?? "Customer can")}</span><div class="fmcl-customer-levels" role="group" aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_8081e27d7264bc","Customer checklist access") ?? "Customer checklist access")}"><button type="button" class="${String(!customerLevel ? 'on' : '')}" data-fmcl-customer-level="private">${(globalThis.PlatformLanguage?.htmlText("checklists","m_2d4ff8a83b1b5c","None") ?? "None")}</button><button type="button" class="${String(customerLevel === 'view' ? 'on' : '')}" data-fmcl-customer-level="view">${(globalThis.PlatformLanguage?.htmlText("checklists","m_f3c2eb85778751","Read-only") ?? "Read-only")}</button><button type="button" class="${String(customerLevel === 'complete' ? 'on' : '')}" data-fmcl-customer-level="complete">${(globalThis.PlatformLanguage?.htmlText("checklists","m_e8e493437c1a17","Complete") ?? "Complete")}</button><button type="button" class="${String(customerLevel === 'edit' ? 'on' : '')}" data-fmcl-customer-level="edit">${(globalThis.PlatformLanguage?.htmlText("checklists","m_5b9378df7220c1","Edit") ?? "Edit")}</button></div></div></div></span>
            <span class="fmcl-cl-tools"><span class="fmcl-count">${String(doneCount)}/${String(items.length)}</span><button class="fmcl-icon-btn danger" type="button" data-fmcl-cl-delete aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_0b91b01a0b286a","Delete checklist") ?? "Delete checklist")}"><i class="fas fa-trash"></i></button></span>
          </div>
          <div class="fmcl-voice-mount" data-fmcl-voice-mount></div>
          ${String(items.length ? `<div class="fmcl-items">${items.map((item) => itemHtml(checklist, item)).join('')}</div>` : `<div class="fmcl-empty-voice"><div class="fmcl-empty-voice-inner"><button class="fmcl-empty-voice-button" type="button" data-fmcl-empty-voice aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_2d8f75a3258628","Create the list with just your voice") ?? "Create the list with just your voice")}"><i class="fas fa-microphone"></i></button><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_274d5307ddcfc4","Create the list with just your voice.") ?? "Create the list with just your voice.")}</span></div><div class="fmcl-empty-voice-mount" data-fmcl-empty-voice-mount></div></div>`)}
          <div class="fmcl-add"><input data-fmcl-new-item placeholder="${(globalThis.PlatformLanguage?.htmlText("checklists","m_7b2113cc856add","Add an item…") ?? "Add an item…")}"><div class="fmcl-item-type-toggle" role="group" aria-label="${(globalThis.PlatformLanguage?.htmlText("checklists","m_ba5dd11f1bb044","Item type") ?? "Item type")}"><button class="on" type="button" data-fmcl-new-item-type="todo" aria-pressed="true">${(globalThis.PlatformLanguage?.htmlText("checklists","m_d2ab565901fd0a","Task") ?? "Task")}</button><button type="button" data-fmcl-new-item-type="rating" aria-pressed="false">${(globalThis.PlatformLanguage?.htmlText("checklists","m_ff908def5f42c6","Rating") ?? "Rating")}</button></div><button class="fmcl-btn primary" type="button" data-fmcl-add-item><i class="fas fa-plus"></i></button></div>
        </div></div>
      </section>`;
    };
        ...
        section.querySelectorAll('[data-fmcl-item]').forEach((row) => {
          ...
          row.querySelector('[data-fmcl-record-evidence]')?.addEventListener('click', async () => {
            ...
            try {
              ...
              mount.innerHTML = `<div class="fm-as-processing"><i class="fas fa-circle-notch fa-spin"></i><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_9dcf317e5e0b3b","Attaching voice explanation...") ?? "Attaching voice explanation...")}</span></div>`;
          ...
          row.querySelectorAll('[data-fmcl-evidence]').forEach((button) => button.addEventListener('click', async () => {
            ...
            if (kind === 'audio' && window.FirstMateAudioNotes?.recordInline) {
              ...
              try {
                const recording = await window.FirstMateAudioNotes.recordInline({ mount, maxSeconds:600 });
                mount.innerHTML = `<div class="fm-as-processing"><i class="fas fa-circle-notch fa-spin"></i><span>${(globalThis.PlatformLanguage?.htmlText("checklists","m_0960411ba118b7","Attaching voice explanation…") ?? "Attaching voice explanation…")}</span></div>`;
                const wav = await window.FirstMateAudioNotes.toWavFile(recording.file, 16_000);

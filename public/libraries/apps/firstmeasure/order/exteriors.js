...
(function(){
  ...
  let guideNode=null, guideIndex=0, photoSummary=false, cameraStream=null, cameraEpoch=0, cameraStarting=false, cameraFallback=false, cameraMessage='', capturing=false;
  ...
  function resetGuide(){stopCamera();guideNode?.remove();guideNode=null;guideIndex=0;photoSummary=false;cameraMessage='';cameraFallback=false;}
  ...
  async function startCamera(){
    ...
    const native=window.PhoneFeatures?.isNative?.();
    if(!navigator.mediaDevices?.getUserMedia||(native&&!window.PhoneFeatures.info?.()?.capabilities?.includes('liveCamera'))){cameraFallback=true;cameraMessage=native?'Use Take photo to open your camera.':'Choose Upload to add photos from this device.';updateGuide();return;}
  ...
  async function captureGuided(){
    ...
    capturing=true;updateGuide();
    try{const canvas=document.createElement('canvas'),scale=Math.min(1,2560/video.videoWidth);canvas.width=Math.round(video.videoWidth*scale);canvas.height=Math.round(video.videoHeight*scale);canvas.getContext('2d').drawImage(video,0,0,canvas.width,canvas.height);const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',.9));if(blob&&epoch===generation&&active())void upload(new File([blob],`${key.replace(':','-')}-${Date.now()}.jpg`,{type:'image/jpeg'}),key,true);}
    finally{capturing=false;updateGuide();}
  }
  function visitGuide(index){guideIndex=Math.max(0,Math.min(ctx.count*8-1,index));structure=Math.floor(guideIndex/8);photoSummary=false;render();void startCamera();}
  ...
  function mountGuide(){
    if(!guideNode){
      ...
      guideNode.innerHTML=`<div class="ext-capture-heading"><span data-guide-step></span><button type="button" class="ext-tool" data-guide-summary>Photo summary</button></div><h3 data-guide-title></h3><p data-guide-instruction></p><div class="ext-camera"><video autoplay muted playsinline aria-label="Live rear camera preview"></video><div class="ext-camera-idle"><i class="fas fa-camera" aria-hidden="true"></i><span>Frame the whole house</span></div><div class="ext-angle-guide">${houseModel()}<strong data-guide-angle></strong></div><span class="ext-camera-status" role="status"></span></div><div class="ext-shutter-row"><button type="button" class="ext-tool" data-guide-upload><i class="fas fa-images" aria-hidden="true"></i> Upload</button><button type="button" class="ext-shutter" data-guide-capture aria-label="Take photo"><i class="fas fa-camera" aria-hidden="true"></i></button><button type="button" class="ext-tool" data-guide-retry-camera>Camera</button></div><p class="ext-capture-status" role="status"></p><div class="ext-angle-photos" aria-label="Photos for this angle"></div><footer class="ext-guide-footer"><div class="ext-guide-progress" role="progressbar" aria-label="Exterior photo coverage" aria-valuemin="0"><span></span></div><div class="ext-guide-progress-label"></div><div class="ext-guide-buttons"><button type="button" class="ext-tool" data-guide-back>Back</button><button type="button" class="ext-tool" data-guide-forward></button></div></footer>`;
      ...
      guideNode.querySelector('[data-guide-summary]').onclick=finishGuide;
  ...
  function updateGuide(){
    ...
    guideNode.querySelector('[data-guide-retry-camera]').hidden=!!cameraStream||cameraStarting;
    guideNode.querySelector('.ext-capture-status').textContent=photos.length?`${photos.length} photo${photos.length===1?'':'s'} for this angle · Latest is primary. Take another anytime.`:'Take a photo, upload, or skip and return later.';
    const strip=guideNode.querySelector('.ext-angle-photos');strip.innerHTML=photos.map(([k,f])=>`<div><img src="${esc(referenceThumbnail(f))}" alt="${esc(label(view))} photo"><span>${k===key?'Primary · ':''}${f.uploading?'Uploading…':f.error?'Upload failed':'Saved'}</span>${f.error?`<button type="button" data-guide-retry="${esc(k)}">Retry</button>`:''}${k!==key?`<button type="button" data-guide-primary="${esc(k)}">Make primary</button>`:''}<button type="button" data-guide-remove="${esc(k)}" aria-label="Remove photo">×</button></div>`).join('');
    strip.querySelectorAll('[data-guide-retry]').forEach(b=>b.onclick=()=>send(files.get(b.dataset.guideRetry)));
    strip.querySelectorAll('[data-guide-remove]').forEach(b=>b.onclick=()=>removeGuided(b.dataset.guideRemove));
    strip.querySelectorAll('[data-guide-primary]').forEach(b=>b.onclick=()=>{const chosen=files.get(b.dataset.guidePrimary),old=files.get(key);files.delete(b.dataset.guidePrimary);if(old){old.angleKey=key;files.set(b.dataset.guidePrimary,old);}delete chosen.angleKey;files.set(key,chosen);render();});
    const progress=guideNode.querySelector('[role=progressbar]');progress.setAttribute('aria-valuemax',total);progress.setAttribute('aria-valuenow',done);progress.firstElementChild.style.width=`${done/total*100}%`;
    guideNode.querySelector('.ext-guide-progress-label').textContent=`${done} of ${total} views uploaded`;
    guideNode.querySelector('[data-guide-forward]').textContent=guideIndex+1===total?'See photo summary':photos.length?'Next angle →':'Skip for now →';
  }
...
`;
    ...
    const complete=ready(),extras=[...files].filter(([k])=>k.includes(':additional')||k.startsWith('tray:'));
    const summaryGrid=grid.slice(grid.indexOf('<div class="ext-toolbar">')).replace(/<div class="ext-house">[\s\S]*?<\/div>/,`<div class="ext-house">${houseModel()}</div>`);
    return `<section class="ext-summary-guide"><div class="ext-photo-summary-head"><div><h3>${complete?'All views uploaded':'Your exterior photos'}</h3><p>${complete?`Great — all ${ctx.count*8} views are uploaded.`:'Tap any view to add photos or revisit a skipped angle.'}</p></div></div>${summaryGrid}<div class="ext-photo-extras"><h3>Additional photos</h3><button type="button" class="ext-tool" data-extra-camera>Take photo</button><button type="button" class="ext-tool" data-extra-upload>Upload photos</button></div>${extras.map(([k,f])=>`<div class="ext-summary-extra"><img src="${esc(referenceThumbnail(f))}" alt="Additional reference"><span>${esc(f.angleKey?label(f.angleKey.split(':')[1]):f.name)} · ${f.uploading?'Uploading…':f.error?'Upload failed':'Uploaded'}${k.startsWith('tray:')?' · Needs assignment':''}</span>${f.error?`<button class="ext-tool" type="button" data-retry="${esc(k)}">Retry</button>`:''}${k.startsWith('tray:')?`<button class="ext-tool" type="button" data-select="${esc(k)}">Assign</button>`:''}<button class="ext-tool" type="button" data-remove="${esc(k)}" aria-label="Remove additional photo">×</button></div>`).join('')}</section>`;
  }
  function render(){const scroller=root?.closest('.r-scroll');const savedScroll=scroller?.scrollTop;const focused=document.activeElement;const caret=focused&&/^(INPUT|TEXTAREA)$/.test(focused.tagName)?[focused.selectionStart,focused.selectionEnd]:null;restoreShared();if(!ctx)return;const step=document.getElementById('rStepType');if(!step)return;
    ...
    root.querySelector('[data-extra-camera]')?.addEventListener('click',()=>pickGuided(null,true));

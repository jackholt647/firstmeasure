...
`;
    ...
    return `<section class="ext-summary-guide"><div class="ext-photo-extras"><button type="button" class="ext-extra-drop" data-extra-upload><i class="fas fa-cloud-arrow-up" aria-hidden="true"></i><strong>Additional photos</strong><span>Tap to upload</span></button></div><div class="ext-photo-summary-head"><div><h3>${complete?'All views uploaded':'Your exterior photos'}</h3><p>${complete?`Great � all ${ctx.count*8} views are uploaded.`:'Choose a primary photo for each angle.'}</p></div></div>${selection?`<div class="ext-selection"><strong>${esc(selection.name)}</strong><p>Tap an angle to assign this photo.</p><button type="button" class="ext-tool" data-additional>Keep as extra</button><button type="button" class="ext-tool" data-cancel-selection>Deselect</button></div>`:''}${Array.from({length:ctx.count},(_,i)=>`${ctx.count>1?`<h3>House ${i+1}</h3>`:''}${views.map(v=>group(i+':'+v,label(v))).join('')}`).join('')}${group('','Additional photos')}</section>`;
  }

...
(function(){
  ...
  function projectTile(rowValue, options = {}){
    ...
    const meta = row.hasTime && row.startAt
      ...
      : `<span class="crew-day-count"><span class="${String(row.overdue || dayIndex > dayTotal ? 'late' : '')}">${String(esc(dayIndex))}</span>${((v2) => globalThis.PlatformLanguage?.htmlText("crew","m_3b067069408773",`/${v2} days`,{v2}) ?? `/${v2} days`)(esc(dayTotal))}</span>`;
    return `<button type="button" class="crew-project-tile ${row.overdue ? 'overdue' : ''}" data-crew-project="${esc(row.id)}" style="--scope-color:${esc(row.scopeColor)}">
      ...
      <span class="crew-project-meta">${meta}${row.overdue ? `<span class="crew-overdue-label">${(globalThis.PlatformLanguage?.htmlText("crew","m_cda60f7c71e465","Overdue") ?? "Overdue")}</span>` : ''}${options.showPayout && row.payoutCents ? `<span class="crew-pill good">${esc(currency(row.payoutCents))}</span>` : ''}</span>
    </button>`;
  }
  ...
  function modal(content, onClose){
    ...
    back.innerHTML = `<div class="crew-modal" role="dialog" aria-modal="true">${String(content)}<button type="button" class="crew-modal-close" aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button></div>`;
  ...
  function shiftSummaryModal(result = {}){
    ...
    return modal(`<div class="crew-shift-hero"><i class="fas fa-check"></i><h2>${(globalThis.PlatformLanguage?.htmlText("crew","m_2c6d91b557af39","You have clocked out") ?? "You have clocked out")}</h2><strong>${String(esc(durationText(worked)))}</strong><span class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_2a5410f131a4ba","Nice work today.") ?? "Nice work today.")}</span></div>
      <div class="crew-summary-grid"><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_d53ec0c2edba2d","Shift length") ?? "Shift length")}</span><strong>${String(esc(durationText(length)))}</strong></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_410d93bf32f6bb","Clocked in") ?? "Clocked in")}</span><strong>${String(esc(timeText(first(summary.clock_in_at, summary.clocked_in_at, summary.started_at), '-')))}</strong></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_f6e32dfe019e40","Clocked out") ?? "Clocked out")}</span><strong>${String(esc(timeText(first(summary.clock_out_at, summary.clocked_out_at, summary.ended_at), '-')))}</strong></div></div>
      ${String(reminders.length ? `<div><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_007c1319e7f391","Before you finish") ?? "Before you finish")}</h3></div><div class="crew-reminders">${reminders.map((item) => `<div class="crew-reminder"><i class="fas fa-circle-exclamation"></i><span>${esc(item.title || item.label || item)}</span></div>`).join('')}</div></div>` : '')}`);
  }
  ...
  function todoRowHtml(todo = {}, options = {}){
    ...
    return `<div class="crew-todo-item ${String(done ? 'done' : '')} ${String(priority ? 'priority' : '')}" data-crew-todo="${String(esc(clean(todo.project_id)))}::${String(esc(clean(todo.id)))}">
      <button type="button" class="crew-cl-check" data-todo-toggle aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_e2ee136f98850f","Complete to-do") ?? "Complete to-do")}"><i class="fas fa-check"></i></button>
  ...
  function mountCrewOverview(root, context = {}){
    ...
    const renderMyTodos = () => {
      ...
      todosRoot.innerHTML = `<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_99f046ad5a9f1f","Your to-dos") ?? "Your to-dos")}</h3><small>${String(myTodos.length)}</small></div><div class="crew-todo-list">${String(myTodos.map((todo) => todoRowHtml(todo, { showProject:true })).join(''))}</div></section>`;
    ...
    const equipmentTodayHtml = (projects) => {
      ...
      return `<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_df3b05b5b01d4b","My equipment today") ?? "My equipment today")}</h3><small>${String(units.length)}</small></div><div class="crew-row-list">${String(units.map((unit) => `
        ...
        ${fieldMeterEntryOn() && window.EquipmentAPI ? `<div class="crew-row-actions"><button class="crew-btn" type="button" data-equipment-meter="${esc(unit.id)}" data-equipment-meter-name="${esc(unit.name)}"><i class="fas fa-gauge-high"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_24724e052f3eaf"," Log hours") ?? " Log hours")}</button></div>` : ''}</div>`).join(''))}</div></section>`;
    };
    ...
    const renderDashboard = () => {
      ...
      if (!projects.length) dashboardRoot.innerHTML = stateHtml('empty', emptyMessage);
        ...
        dashboardRoot.innerHTML = `<section class="crew-card"><div class="crew-card-title"><h3>${String(heading)}</h3><small>${((v1,v2) => globalThis.PlatformLanguage?.htmlText("crew","m_46453ef0cafea3",`${v1} project${v2}`,{v1,v2}) ?? `${v1} project${v2}`)(active.length,active.length === 1 ? '' : 's')}</small></div><div class="crew-project-list">${String(active.map(projectTile).join('') || `<div class="crew-sub">${supervisorVariant ? 'No projects are running today.' : 'No projects scheduled for today.'}</div>`)}</div></section>${String(overdue.length ? `<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_7d9240ca995f28","Still needs attention") ?? "Still needs attention")}</h3><small>${((v0) => globalThis.PlatformLanguage?.htmlText("crew","m_4eb5305d6a5157",`${v0} overdue`,{v0}) ?? `${v0} overdue`)(overdue.length)}</small></div><div class="crew-project-list">${overdue.map(projectTile).join('')}</div></section>` : '')}${String(equipmentTodayHtml(projects))}`;
  ...
  function mountCrewReceipts(root, context = {}){
    ...
    root.innerHTML = `<div class="crew-shell"><div class="crew-page narrow"><div class="crew-head"><div><div class="crew-eyebrow">${(globalThis.PlatformLanguage?.htmlText("crew","m_0cda4b1788c220","Expense evidence") ?? "Expense evidence")}</div><h1 class="crew-title">${(globalThis.PlatformLanguage?.htmlText("crew","m_fc54001a0cc000","Receipts") ?? "Receipts")}</h1><p class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_fab0626eca778a","Photograph or upload receipts. FirstMate reads and categorizes every file independently.") ?? "Photograph or upload receipts. FirstMate reads and categorizes every file independently.")}</p></div></div>${String(canUpload ? `<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_ee019548f0f938","Upload receipts") ?? "Upload receipts")}</h3><small>${(globalThis.PlatformLanguage?.htmlText("crew","m_40206ff6752102","Mixed formats supported") ?? "Mixed formats supported")}</small></div>${canRequest ? `<div class="crew-form-grid"><label class="crew-field"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_4560726249d4c1","How was this paid?") ?? "How was this paid?")}</span><select data-funding-source><option value="company_card">${(globalThis.PlatformLanguage?.htmlText("crew","m_d20774f2762b5c","Company card — no reimbursement") ?? "Company card — no reimbursement")}</option><option value="personal">${(globalThis.PlatformLanguage?.htmlText("crew","m_d1c49c6ea46580","Personal funds — request reimbursement") ?? "Personal funds — request reimbursement")}</option></select></label><label class="crew-field"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_db332aed2a2790","Project (optional)") ?? "Project (optional)")}</span><select data-reimbursement-project><option value="">${(globalThis.PlatformLanguage?.htmlText("crew","m_ca5b642c8cc4c2","Office / unassigned") ?? "Office / unassigned")}</option></select></label><label class="crew-field wide"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_63b99552978591","Note (optional)") ?? "Note (optional)")}</span><input data-reimbursement-note placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_3d7a838b7b1fce","What was purchased and why?") ?? "What was purchased and why?")}"></label></div>` : ''}<div class="crew-form-actions" style="justify-content:flex-start"><button class="crew-btn primary" type="button" data-camera><i class="fas fa-camera"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_cd5157f3b879c4"," Take photo") ?? " Take photo")}</button><button class="crew-btn" type="button" data-file><i class="fas fa-upload"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_b8cff41364db04"," Choose files") ?? " Choose files")}</button><input hidden type="file" accept="image/*" capture="environment" data-camera-input><input hidden type="file" multiple accept="${RECEIPT_ACCEPT}" data-file-input></div><div class="crew-sub" data-upload-status></div><div data-upload-batch></div></section>` : '')}<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_59338e296bcca9","Recent receipts") ?? "Recent receipts")}</h3><small data-receipt-count>${String(receipts.length)}</small></div><div data-receipts>${String(stateHtml())}</div></section></div></div>`;
  ...
  function earningsProjectCard(itemValue = {}){
    ...
    return `<div class="crew-row"><div class="crew-row-copy"><strong>${esc(first(item.title,item.project_title,item.project_id,'Other earnings'))}</strong><span>${owed > 0 ? `${esc(currency(owed))} currently owed` : (projected > 0 ? `${esc(currency(projected))} projected` : `${esc(currency(paid))} paid`)}</span><div class="crew-earnings-breakdown">${accrued ? `<span>${((v0) => globalThis.PlatformLanguage?.htmlText("crew","m_326110f5babd31",`${v0} accrued`,{v0}) ?? `${v0} accrued`)(esc(currency(accrued)))}</span>` : ''}${inPayroll ? `<span>${((v0) => globalThis.PlatformLanguage?.htmlText("crew","m_a0f499807b61d8",`${v0} in payroll`,{v0}) ?? `${v0} in payroll`)(esc(currency(inPayroll)))}</span>` : ''}${paid ? `<span>${((v0) => globalThis.PlatformLanguage?.htmlText("crew","m_6acae344e57858",`${v0} paid`,{v0}) ?? `${v0} paid`)(esc(currency(paid)))}</span>` : ''}</div></div><div class="crew-row-value">${esc(currency(amount))}<small><span class="crew-pill ${status === 'paid' || status === 'settled' ? 'good' : 'warn'}">${esc(status)}</span></small></div></div>`;
  }
  ...
  function renderEarningsPage(root, result = {}, title = 'Earnings by project', options = {}){
    ...
    const details = options.details === true && entries.length
      ? `<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_5807fca7eb02a0","Earning details") ?? "Earning details")}</h3><small>${String(entries.length)}</small></div><div class="crew-row-list">${String(entries.slice(0,100).map(earningEntryCard).join(''))}</div></section>`
      : '';
    root.innerHTML = `<div class="crew-summary-grid"><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_5f59818bf1cb7d","Owed") ?? "Owed")}</span><strong>${String(esc(currency(owed)))}</strong><small>${(globalThis.PlatformLanguage?.htmlText("crew","m_3d6345f45141a8","Accrued and currently in payroll") ?? "Accrued and currently in payroll")}</small></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_956173c8527121","Paid") ?? "Paid")}</span><strong>${String(esc(currency(paid)))}</strong><small>${(globalThis.PlatformLanguage?.htmlText("crew","m_192360a7033431","Completed payroll") ?? "Completed payroll")}</small></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_929d3bd2149645","Projected") ?? "Projected")}</span><strong>${String(esc(currency(projected)))}</strong><small>${(globalThis.PlatformLanguage?.htmlText("crew","m_047c1d69a096b3","Not owed until accrued") ?? "Not owed until accrued")}</small></div></div>${String(result.truncated ? `<div class="crew-earnings-note">${(globalThis.PlatformLanguage?.htmlText("crew","m_c023f5308157d7","Showing the newest payroll records. Choose a narrower date range to review older earnings.") ?? "Showing the newest payroll records. Choose a narrower date range to review older earnings.")}</div>` : '')}<section class="crew-card"><div class="crew-card-title"><h3>${String(esc(title))}</h3><small>${String(projects.length)}</small></div>${String(projects.length ? `<div class="crew-row-list">${projects.map(earningsProjectCard).join('')}</div>` : stateHtml('empty','No payroll earnings are available yet.'))}</section>${String(details)}`;
  }

  function mountCrewPayouts(root, context = {}){
    let destroyed = false;
    root.innerHTML = `<div class="crew-shell"><div class="crew-page"><div class="crew-head"><div><div class="crew-eyebrow">${(globalThis.PlatformLanguage?.htmlText("crew","m_af7280e8ac9efa","My payroll") ?? "My payroll")}</div><h1 class="crew-title">${(globalThis.PlatformLanguage?.htmlText("crew","m_685ff0ff145929","Earnings") ?? "Earnings")}</h1><p class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_97d70e1d5cb7d4","What you are owed, what is in payroll, and what has been paid—organized by project.") ?? "What you are owed, what is in payroll, and what has been paid—organized by project.")}</p></div></div><div data-earnings>${String(stateHtml('loading','Loading your earnings'))}</div></div></div>`;
  ...
  function mountProjectOverviewLegacy(context = {}){
    ...
    const render = () => {
      ...
      const chips = [
        row.address ? `<a class="crew-strip-chip" href="https://maps.google.com/?q=${String(encodeURIComponent(row.address))}" target="_blank" rel="noopener"><i class="fas fa-location-dot"></i><div class="crew-strip-copy"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_53d803cdbe9ab1","Address") ?? "Address")}</span><strong>${String(esc(row.address))}</strong></div></a>` : '',
        customer ? `<div class="crew-strip-chip"><i class="fas fa-user"></i><div class="crew-strip-copy"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_ae8e4953e07d70","Customer") ?? "Customer")}</span><strong>${String(esc(customer))}</strong></div></div>` : '',
        phone ? `<a class="crew-strip-chip" href="tel:${String(esc(phone.replace(/[^0-9+]/g,'')))}"><i class="fas fa-phone"></i><div class="crew-strip-copy"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_ed04c65845180f","Phone") ?? "Phone")}</span><strong>${String(esc(phone))}</strong></div></a>` : ''
      ].filter(Boolean).join('');
      ...
      const todoSection = todos.length
        ? `<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_a6534938817ec3","To-dos") ?? "To-dos")}</h3><small>${((v0) => globalThis.PlatformLanguage?.htmlText("crew","m_6c4162aa600d83",`${v0} open`,{v0}) ?? `${v0} open`)(openTodos.length)}</small></div><div class="crew-todo-list">${String([...openTodos, ...doneTodos].map((todo) => todoRowHtml(todo)).join(''))}</div></section>`
        : '';
      root.innerHTML = `<div class="crew-project-page"><div class="crew-page">${String(chips ? `<div class="crew-overview-strip">${chips}</div>` : '')}${String(todoSection)}<section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_d6113dda50d96c","Project notes") ?? "Project notes")}</h3></div><div class="crew-notes">${String(esc(row.notes || 'No project notes have been added.'))}</div></section></div></div>`;
  ...
  function materialListHtml(listValue = {}){
    ...
    return `<section class="crew-card"><div class="crew-material-head"><div><h3>${String(esc(first(list.title, list.name, 'Materials')))}</h3><span class="crew-sub">${String(esc(deliveryLabel))}</span></div><span class="crew-pill ${String(deliveryStatus === 'delivered' ? 'good' : '')}">${((v3,v4) => globalThis.PlatformLanguage?.htmlText("crew","m_87d55382163ac0",`${v3} item${v4}`,{v3,v4}) ?? `${v3} item${v4}`)(items.length,items.length === 1 ? '' : 's')}</span></div>${String(deliveryRows ? `<div class="crew-material-label">${(globalThis.PlatformLanguage?.htmlText("crew","m_bdafd7318e7a2b","Deliveries") ?? "Deliveries")}</div><div class="crew-row-list" style="margin-top:8px">${deliveryRows}</div>` : '')}<div class="crew-material-label">${(globalThis.PlatformLanguage?.htmlText("crew","m_0732999aff5fb0","Items") ?? "Items")}</div><div class="crew-material-items" style="margin-top:8px">${String(items.length ? items.map((item) => `<div class="crew-material-item"><div><strong>${esc(itemDescription(item))}</strong><span>${esc(`${Number(item.quantity || item.qty || 1)} ${first(item.unit, '')}`)}</span></div><b>${esc(itemTotalCents(item) ? currency(itemTotalCents(item)) : '')}</b></div>`).join('') : `<div class="crew-state" style="min-height:90px">${(globalThis.PlatformLanguage?.htmlText("crew","m_106189435c8ab8","No items yet.") ?? "No items yet.")}</div>`)}</div></section>`;
  }
  ...
  function mountProjectMaterials(context = {}){
    ...
    const render = () => {
      ...
      root.innerHTML = `<div class="crew-project-page"><div class="crew-page"><div class="crew-head"><div><div class="crew-eyebrow">${(globalThis.PlatformLanguage?.htmlText("crew","m_4de5ecee190da3","Project materials") ?? "Project materials")}</div><h2 class="crew-title" style="font-size:26px">${(globalThis.PlatformLanguage?.htmlText("crew","m_691187e28aba8e","Materials") ?? "Materials")}</h2><p class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_24901b36a1137d","Delivery lists and items added in the field.") ?? "Delivery lists and items added in the field.")}</p></div><div class="crew-toolbar"><button class="crew-btn" type="button" data-material-receipt><i class="fas fa-receipt"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_42e6bde19da3c1"," Receipts") ?? " Receipts")}</button><button class="crew-btn primary" type="button" data-material-add><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_f512a53a38a9b7"," Add item") ?? " Add item")}</button><input hidden type="file" multiple accept="${String(RECEIPT_ACCEPT)}" data-material-receipt-input></div></div><section class="crew-card crew-inline-form" data-material-form><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_a13e9871c5ffa9","Add a material") ?? "Add a material")}</h3></div><div class="crew-form"><div class="crew-form-grid"><label class="crew-field wide">${(globalThis.PlatformLanguage?.htmlText("crew","m_fd0199dc45d261","Material list") ?? "Material list")}<select data-material-list><option value="">${String(esc(defaultListLabel))}</option>${String(lists.map((list) => `<option value="${esc(list.id)}">${esc(first(list.title,list.name,list.id))}</option>`).join(''))}</select></label><label class="crew-field wide">${(globalThis.PlatformLanguage?.htmlText("crew","m_5be12a31e41de3","Item") ?? "Item")}<input data-material-description placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_5b79f2667269d3","e.g. 1 box coil nails") ?? "e.g. 1 box coil nails")}"></label><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_9c689ddee2f502","Quantity") ?? "Quantity")}<input data-material-quantity type="number" min="0" step="0.01" value="1"></label><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_4b91b73dae1ff3","Unit") ?? "Unit")}<input data-material-unit placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_e23846736f670a","box, each, roll") ?? "box, each, roll")}"></label><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_bc7b189271d609","Unit price") ?? "Unit price")}<input data-material-price inputmode="decimal" placeholder="0.00"></label></div><div class="crew-form-actions"><button class="crew-btn ghost" type="button" data-material-cancel>${(globalThis.PlatformLanguage?.htmlText("crew","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button class="crew-btn primary" type="button" data-material-save>${(globalThis.PlatformLanguage?.htmlText("crew","m_355f235735b2c7","Save item") ?? "Save item")}</button></div><div class="crew-sub" data-material-status></div></div></section><div data-material-receipt-batch>${String(receiptBatchHtml(receiptBatchItems, { review:true }))}</div><div data-receipt-review></div><div data-material-lists>${String(lists.length ? lists.map(materialListHtml).join('') : stateHtml('empty','No material lists are available for this project.'))}</div></div></div>`;
      if (!canAppend) {
    ...
    const renderReceiptReview = () => {
      ...
      const lists = materialLists(result);
      const defaultListLabel = lists.length ? 'Use default project list' : 'Create Crew Added Materials list';
      mount.innerHTML = `<section class="crew-card"><div class="crew-card-title"><div><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_673543e917499c","Review receipt items") ?? "Review receipt items")}</h3><small>${String(esc(first(extraction.vendor_name,extraction.vendor,receipt.title,obj(receipt.file).file_name,'Receipt')))}</small></div><span class="crew-pill warn">${(globalThis.PlatformLanguage?.htmlText("crew","m_b0bb1e74e2a6d3","Review") ?? "Review")}</span></div><div class="crew-summary-grid"><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_9403c7637d4905","Total") ?? "Total")}</span><strong>${String(esc(currency(Number(receipt.total_cents || extraction.total_cents || 0))))}</strong></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_16adeee1c6af56","Purchased") ?? "Purchased")}</span><strong style="font-size:15px">${String(esc(shortDate(first(receipt.purchase_date,extraction.purchase_date),'Unknown')))}</strong></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_727840a47c2e4c","Time") ?? "Time")}</span><strong style="font-size:15px">${String(esc(first(receipt.purchase_time,extraction.purchase_time,'Unknown')))}</strong></div></div><div class="crew-receipt-review" style="margin-top:12px">${String(lines.length ? lines.map((line,index) => `<div class="crew-receipt-line" data-receipt-line="${index}"><input type="checkbox" checked aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_089095261636d7","Include item") ?? "Include item")}"><input type="text" value="${esc(itemDescription(line))}" aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_05f179e4bf5630","Item description") ?? "Item description")}"><input type="number" inputmode="decimal" step="0.01" value="${esc((itemTotalCents(line)/100).toFixed(2))}" aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_70e19044e29070","Item total") ?? "Item total")}"></div>`).join('') : `<div class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_4e17cec39aa192","No line items were found. You can still keep the receipt and add items manually.") ?? "No line items were found. You can still keep the receipt and add items manually.")}</div>`)}</div><label class="crew-field" style="margin-top:10px">${(globalThis.PlatformLanguage?.htmlText("crew","m_3504dc9d7f7408","Append to") ?? "Append to")}<select data-receipt-list><option value="">${String(esc(defaultListLabel))}</option>${String(lists.map((list) => `<option value="${esc(list.id)}">${esc(first(list.title,list.name,list.id))}</option>`).join(''))}</select></label><div class="crew-form-actions" style="margin-top:12px"><button class="crew-btn ghost" type="button" data-receipt-dismiss>${(globalThis.PlatformLanguage?.htmlText("crew","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button class="crew-btn primary" type="button" data-receipt-append ${String(lines.length ? '' : 'disabled')}><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_3a147f783ac4bf"," Add selected items") ?? " Add selected items")}</button></div><div class="crew-sub" data-receipt-status></div></section>`;
  ...
  function mountProjectPayments(context = {}){
    ...
    const render = () => {
      ...
      root.innerHTML = `<div class="crew-project-page"><div class="crew-page"><div class="crew-summary-grid"><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_59d1f6f44b2d3a","Amount due") ?? "Amount due")}</span><strong>${String(esc(currency(data.due)))}</strong></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_2e84162dfa6e1d","Next payment") ?? "Next payment")}</span><strong>${String(esc(currency(Number(data.next.amount_cents || 0))))}</strong><small>${String(esc(shortDate(data.next.due_at,'Not scheduled')))}</small></div><div class="crew-summary"><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_21a721f5fdec30","Project total") ?? "Project total")}</span><strong>${String(esc(currency(data.total)))}</strong><small>${String(esc(`${currency(data.paid)} collected`))}</small></div></div><section class="crew-card crew-inline-form" data-payment-form><div class="crew-card-title"><div><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_cf2e333ad7c693","Record an on-site payment") ?? "Record an on-site payment")}</h3><small>${(globalThis.PlatformLanguage?.htmlText("crew","m_9dfda30a653c48","This records money already received; it does not process a card or bank transfer.") ?? "This records money already received; it does not process a card or bank transfer.")}</small></div></div><div class="crew-form"><div class="crew-form-grid"><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_2b8c3448fa87a1","Amount") ?? "Amount")}<input data-payment-amount inputmode="decimal" value="${String(esc(((Number(data.next.amount_cents)||data.due)/100).toFixed(2)))}"></label><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_6952fe71f8dc85","Method") ?? "Method")}<select data-payment-method><option value="check">${(globalThis.PlatformLanguage?.htmlText("crew","m_cc74e4e6c905ec","Check") ?? "Check")}</option><option value="cash">${(globalThis.PlatformLanguage?.htmlText("crew","m_f758b041cf8d5c","Cash") ?? "Cash")}</option></select></label><label class="crew-field wide">${(globalThis.PlatformLanguage?.htmlText("crew","m_963906ea70bf9a","Note") ?? "Note")}<input data-payment-note placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_303cd92e76c991","Check number or optional note") ?? "Check number or optional note")}"></label></div><div class="crew-form-actions"><button class="crew-btn ghost" type="button" data-payment-cancel>${(globalThis.PlatformLanguage?.htmlText("crew","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button class="crew-btn primary" type="button" data-payment-save>${(globalThis.PlatformLanguage?.htmlText("crew","m_3b7bc8bf145411","Record received payment") ?? "Record received payment")}</button></div><div class="crew-sub" data-payment-status></div></div></section><section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_5b23b718776ab9","Past payments") ?? "Past payments")}</h3><small>${String(data.payments.length)}</small></div>${String(data.payments.length ? `<div class="crew-row-list">${data.payments.map((payment) => `<div class="crew-row"><div class="crew-row-copy"><strong>${esc(first(obj(payment.method).label,obj(payment.method).type,obj(payment.method).kind,payment.kind,'Payment'))}</strong><span>${((v1,v2) => globalThis.PlatformLanguage?.htmlText("crew","m_bfcaea81bcc4ed",`${v1} &middot; ${v2}`,{v1,v2}) ?? `${v1} &middot; ${v2}`)(esc(shortDate(first(payment.received_at,payment.created_at))),esc(clean(payment.status||'settled')))}</span></div><div class="crew-row-value">${esc(currency(Number(payment.amount_cents||0)))}</div></div>`).join('')}</div>` : stateHtml('empty','No payments have been recorded.'))}</section></div></div>`;
      if (!canTakePayment) {
  ...
  function mountProjectChangeOrders(context = {}){
    ...
    const line = () => `<div class="crew-co-line" data-co-line><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_5be12a31e41de3","Item") ?? "Item")}<input data-co-description placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_d274affab502e2","Additional work") ?? "Additional work")}"></label><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_1a29aea570fbc4","Qty") ?? "Qty")}<input data-co-quantity type="number" step="0.01" value="1"></label><label class="crew-field">${(globalThis.PlatformLanguage?.htmlText("crew","m_bc7b189271d609","Unit price") ?? "Unit price")}<input data-co-price inputmode="decimal" placeholder="0.00"></label><button class="crew-icon-btn danger" type="button" data-co-remove aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_f643f568915438","Remove") ?? "Remove")}"><i class="fas fa-trash"></i></button></div>`;
    const render = () => {
      const orders=changeOrders(result);
      root.innerHTML=`<div class="crew-project-page"><div class="crew-page"><div class="crew-head"><div><div class="crew-eyebrow">${(globalThis.PlatformLanguage?.htmlText("crew","m_a7e3569f7a7d11","Project amendments") ?? "Project amendments")}</div><h2 class="crew-title" style="font-size:26px">${(globalThis.PlatformLanguage?.htmlText("crew","m_9b66c8f287b44d","Change orders") ?? "Change orders")}</h2><p class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_9f6e73064d4fa1","Add work and prepare a customer signing and payment link.") ?? "Add work and prepare a customer signing and payment link.")}</p></div><button class="crew-btn primary" type="button" data-co-add><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_d80ab804f02b15"," New change order") ?? " New change order")}</button></div><section class="crew-card crew-inline-form" data-co-form><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_530a38e148745f","New change order") ?? "New change order")}</h3></div><div class="crew-form"><div class="crew-form-grid"><label class="crew-field wide">${(globalThis.PlatformLanguage?.htmlText("crew","m_29dbd3d8b69f55","Title") ?? "Title")}<input data-co-title value="Change order"></label><label class="crew-field wide">${(globalThis.PlatformLanguage?.htmlText("crew","m_b3ecc234f63212","Details") ?? "Details")}<textarea data-co-details placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_9a2af7162eb5c6","Describe why this work is being added") ?? "Describe why this work is being added")}"></textarea></label></div><div class="crew-co-lines" data-co-lines>${String(line())}</div><button class="crew-btn ghost" type="button" data-co-line-add style="justify-self:start"><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_e10157c757b2aa"," Add line") ?? " Add line")}</button><div class="crew-form-actions"><button class="crew-btn ghost" type="button" data-co-cancel>${(globalThis.PlatformLanguage?.htmlText("crew","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button class="crew-btn primary" type="button" data-co-save>${(globalThis.PlatformLanguage?.htmlText("crew","m_657d2fe886023d","Create change order") ?? "Create change order")}</button></div><div class="crew-sub" data-co-status></div></div></section><section class="crew-card"><div class="crew-card-title"><h3>${(globalThis.PlatformLanguage?.htmlText("crew","m_9b66c8f287b44d","Change orders") ?? "Change orders")}</h3><small>${String(orders.length)}</small></div>${String(orders.length?`<div class="crew-row-list">${orders.map((order)=>{const token=publicToken(order);const hasRecipient=arr(order.contacts).some((contact)=>clean(contact?.email));return `<div class="crew-row has-actions"><div class="crew-row-copy"><strong>${esc(first(order.title,'Change order'))}</strong><span>${((v1,v2) => globalThis.PlatformLanguage?.htmlText("crew","m_bfcaea81bcc4ed",`${v1} &middot; ${v2}`,{v1,v2}) ?? `${v1} &middot; ${v2}`)(esc(changeOrderStatusLabel(order)),esc(shortDate(order.updated_at||order.created_at)))}</span></div><div class="crew-row-actions"><div class="crew-row-value">${esc(currency(changeOrderTotal(order)))}</div>${clean(order.status)==='draft'?`<button class="crew-btn" type="button" data-co-send="${esc(order.id)}">${hasRecipient?'Send':'Prepare link'}</button>`:''}${token?`<button class="crew-btn primary" type="button" data-co-open="${esc(order.id)}">${(globalThis.PlatformLanguage?.htmlText("crew","m_4b012fc82f667a","Sign & pay") ?? "Sign & pay")}</button>`:''}</div></div>`}).join('')}</div>`:stateHtml('empty','No change orders have been created.'))}</section></div></div>`;
      if (!canManage) {
  ...
  function mountProjectChecklist(context={}){
    ...
    const itemHtml=(checklist,item)=>{
      ...
      const control=rating
        ...
        :'';
      const check=rating?'':`<button type="button" class="crew-cl-check" data-cl-toggle ${String(canComplete?'':'disabled')} aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_892c2703c8b0d7","Complete item") ?? "Complete item")}"><i class="fas fa-check"></i></button>`;
      const actions=canEdit?`<span class="crew-cl-actions"><button class="crew-icon-btn" type="button" data-cl-edit aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_5b9378df7220c1","Edit") ?? "Edit")}"><i class="fas fa-pen"></i></button><button class="crew-icon-btn danger" type="button" data-cl-delete aria-label="${(globalThis.PlatformLanguage?.htmlText("crew","m_4fc60207629a44","Delete") ?? "Delete")}"><i class="fas fa-trash"></i></button></span>`:'';
      ...
      const drawerHtml=rating?`<div class="crew-cl-drawer ${String(drawer.open?'open':'')}"><div><div class="crew-cl-drawer-inner"><textarea data-cl-note placeholder="${String(requireNote?'Describe what needs attention…':'Add a note for this item…')}">${String(esc(drawer.draft??item.note))}</textarea>${String(drawer.hint?`<div class="crew-cl-drawer-hint">${(globalThis.PlatformLanguage?.htmlText("crew","m_16bae870307854","A note is required before this item can be marked bad.") ?? "A note is required before this item can be marked bad.")}</div>`:'')}<div class="crew-cl-drawer-actions"><button class="crew-btn ghost" type="button" data-cl-note-cancel>${(globalThis.PlatformLanguage?.htmlText("crew","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button><button class="crew-btn primary" type="button" data-cl-note-save>${String(drawer.pendingRating?'Save rating':'Save note')}</button></div></div></div></div>`:'';
      ...
      const evidenceHtml=requirements.length?`<div class="crew-cl-evidence ${drawer.evidenceHint?'hint':''}" data-cl-evidence>
        ${requirements.map((requirement)=>`<div class="crew-cl-requirement ${String(requirement.met?'met':'missing')}"><i class="fas ${String(requirement.met?'fa-check':requirement.icon)}"></i><div class="crew-cl-requirement-copy"><strong>${String(esc(requirement.label))}</strong><span>${((v3,v4,v5) => globalThis.PlatformLanguage?.htmlText("crew","m_c35ccba2063e20",`${v3}/${v4} attached${v5}`,{v3,v4,v5}) ?? `${v3}/${v4} attached${v5}`)(requirement.matches.length,requirement.minimum,requirement.met?' · Ready':'')}</span></div>${String(canComplete?(requirement.recordOnly?`<button class="crew-cl-upload crew-cl-record" type="button" data-cl-evidence-record="${esc(requirement.key)}" ${drawer.uploading?'disabled':''}><i class="fas fa-microphone"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_e05fb49213187a","Record") ?? "Record")}</button>`:`<button class="crew-cl-upload" type="button" data-cl-evidence-add="${esc(requirement.key)}" ${drawer.uploading?'disabled':''}><i class="fas ${drawer.uploading===requirement.key?'fa-spinner fa-spin':'fa-arrow-up-from-bracket'}"></i>${drawer.uploading===requirement.key?'Uploading':'Add'}</button><input type="file" hidden multiple data-cl-evidence-input="${esc(requirement.key)}" accept="${esc(requirement.accept)}">`):'')}</div>${String(canComplete&&requirement.recordOnly?`<div class="crew-cl-evidence-recorder" data-cl-evidence-recorder="${esc(requirement.key)}"></div>`:'')}`).join('')}
        ${drawer.evidenceHint&&requirements.some((entry)=>!entry.met)?`<div class="crew-cl-evidence-hint"><i class="fas fa-circle-exclamation"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_b84b57a4a1d9f6"," Add the required files before completing this item.") ?? " Add the required files before completing this item.")}</div>`:''}
    ...
    const checklistHtml=(checklist)=>{
      ...
      const accessPill=checklist.can_edit?`<span class="crew-pill good">${(globalThis.PlatformLanguage?.htmlText("crew","m_d9e27fdaa430b5","Editable") ?? "Editable")}</span>`:checklist.can_complete?`<span class="crew-pill">${quality?'Rate items':'Check off'}</span>`:`<span class="crew-pill warn">${(globalThis.PlatformLanguage?.htmlText("crew","m_65ed6dd2cd3755","Read only") ?? "Read only")}</span>`;
      ...
      const assignmentPill=checklist.assignment_required?`<span class="crew-pill warn">${(globalThis.PlatformLanguage?.htmlText("crew","m_4bde5430c931ff","Needs assignment") ?? "Needs assignment")}</span>`:assignedCount?`<span class="crew-pill good"><i class="fas fa-user-check"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_5a1e07a6bc5770"," Assigned") ?? " Assigned")}</span>`:Object.keys(obj(checklist.assignment_policy)).length?`<span class="crew-pill"><i class="fas fa-users"></i>${(globalThis.PlatformLanguage?.htmlText("crew","m_c1297bb46922d3"," Any eligible") ?? " Any eligible")}</span>`:'';
      return `<section class="crew-card" data-cl-list="${esc(checklist.id)}"><div class="crew-cl-head"><span class="crew-cl-icon ${supervisor?'supervisor':''}"><i class="fas ${esc(first(checklist.icon,quality?'fa-clipboard-check':'fa-list-check'))}"></i></span><div class="crew-cl-copy"><h3>${esc(checklist.title)}</h3></div><div class="crew-cl-progress"><span class="crew-cl-count">${doneCount}/${total}</span><span class="crew-cl-bar"><i style="width:${percent}%"></i></span></div></div><div class="crew-cl-meta">${supervisor?`<span class="crew-pill" style="background:#eff8ff;color:#175cd3">${(globalThis.PlatformLanguage?.htmlText("crew","m_80a95b33172678","Supervisor") ?? "Supervisor")}</span>`:`<span class="crew-pill">${(globalThis.PlatformLanguage?.htmlText("crew","m_5b8ee9e9110e54","Crew") ?? "Crew")}</span>`}${quality?`<span class="crew-pill" style="background:#fdf4ff;color:#9f1ab1">${(globalThis.PlatformLanguage?.htmlText("crew","m_c2c285ec01b1b5","Quality review") ?? "Quality review")}</span>`:''}${assignmentPill}${accessPill}</div><div class="crew-cl-items">${items.length?items.map((item)=>itemHtml(checklist,item)).join(''):`<div class="crew-sub">${(globalThis.PlatformLanguage?.htmlText("crew","m_bfe2f5665c1cf0","No items on this checklist yet.") ?? "No items on this checklist yet.")}</div>`}</div>${checklist.can_edit?`<div class="crew-cl-add"><input data-cl-new placeholder="${(globalThis.PlatformLanguage?.htmlText("crew","m_7b2113cc856add","Add an item…") ?? "Add an item…")}"><select data-cl-new-type><option value="todo" ${String(quality?'':'selected')}>${(globalThis.PlatformLanguage?.htmlText("crew","m_dc3195e2b61dfc","Check item") ?? "Check item")}</option><option value="rating" ${String(quality?'selected':'')}>${(globalThis.PlatformLanguage?.htmlText("crew","m_98cd0b14518eb3","Rating item") ?? "Rating item")}</option></select><button class="crew-btn primary" type="button" data-cl-add><i class="fas fa-plus"></i></button></div>`:''}</section>`;
    };
    const render=()=>{
      ...
      root.querySelectorAll('[data-cl-item]').forEach((row)=>{
        ...
        row.querySelectorAll('[data-cl-evidence-record]').forEach((button)=>button.addEventListener('click',async()=>{
          ...
          try{
            ...
            mount.innerHTML=`<div class="crew-cl-recording-process"><i class="fas fa-circle-notch fa-spin"></i><span>${(globalThis.PlatformLanguage?.htmlText("crew","m_0960411ba118b7","Attaching voice explanation…") ?? "Attaching voice explanation…")}</span></div>`;
  ...
  registerProject({id:'project.crew_checklists',title:(globalThis.PlatformLanguage?.text("crew","m_4890d3d11dc3eb","Checklists") ?? "Checklists"),icon:'fa-list-check',kind:'checklists',mount:mountProjectChecklist,terminologyKey:'checklists.crew_project_tab'});
})();

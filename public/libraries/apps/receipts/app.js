...
(function(){
  ...
  function reimbursementQueueHtml(rowsValue = [], visible = true){
    ...
    return `<section class="rb-reimbursement-queue" aria-label="Employee reimbursements"><div class="rb-reimbursement-head"><div><strong>${(globalThis.PlatformLanguage?.text("receipts","m_5c2c0f1194429c","Employee reimbursements") ?? "Employee reimbursements")}</strong><p>${(globalThis.PlatformLanguage?.text("receipts","m_1019e518a22db1","Receipt-backed requests awaiting office or payroll action.") ?? "Receipt-backed requests awaiting office or payroll action.")}</p></div><button type="button" class="rb-reimbursement-toggle" role="switch" aria-label="Show employee reimbursements" aria-checked="${visible}" data-reimbursement-toggle><span>${visible ? 'On' : 'Off'}</span><i aria-hidden="true"></i></button></div>${visible ? `<span class="rb-reimbursement-count">${((v0) => globalThis.PlatformLanguage?.text("receipts","m_6c4162aa600d83",`${v0} open`,{v0}) ?? `${v0} open`)(actionable.length)}</span>` : ''}${String(!visible ? '<div class="rb-reimbursement-empty">Reimbursements are hidden. Turn them on to view requests.</div>' : actionable.length ? `<div class="rb-reimbursement-list">${actionable.map((row) => {
  ...
  async function createGlobalApp(context = {}){
    ...
    let reimbursementVisible = true;
    try { reimbursementVisible = window.localStorage?.getItem(preferenceKey) !== 'off'; } catch (_) {}
    ...
    try {
      ...
      const renderReimbursements = () => reimbursementQueueHtml(visibleReimbursements(), reimbursementVisible);
      ...
      browserRoot.addEventListener('click', async (event) => {
        const toggle = event.target.closest('[data-reimbursement-toggle]');
        if (toggle) {
          reimbursementVisible = !reimbursementVisible;
          try { window.localStorage?.setItem(preferenceKey, reimbursementVisible ? 'on' : 'off'); } catch (_) {}
          const aside = browserRoot.querySelector('.pf-content-aside');
          if (aside) aside.innerHTML = renderReimbursements();
          aside?.querySelector('[data-reimbursement-toggle]')?.focus();
          return;
        }
    ...
    return { destroy(){ destroyed = true; unregister?.(); handle?.destroy?.(); root.classList.remove('rb-global-app'); delete root.dataset.mobileView; } };
  }

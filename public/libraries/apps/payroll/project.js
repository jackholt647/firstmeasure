  ...
  function createApp(context = {}){
    ...
    function activity(){
      ...
      return `<section class="pc-card"><div class="pc-card-head"><strong>${String(escapeHtml(word('commission_payments', 'Commission Payments')))}</strong><div class="pc-card-summary"><span>${String(state.entries.length)} ${String(escapeHtml(state.entries.length === 1 ? word('commission_payment', 'Commission Payment').toLowerCase() : word('commission_payments', 'Commission Payments').toLowerCase()))}</span><span class="pc-summary-upcoming">${String(money(upcoming))} ${String(escapeHtml(word('upcoming', 'Upcoming').toLowerCase()))}</span><span class="pc-summary-accrued">${String(money(accrued))} ${String(escapeHtml(word('accrued', 'Accrued').toLowerCase()))}</span></div></div><div class="pc-table-wrap"><table class="pc-table"><thead><tr><th>${String(escapeHtml(word('commission_recipient', 'Commission Recipient')))}</th><th>${String(escapeHtml(word('commission_payment', 'Commission Payment')))}</th><th>${String(escapeHtml(word('timing', 'Timing')))}</th><th class="pc-amount">${(globalThis.PlatformLanguage?.htmlText("payroll","m_2b8c3448fa87a1","Amount") ?? "Amount")}</th></tr></thead><tbody>${String(state.entries.map((entry) => {
        ...
        return `<tr><td><strong>${escapeHtml(payee.name || payee.id)}</strong><span>${payee.worker_type === 'subcontractor' ? 'Subcontractor' : 'Employee'}</span></td><td><strong>${escapeHtml(paymentTitle || title(entry.kind))}</strong><span class="pc-pill ${escapeHtml(status)}">${escapeHtml(status === 'projected' ? 'Upcoming' : title(status))}</span>${ruleTotal && share ? `<span class="pc-effective">${((v0,v1) => globalThis.PlatformLanguage?.htmlText("payroll","m_5640a82dc88247",`${v0}% of ${v1} total commission`,{v0,v1}) ?? `${v0}% of ${v1} total commission`)(escapeHtml(String(share)),escapeHtml(money(ruleTotal)))}</span>` : ''}</td><td><strong>${escapeHtml(timing)}</strong></td><td class="pc-amount ${amount < 0 ? 'negative' : ''}"><div class="pc-entry-actions"><span>${escapeHtml(money(amount))}${related.length ? `<small class="pc-effective">${((v0) => globalThis.PlatformLanguage?.htmlText("payroll","m_bebbfb93134c88",`Effective ${v0}`,{v0}) ?? `Effective ${v0}`)(escapeHtml(money(effective)))}</small>` : ''}</span>${canOverride ? `<button type="button" class="pc-btn" data-pc-override="${escapeHtml(entry.id)}">${(globalThis.PlatformLanguage?.htmlText("payroll","m_fb4ec7a8e31951","Override") ?? "Override")}</button>` : ''}</div></td></tr>${state.overrideEntryId === entry.id ? `<tr class="pc-override-row"><td colspan="4"><form class="pc-override-form" data-pc-override-form="${escapeHtml(entry.id)}"><label>${(globalThis.PlatformLanguage?.htmlText("payroll","m_20e98d8035ad4e","Final amount") ?? "Final amount")}<input class="pc-input" name="amount" type="number" min="0" step="0.01" value="${escapeHtml((effective / 100).toFixed(2))}" required></label><label>${(globalThis.PlatformLanguage?.htmlText("payroll","m_6480ed19528b5a","Reason") ?? "Reason")}<input class="pc-input" name="reason" maxlength="1000" placeholder="${(globalThis.PlatformLanguage?.htmlText("payroll","m_a7ee2c02dfc73f","Required audit reason") ?? "Required audit reason")}" required></label><button type="submit" class="pc-btn primary" ${state.saving ? 'disabled' : ''}>${(globalThis.PlatformLanguage?.htmlText("payroll","m_27d5f2020ffae7","Save override") ?? "Save override")}</button><button type="button" class="pc-btn" data-pc-override-cancel>${(globalThis.PlatformLanguage?.htmlText("payroll","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button></form></td></tr>` : ''}`;
      }).join('') || `<tr><td colspan="4"><div class="pc-empty">${(globalThis.PlatformLanguage?.htmlText("payroll","m_057f4863918f13","No projected or accrued commissions on this project yet.") ?? "No projected or accrued commissions on this project yet.")}</div></td></tr>`)}</tbody></table></div></section>`;
    }
    function render(){
      if (state.destroyed) return;
      if (state.loading) { host.innerHTML = `<div class="pc-state"><div><i class="fas fa-circle-notch fa-spin"></i>${(globalThis.PlatformLanguage?.htmlText("payroll","m_0c4d44e49d9fdf","Loading commission participants…") ?? "Loading commission participants…")}</div></div>`; return; }
      if (state.error) { host.innerHTML = `<div class="pc-state"><div><i class="fas fa-triangle-exclamation"></i>${String(escapeHtml(state.error))}<br><button class="pc-btn" data-pc-refresh style="margin-top:12px">${(globalThis.PlatformLanguage?.htmlText("payroll","m_ef39ad5e614a24","Try again") ?? "Try again")}</button></div></div>`; bind(); return; }
      const roleTileCount = state.roles.length + 1; const roleColumns = roleTileCount > 4 ? 3 : Math.max(1, roleTileCount);
      host.innerHTML = `<div class="pc-shell"><header class="pc-head"><div><h2>${String(escapeHtml(word('commissions', 'Commissions')))}</h2><p>${((v1,v2) => globalThis.PlatformLanguage?.htmlText("payroll","m_57d8adf5033ef1",`Assign ${v1} and review their ${v2}.`,{v1,v2}) ?? `Assign ${v1} and review their ${v2}.`)(escapeHtml(word('commission_recipients', 'Commission Recipients').toLowerCase()),escapeHtml(word('commission_payments', 'Commission Payments').toLowerCase()))}</p></div><button type="button" class="pc-btn" data-pc-refresh><i class="fas fa-rotate-right"></i>${(globalThis.PlatformLanguage?.htmlText("payroll","m_78973ce0cf3403","Refresh") ?? "Refresh")}</button></header>
        <div class="pc-roles" style="--pc-role-columns:${String(roleColumns)}">${String(state.roles.map(roleCard).join(''))}<section class="pc-role pc-new-role"><div class="pc-role-top"><div><strong>${(globalThis.PlatformLanguage?.htmlText("payroll","m_433e848e538fe5","Add new") ?? "Add new")}</strong><small>${(globalThis.PlatformLanguage?.htmlText("payroll","m_1050e39f362189","Create another commission recipient role") ?? "Create another commission recipient role")}</small></div><i class="fas fa-plus"></i></div><div class="pc-role-body"><input class="pc-input" data-pc-new-title placeholder="${(globalThis.PlatformLanguage?.htmlText("payroll","m_3aa3842fbcac51","Role title, e.g. Estimator") ?? "Role title, e.g. Estimator")}"><button type="button" class="pc-btn primary" data-pc-new-role ${String(state.saving ? 'disabled' : '')}>${(globalThis.PlatformLanguage?.htmlText("payroll","m_5470b926d9160c","Add role") ?? "Add role")}</button></div></section></div>${String(activity())}</div>`;
      bind();
    }
    function bind(){

...
(function(){
  ...
  function createApp(context = {}){
    ...
    function subgroupChips(row, currency){
      ...
      if (!keys.length) return `<span class="fmp-subgroup other">${(globalThis.PlatformLanguage?.text("payroll","m_4a04382820d2e1","Other") ?? "Other")}</span>`;
      return keys.map((key) => {
    ...
    function workerSection(title, icon, workerType, rows, occurrence){
      ...
      return `<section class="fmp-worker-section">
        <div class="fmp-worker-head${String(contractor ? ' contractor' : '')}">
          ...
          <span>${((v3,v4) => globalThis.PlatformLanguage?.text("payroll","m_6448e0d7c2d07a",`${v3} payee${v4}`,{v3,v4}) ?? `${v3} payee${v4}`)(rows.length,rows.length === 1 ? '' : 's')}</span>
        </div>
        <div class="fmp-worker-list">
          ${String(rows.length ? rows.map((row) => {
            ...
            const status = row.batchItem ? statusBadge(row.batchItem.status, { small:true }) : (accrued > 0 ? '<span class="fmp-status small run"><i class="fas fa-coins"></i>Accrued</span>' : '<span class="fmp-status small"><i class="fas fa-chart-line"></i>Forecast</span>');
            return `<article class="fmp-worker-row">
              ...
              <div class="fmp-worker-money">
                ...
                <small>${esc(formatMoney(accrued, currency, { compact:true }))} accrued${projected ? ` + ${esc(formatMoney(projected, currency, { compact:true }))} projected` : ''}</small>
                ${itemActions(occurrence.batch, row.batchItem)}
              </div>
            </article>`;
          }).join('') : `<div class="fmp-empty-section"><i class="fas ${contractor ? 'fa-helmet-safety' : 'fa-user-group'}"></i>No ${contractor ? 'subcontractors' : 'employees'} in this payroll.</div>`)}
        </div>
      </section>`;
    }
    ...
    function approvalSummary(batch){
      ...
      return `<div style="font-size:11px;color:#667085;margin-top:4px">${(globalThis.PlatformLanguage?.text("payroll","m_753b856d98671a","Approval: ") ?? "Approval: ")}<strong>${String(esc(approval.status))}</strong>${String(requested.length ? ` · ${requested.map((person) => { const decision=decisions.find((entry) => clean(entry.user_id) === clean(person.user_id)); return `${esc(person.name || person.user_id)}${decision ? ` (${esc(decision.decision)})` : ''}`; }).join(', ')}` : '')}</div>`;
    }
    ...
    function batchControls(occurrence){
      ...
      if (!clean(batch.id)) {
        ...
        return `<div class="fmp-batch-banner">
          <div class="fmp-batch-none"><b>${(globalThis.PlatformLanguage?.text("payroll","m_7bda291a7ec7d2","No batch created.") ?? "No batch created.")}</b>${(globalThis.PlatformLanguage?.text("payroll","m_222255f0484ee6"," Create one when the accrued amount is ready for payroll. Projected earnings stay out until they accrue.") ?? " Create one when the accrued amount is ready for payroll. Projected earnings stay out until they accrue.")}</div>
      ...
      return `<div class="fmp-batch-banner">
        <div class="fmp-batch-copy">
          ...
          <span><strong>${((v1) => globalThis.PlatformLanguage?.text("payroll","m_2a99efe3caf1f5",`Batch ${v1}`,{v1}) ?? `Batch ${v1}`)(esc(batch.id))}</strong><small>${((v2,v3,v4) => globalThis.PlatformLanguage?.text("payroll","m_3d4c7aa8503d45",`${v2} payee${v3} · ${v4}`,{v2,v3,v4}) ?? `${v2} payee${v3} · ${v4}`)(array(batch.items).length,array(batch.items).length === 1 ? '' : 's',esc(formatMoney(batch.total_cents, currency)))}</small>${String(approvalSummary(batch))}</span>
        </div>
        <div class="fmp-card-actions">
          ${String(!approvalStatus || ['draft','rejected'].includes(approvalStatus) ? `<button class="fmp-btn" data-action="approval-submit" data-batch-id="${esc(batch.id)}"><i class="fas fa-user-check"></i> Request approval</button>` : '')}
          ${String(approvalStatus === 'pending' ? `<button class="fmp-btn success" data-action="approval-decide" data-batch-id="${esc(batch.id)}"><i class="fas fa-check"></i> Approve as me</button>` : '')}
          ${String(approvalStatus === 'approved' ? `<button class="fmp-btn success" data-action="approval-finalize" data-batch-id="${esc(batch.id)}"><i class="fas fa-lock"></i> Finalize</button>` : '')}
          ${String(approvalStatus === 'finalized' && clean(batch.status) === 'draft' ? `<button class="fmp-btn" data-action="approval-reopen" data-batch-id="${esc(batch.id)}"><i class="fas fa-pen-to-square"></i> Reopen & refresh</button>` : '')}
          ${String(canRun ? `<button class="fmp-btn" data-action="batch-action" data-batch-id="${esc(batch.id)}" data-next-status="run" ${isBusy(`batch:${batch.id}:run`) ? 'disabled' : ''}><i class="fas ${isBusy(`batch:${batch.id}:run`) ? 'fa-spinner fmp-spinner' : 'fa-play'}"></i> Mark all Run</button>` : '')}
          ${String(canPay ? `<button class="fmp-btn success" data-action="batch-action" data-batch-id="${esc(batch.id)}" data-next-status="paid" ${isBusy(`batch:${batch.id}:paid`) ? 'disabled' : ''}><i class="fas ${isBusy(`batch:${batch.id}:paid`) ? 'fa-spinner fmp-spinner' : 'fa-check-double'}"></i> Mark all Paid</button>` : '')}
        </div>
      </div>`;
    }
      ...
      return `<article class="fmp-schedule-card${String(index === 0 ? ' next' : '')}${String(open ? ' open' : '')}">
        <div class="fmp-schedule-head" data-action="toggle-schedule" data-key="${String(esc(key))}" role="button" tabindex="0" aria-expanded="${String(open ? 'true' : 'false')}">
          <div class="fmp-schedule-title">
            ...
            <div class="fmp-title-copy"><h2>${String(esc(schedule.name || 'Payroll schedule'))}${String(index === 0 ? '<span class="fmp-next-label">Next</span>' : '')}</h2><p>${((v8,v9) => globalThis.PlatformLanguage?.text("payroll","m_613f927946b35b",`Pay date ${v8} · ${v9}`,{v8,v9}) ?? `Pay date ${v8} · ${v9}`)(esc(formatDate(occurrence.pay_date)),esc(recurrenceLabel(schedule)))}</p></div>
          </div>
          <div class="fmp-schedule-totals">
            <div class="fmp-mini-total"><span>${(globalThis.PlatformLanguage?.text("payroll","m_c34ad4e79e396b","Accrued") ?? "Accrued")}</span><b>${String(esc(formatMoney(amounts.accrued, currency)))}</b></div>
            <div class="fmp-mini-total projected"><span>${(globalThis.PlatformLanguage?.text("payroll","m_929d3bd2149645","Projected") ?? "Projected")}</span><b>${String(esc(formatMoney(amounts.projected, currency)))}</b></div>
            <div class="fmp-mini-total forecast"><span>${(globalThis.PlatformLanguage?.text("payroll","m_d418a1dd99512f","Forecast") ?? "Forecast")}</span><b>${String(esc(formatMoney(amounts.forecast, currency)))}</b></div>
          </div>
        ...
        ${String(open ? `<div class="fmp-schedule-body">
          <div class="fmp-occurrence-meta">
            ...
            <span class="fmp-meta-chip"><i class="fas ${schedule.timing_basis === 'completed' ? 'fa-flag-checkered' : 'fa-person-digging'}"></i>Recognize when ${schedule.timing_basis === 'completed' ? 'completed' : 'worked'}</span>
            <span class="fmp-meta-chip"><i class="fas fa-scissors"></i>Cutoff ${esc(formatDateTime(occurrence.cutoff_at))}</span>
            <span class="fmp-meta-chip"><i class="fas fa-shield-halved"></i>${esc(number(schedule.clawback_cap_percent))}% max clawback</span>
          </div>
    ...
    function historyCard(batch, scheduleMap){
      ...
      return `<article class="fmp-history-card">
        <div class="fmp-history-head" data-action="toggle-history" data-key="${String(esc(batch.id))}" role="button" tabindex="0" aria-expanded="${String(open ? 'true' : 'false')}">
          <div class="fmp-history-title"><span class="icon"><i class="fas fa-receipt"></i></span><span><strong>${String(esc(schedule.name || 'Payroll batch'))}</strong><small>${((v3,v4,v5) => globalThis.PlatformLanguage?.text("payroll","m_590c41b7b858c6",`Pay date ${v3} · ${v4} payee${v5}`,{v3,v4,v5}) ?? `Pay date ${v3} · ${v4} payee${v5}`)(esc(formatDate(batch.pay_date)),items.length,items.length === 1 ? '' : 's')}</small></span></div>
          <div class="fmp-history-stat"><span>${(globalThis.PlatformLanguage?.text("payroll","m_9403c7637d4905","Total") ?? "Total")}</span><b>${String(esc(formatMoney(batch.total_cents, currency)))}</b></div>
          <div class="fmp-history-stat"><span>${(globalThis.PlatformLanguage?.text("payroll","m_af4c235bc74266","Employees") ?? "Employees")}</span><b>${String(esc(formatMoney(batch.employee_total_cents, currency)))}</b></div>
          <div class="fmp-history-stat hide-medium"><span>${(globalThis.PlatformLanguage?.text("payroll","m_e761408c42ed5e","Subcontractors") ?? "Subcontractors")}</span><b>${String(esc(formatMoney(batch.subcontractor_total_cents, currency)))}</b></div>
        ...
        ${String(open ? `<div class="fmp-history-body">
          ...
          <table class="fmp-history-table"><thead><tr><th>Payee</th><th>Type</th><th>Breakdown</th><th class="fmp-right">Gross</th><th class="fmp-right">Deductions</th><th class="fmp-right">Net</th><th>Status</th><th>Paid at</th></tr></thead>
          ...
          ${!['paid', 'void'].includes(clean(batch.status)) ? `<div class="fmp-card-actions" style="justify-content:flex-end;margin-top:12px">
            ${!clean(batch.approval?.status) || clean(batch.approval?.status) === 'draft' || clean(batch.approval?.status) === 'rejected' ? `<button class="fmp-btn" data-action="approval-submit" data-batch-id="${esc(batch.id)}"><i class="fas fa-user-check"></i> Request approval</button>` : ''}
            ${clean(batch.approval?.status) === 'pending' ? `<button class="fmp-btn success" data-action="approval-decide" data-batch-id="${esc(batch.id)}"><i class="fas fa-check"></i> Approve as me</button>` : ''}
            ${clean(batch.approval?.status) === 'approved' ? `<button class="fmp-btn success" data-action="approval-finalize" data-batch-id="${esc(batch.id)}"><i class="fas fa-lock"></i> Finalize payroll</button>` : ''}
            ${clean(batch.approval?.status) === 'finalized' && clean(batch.status) === 'draft' ? `<button class="fmp-btn" data-action="batch-action" data-batch-id="${esc(batch.id)}" data-next-status="run"><i class="fas fa-play"></i> Mark all Run</button>` : ''}
            ${(clean(batch.approval?.status) === 'finalized' && clean(batch.status) === 'draft') || clean(batch.status) === 'run' ? `<button class="fmp-btn" data-action="approval-reopen" data-batch-id="${esc(batch.id)}"><i class="fas fa-pen-to-square"></i> Reopen & refresh</button>` : ''}
            ${clean(batch.approval?.status) === 'finalized' || ['run','partial'].includes(clean(batch.status)) ? `<button class="fmp-btn success" data-action="batch-action" data-batch-id="${esc(batch.id)}" data-next-status="paid"><i class="fas fa-check-double"></i> Mark all Paid</button>` : ''}
          </div>` : ''}
        </div>` : '')}
      </article>`;
    }
    ...
    function rangeHtml(){
      return `<section class="fmp-range">
        <div class="fmp-range-fields">
          <div class="fmp-field"><label for="fmpRangeFrom">${(globalThis.PlatformLanguage?.text("payroll","m_c313c42d1f7a10","From") ?? "From")}</label><input id="fmpRangeFrom" type="date" value="${String(esc(state.from))}"></div>
          <div class="fmp-range-arrow"><i class="fas fa-arrow-right"></i></div>
          <div class="fmp-field"><label for="fmpRangeThrough">${(globalThis.PlatformLanguage?.text("payroll","m_6089f8623b0f11","Through") ?? "Through")}</label><input id="fmpRangeThrough" type="date" value="${String(esc(state.through))}" min="${String(esc(state.from))}"></div>
          <button class="fmp-btn dark" data-action="apply-range"><i class="fas fa-calendar-check"></i>${(globalThis.PlatformLanguage?.text("payroll","m_4a4ef7a972510c"," Apply range") ?? " Apply range")}</button>
        </div>
        <div class="fmp-range-presets">
          <button class="fmp-preset" data-action="preset" data-days="30">${(globalThis.PlatformLanguage?.text("payroll","m_abb8a16c33a945","30 days") ?? "30 days")}</button><button class="fmp-preset" data-action="preset" data-days="90">${(globalThis.PlatformLanguage?.text("payroll","m_96c3192473b128","90 days") ?? "90 days")}</button><button class="fmp-preset" data-action="preset" data-days="180">${(globalThis.PlatformLanguage?.text("payroll","m_6fdfbc2f933786","6 months") ?? "6 months")}</button><button class="fmp-preset" data-action="preset" data-days="365">${(globalThis.PlatformLanguage?.text("payroll","m_4b231e0cddf70e","1 year") ?? "1 year")}</button>
          <button class="fmp-projection-toggle${String(state.includeProjected ? ' active' : '')}" data-action="toggle-projected" aria-pressed="${String(state.includeProjected ? 'true' : 'false')}"><span class="fmp-toggle-dot"></span>${(globalThis.PlatformLanguage?.text("payroll","m_44e7c844bf6527","Projected earnings") ?? "Projected earnings")}</button>
        </div>
      </section>`;
    }
    ...
    function metricsHtml(){
      ...
      return `<section class="fmp-metrics">
        <div class="fmp-metric"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_3880ce31dedd06","Next pay date") ?? "Next pay date")}</span><span class="value">${String(next ? esc(formatDate(next.pay_date, { year:false })) : '—')}</span><span class="detail">${String(next ? esc(next.schedule?.name || '') : 'No payroll scheduled')}</span></div>
        <div class="fmp-metric accrued"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_02e36c84f7302b","Accrued in range") ?? "Accrued in range")}</span><span class="value">${String(esc(formatMoney(totals.accrued, currency, { compact:true })))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_9561193cdfc6f5","Ready across scheduled cycles") ?? "Ready across scheduled cycles")}</span></div>
        <div class="fmp-metric projected"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_d20939f1e6df77","Projected additional") ?? "Projected additional")}</span><span class="value">${String(esc(formatMoney(totals.projected, currency, { compact:true })))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_d2b7b8565a580d","Not included in created batches") ?? "Not included in created batches")}</span></div>
        <div class="fmp-metric forecast"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_34d6d8e9d655bc","Range forecast") ?? "Range forecast")}</span><span class="value">${String(esc(formatMoney(totals.forecast, currency, { compact:true })))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_596454d77d55c3","Accrued plus projected labor") ?? "Accrued plus projected labor")}</span></div>
        <div class="fmp-metric"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_9714bf1827f13f","Payroll batches") ?? "Payroll batches")}</span><span class="value">${String(batches.length)}</span><span class="detail">${((v6,v7) => globalThis.PlatformLanguage?.text("payroll","m_29bcd8d58a79a8",`${v6} active schedule group${v7}`,{v6,v7}) ?? `${v6} active schedule group${v7}`)(array(state.data?.schedules).length,array(state.data?.schedules).length === 1 ? '' : 's')}</span></div>
      </section>`;
    }

    function diagnosticsHtml(){
      return array(state.data?.diagnostics).map((diagnostic) => {
        if (clean(diagnostic.code) === 'unassigned_entries') return `<div class="fmp-diagnostic"><i class="fas fa-triangle-exclamation"></i><span><strong>${((v0,v1) => globalThis.PlatformLanguage?.text("payroll","m_ccb0a7fbff8fcd",`${v0} payroll entr${v1} not assigned to a pay schedule.`,{v0,v1}) ?? `${v0} payroll entr${v1} not assigned to a pay schedule.`)(number(diagnostic.count),number(diagnostic.count) === 1 ? 'y is' : 'ies are')}</strong>${((v2) => globalThis.PlatformLanguage?.text("payroll","m_431b1e58e0a1e8",` Assign a schedule in Payroll settings so ${v2} can appear in an upcoming payroll.`,{v2}) ?? ` Assign a schedule in Payroll settings so ${v2} can appear in an upcoming payroll.`)(number(diagnostic.count) === 1 ? 'it' : 'they')}</span></div>`;
    ...
    function locationHtml(value, label){
      ...
      if (!captured) return `<span class="fmp-location missing" title="${String(esc(location.reason || location.status || 'Not recorded'))}"><i class="fas fa-location-dot-slash"></i>${((v1) => globalThis.PlatformLanguage?.text("payroll","m_a2abbd4a491432",`${v1} unavailable`,{v1}) ?? `${v1} unavailable`)(esc(label))}</span>`;
    ...
    function timesheetMetricsHtml(){
      ...
      return `<section class="fmp-timesheet-metrics">
        <div class="fmp-metric"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_a674a4cf49532a","Currently clocked in") ?? "Currently clocked in")}</span><span class="value">${String(number(summary.active))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_dd20fcf3b03743","Live active shifts") ?? "Live active shifts")}</span></div>
        <div class="fmp-metric forecast"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_d3ba5228539462","Pending review") ?? "Pending review")}</span><span class="value">${String(number(summary.pending))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_109e68d6521f7d","Closed, not yet accrued") ?? "Closed, not yet accrued")}</span></div>
        <div class="fmp-metric accrued"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_51be70f25415d9","Approved") ?? "Approved")}</span><span class="value">${String(number(summary.approved))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_1a9b825b05ac9b","Posted to payroll ledger") ?? "Posted to payroll ledger")}</span></div>
        <div class="fmp-metric"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_059a429eefd80b","Worked time") ?? "Worked time")}</span><span class="value">${String(esc(formatDuration(summary.worked_seconds)))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_40a0a105f15ffa","Across this date range") ?? "Across this date range")}</span></div>
        <div class="fmp-metric"><span class="label">${(globalThis.PlatformLanguage?.text("payroll","m_c8563e08f09a81","Missing location") ?? "Missing location")}</span><span class="value">${String(number(summary.missing_location))}</span><span class="detail">${(globalThis.PlatformLanguage?.text("payroll","m_3c8a8a9742e909","Denied, unavailable, or legacy") ?? "Denied, unavailable, or legacy")}</span></div>
      </section>`;
    }

    function timesheetRangeHtml(){
      return `<section class="fmp-range"><div class="fmp-range-fields">
        <div class="fmp-field"><label for="fmpTimesheetFrom">${(globalThis.PlatformLanguage?.text("payroll","m_c313c42d1f7a10","From") ?? "From")}</label><input id="fmpTimesheetFrom" type="date" value="${String(esc(state.timesheetFrom))}"></div>
        <div class="fmp-range-arrow"><i class="fas fa-arrow-right"></i></div>
        <div class="fmp-field"><label for="fmpTimesheetThrough">${(globalThis.PlatformLanguage?.text("payroll","m_6089f8623b0f11","Through") ?? "Through")}</label><input id="fmpTimesheetThrough" type="date" value="${String(esc(state.timesheetThrough))}" min="${String(esc(state.timesheetFrom))}"></div>
        <button class="fmp-btn dark" data-action="apply-timesheet-range"><i class="fas fa-calendar-check"></i>${(globalThis.PlatformLanguage?.text("payroll","m_4a4ef7a972510c"," Apply range") ?? " Apply range")}</button>
      </div><div class="fmp-range-presets"><button class="fmp-preset" data-action="timesheet-preset" data-days="7">${(globalThis.PlatformLanguage?.text("payroll","m_016fb803931b09","7 days") ?? "7 days")}</button><button class="fmp-preset" data-action="timesheet-preset" data-days="14">${(globalThis.PlatformLanguage?.text("payroll","m_6e4cfc8d97e558","14 days") ?? "14 days")}</button><button class="fmp-preset" data-action="timesheet-preset" data-days="30">${(globalThis.PlatformLanguage?.text("payroll","m_abb8a16c33a945","30 days") ?? "30 days")}</button><button class="fmp-preset" data-action="timesheet-preset" data-days="90">${(globalThis.PlatformLanguage?.text("payroll","m_96c3192473b128","90 days") ?? "90 days")}</button></div></section>`;
    }

    function timesheetsHtml(){
      if (state.loading && !state.timesheetData) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-spinner fmp-spinner"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_7eb1d571aa817d","Loading timesheets") ?? "Loading timesheets")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_cf8c14c49e68b2","Reading shifts, approvals, and clock locations.") ?? "Reading shifts, approvals, and clock locations.")}</p></div>`;
      if (state.error && !state.timesheetData) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-triangle-exclamation"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_3bab786d0ed977","Timesheets could not load") ?? "Timesheets could not load")}</h2><p>${String(esc(state.error.message || state.error))}</p><button class="fmp-btn primary" data-action="refresh"><i class="fas fa-rotate"></i>${(globalThis.PlatformLanguage?.text("payroll","m_cbfbb44ff35f0f"," Try again") ?? " Try again")}</button></div>`;
      const rows = array(state.timesheetData?.timesheets);
      if (!rows.length) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-clock"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_0a20a95c3fb7ae","No shifts in this range") ?? "No shifts in this range")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_ac23dfa7d4a35f","Clocked shifts will appear here for review and payroll approval.") ?? "Clocked shifts will appear here for review and payroll approval.")}</p></div>`;
      return `<div class="fmp-timesheet-table-wrap"><table class="fmp-timesheet-table"><thead><tr><th>${(globalThis.PlatformLanguage?.text("payroll","m_cd4c6b59af9d33","Worker") ?? "Worker")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_a5a3b1f08ca908","Clock in") ?? "Clock in")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_a955f5181dc29d","Clock out") ?? "Clock out")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_43a63afbcc6e8e","Worked") ?? "Worked")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_aaebd7ccba0b30","Project") ?? "Project")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_16a2f7a51e5335","Locations") ?? "Locations")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_1352cafa75b8da","Status") ?? "Status")}</th><th></th></tr></thead><tbody>${String(rows.map((shift) => {
        ...
        return `<tr><td><span class="fmp-timesheet-person"><strong>${esc(shift.user?.name || shift.user_id)}</strong><span>${esc(shift.user?.email || '')}</span></span></td>
          <td>${esc(formatDateTime(shift.clocked_in_at))}</td><td>${shift.clocked_out_at ? esc(formatDateTime(shift.clocked_out_at)) : '<span class="fmp-approval active">Active</span>'}</td>
          <td><strong>${esc(formatDuration(shift.worked_seconds))}</strong><br><small>${esc(formatDuration(shift.break_seconds))} break</small></td>
          <td>${esc(shift.project?.title || 'Unallocated')}</td><td><div style="display:grid;gap:5px">${locationHtml(shift.clock_in_location, 'In')}${closed ? locationHtml(shift.clock_out_location, 'Out') : ''}</div></td>
          ...
          <td><div class="fmp-timesheet-actions"><button class="fmp-btn small" data-action="timesheet-edit" data-shift-id="${esc(shift.id)}" title="Correct times"><i class="fas fa-pen"></i></button>${closed && approval !== 'approved' ? `<button class="fmp-btn small success" data-action="timesheet-approve" data-shift-id="${esc(shift.id)}"><i class="fas fa-check"></i> Approve</button>` : ''}${closed && approval !== 'rejected' ? `<button class="fmp-btn small" data-action="timesheet-reject" data-shift-id="${esc(shift.id)}"><i class="fas fa-xmark"></i></button>` : ''}</div></td></tr>`;
      }).join(''))}</tbody></table></div>`;
    }

    function contractorsHtml(){
      if (state.loading && !state.contractorData) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-spinner fmp-spinner"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_65de6379fbec63","Loading payees") ?? "Loading payees")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_f47bd5fe91454a","Reading people, subcontractor companies, and payment terms.") ?? "Reading people, subcontractor companies, and payment terms.")}</p></div>`;
      const users = array(state.contractorData?.users);
      const connections = array(state.contractorData?.connections);
      return `<section class="fmp-range"><div><strong>${(globalThis.PlatformLanguage?.text("payroll","m_bbcf73eb789ddf","Workers and subcontractors") ?? "Workers and subcontractors")}</strong><div style="color:#667085;font-size:12px;margin-top:4px">${(globalThis.PlatformLanguage?.text("payroll","m_e7c30e4e0c04f1","Classify individual workers here. Company subcontractors retain their organization record and payment terms; project assignments continue to use the existing crew and resource assignment tools.") ?? "Classify individual workers here. Company subcontractors retain their organization record and payment terms; project assignments continue to use the existing crew and resource assignment tools.")}</div></div></section>
        <div class="fmp-timesheet-table-wrap"><table class="fmp-timesheet-table"><thead><tr><th>${(globalThis.PlatformLanguage?.text("payroll","m_4e3a8f50faa2d5","Payee") ?? "Payee")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_9bb65d62d4caa7","Record type") ?? "Record type")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_0ee677e8200886","Classification") ?? "Classification")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_462e38dd9a2f59","Payment terms") ?? "Payment terms")}</th><th></th></tr></thead><tbody>
        ${String(users.map((user) => { const classification=clean(user.worker_classification || user.worker_type || 'employee'); const terms=object(user.payment_terms); return `<tr><td><strong>${esc(user.name || user.display_name || user.email || user.id)}</strong><br><small>${esc(user.email || '')}</small></td><td>Individual</td><td><select data-contractor-classification="${esc(user.id)}"><option value="employee" ${classification === 'employee' ? 'selected' : ''}>Employee</option><option value="independent_contractor" ${classification === 'independent_contractor' ? 'selected' : ''}>Independent contractor</option></select></td><td><select data-contractor-basis="${esc(user.id)}"><option value="payroll_schedule" ${clean(terms.basis || 'payroll_schedule') === 'payroll_schedule' ? 'selected' : ''}>Payroll schedule</option><option value="net_days" ${clean(terms.basis) === 'net_days' ? 'selected' : ''}>Net days</option></select> <input style="width:70px" type="number" min="0" max="365" value="${number(terms.net_days)}" data-contractor-days="${esc(user.id)}" aria-label="Net days"></td><td><button class="fmp-btn small" data-action="save-worker-payee" data-user-id="${esc(user.id)}">Save</button></td></tr>`; }).join(''))}
        ${String(connections.map((connection) => { const terms=object(connection.payment_terms); return `<tr><td><strong>${esc(connection.name || connection.company_name || connection.id)}</strong><br><small>${esc(connection.primary_contact?.email || '')}</small></td><td>Subcontractor company</td><td>Subcontractor</td><td><select data-connection-basis="${esc(connection.id)}"><option value="payroll_schedule" ${clean(terms.basis || 'payroll_schedule') === 'payroll_schedule' ? 'selected' : ''}>Payment cycle</option><option value="net_days" ${clean(terms.basis) === 'net_days' ? 'selected' : ''}>Net days</option></select> <input style="width:70px" type="number" min="0" max="365" value="${number(terms.net_days)}" data-connection-days="${esc(connection.id)}" aria-label="Net days"></td><td><button class="fmp-btn small" data-action="save-company-payee" data-connection-id="${esc(connection.id)}">Save</button></td></tr>`; }).join(''))}
        ${String(!users.length && !connections.length ? '<tr><td colspan="5">No workforce or subcontractor records are available.</td></tr>' : '')}</tbody></table></div>`;
    }

    function exportsHtml(){
      if (state.loading && !state.exportData) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-spinner fmp-spinner"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_4bf939ec84b6aa","Loading reports") ?? "Loading reports")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_47c2ba7f4c91da","Reading export definitions and generated documents.") ?? "Reading export definitions and generated documents.")}</p></div>`;
      ...
      return `<section class="fmp-export-intro"><div><h2>${(globalThis.PlatformLanguage?.text("payroll","m_d9207f2ac07c2a","Prepare payroll records") ?? "Prepare payroll records")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_7e5e7151e747b9","Choose one payroll run when you are paying workers or handing final numbers to your payroll provider. Choose a date range when your accountant or office needs activity across several payrolls. CSV files are best for spreadsheets and imports; PDFs are formatted for reading, sharing, or printing.") ?? "Choose one payroll run when you are paying workers or handing final numbers to your payroll provider. Choose a date range when your accountant or office needs activity across several payrolls. CSV files are best for spreadsheets and imports; PDFs are formatted for reading, sharing, or printing.")}</p></div><i class="fas fa-file-invoice-dollar" style="font-size:28px;color:#98a2b3"></i></section>
        <section class="fmp-export-builder"><div class="fmp-export-step">${(globalThis.PlatformLanguage?.text("payroll","m_c189699a20e926","1. What should these reports cover?") ?? "1. What should these reports cover?")}</div><div class="fmp-export-mode"><button data-action="export-mode" data-mode="payroll_run" class="${String(state.exportMode === 'payroll_run' ? 'active' : '')}"><strong>${(globalThis.PlatformLanguage?.text("payroll","m_155e03aed682c6","A specific payroll run") ?? "A specific payroll run")}</strong><span>${(globalThis.PlatformLanguage?.text("payroll","m_e6457fc3facbdb","Use for payroll processing, approval records, and provider handoff.") ?? "Use for payroll processing, approval records, and provider handoff.")}</span></button><button data-action="export-mode" data-mode="date_range" class="${String(state.exportMode === 'date_range' ? 'active' : '')}"><strong>${(globalThis.PlatformLanguage?.text("payroll","m_0c55e913f098e3","A date range") ?? "A date range")}</strong><span>${(globalThis.PlatformLanguage?.text("payroll","m_f629af4c52c3ee","Use for bookkeeping, commissions, timesheets, and reimbursement review.") ?? "Use for bookkeeping, commissions, timesheets, and reimbursement review.")}</span></button></div>
        ${String(state.exportMode === 'payroll_run' ? (batches.length ? `<div class="fmp-export-selection"><div class="fmp-field"><label for="fmpExportBatch">Payroll run</label><select id="fmpExportBatch">${batches.map((batch) => `<option value="${esc(batch.id)}" ${clean(batch.id) === clean(selectedBatch?.id) ? 'selected' : ''}>${esc(batchOption(batch))}</option>`).join('')}</select></div><button class="fmp-btn" data-action="create-all-exports" data-format="csv"><i class="fas fa-table"></i> Prepare all CSVs</button><button class="fmp-btn" data-action="create-all-exports" data-format="pdf"><i class="fas fa-print"></i> Prepare all PDFs</button></div>` : `<div class="fmp-diagnostic"><i class="fas fa-circle-info"></i><span><strong>There are no payroll runs to export yet.</strong> Create a payroll batch first, then return here to select it.</span><button class="fmp-btn small" data-action="view" data-view="upcoming">Go to upcoming payroll</button></div>`) : `<div class="fmp-export-selection"><div class="fmp-field"><label for="fmpExportFrom">Start date</label><input id="fmpExportFrom" type="date" value="${esc(state.exportFrom)}"></div><div class="fmp-field"><label for="fmpExportThrough">End date</label><input id="fmpExportThrough" type="date" value="${esc(state.exportThrough)}" min="${esc(state.exportFrom)}"></div></div>`)}
        <div class="fmp-export-step" style="margin-top:18px">${(globalThis.PlatformLanguage?.text("payroll","m_2dc0a7287892b3","2. Choose the records you need") ?? "2. Choose the records you need")}</div><div class="fmp-report-grid">${String(availableReports.map((report) => `<article class="fmp-report-card"><div><h3>${esc(report.label)}</h3><p>${esc(report.description)}<span class="fmp-report-purpose">${esc(report.purpose)}</span></p></div><div class="fmp-report-actions"><button class="fmp-btn small" data-action="create-export" data-export-type="${esc(report.type)}" data-format="csv" ${state.exportMode === 'payroll_run' && !selectedBatch ? 'disabled' : ''}><i class="fas fa-file-csv"></i> Download CSV</button><button class="fmp-btn small" data-action="create-export" data-export-type="${esc(report.type)}" data-format="pdf" ${state.exportMode === 'payroll_run' && !selectedBatch ? 'disabled' : ''}><i class="fas fa-file-pdf"></i> Download PDF</button></div></article>`).join(''))}</div></section>
        <div class="fmp-export-history-head"><div><h2>${(globalThis.PlatformLanguage?.text("payroll","m_6b2b4d7c5da0fa","Previously prepared reports") ?? "Previously prepared reports")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_700c117178092f","Coverage is shown here so you can tell exactly what each file contains before downloading it again.") ?? "Coverage is shown here so you can tell exactly what each file contains before downloading it again.")}</p></div><span style="font-size:11px;color:#667085;font-weight:800">${((v4,v5) => globalThis.PlatformLanguage?.text("payroll","m_99071c28413a8e",`${v4} saved file${v5}`,{v4,v5}) ?? `${v4} saved file${v5}`)(artifacts.length,artifacts.length === 1 ? '' : 's')}</span></div>
        <div class="fmp-timesheet-table-wrap"><table class="fmp-timesheet-table"><thead><tr><th>${(globalThis.PlatformLanguage?.text("payroll","m_c47c2ce6bb05c0","Report") ?? "Report")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_b59aa39327a43b","What it covers") ?? "What it covers")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_cdf11b397d38fc","Format") ?? "Format")}</th><th>${(globalThis.PlatformLanguage?.text("payroll","m_984cc664c28b29","Prepared") ?? "Prepared")}</th><th></th></tr></thead><tbody>${String(artifacts.map((artifact) => { const format=clean(metadataFor(artifact).format || (clean(artifact.content_type).includes('pdf') ? 'pdf' : 'csv')); return `<tr><td><strong>${esc(reportName(artifact))}</strong><span class="fmp-file-name" title="${esc(artifact.file_name)}">${esc(artifact.file_name)}</span></td><td><strong>${esc(coverage(artifact))}</strong><br><small>${number(metadataFor(artifact).row_count)} record${number(metadataFor(artifact).row_count) === 1 ? '' : 's'}</small></td><td><span class="fmp-file-format ${esc(format)}">${esc(format.toUpperCase())}</span></td><td>${esc(formatDateTime(artifact.created_at))}</td><td><a class="fmp-btn small" href="${esc(window.PayrollAPI.artifacts.downloadUrl(state.orgId, artifact.id))}" download><i class="fas fa-download"></i> Download again</a></td></tr>`; }).join(''))}${String(!artifacts.length ? '<tr><td colspan="5">No reports have been prepared yet. Choose a report above to create the first one.</td></tr>' : '')}</tbody></table></div>`;
    }
    ...
    function bodyHtml(){
      ...
      if (state.loading && !state.data) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-spinner fmp-spinner"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_d0469f4ea4c0cc","Loading payroll") ?? "Loading payroll")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_961537c1a70067","Building upcoming schedule groups and payroll previews.") ?? "Building upcoming schedule groups and payroll previews.")}</p></div>`;
      if (state.error && !state.data) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-triangle-exclamation"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_a805757c14ef68","Payroll could not load") ?? "Payroll could not load")}</h2><p>${String(esc(state.error.message || state.error))}</p><button class="fmp-btn primary" data-action="refresh"><i class="fas fa-rotate"></i>${(globalThis.PlatformLanguage?.text("payroll","m_cbfbb44ff35f0f"," Try again") ?? " Try again")}</button></div>`;
      if (!array(state.data?.schedules).length) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-calendar-plus"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_af98d4fa763589","Create your first pay schedule") ?? "Create your first pay schedule")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_11167f4970bb3b","Payroll groups workers by schedule. Configure weekly, biweekly, semi-monthly, or monthly cycles here, then return to preview and run payroll.") ?? "Payroll groups workers by schedule. Configure weekly, biweekly, semi-monthly, or monthly cycles here, then return to preview and run payroll.")}</p><button class="fmp-btn primary" data-action="settings"><i class="fas fa-gear"></i>${(globalThis.PlatformLanguage?.text("payroll","m_00b847ae2b1181"," Configure payroll") ?? " Configure payroll")}</button></div>`;
      if (state.view === 'history') {
        const history = array(state.data?.history);
        if (!history.length) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-clock-rotate-left"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_df287cf95e8e63","No payroll history yet") ?? "No payroll history yet")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_100196122a2e0d","Created batches will appear here with their employee, subcontractor, run, and paid details.") ?? "Created batches will appear here with their employee, subcontractor, run, and paid details.")}</p><button class="fmp-btn" data-action="view" data-view="upcoming">${(globalThis.PlatformLanguage?.text("payroll","m_f363a044460dbe","View upcoming payroll") ?? "View upcoming payroll")}</button></div>`;
        const scheduleMap = schedulesById();
        return `<div class="fmp-history">${history.map((batch) => historyCard(batch, scheduleMap)).join('')}</div>`;
      }
      const upcoming = array(state.data?.upcoming);
      if (!upcoming.length) return `<div class="fmp-state"><span class="state-icon"><i class="fas fa-calendar-xmark"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_04de86b118c040","No payroll dates in this range") ?? "No payroll dates in this range")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_21151a08806ec5","Choose a wider date range to see future payroll occurrences. You can look ahead as far as your planning horizon requires.") ?? "Choose a wider date range to see future payroll occurrences. You can look ahead as far as your planning horizon requires.")}</p><button class="fmp-btn" data-action="preset" data-days="365"><i class="fas fa-calendar-days"></i>${(globalThis.PlatformLanguage?.text("payroll","m_0ad0e2722333d3"," Show the next year") ?? " Show the next year")}</button></div>`;
    ...
    function render(){
      ...
      if (state.view === 'settings') {
        ...
        if (!window.FirstMatePayrollSettings?.mount) {
          settingsHost.innerHTML = `<div class="fmp-state"><span class="state-icon"><i class="fas fa-triangle-exclamation"></i></span><h2>${(globalThis.PlatformLanguage?.text("payroll","m_46a683ef876bd0","Payroll settings could not load") ?? "Payroll settings could not load")}</h2><p>${(globalThis.PlatformLanguage?.text("payroll","m_c5efd7b69468f5","The payroll configuration bundle is unavailable.") ?? "The payroll configuration bundle is unavailable.")}</p><button class="fmp-btn" data-action="settings-back"><i class="fas fa-arrow-left"></i>${(globalThis.PlatformLanguage?.text("payroll","m_a1209799a6f896"," Back to payroll") ?? " Back to payroll")}</button></div>`;
          return;
        }
      ...
      root.innerHTML = `<div class="fmp-shell">
        <header class="fmp-top">
          <div class="fmp-heading"><span class="fmp-heading-icon"><i class="fas fa-money-check-dollar"></i></span><span class="fmp-heading-copy"><h1>${(globalThis.PlatformLanguage?.text("payroll","m_45e0abb75231e4","Payroll") ?? "Payroll")}</h1><p>${(globalThis.PlatformLanguage?.text("payroll","m_bd05cc1ca9fa01","Accrued pay, forecasts, payroll batches, and payment status by schedule.") ?? "Accrued pay, forecasts, payroll batches, and payment status by schedule.")}</p></span></div>
          <div class="fmp-top-actions"><button class="fmp-btn" data-action="create-off-cycle"><i class="fas fa-calendar-plus"></i><span>${(globalThis.PlatformLanguage?.text("payroll","m_7a7fb35aa48895","Off-cycle run") ?? "Off-cycle run")}</span></button><button class="fmp-btn" data-action="settings" title="${(globalThis.PlatformLanguage?.text("payroll","m_86fa12758c2c13","Payroll settings") ?? "Payroll settings")}"><i class="fas fa-sliders"></i><span>${(globalThis.PlatformLanguage?.text("payroll","m_7d461dc7d355cc","Settings") ?? "Settings")}</span></button><button class="fmp-btn" data-action="refresh" ${String(state.loading ? 'disabled' : '')}><i class="fas ${String(state.loading ? 'fa-spinner fmp-spinner' : 'fa-rotate')}"></i><span>${(globalThis.PlatformLanguage?.text("payroll","m_78973ce0cf3403","Refresh") ?? "Refresh")}</span></button></div>
        </header>

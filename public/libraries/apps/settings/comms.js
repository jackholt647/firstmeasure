...
(function(){
  ...
  function shell(view){
    return `<div class="fmcos"><nav class="fmcos-tabs" aria-label="${(globalThis.PlatformLanguage?.text("settings","m_d2ffc030cab2fb","Communications settings sections") ?? "Communications settings sections")}">
      <button class="fmcos-tab ${String(view === 'general' ? 'active' : '')}" data-co-view="general"><i class="fa-solid fa-sliders"></i>${(globalThis.PlatformLanguage?.text("settings","m_a8b3e178ada9e1"," General") ?? " General")}</button>
      <button class="fmcos-tab ${String(view === 'templates' ? 'active' : '')}" data-co-view="templates"><i class="fa-solid fa-message"></i>${(globalThis.PlatformLanguage?.text("settings","m_d708ded93ed154"," Message templates") ?? " Message templates")}</button>
  ...
  async function mount(el, options = {}){
    ...
    if (!api || !orgId) {
      el.innerHTML = `<div style="padding:24px;color:#b42318;font-weight:850">${(globalThis.PlatformLanguage?.text("settings","m_7ece649be9adf9","Communications settings failed to load.") ?? "Communications settings failed to load.")}</div>`;
      return;
    }
    const controller = {
  ...
  async function renderView(controller){
    ...
    content.innerHTML = `<div class="cs-note" style="padding:18px;">${(globalThis.PlatformLanguage?.text("settings","m_41963dee42009b","Loading communications settings…") ?? "Loading communications settings…")}</div>`;
    try {
  ...
  function renderGeneral(controller){
    ...
    content.innerHTML = `<div class="fmcos-grid">
      <section class="fmcos-card"><p class="fmcos-title"><i class="fa-solid fa-envelope"></i>${(globalThis.PlatformLanguage?.text("settings","m_3a7245a5ee6cd0","Company inbox") ?? "Company inbox")}</p>
        ${String(controller.inboxAddress ? `<div class="fmcos-inbox"><i class="fa-solid fa-at"></i>${esc(controller.inboxAddress)}</div><p class="fmcos-sub">Customer replies land on the matching project automatically.</p>` : '<p class="fmcos-sub">The company inbox is provisioned the first time email is used.</p>')}</section>
      <section class="fmcos-card"><p class="fmcos-title"><i class="fa-solid fa-bell"></i>${(globalThis.PlatformLanguage?.text("settings","m_23fbb6d4f23d62","Inbound notifications") ?? "Inbound notifications")}</p><p class="fmcos-sub">${(globalThis.PlatformLanguage?.text("settings","m_8eba04bd967401","Choose who is notified when a customer emails or texts in.") ?? "Choose who is notified when a customer emails or texts in.")}</p>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_dfc39bd0cac3f5","Notify the team") ?? "Notify the team")}</span>${String(toggle('notify_enabled', notifications.enabled !== false))}</div>
      ...
      <section class="fmcos-card"><p class="fmcos-title"><i class="fa-solid fa-share-from-square"></i>${(globalThis.PlatformLanguage?.text("settings","m_6b7a5ab17b95c9","Unmatched email") ?? "Unmatched email")}</p><p class="fmcos-sub">${(globalThis.PlatformLanguage?.text("settings","m_12b5abb0bd300a","Forward messages that have no usable project target.") ?? "Forward messages that have no usable project target.")}</p>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_4028fc8e76bfe1","Enable forwarding") ?? "Enable forwarding")}</span>${String(toggle('forwarding_enabled', forwarding.enabled === true))}</div>
        <div><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_bc01ab81360626","Fallback email") ?? "Fallback email")}</span><input type="email" data-co-text="forwarding_email" value="${String(esc(clean(forwarding.fallback_email)))}" placeholder="${(globalThis.PlatformLanguage?.text("settings","m_770640beb7d631","inbox@example.com") ?? "inbox@example.com")}"></div></section>
      <section class="fmcos-card"><p class="fmcos-title"><i class="fa-solid fa-microphone-lines"></i>${(globalThis.PlatformLanguage?.text("settings","m_048a7361aa1b5d","Voice messages") ?? "Voice messages")}</p><p class="fmcos-sub">${(globalThis.PlatformLanguage?.text("settings","m_01253a5e3076f3","Choose how recorded notes are delivered by channel.") ?? "Choose how recorded notes are delivered by channel.")}</p>
        <div><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_7a2ee5bebd134a","Text messages") ?? "Text messages")}</span><select data-co-text="voice_sms"><option value="attachment" ${String(clean(voice.sms) !== 'dictation' && clean(voice.sms) !== 'off' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_fc578eefcdfca0","Audio + transcript (MMS)") ?? "Audio + transcript (MMS)")}</option><option value="dictation" ${String(clean(voice.sms) === 'dictation' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_c2373e9f83affc","Dictation only") ?? "Dictation only")}</option><option value="off" ${String(clean(voice.sms) === 'off' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_273e689aeb0785","Off") ?? "Off")}</option></select></div>
        <div><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_5d2b9327181e33","Email") ?? "Email")}</span><select data-co-text="voice_email"><option value="dictation" ${String(clean(voice.email) !== 'off' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_c2373e9f83affc","Dictation only") ?? "Dictation only")}</option><option value="off" ${String(clean(voice.email) === 'off' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_273e689aeb0785","Off") ?? "Off")}</option></select></div></section>
      <section class="fmcos-card wide"><p class="fmcos-title"><i class="fa-solid fa-wand-magic-sparkles"></i>${(globalThis.PlatformLanguage?.text("settings","m_4c8e66d639bed7","Comms AI agent") ?? "Comms AI agent")}</p><p class="fmcos-sub">${(globalThis.PlatformLanguage?.text("settings","m_9231e287f19194","Company-wide behavior across email, texts, and portal chat.") ?? "Company-wide behavior across email, texts, and portal chat.")}</p>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_044994db3c4b47","Enable the comms agent") ?? "Enable the comms agent")}</span>${String(toggle('agent_enabled', agent.enabled !== false))}</div>
        <div><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_85ff81dc918529","Agent name") ?? "Agent name")}</span><input type="text" data-co-text="agent_name" value="${String(esc(clean(agent.agent_name) || 'Comms Agent'))}" maxlength="80"></div>
        <div><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_04016b6467ee76","Global instructions") ?? "Global instructions")}</span><textarea data-co-text="custom_instructions" placeholder="${(globalThis.PlatformLanguage?.text("settings","m_2d8ec1abaee245","Tone, policies, and promises to avoid.") ?? "Tone, policies, and promises to avoid.")}">${String(esc(String(agent.custom_instructions ?? '')))}</textarea></div>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_fcbd394b0a74de","Allowed to send messages") ?? "Allowed to send messages")}</span>${String(toggle('allow_send', agent.allow_send !== false))}</div>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_b707adca6df34d","Allowed to create appointments") ?? "Allowed to create appointments")}</span>${String(toggle('allow_scheduling', agent.allow_scheduling !== false))}</div>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_f7fbbb14ff6fd8","Allowed to read available times") ?? "Allowed to read available times")}</span>${String(toggle('allow_availability_read', agent.allow_availability_read === true))}</div>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_f5b101a8a73792","Allowed to commit customer reschedules") ?? "Allowed to commit customer reschedules")}</span>${String(toggle('allow_rescheduling', agent.allow_rescheduling === true))}</div></section>
      <section class="fmcos-card"><p class="fmcos-title"><i class="fa-solid fa-reply"></i>${(globalThis.PlatformLanguage?.text("settings","m_27309f9c4210cc","AI auto-response") ?? "AI auto-response")}</p><p class="fmcos-sub">${(globalThis.PlatformLanguage?.text("settings","m_e4e2a3aa8b5ed8","Draft replies for review or send automatically.") ?? "Draft replies for review or send automatically.")}</p>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_1b7f88942c0182","Auto-respond") ?? "Auto-respond")}</span>${String(toggle('auto_enabled', autoResponse.enabled === true))}</div>
        <div><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_669e1e42b35fa1","Mode") ?? "Mode")}</span><select data-co-text="auto_mode"><option value="draft" ${String(clean(autoResponse.mode) !== 'send' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_a26f603c848435","Draft for review") ?? "Draft for review")}</option><option value="send" ${String(clean(autoResponse.mode) === 'send' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_d8eaf5654c6968","Send automatically") ?? "Send automatically")}</option></select></div>
        <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_193abbb507fbd0","Respond to email") ?? "Respond to email")}</span>${String(toggle('auto_email', channels.email !== false))}</div><div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_fab27481d54e6f","Respond to texts") ?? "Respond to texts")}</span>${String(toggle('auto_sms', channels.sms !== false))}</div></section>
    </div><button type="button" class="fmcos-save" data-co-save>${(globalThis.PlatformLanguage?.text("settings","m_89abc4aa7105b9","Save communications settings") ?? "Save communications settings")}</button>`;
  ...
  function renderTemplates(controller){
    ...
    const content = controller.el.querySelector('[data-co-content]');
    content.innerHTML = `<div class="fmcos-template-head"><div><h3>${(globalThis.PlatformLanguage?.text("settings","m_3cc8ac595c76f1","Message templates") ?? "Message templates")}</h3><p>${(globalThis.PlatformLanguage?.text("settings","m_8d85ab7a35841d","Create reusable SMS and email drafts. Variables resolve when a teammate selects a template.") ?? "Create reusable SMS and email drafts. Variables resolve when a teammate selects a template.")}</p></div><button class="fmcos-btn" data-template-new><i class="fa-solid fa-plus"></i>${(globalThis.PlatformLanguage?.text("settings","m_fc61a22aa7a62e"," New template") ?? " New template")}</button></div>
      <div class="fmcos-template-workspace">
        <aside class="fmcos-template-list"><div class="fmcos-template-filter"><select data-template-filter><option value="">${(globalThis.PlatformLanguage?.text("settings","m_90d8f15c2db3ca","All channels") ?? "All channels")}</option><option value="sms">${(globalThis.PlatformLanguage?.text("settings","m_e1dd22c36fbc9b","SMS") ?? "SMS")}</option><option value="email">${(globalThis.PlatformLanguage?.text("settings","m_5d2b9327181e33","Email") ?? "Email")}</option></select></div><div class="fmcos-template-items" data-template-items></div></aside>
    ...
    const drawList = () => {
      ...
      content.querySelector('[data-template-items]').innerHTML = visible.length ? visible.map((item) => `<button class="fmcos-template-item ${clean(item.id) === controller.selectedId ? 'active' : ''}" data-template-id="${esc(clean(item.id))}"><strong>${esc(item.name)}</strong><div class="fmcos-template-meta"><span>${esc(clean(item.channel).toUpperCase())}</span>${item.category ? `<span>${esc(item.category)}</span>` : ''}${item.active === false ? `<span>${(globalThis.PlatformLanguage?.text("settings","m_425a653d4590da","Inactive") ?? "Inactive")}</span>` : ''}</div></button>`).join('') : `<div class="fmcos-empty">${(globalThis.PlatformLanguage?.text("settings","m_c9475097bcee66","No templates in this channel.") ?? "No templates in this channel.")}</div>`;
  ...
  function drawTemplateEditor(controller, template){
    ...
    if (!template) {
      editor.innerHTML = `<div class="fmcos-empty"><i class="fa-regular fa-message" style="font-size:28px"></i><br>${(globalThis.PlatformLanguage?.text("settings","m_0bce23feafbaf5","Select a template or create a new one.") ?? "Select a template or create a new one.")}</div>`;
      return;
    }
    ...
    editor.innerHTML = `<div class="fmcos-template-fields">
      <label class="fmcos-field"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_c4b7cbfdc9b700","Template name") ?? "Template name")}</span><input type="text" data-template-name value="${String(esc(template.name))}" maxlength="120"></label>
      <label class="fmcos-field"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_ed28d6fb9ea58c","Channel") ?? "Channel")}</span><select data-template-channel><option value="sms" ${String(template.channel === 'sms' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_e1dd22c36fbc9b","SMS") ?? "SMS")}</option><option value="email" ${String(template.channel === 'email' ? 'selected' : '')}>${(globalThis.PlatformLanguage?.text("settings","m_5d2b9327181e33","Email") ?? "Email")}</option></select></label>
      <label class="fmcos-field"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_c478c1907a4d4f","Category") ?? "Category")}</span><input type="text" data-template-category value="${String(esc(template.category || ''))}" maxlength="80"></label>
    </div>
    <label class="fmcos-field" data-template-subject-wrap ${String(template.channel === 'email' ? '' : 'hidden')}><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_eb218e4bd1f2dd","Email subject") ?? "Email subject")}</span><input type="text" data-template-subject value="${String(esc(template.subject || ''))}" maxlength="998"></label>
    <label class="fmcos-field"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_a16cfd85cfd122","Message") ?? "Message")}</span><textarea data-template-body maxlength="100000">${String(esc(template.body))}</textarea></label>
    <div class="fmcos-variable-bar"><span>${(globalThis.PlatformLanguage?.text("settings","m_55e76df1f0d41a","Insert variable") ?? "Insert variable")}</span>${String(variables.map((item) => `<button class="fmcos-chip" type="button" data-template-variable="${esc(item.key)}" title="${esc(item.example || '')}">{{${esc(item.key)}}}</button>`).join(''))}</div>
    <div class="fmcos-preview"><span class="fmcos-preview-label">${(globalThis.PlatformLanguage?.text("settings","m_4cde618f7445e1","Example preview") ?? "Example preview")}</span><div class="fmcos-preview-bubble" data-template-preview></div><div class="fmcos-hint" data-template-missing></div></div>
    <div class="fmcos-row"><span class="fmcos-label">${(globalThis.PlatformLanguage?.text("settings","m_9f4aefdd323d29","Available to teammates") ?? "Available to teammates")}</span>${String(toggle('template_active', template.active !== false))}</div>
    <div class="fmcos-template-actions"><div>${String(template.id ? '<button class="fmcos-btn danger" data-template-delete>Delete</button>' : '')}</div><div><button class="fmcos-save" data-template-save>${String(template.id ? 'Save template' : 'Create template')}</button></div></div>`;

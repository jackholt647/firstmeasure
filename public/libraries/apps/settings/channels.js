...
(function(root){
  ...
  function mount(pane, options = {}){
    ...
    pane.innerHTML = `<div class="chs-loading">${(globalThis.PlatformLanguage?.htmlText("settings","m_c7905f57f9bef9","Loading channel settings…") ?? "Loading channel settings…")}</div>`;
    ...
    function channelRow(channel){
      ...
      return `
        <div class="chs-row${String(archived ? ' archived' : '')}" data-channel="${String(esc(channel.id))}">
          ...
          <span class="name">${String(esc(channel.display_name || channel.name))}<small>${((v4,v5) => globalThis.PlatformLanguage?.htmlText("settings","m_cc5d9caa3b69a6",`${v4} members${v5}`,{v4,v5}) ?? `${v4} members${v5}`)(channel.member_count || 0,channel.topic ? ` · ${esc(channel.topic)}` : '')}</small></span>
          ${String(channel.type === 'private' ? `<span class="chs-pill">${(globalThis.PlatformLanguage?.htmlText("settings","m_c38b42b9122e62","Private") ?? "Private")}</span>` : '')}
          ${String(archived ? `<span class="chs-pill">${(globalThis.PlatformLanguage?.htmlText("settings","m_26b28a86126a93","Archived") ?? "Archived")}</span>` : '')}
    ...
    function render(channels, preferences = {}){
        ...
        <div class="chs-root">
          <div class="chs-head">
            <div class="chs-head-copy">
              <h3><span class="chs-head-icon"><i class="fas fa-hashtag"></i></span>${(globalThis.PlatformLanguage?.htmlText("settings","m_bebb358d67ea41"," Channels") ?? " Channels")}</h3>
              <p>${(globalThis.PlatformLanguage?.htmlText("settings","m_fb7493b5014d0e","Internal team messaging: channels, threads, reactions, direct messages, and per-project message threads. Manage the org-wide channel list here.") ?? "Internal team messaging: channels, threads, reactions, direct messages, and per-project message threads. Manage the org-wide channel list here.")}</p>
            </div>
            <div class="chs-actions-top">
              <button class="chs-btn" id="chsOpenApp"><i class="fas fa-arrow-up-right-from-square"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_4cb9e31714b89c"," Open Channels") ?? " Open Channels")}</button>
              <button class="chs-btn primary" id="chsCreate"><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_b325c4ac4d5599"," New channel") ?? " New channel")}</button>
            </div>
          </div>
          <div class="chs-grid">
            <div class="chs-card wide">
              <div class="chs-card-head"><strong><i class="fas fa-hashtag"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_ab3fbcb19bd5ed"," Team channels") ?? " Team channels")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_cc4d3edf151612","Public channels are open to everyone in the company; private channels are invite-only. Archiving hides a channel and locks new messages without deleting history.") ?? "Public channels are open to everyone in the company; private channels are invite-only. Archiving hides a channel and locks new messages without deleting history.")}</p></div>
              <div id="chsTeamList">${String(team.length ? team.map(channelRow).join('') : `<div class="chs-empty">${(globalThis.PlatformLanguage?.htmlText("settings","m_274532f34bdf27","No channels yet — create the first one.") ?? "No channels yet — create the first one.")}</div>`)}</div>
            </div>
            <div class="chs-card">
              <div class="chs-card-head"><strong><i class="fas fa-user-shield"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_9e683e58247212"," Permissions") ?? " Permissions")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_656b94e3524e0c","Grant these in user permission sets (Settings → Users).") ?? "Grant these in user permission sets (Settings → Users).")}</p></div>
              <div class="chs-perm"><code>${(globalThis.PlatformLanguage?.htmlText("settings","m_a4c36d4228f838","create_channels") ?? "create_channels")}</code><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_f33f158a12c9f3","Create public and private channels. Owners and admins always can.") ?? "Create public and private channels. Owners and admins always can.")}</span></div>
              <div class="chs-perm"><code>${(globalThis.PlatformLanguage?.htmlText("settings","m_5f557f484e28f6","manage_channels") ?? "manage_channels")}</code><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_a20984ac87e216","Edit or archive any channel, manage members, and remove or restore anyone's message. Editing stays author-only for everyone.") ?? "Edit or archive any channel, manage members, and remove or restore anyone's message. Editing stays author-only for everyone.")}</span></div>
              <div class="chs-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_ee0e0c24230d26","Message authorship is enforced server-side: only the person who wrote a message can edit it, ever.") ?? "Message authorship is enforced server-side: only the person who wrote a message can edit it, ever.")}</div>
            </div>
            <div class="chs-card">
              <div class="chs-card-head"><strong><i class="fas fa-toggle-on"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_5539ab93aa7a52"," Features") ?? " Features")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_24941aa4ad361a","Threads, reactions, DMs, attachments, pins, and search are individual capability flags.") ?? "Threads, reactions, DMs, attachments, pins, and search are individual capability flags.")}</p></div>
              <div class="chs-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_aa00d6f0409fa0","Toggle Channels features per-organization in ") ?? "Toggle Channels features per-organization in ")}<a id="chsOpenFlags">${(globalThis.PlatformLanguage?.htmlText("settings","m_cc445c917318c1","Features &amp; Apps") ?? "Features &amp; Apps")}</a>${(globalThis.PlatformLanguage?.htmlText("settings","m_d1aa801311165f"," under the “Communications” category (keys starting with ") ?? " under the “Communications” category (keys starting with ")}<code>${(globalThis.PlatformLanguage?.htmlText("settings","m_3fc867806515a5","channels.") ?? "channels.")}</code>).</div>
            </div>
            <div class="chs-card wide">
              <div class="chs-card-head"><strong><i class="fas fa-record-vinyl"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_f9eec5552531d5"," Huddle retention") ?? " Huddle retention")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_f2d903f955925d","When enabled, huddles retain their mixed audio and can include cameras and shared screens. A visible recording indicator appears for everyone, and the result is retained in the huddle conversation and channel files.") ?? "When enabled, huddles retain their mixed audio and can include cameras and shared screens. A visible recording indicator appears for everyone, and the result is retained in the huddle conversation and channel files.")}</p></div>
              <div class="chs-pref-list">
                <div class="chs-pref-row">
                  <div><label for="chsRecordHuddles">${(globalThis.PlatformLanguage?.htmlText("settings","m_8448509a8602f7","Retain huddle recordings") ?? "Retain huddle recordings")}</label><small>${(globalThis.PlatformLanguage?.htmlText("settings","m_4a5d2f8c4c7927","Organization-wide. Turn this off to keep huddles live-only.") ?? "Organization-wide. Turn this off to keep huddles live-only.")}</small></div>
                  <label class="chs-switch"><input id="chsRecordHuddles" type="checkbox" ${String(huddleRecordingEnabled ? 'checked' : '')}>${(globalThis.PlatformLanguage?.htmlText("settings","m_6184a3f1c019ec"," Record huddles") ?? " Record huddles")}</label>
                </div>
                <div class="chs-pref-row">
                  <div><label for="chsRecordHuddleVideo">${(globalThis.PlatformLanguage?.htmlText("settings","m_c0fafd4c71c164","Include video and shared screens") ?? "Include video and shared screens")}</label><small>${(globalThis.PlatformLanguage?.htmlText("settings","m_8edacd3ed2648a","Audio-only huddles still work normally when nobody enables a camera or screen.") ?? "Audio-only huddles still work normally when nobody enables a camera or screen.")}</small></div>
                  <label class="chs-switch"><input id="chsRecordHuddleVideo" type="checkbox" ${String(huddleVideoRecordingEnabled ? 'checked' : '')} ${String(huddleRecordingEnabled ? '' : 'disabled')}>${(globalThis.PlatformLanguage?.htmlText("settings","m_2d17177a23d2ec"," Record video") ?? " Record video")}</label>
                </div>
              </div>
            </div>
            <div class="chs-card wide">
              <div class="chs-card-head"><strong><i class="fas fa-bell"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_55539f4f3c8f55"," Your channel preferences") ?? " Your channel preferences")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_a306c13ae7d201","These preferences follow your account and apply on every device.") ?? "These preferences follow your account and apply on every device.")}</p></div>
              <div class="chs-pref-list">
                <div class="chs-pref-row">
                  <div><label for="chsDefaultNotifications">${(globalThis.PlatformLanguage?.htmlText("settings","m_ed01a1da4c98c1","Default notifications") ?? "Default notifications")}</label><small>${(globalThis.PlatformLanguage?.htmlText("settings","m_250e156ead090a","Individual channel choices can override this.") ?? "Individual channel choices can override this.")}</small></div>
                  <select id="chsDefaultNotifications">
                    <option value="all">${(globalThis.PlatformLanguage?.htmlText("settings","m_6275935eadd8a4","All new messages") ?? "All new messages")}</option>
                    <option value="mentions">${(globalThis.PlatformLanguage?.htmlText("settings","m_722ecd47834278","Mentions and replies") ?? "Mentions and replies")}</option>
                    <option value="muted">${(globalThis.PlatformLanguage?.htmlText("settings","m_3c7c4f2cd743b8","Nothing") ?? "Nothing")}</option>
                  </select>
                </div>
                <div class="chs-pref-row">
                  <div><label for="chsSendMode">${(globalThis.PlatformLanguage?.htmlText("settings","m_10b3b04881d2ff","Send messages with") ?? "Send messages with")}</label><small>${(globalThis.PlatformLanguage?.htmlText("settings","m_b1f07f61089504","The other key combination inserts a new line.") ?? "The other key combination inserts a new line.")}</small></div>
                  <select id="chsSendMode">
                    <option value="enter">${(globalThis.PlatformLanguage?.htmlText("settings","m_92c001f63daac8","Enter") ?? "Enter")}</option>
                    <option value="modified_enter">${(globalThis.PlatformLanguage?.htmlText("settings","m_a34f170b65e07e","Ctrl/Cmd + Enter") ?? "Ctrl/Cmd + Enter")}</option>
                  </select>
                </div>
                <div class="chs-pref-row">
                  <div><label for="chsDnd">${(globalThis.PlatformLanguage?.htmlText("settings","m_be9029a365cbbe","Pause notifications") ?? "Pause notifications")}</label><small>${(globalThis.PlatformLanguage?.htmlText("settings","m_58b5ec56db50e3","Activity remains available for you to review later.") ?? "Activity remains available for you to review later.")}</small></div>
                  <label class="chs-switch"><input id="chsDnd" type="checkbox">${(globalThis.PlatformLanguage?.htmlText("settings","m_fcfd131557bcd7"," Do not disturb") ?? " Do not disturb")}</label>
                </div>
              </div>
            </div>
            <div class="chs-card wide">
              <div class="chs-card-head"><strong><i class="fas fa-folder"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_e1fc3bc4c3c18e"," Project message threads") ?? " Project message threads")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_387d32582f5cb6","Each project gets its own message thread, shown in the project's Notes area and under “Project messages” in the Channels app. These are created automatically and follow project access.") ?? "Each project gets its own message thread, shown in the project's Notes area and under “Project messages” in the Channels app. These are created automatically and follow project access.")}</p></div>
              <div id="chsProjectList">${String(projects.length ? projects.slice(0, 12).map(channelRow).join('') : `<div class="chs-empty">${(globalThis.PlatformLanguage?.htmlText("settings","m_b9e88dd13732d2","No project messages yet.") ?? "No project messages yet.")}</div>`)}</div>
            </div>
          </div>
        </div>`;
      ...
      pane.querySelector('#chsCreate')?.addEventListener('click', () => {
        ...
        row.innerHTML = `
          ...
          <input type="text" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_2e0e0d6bedcbd2","channel-name") ?? "channel-name")}" style="flex:1;min-width:0;border:1px solid #d0d5dd;border-radius:9px;padding:8px 11px;font:800 12.5px/1.4 inherit;color:#344054;outline:0">
          <button class="chs-btn small" data-create-confirm>${(globalThis.PlatformLanguage?.htmlText("settings","m_3c21a9590eb762","Create") ?? "Create")}</button>
          <button class="chs-btn small" data-create-cancel>${(globalThis.PlatformLanguage?.htmlText("settings","m_cbef679b21abb4","Cancel") ?? "Cancel")}</button>`;
        list.prepend(row);

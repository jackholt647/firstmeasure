...
(function(){
  ...
  function contactFields(contact = {}, errors = {}){
    const field = (name,label,wide=false,placeholder='',type='text') => {
      ...
      return `<label class="dm-field ${wide?'dm-wide':''} ${error?'invalid':''}"><span>${esc(label)}${required?`<em class="dm-required">${(globalThis.PlatformLanguage?.htmlText("settings","m_db97f048cd99aa","Required") ?? "Required")}</em>`:''}</span><input class="dm-input" name="${esc(name)}" type="${type}" value="${esc(contact[name] || '')}" placeholder="${esc(placeholder)}" ${required?'required':''} ${error?'aria-invalid="true" aria-describedby="dm-error-'+esc(name)+'"':''}>${error?`<small class="dm-field-error" id="dm-error-${esc(name)}">${esc(error)}</small>`:''}</label>`;
    };
  ...
  function openWizard(options, initialPath = '', initialStep = ''){
    ...
    function bodyHtml(){
      if (state.busy) return `<div class="dm-loading"><span><i class="fas fa-circle-notch fa-spin"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_1171f9997c2e9d"," Working securely…") ?? " Working securely…")}</span></div>`;
      if (state.step === 'choose') return `<div class="dm-choice-grid">
        <button class="dm-choice ${String(state.path==='register'?'selected':'')}" data-path="register"><span class="icon"><i class="fas fa-magnifying-glass"></i></span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_ace49b1ec21514","Register a new domain") ?? "Register a new domain")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_21c37b64d61db2","Search available names, see the annual price, and register one without leaving FirstMate.") ?? "Search available names, see the annual price, and register one without leaving FirstMate.")}</span></button>
        <button class="dm-choice ${String(state.path==='transfer'?'selected':'')}" data-path="transfer"><span class="icon"><i class="fas fa-arrow-right-arrow-left"></i></span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_d7d9771f85e390","Transfer a domain") ?? "Transfer a domain")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_0f29ad8eda1299","Move a domain from another provider using its authorization code.") ?? "Move a domain from another provider using its authorization code.")}</span></button>
        <button class="dm-choice ${String(state.path==='existing'?'selected':'')}" data-path="existing"><span class="icon"><i class="fas fa-link"></i></span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_bbc1cdea0b61c7","Use an existing domain") ?? "Use an existing domain")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_90012b00013e8d","Keep it where it is and connect it to a FirstMate website with DNS.") ?? "Keep it where it is and connect it to a FirstMate website with DNS.")}</span></button>
      </div>`;
      if (state.step === 'domain' && state.path === 'register') return `<div class="dm-stack">
        <form class="dm-search-row" data-search><input class="dm-input" name="domain" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_992a2b48510215","yourcompany.com") ?? "yourcompany.com")}" value="${String(esc(state.domain))}" required><select class="dm-select" name="period"><option value="1">${(globalThis.PlatformLanguage?.htmlText("settings","m_4b231e0cddf70e","1 year") ?? "1 year")}</option><option value="2">${(globalThis.PlatformLanguage?.htmlText("settings","m_36823ec15426a4","2 years") ?? "2 years")}</option><option value="3">${(globalThis.PlatformLanguage?.htmlText("settings","m_2ddbc007deaf45","3 years") ?? "3 years")}</option></select><button class="dm-btn primary" type="submit"><i class="fas fa-magnifying-glass"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_df7ced82563784"," Search") ?? " Search")}</button></form>
        <p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_d2472bc3032884","Prices include private registration where available. Quotes remain valid for 10 minutes.") ?? "Prices include private registration where available. Quotes remain valid for 10 minutes.")}</p>
        <div class="dm-results">${String(state.quote ? (state.quote.items||[]).map((item) => `<button type="button" class="dm-result ${state.item?.domain===item.domain?'selected':''}" data-domain="${esc(item.domain)}" ${item.available?'':'disabled'}><span><strong>${esc(item.domain)}</strong><small>${item.available?(item.premium?'Premium domain · available':'Available'):(item.reason||'Unavailable')}</small></span>${item.available?`<span class="dm-price">${money(item.price)} <small>${((v1) => globalThis.PlatformLanguage?.htmlText("settings","m_acf892860eeea9",`/ ${v1} yr`,{v1}) ?? `/ ${v1} yr`)(Number(state.quote.period||1))}</small></span>`:`<span class="dm-badge">${(globalThis.PlatformLanguage?.htmlText("settings","m_ce86fe78082565","Unavailable") ?? "Unavailable")}</span>`}</button>`).join('') : '')}</div>
      </div>`;
      if (state.step === 'domain' && state.path === 'transfer') return `<div class="dm-stack">
        <label class="dm-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_0583d77b73123f","Domain") ?? "Domain")}</span><input class="dm-input" name="domain" value="${String(esc(state.domain))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_992a2b48510215","yourcompany.com") ?? "yourcompany.com")}" required></label>
        <label class="dm-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_9aa8861c7c8cd7","Transfer authorization code") ?? "Transfer authorization code")}</span><input class="dm-input" name="auth_code" value="${String(esc(state.authCode))}" type="password" autocomplete="off" required></label>
        <div class="dm-callout"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_55276993d9d6f3","Before continuing:") ?? "Before continuing:")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_64d5141284d640"," unlock the domain at its current provider and request its transfer authorization code. Most transfers take several days after approval.") ?? " unlock the domain at its current provider and request its transfer authorization code. Most transfers take several days after approval.")}</div>
        ${String(state.item ? `<div class="dm-summary"><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_4cf18dcb73c18f","Transfer price") ?? "Transfer price")}</span><strong>${((v0) => globalThis.PlatformLanguage?.htmlText("settings","m_4d60013fef9dbb",`${v0} USD`,{v0}) ?? `${v0} USD`)(money(state.item.price))}</strong></div><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_0dbf10eeda5484","Term") ?? "Term")}</span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_4b231e0cddf70e","1 year") ?? "1 year")}</strong></div></div>` : '')}
      </div>`;
      if (state.step === 'domain') return `<div class="dm-stack"><label class="dm-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_0583d77b73123f","Domain") ?? "Domain")}</span><input class="dm-input" name="domain" value="${String(esc(state.domain))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_992a2b48510215","yourcompany.com") ?? "yourcompany.com")}" required></label><div class="dm-callout">${(globalThis.PlatformLanguage?.htmlText("settings","m_0c5ea6ccc9c380","The domain stays with its current provider. After review, we’ll give you one TXT record to add so FirstMate can verify ownership.") ?? "The domain stays with its current provider. After review, we’ll give you one TXT record to add so FirstMate can verify ownership.")}</div></div>`;
      if (state.step === 'contact') return contactFields(state.contact, state.fieldErrors);
      if (state.step === 'billing') return `<div class="dm-stack"><div class="dm-callout warn"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_8f6d475b4c012b","Billing is not connected yet.") ?? "Billing is not connected yet.")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_8503c67dd86572"," This onboarding flow includes the billing checkpoint, but no card is collected and no payment is processed by FirstMate in this version.") ?? " This onboarding flow includes the billing checkpoint, but no card is collected and no payment is processed by FirstMate in this version.")}</div><div class="dm-summary"><div class="dm-summary-row"><span>${((v0) => globalThis.PlatformLanguage?.htmlText("settings","m_b8751f2d4f8ddb",`${v0} total`,{v0}) ?? `${v0} total`)(state.path==='transfer'?'Transfer':'Registration')}</span><strong>${((v1) => globalThis.PlatformLanguage?.htmlText("settings","m_d4f554dfb70d83",`${v1} USD`,{v1}) ?? `${v1} USD`)(money(state.item?.price))}</strong></div><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_5a3350b9db25e3","Payment status") ?? "Payment status")}</span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_f56737918370dc","Deferred for this test workflow") ?? "Deferred for this test workflow")}</strong></div><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_003d0cd4d5dd21","Renewal") ?? "Renewal")}</span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_13fce6e531a8b0","Automatic renewal enabled") ?? "Automatic renewal enabled")}</strong></div></div></div>`;
      if (state.step === 'review') return `<div class="dm-stack"><div class="dm-summary">
        <div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_0583d77b73123f","Domain") ?? "Domain")}</span><strong>${String(esc(state.domain))}</strong></div>
        <div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_b06faf127e1505","Setup") ?? "Setup")}</span><strong>${String(state.path==='register'?'New registration':state.path==='transfer'?'Transfer':'Existing domain connection')}</strong></div>
        ${String(state.path==='existing'?'':`<div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_9403c7637d4905","Total") ?? "Total")}</span><strong>${((v0) => globalThis.PlatformLanguage?.htmlText("settings","m_4d60013fef9dbb",`${v0} USD`,{v0}) ?? `${v0} USD`)(money(state.item?.price))}</strong></div><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_ebde2cac915f72","Registrant") ?? "Registrant")}</span><strong>${esc(`${state.contact.first_name||''} ${state.contact.last_name||''}`.trim())}</strong></div>`)}
        <div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_88ce17162a6464","Billing collected") ?? "Billing collected")}</span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_2f0222913078f4","No") ?? "No")}</strong></div>
      </div><div class="dm-callout ${String(state.path==='existing'||orderTestMode?'':'danger')}">${String(state.path==='existing'?`<strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_d540cb574bf54c","No transfer or purchase.") ?? "No transfer or purchase.")}</strong> This creates a DNS verification record only.`:orderTestMode?`<strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_306264fc95bf71","Test mode.") ?? "Test mode.")}</strong> This simulates the ${state.path==='transfer'?'transfer':'registration'} and does not submit an order to OpenSRS or incur a domain charge.`:`<strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_2c10be3701f2a3","Final confirmation.") ?? "Final confirmation.")}</strong> Submitting can create a live ${state.path==='transfer'?'transfer order':'domain registration'} and incur the displayed domain cost even though FirstMate billing is not connected.`)}</div>
      <label class="dm-check"><input type="checkbox" data-attest> <span>${(globalThis.PlatformLanguage?.htmlText("settings","m_7e9b2776dcfade","I confirm the information is accurate, accept the domain registration terms, and authorize this request.") ?? "I confirm the information is accurate, accept the domain registration terms, and authorize this request.")}</span></label>
      </div>`;
      if (state.step === 'verify') {
        const verification = obj(state.registration?.verification);
        if (state.registration?.status === 'ready_to_connect' || state.registration?.verified_at) return `<div class="dm-success"><div class="icon"><i class="fas fa-check"></i></div><h3>${(globalThis.PlatformLanguage?.htmlText("settings","m_3e37522560abc5","Verification complete") ?? "Verification complete")}</h3><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_a34714579052b0","Continue to choose the website and default email address for this domain.") ?? "Continue to choose the website and default email address for this domain.")}</p></div>`;
        return state.path === 'existing'
          ? `<div class="dm-stack"><div class="dm-callout"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_15d09fc348b12e","Add this TXT record in your DNS:") ?? "Add this TXT record in your DNS:")}</strong></div><div class="dm-summary"><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_523563ae2fd488","Host") ?? "Host")}</span><strong>${String(esc(verification.host||''))}</strong></div><div class="dm-summary-row"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_ec6b76d100b0ec","Value") ?? "Value")}</span><strong style="word-break:break-all">${String(esc(verification.value||''))}</strong></div></div><p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_2316222487e5d5","DNS changes can take time to appear. You can close this workflow and return later.") ?? "DNS changes can take time to appear. You can close this workflow and return later.")}</p></div>`
          : state.registration?.simulated
            ? `<div class="dm-stack"><div class="dm-callout warn"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_8b24faed53b69a","Test verification is pending.") ?? "Test verification is pending.")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_74c889fbc5e08d"," In live mode FirstMate waits for the registrar to confirm the registrant. Use the test control below to simulate that confirmation and inspect the end-user experience.") ?? " In live mode FirstMate waits for the registrar to confirm the registrant. Use the test control below to simulate that confirmation and inspect the end-user experience.")}</div><p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_7ced1188e01de2","No verification or reminder email is sent by this test workflow.") ?? "No verification or reminder email is sent by this test workflow.")}</p></div>`
            : `<div class="dm-stack"><div class="dm-callout"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_93a318cf3fe7c1","Waiting for registrar confirmation.") ?? "Waiting for registrar confirmation.")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_62a9f6ffb8e41b"," FirstMate will keep checking the registrant verification status. A reminder option becomes available only if it is still incomplete after one hour.") ?? " FirstMate will keep checking the registrant verification status. A reminder option becomes available only if it is still incomplete after one hour.")}</div><p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_b4e522c39f6665","You can close this workflow and return later without losing progress.") ?? "You can close this workflow and return later without losing progress.")}</p></div>`;
      }
      if (state.step === 'resources') {
        const sites = state.sites.filter((site)=>clean(site.site_kind)!=='customer_portal'&&clean(site.status)!=='archived');
        return `<div class="dm-stack"><div><h4 style="margin:0 0 5px;font-size:13px">${(globalThis.PlatformLanguage?.htmlText("settings","m_9dc897dcba7a79","Choose a website") ?? "Choose a website")}</h4><p class="dm-note" style="margin-bottom:10px">${(globalThis.PlatformLanguage?.htmlText("settings","m_d9cc78af9742bf","This domain will become the selected website’s primary domain.") ?? "This domain will become the selected website’s primary domain.")}</p><div class="dm-site-grid">${String(sites.map((site)=>`<button type="button" class="dm-site-tile ${state.websiteMode===`existing:${site.id}`?'selected':''}" data-website-choice="existing:${esc(site.id)}"><span class="icon"><i class="fas fa-window-maximize"></i></span><strong>${esc(site.name||'Website')}</strong><small>${esc(clean(site.status||'draft').replace(/_/g,' '))}</small></button>`).join(''))}<button type="button" class="dm-site-tile ${String(state.websiteMode==='create'?'selected':'')}" data-website-choice="create"><span class="icon"><i class="fas fa-plus"></i></span><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_a7b6365a404298","Create a new website") ?? "Create a new website")}</strong><small>${(globalThis.PlatformLanguage?.htmlText("settings","m_30e9f05dba7fa9","Start a fresh hosted website for this domain.") ?? "Start a fresh hosted website for this domain.")}</small></button></div></div>
        ${String(state.websiteMode==='create'?`<label class="dm-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_e64a543b1d8328","New website name ") ?? "New website name ")}<em class="dm-required">${(globalThis.PlatformLanguage?.htmlText("settings","m_db97f048cd99aa","Required") ?? "Required")}</em></span><input class="dm-input" name="website_name" value="${esc(state.websiteName)}" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_99786b12078773","Main Website") ?? "Main Website")}" required></label>`:'')}
        <div><h4 style="margin:0 0 5px;font-size:13px">${(globalThis.PlatformLanguage?.htmlText("settings","m_715488f4d0b6d4","Default email address") ?? "Default email address")}</h4><p class="dm-note" style="margin-bottom:9px">${(globalThis.PlatformLanguage?.htmlText("settings","m_33079737eef674","FirstMate will use this address for incoming email and as the default sender for automated website email. Provider and DNS activation will be completed during email setup.") ?? "FirstMate will use this address for incoming email and as the default sender for automated website email. Provider and DNS activation will be completed during email setup.")}</p><label class="dm-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_4327ac34fcf20c","Email address ") ?? "Email address ")}<em class="dm-required">${(globalThis.PlatformLanguage?.htmlText("settings","m_db97f048cd99aa","Required") ?? "Required")}</em></span><div class="dm-email-builder"><input class="dm-input" name="default_email_local_part" value="${String(esc(state.defaultEmailLocalPart))}" aria-label="${(globalThis.PlatformLanguage?.htmlText("settings","m_e7e9c2f0efc1df","Default email name") ?? "Default email name")}" required><span class="dm-email-domain">@${String(esc(state.domain))}</span></div></label></div></div>`;
      }
      return `<div class="dm-success"><div class="icon"><i class="fas fa-check"></i></div><h3>${((v0) => globalThis.PlatformLanguage?.htmlText("settings","m_4ce45455f4515a",`${v0} is ready`,{v0}) ?? `${v0} is ready`)(esc(state.domain))}</h3><p>${String(state.registration?.website_id?'The domain and default email address are attached to your website. Open Web Editor to continue building.':'The domain setup has been saved.')}</p></div>`;
    }

    function footerActionsHtml(){
      if (state.busy) return '';
      if (state.step === 'done') return `<button class="dm-btn primary" data-finish><i class="fas fa-arrow-up-right-from-square"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_c600b08bd97509"," Open Web Editor") ?? " Open Web Editor")}</button>`;
      if (state.step === 'verify') {
        ...
        if (verified) return `<button class="dm-btn" data-back>${(globalThis.PlatformLanguage?.htmlText("settings","m_121372231b5699","Back") ?? "Back")}</button><button class="dm-btn primary" data-next>${(globalThis.PlatformLanguage?.htmlText("settings","m_854c72abba5166","Continue ") ?? "Continue ")}<i class="fas fa-arrow-right"></i></button>`;
        if (state.registration?.simulated) return `<button class="dm-btn" data-back>${(globalThis.PlatformLanguage?.htmlText("settings","m_121372231b5699","Back") ?? "Back")}</button><button class="dm-btn primary" data-check><i class="fas fa-flask"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_357b27f0dd099b"," Complete test verification") ?? " Complete test verification")}</button>`;
        return `<button class="dm-btn" data-back>${(globalThis.PlatformLanguage?.htmlText("settings","m_121372231b5699","Back") ?? "Back")}</button>${String(state.path==='existing'?`<button class="dm-btn" data-resend>${(globalThis.PlatformLanguage?.htmlText("settings","m_0007afaec666fd","Copy DNS values") ?? "Copy DNS values")}</button>`:reminderReady?`<button class="dm-btn" data-resend>${(globalThis.PlatformLanguage?.htmlText("settings","m_4ce77002a418e4","Send reminder email") ?? "Send reminder email")}</button>`:'')}<button class="dm-btn primary" data-check><i class="fas fa-rotate"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_ffb92d3f243a29"," Check status") ?? " Check status")}</button>`;
      }
    ...
    wizard = window.FirstMateSetupWizard.open({
      ...
      railNote:() => `${orderTestMode?`<span class="dm-test-indicator"><i class="fas fa-flask"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_1fbbd370250cd9"," Test mode") ?? " Test mode")}</span><br>`:''}Private registration, renewal protection, and domain lock are enabled where supported.`,
  ...
  async function legacyMount(options = {}){
    ...
    pane.innerHTML=`<div class="dm-card"><p class="dm-note"><i class="fas fa-circle-notch fa-spin"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_27be9dd638fd07"," Loading domains…") ?? " Loading domains…")}</p></div>`;
    try {
      ...
      pane.innerHTML=`<div class="dm-page"><div class="dm-hero"><div><h3>${(globalThis.PlatformLanguage?.htmlText("settings","m_7e6427ce4061f4","Domains & Hosting") ?? "Domains & Hosting")}</h3><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_b220ed6c45c214","Register, transfer, or connect domains, then attach them to a FirstMate website and default email address.") ?? "Register, transfer, or connect domains, then attach them to a FirstMate website and default email address.")}</p></div><button class="dm-btn primary" data-new-domain><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_36735857529693"," Add domain") ?? " Add domain")}</button></div>
        ${String(config.configured&&config.encryption_configured?'':`<div class="dm-callout warn"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_310a794dc5d358","Domain purchasing is not configured.") ?? "Domain purchasing is not configured.")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_0c9c4c4df16b0c"," Existing-domain connections remain available, but registration and transfer require server credentials and encryption.") ?? " Existing-domain connections remain available, but registration and transfer require server credentials and encryption.")}</div>`)}
        <section class="dm-card"><div class="dm-card-head"><div><h4>${(globalThis.PlatformLanguage?.htmlText("settings","m_7e3507920a3b24","Your domains") ?? "Your domains")}</h4><p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_2704e3a346d938","Verification and website connection status update here.") ?? "Verification and website connection status update here.")}</p></div></div><div class="dm-owned">${String(domains.length?domains.map((domain)=>`<div class="dm-owned-row"><div><strong>${esc(domain.domain||'')}</strong><p class="dm-note">${esc(domain.simulated?'Simulated domain':domain.acquisition_type==='existing'?'Connected domain':domain.acquisition_type==='transfer'?'Domain transfer':'Registered domain')}${domain.website_id?' · Website connected':''}</p></div><span class="dm-badge ${esc(domain.status||'')}">${esc(clean(domain.status||'pending').replace(/_/g,' '))}</span></div>`).join(''):`<p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_6e11ef5cf54adb","No domains have been added yet.") ?? "No domains have been added yet.")}</p>`)}</div></section></div>`;
      let wizard=null;
  ...
  async function mount(options = {}){
    ...
    const orgId=clean(options.orgId);
    if(!pane||!orgId)return;
    pane.innerHTML=`<div class="dm-card"><p class="dm-note"><i class="fas fa-circle-notch fa-spin"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_a80f8f278ef5a0"," Loading domains...") ?? " Loading domains...")}</p></div>`;
    try{
      const [configResult,listResult,registrantDefaults]=await Promise.all([
        window.DomainsAPI.config(),
        window.DomainsAPI.list(orgId),
      ...
      const domainCard=(domain)=>`<article class="dm-owned-row">
        ...
        <div class="dm-card-actions"><button class="dm-btn" data-manage-domain="${String(esc(domain.domain))}"><i class="fas fa-sliders"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_773cb43b56c274"," Manage") ?? " Manage")}</button></div>
      </article>`;
      const listHtml=()=>`<div class="dm-page">
        ${String(options.embedded?'':`<div class="dm-hero"><div><h3>${(globalThis.PlatformLanguage?.htmlText("settings","m_7e6427ce4061f4","Domains & Hosting") ?? "Domains & Hosting")}</h3><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_b220ed6c45c214","Register, transfer, or connect domains, then attach them to a FirstMate website and default email address.") ?? "Register, transfer, or connect domains, then attach them to a FirstMate website and default email address.")}</p></div><button class="dm-btn primary" data-new-domain><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_36735857529693"," Add domain") ?? " Add domain")}</button></div>`)}
        ${String(options.embedded?`<div class="dm-card-head"><div><h4>${(globalThis.PlatformLanguage?.htmlText("settings","m_7e3507920a3b24","Your domains") ?? "Your domains")}</h4><p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_46f4295156601c","Manage website routing, renewal protection, nameservers, privacy, and transfers.") ?? "Manage website routing, renewal protection, nameservers, privacy, and transfers.")}</p></div><button class="dm-btn primary" data-new-domain><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_36735857529693"," Add domain") ?? " Add domain")}</button></div>`:'')}
        ${String(config.configured&&config.encryption_configured?'':`<div class="dm-callout warn"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_310a794dc5d358","Domain purchasing is not configured.") ?? "Domain purchasing is not configured.")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_0c9c4c4df16b0c"," Existing-domain connections remain available, but registration and transfer require server credentials and encryption.") ?? " Existing-domain connections remain available, but registration and transfer require server credentials and encryption.")}</div>`)}
        <section class="dm-card"><div class="dm-card-head"><div><h4>${String(options.embedded?'Domain portfolio':'Your domains')}</h4><p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_1e4516f36d3029","Each card shows the website currently served on that domain.") ?? "Each card shows the website currently served on that domain.")}</p></div></div>
          <div class="dm-owned">${String(domains.length?domains.map(domainCard).join(''):`<p class="dm-note">${(globalThis.PlatformLanguage?.htmlText("settings","m_6e11ef5cf54adb","No domains have been added yet.") ?? "No domains have been added yet.")}</p>`)}</div>
        </section></div>`;
      const detailHtml=(domain)=>{
        ...
        return `<div class="dm-page">
          <div class="dm-hero"><div class="dm-detail-heading"><button class="dm-detail-back" data-domain-back aria-label="${(globalThis.PlatformLanguage?.htmlText("settings","m_993cffffd61784","Back to all domains") ?? "Back to all domains")}" title="${(globalThis.PlatformLanguage?.htmlText("settings","m_993cffffd61784","Back to all domains") ?? "Back to all domains")}"><i class="fas fa-arrow-left"></i></button><div><h3>${String(esc(domain.domain))}</h3><p>${((v1,v2) => globalThis.PlatformLanguage?.htmlText("settings","m_bfcaea81bcc4ed",`${v1} &middot; ${v2}`,{v1,v2}) ?? `${v1} &middot; ${v2}`)(esc(typeLabel(domain)),esc(clean(domain.status||'pending').replace(/_/g,' ')))}</p></div></div>${String(domain.website_id?`<button class="dm-btn" data-open-website="${esc(domain.website_id)}"><i class="fas fa-arrow-up-right-from-square"></i>${((v1) => globalThis.PlatformLanguage?.htmlText("settings","m_5280d67fd63d3b",` Open ${v1}`,{v1}) ?? ` Open ${v1}`)(esc(domain.website?.name||'website'))}</button>`:'')}</div>
          ${String(message?`<div class="dm-callout" role="status"><i class="fas fa-circle-info"></i> ${esc(message)}</div>`:'')}
          ${String(busy?`<div class="dm-callout"><i class="fas fa-circle-notch fa-spin"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_45b749fc916282"," Saving domain changes...") ?? " Saving domain changes...")}</div>`:'')}
          <div class="dm-manage-grid">
            <section class="dm-manage-card"><h4><i class="fas fa-window-maximize"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_341b4942338454"," Website & email") ?? " Website & email")}</h4>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_902cd6598c5257","Connected website") ?? "Connected website")}</strong><p>${String(esc(domain.website?.name||'No website connected'))}</p></div>${String(domain.website_id?`<button class="dm-btn" data-open-website="${esc(domain.website_id)}">${(globalThis.PlatformLanguage?.htmlText("settings","m_c25cc66b28cc9d","Open") ?? "Open")}</button>`:`<span class="dm-badge">${(globalThis.PlatformLanguage?.htmlText("settings","m_cd62fc8a4b19b2","Not connected") ?? "Not connected")}</span>`)}</div>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_715488f4d0b6d4","Default email address") ?? "Default email address")}</strong><p>${String(esc(domain.default_email||'Not configured'))}</p></div><span class="dm-badge">${String(domain.email_configuration_status==='active'?'Active':'Pending setup')}</span></div>
            </section>
            <section class="dm-manage-card"><h4><i class="fas fa-shield-halved"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_12ca16778536e1"," Protection & renewal") ?? " Protection & renewal")}</h4>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_4d5a168f5fd12e","Domain privacy") ?? "Domain privacy")}</strong><p>${String(external?'Managed at the current registrar.':'Registrant information stays private where supported.')}</p></div><button class="dm-switch on" disabled aria-label="${(globalThis.PlatformLanguage?.htmlText("settings","m_d391c915a7335b","Domain privacy enabled") ?? "Domain privacy enabled")}"></button></div>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_7acab65048e339","Auto-renew") ?? "Auto-renew")}</strong><p>${String(external?'Managed at the current registrar.':'Renew automatically before expiration.')}</p></div><button class="dm-switch ${String(domain.auto_renew!==false?'on':'')}" data-domain-toggle="auto_renew" ${String(external||busy?'disabled':'')}></button></div>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_eb664d5a225fc1","Registrar lock") ?? "Registrar lock")}</strong><p>${String(external?'Managed at the current registrar.':'Blocks unauthorized transfers and registry changes.')}</p></div><button class="dm-switch ${String(domain.locked!==false?'on':'')}" data-domain-toggle="locked" ${String(external||busy?'disabled':'')}></button></div>
            </section>
            <section class="dm-manage-card"><h4><i class="fas fa-network-wired"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_f7e82cb670723d"," DNS & nameservers") ?? " DNS & nameservers")}</h4>
              ...
              <textarea class="dm-textarea" data-domain-nameservers placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_7646ae24b6c57c","abby.ns.cloudflare.com&#10;mark.ns.cloudflare.com") ?? "abby.ns.cloudflare.com&#10;mark.ns.cloudflare.com")}" ${String(external||busy?'disabled':'')}>${String(esc(nameservers))}</textarea>
              <div class="dm-card-actions"><button class="dm-btn primary" data-save-nameservers ${String(external||busy?'disabled':'')}><i class="fas fa-floppy-disk"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_bba93a61b42863"," Save nameservers") ?? " Save nameservers")}</button></div>
            </section>
            <section class="dm-manage-card"><h4><i class="fas fa-address-card"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_62d1668cdfb9ec"," Registration") ?? " Registration")}</h4>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_95af41e20221e6","Expiration") ?? "Expiration")}</strong><p>${String(esc(dateLabel(domain.expires_at)))}</p></div></div>
              <div class="dm-setting-row"><div class="dm-setting-copy"><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_78ef82dff0cf42","Registrant contact") ?? "Registrant contact")}</strong><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_c3d10b3fb197a6","Protected from the public directory. Contact changes require registry verification.") ?? "Protected from the public directory. Contact changes require registry verification.")}</p></div></div>
            </section>
            <section class="dm-manage-card dm-danger-zone"><h4><i class="fas fa-arrow-right-arrow-left"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_0aa7532fd241c1"," Domain actions") ?? " Domain actions")}</h4>
              ...
              <div class="dm-card-actions">${String(external?'':`<button class="dm-btn" data-transfer-out ${busy?'disabled':''}><i class="fas fa-paper-plane"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_e0769df0810a0d"," Transfer out") ?? " Transfer out")}</button>`)}<button class="dm-btn danger" data-disconnect ${String(busy?'disabled':'')}><i class="fas fa-link-slash"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_8a064cc0e8391e"," Disconnect from FirstMate") ?? " Disconnect from FirstMate")}</button></div>
            </section>
          </div></div>`;
      };
      const render=()=>{

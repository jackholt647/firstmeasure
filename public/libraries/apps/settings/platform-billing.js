  ...
  async function mountWorkspace(host,options={}){
    ...
    function renderStatement(){
      ...
      target.querySelectorAll('[data-invoice]').forEach(button=>button.onclick=()=>{
        ...
        const dialog=modal(row.title,`<p>${esc(row.detail)} · ${esc(row.status)}</p>${row.lines.map(line=>`<div class="bw-row bw-subscription"><span>${esc(line.label)}</span><strong>${money(line.amount_cents)}</strong></div>`).join('')}<div class="bw-row bw-subscription"><strong>Total</strong><strong>${money(invoice.total_cents)}</strong></div>${invoice.url?'<button class="primary" data-receipt>View invoice / receipt</button>':row.service==='usage'&&row.due&&data.can_manage?'<button class="primary" data-pay>Pay invoice</button>':''}`);
    ...
    function render(){
      ...
      <details class="bw-usage"><summary>Usage & storage</summary>${data?.storage?`<p>${(Number(data.storage.quantity)/1073741824).toFixed(2)} GiB stored · measured ${esc(date(data.storage.occurred_at))}</p>`:'<p>Storage has not been measured yet.</p>'}${(data?.estimate.lines||[]).map(line=>`<div class="bw-row"><div>${esc(line.label)}<small> · ${esc(line.quantity)} units · ${esc(line.included)} included</small></div><strong>${money(line.amount_cents)}</strong></div>`).join('')||'<p>No metered charges for this month.</p>'}${data?.sync?`<p><small>Updated ${esc(date(data.sync.at))}${data.sync.error?' · '+esc(data.sync.error):data.sync.complete?'':' · Catch-up in progress'}</small></p>`:''}${data?.late_usage?.length?'<p>Late usage is awaiting an operator adjustment.</p>':''}</details></section>
      <aside class="bw-section bw-aside">${credit?`<div class="bw-row"><h3>Credit auto top-up</h3><button class="link" data-topup>Manage</button></div><h4>${credit.enabled?'On':'Off'}</h4><p>${credit.enabled?'Add '+amount(credit.amount)+' when credits fall below '+amount(credit.threshold)+'.':'Add measurement credits automatically when your balance runs low.'}</p><div style="margin-top:20px"><h4>Credit payment method</h4><p>${esc(credit.card||'No saved card')}</p></div>`:'<h3>Payment timing</h3>'}<p style="margin-top:20px"><small>Subscriptions renew monthly. Add-ons show the updated total and the amount due before you pay.</small></p></aside></div>
      ...
      host.querySelector('[data-add]')?.addEventListener('click',()=>{
        const latest=new Map();for(const p of data.prices)if(p.published&&(!latest.has(p.product_id)||latest.get(p.product_id).version<p.version))latest.set(p.product_id,p);
        const available=[...latest.values()].filter(p=>!subs.some(s=>s.product_id===p.product_id));
        const dialog=modal('Add subscription',available.map(p=>`<div class="bw-subscription"><div class="bw-row"><div><h4>${esc(p.name)}</h4><p>${esc(p.description)}</p><strong>${money(p.monthly_cents)} / month${p.rates.length?' + usage':''}</strong></div><button data-subscribe="${esc(p.id)}">Review & add</button></div></div>`).join('')||'<p>No additional subscriptions are available.</p>');

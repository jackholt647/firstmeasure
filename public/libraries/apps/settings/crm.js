(function(root){
  ...
  function mount(host, options = {}){
    ...
    host.innerHTML = `<div class="crm-settings"><div class="crm-error"><i class="fas fa-spinner fa-spin"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_2bc82e9f77e1eb"," Loading settings") ?? " Loading settings")}</div></div>`;
    ...
    function callRoleChoices(draft){
      ...
      }).join('') || `<span class="crm-hint">${(globalThis.PlatformLanguage?.htmlText("settings","m_a50a98dc11ce7c","No access roles are configured yet. Role IDs can still be entered below.") ?? "No access roles are configured yet. Role IDs can still be entered below.")}</span>`;
    }
    function callUserChoices(draft){
      ...
      }).join('') || `<span class="crm-hint">${(globalThis.PlatformLanguage?.htmlText("settings","m_f019015e9b8cac","No active people were found.") ?? "No active people were found.")}</span>`;
    }
    function callEditorMarkup(){
      const draft = state.callDraft;
      if (!draft) return `<div class="crm-call-empty"><div><i class="fas fa-list-check"></i><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_0e77465d2df6c0","Select a call list") ?? "Select a call list")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_463f28d47865a2","Choose a list to control who sees it and how it appears in Calls.") ?? "Choose a list to control who sees it and how it appears in Calls.")}</span></div></div>`;
      ...
      return `<div class="crm-call-editor-head"><div class="crm-call-editor-title"><span class="crm-call-list-icon"><i class="fas ${String(esc(draft.icon || 'fa-phone'))}"></i></span><div><h4>${String(esc(draft.title || (globalThis.PlatformLanguage?.text("settings","m_d2459194a46e7e","New call list") ?? "New call list")))}</h4><p>${String(isNew ? 'Create an organization-wide queue.' : `${Number(draft.pending_count || 0)} pending call${Number(draft.pending_count || 0) === 1 ? '' : 's'} · ${esc(callAssignmentLabel(draft))}`)}</p></div></div><div class="crm-call-switch-row"><span>${String(draft.status === 'active' ? 'Active' : 'Inactive')}</span><button class="crm-call-switch ${String(draft.status === 'active' ? 'on' : '')}" type="button" data-call-active aria-label="${(globalThis.PlatformLanguage?.htmlText("settings","m_058412d3e429ee","Toggle list status") ?? "Toggle list status")}" aria-pressed="${String(draft.status === 'active')}"></button></div></div>
        <section class="crm-call-section"><div class="crm-call-section-head"><div><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_4939d2238bc1e0","List details") ?? "List details")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_ab20966c7dc4ac","The name and appearance callers see in the Calls tab.") ?? "The name and appearance callers see in the Calls tab.")}</span></div></div><div class="crm-call-form-grid">
          <label class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_48a52dec920f8b","List name") ?? "List name")}</span><input data-call-field="title" value="${String(esc(draft.title))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_6c62aae02ca220","Mid-project check-ins") ?? "Mid-project check-ins")}"></label>
          <label class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_cab566faf07d28","List key") ?? "List key")}</span><input data-call-field="key" value="${String(esc(draft.key))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_999150047f9ffd","mid_project_checkins") ?? "mid_project_checkins")}" ${String(isNew ? '' : 'readonly')}></label>
          <label class="crm-call-field wide"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_aa136ecb65672f","Description") ?? "Description")}</span><textarea data-call-field="description" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_bc621d59c73e9f","Explain when a call belongs here.") ?? "Explain when a call belongs here.")}">${String(esc(draft.description))}</textarea></label>
          <label class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_64d67127744574","Call type") ?? "Call type")}</span><select data-call-field="kind">${String([['lead','Lead intake'],['follow_up','Scheduled follow-up'],['signature','Signed customer'],['production','Production'],['customer','Customer care'],['general','General']].map(([value,label]) => `<option value="${value}" ${draft.kind === value ? 'selected' : ''}>${label}</option>`).join(''))}</select></label>
          <div class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_444eb2577d10b1","Display icon") ?? "Display icon")}</span><div class="crm-call-visual-row">${String([['fa-phone','Phone'],['fa-phone-volume','Outgoing call'],['fa-clock','Clock'],['fa-handshake','Handshake'],['fa-helmet-safety','Production'],['fa-headset','Customer care'],['fa-list-check','Checklist']].map(([value,label]) => `<button class="crm-call-visual-option ${draft.icon === value ? 'selected' : ''}" type="button" data-call-icon="${value}" aria-label="${label}" title="${label}" aria-pressed="${draft.icon === value}"><i class="fas ${value}"></i></button>`).join(''))}</div></div>
          <div class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_9faac7258df0a0","Color treatment") ?? "Color treatment")}</span><div class="crm-call-visual-row">${String([['default','Blue','#2563eb'],['lead','Lead blue','#3538cd'],['followup','Purple','#7f56d9'],['customer','Green','#079455'],['production','Orange','#c2410c']].map(([value,label,color]) => `<button class="crm-call-visual-option tone ${draft.tone === value ? 'selected' : ''}" style="--call-tone:${color}" type="button" data-call-tone="${value}" aria-label="${label}" title="${label}" aria-pressed="${draft.tone === value}"></button>`).join(''))}</div></div>
          <label class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_0505c57e04e7ce","Display order") ?? "Display order")}</span><input type="number" data-call-field="sort_order" value="${String(Number(draft.sort_order || 0))}" min="0" step="10"></label>
        </div></section>
        <section class="crm-call-section"><div class="crm-call-section-head"><div><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_dc49c965d875f0","Who handles these calls?") ?? "Who handles these calls?")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_81d376e79d4d54","Leave the list open to everyone, or route it to specific roles and people.") ?? "Leave the list open to everyone, or route it to specific roles and people.")}</span></div><div class="crm-call-modes"><button class="crm-call-mode ${String(restricted ? '' : 'active')}" type="button" data-call-everyone><i class="fas fa-users"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_7997fdde9802e5"," Everyone") ?? " Everyone")}</button><button class="crm-call-mode ${String(restricted ? 'active' : '')}" type="button" data-call-restricted><i class="fas fa-user-lock"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_9102fddfe2fd7b"," Assigned only") ?? " Assigned only")}</button></div></div>
          <div class="crm-call-choice-group"><span class="crm-call-choice-label">${(globalThis.PlatformLanguage?.htmlText("settings","m_0728ce1a29afca","Roles") ?? "Roles")}</span><div class="crm-call-chips">${String(callRoleChoices(draft))}</div><label class="crm-call-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_88aa87a29a7174","Role IDs") ?? "Role IDs")}</span><input data-call-role-ids value="${String(esc(draft.assigned_role_ids.join(', ')))}" placeholder="${(globalThis.PlatformLanguage?.htmlText("settings","m_8faa76bd574917","office, production") ?? "office, production")}"></label></div>
          <div class="crm-call-choice-group"><span class="crm-call-choice-label">${(globalThis.PlatformLanguage?.htmlText("settings","m_290e094a10f572","Specific people") ?? "Specific people")}</span><div class="crm-call-chips">${String(callUserChoices(draft))}</div></div>
        </section>
        <section class="crm-call-section"><div class="crm-call-section-head"><div><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_5cd547e56ead7c","Workflow connection") ?? "Workflow connection")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_1067e5ca14f97a","Scopes and automations add work to this list by its stable key.") ?? "Scopes and automations add work to this list by its stable key.")}</span></div></div><div class="crm-call-origin"><i class="fas ${String(managed ? 'fa-wand-magic-sparkles' : 'fa-code-branch')}"></i><div><strong>${String(managed ? 'Connected to project scope work' : 'Ready for scopes and automations')}</strong><span>${String(managed ? 'The Roof welcome-call step publishes signed customers here and completes with the call.' : 'Use crm.callLists.add.v1 on a work node’s onReady hook.')}</span></div><button class="crm-call-key" type="button" data-call-copy title="${(globalThis.PlatformLanguage?.htmlText("settings","m_b1d41127305f1f","Copy list key") ?? "Copy list key")}">${String(esc(draft.key || 'set_a_key'))}</button></div></section>
        <section class="crm-call-section"><div class="crm-call-section-head"><div><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_265cb02467b2cb","Caller flow") ?? "Caller flow")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_d8e127a0db34f6","Every list shares the same quick outcome flow.") ?? "Every list shares the same quick outcome flow.")}</span></div></div><div class="crm-call-flow"><div class="crm-call-flow-step"><i class="fas fa-phone"></i><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_8d4eaa0da004be","Call") ?? "Call")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_435c1044ae98b9","Work through one contact at a time.") ?? "Work through one contact at a time.")}</span></div><div class="crm-call-flow-step"><i class="fas fa-clipboard-check"></i><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_f303d69f775dc0","Record outcome") ?? "Record outcome")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_41fadcef284d5a","Answered, voicemail, no answer, or skip.") ?? "Answered, voicemail, no answer, or skip.")}</span></div><div class="crm-call-flow-step"><i class="fas fa-check-double"></i><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_288a893aca6efa","Complete or follow up") ?? "Complete or follow up")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_db3adb9e5481e3","Finish linked work or schedule the next call.") ?? "Finish linked work or schedule the next call.")}</span></div></div></section>
        <div class="crm-call-editor-actions"><span class="crm-call-status" id="crmCallStatus"></span><div class="crm-call-actions"><button class="crm-btn" type="button" data-call-reset>${(globalThis.PlatformLanguage?.htmlText("settings","m_955ed36bc1820f","Discard changes") ?? "Discard changes")}</button><button class="crm-btn primary" type="button" data-call-save ${String(state.callSaving ? 'disabled' : '')}><i class="fas fa-floppy-disk"></i> ${String(state.callSaving ? 'Saving…' : isNew ? 'Create list' : 'Save list')}</button></div></div>`;
    }
    function followUpSettingsMarkup(){
      ...
      const quick=(config.quick_options||[]).map((option,index)=>`<div class="crm-row" data-follow-quick="${String(index)}"><div class="crm-grid-3">${String(field('Button label',`<input class="crm-input" data-follow-quick-field="label" value="${esc(option.label)}">`))}${String(field('Amount',`<input class="crm-input" type="number" min="1" data-follow-quick-field="amount" value="${Number(option.amount)||1}">`))}${String(field('Unit',`<select class="crm-select" data-follow-quick-field="unit"><option value="days" ${option.unit==='days'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_d674899969da48","Days") ?? "Days")}</option><option value="weeks" ${option.unit==='weeks'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_c18bd9b7e40fc5","Weeks") ?? "Weeks")}</option><option value="months" ${option.unit==='months'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_49edaae2aeec64","Months") ?? "Months")}</option></select>`))}</div><button class="crm-btn danger" type="button" data-follow-remove-quick="${String(index)}"><i class="fas fa-trash"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_09a65903b3217c"," Remove") ?? " Remove")}</button></div>`).join('');
      const policySteps=(policy.steps||[]).map((step,index)=>`<div class="crm-row" data-follow-policy-step="${String(index)}"><div class="crm-row-head"><strong>${((v1) => globalThis.PlatformLanguage?.htmlText("settings","m_37c9aa5403dd55",`Attempt ${v1}`,{v1}) ?? `Attempt ${v1}`)(index+1)}</strong><span><button class="crm-btn crm-icon" type="button" data-follow-policy-move="-1" ${String(index===0?'disabled':'')} title="${(globalThis.PlatformLanguage?.htmlText("settings","m_0ef44a71d653fa","Move earlier") ?? "Move earlier")}"><i class="fas fa-arrow-up"></i></button><button class="crm-btn crm-icon" type="button" data-follow-policy-move="1" ${String(index===policy.steps.length-1?'disabled':'')} title="${(globalThis.PlatformLanguage?.htmlText("settings","m_9613af4b86bae6","Move later") ?? "Move later")}"><i class="fas fa-arrow-down"></i></button><button class="crm-btn crm-icon danger" type="button" data-follow-policy-remove ${String(policy.steps.length<=1?'disabled':'')} title="${(globalThis.PlatformLanguage?.htmlText("settings","m_26fe00c3c178fa","Remove attempt") ?? "Remove attempt")}"><i class="fas fa-trash"></i></button></span></div><div class="crm-grid-3">${String(field('Step label',`<input class="crm-input" data-follow-policy-field="label" value="${esc(step.label)}">`))}${String(field('Wait',`<input class="crm-input" type="number" min="1" data-follow-policy-field="amount" value="${Number(step.amount)||1}">`))}${String(field('Unit',`<select class="crm-select" data-follow-policy-field="unit"><option value="days" ${step.unit==='days'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_d674899969da48","Days") ?? "Days")}</option><option value="weeks" ${step.unit==='weeks'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_c18bd9b7e40fc5","Weeks") ?? "Weeks")}</option><option value="months" ${step.unit==='months'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_49edaae2aeec64","Months") ?? "Months")}</option></select>`))}</div></div>`).join('');
      ...
      return `<section class="crm-settings-block"><div class="crm-section-head"><div><h5>${(globalThis.PlatformLanguage?.htmlText("settings","m_a21f2cc9fc344e","Follow-up behavior") ?? "Follow-up behavior")}</h5><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_95effd0be90a6d","One tagged to-do workflow shared by Calls, Today, projects, and automations.") ?? "One tagged to-do workflow shared by Calls, Today, projects, and automations.")}</span></div><button class="crm-btn primary" type="button" data-follow-save><i class="fas fa-floppy-disk"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_ea8389eb2db426"," Save follow-ups") ?? " Save follow-ups")}</button></div><div class="crm-grid-3">${String(field('Terminology',`<input class="crm-input" data-follow-field="label" value="${esc(config.label)}">`))}${String(field('Default to-do title',`<input class="crm-input" data-follow-field="default_title" value="${esc(config.default_title)}">`))}${String(field('Default time (optional)',`<input class="crm-input" type="time" data-follow-field="default_time" value="${esc(config.default_time||'')}">`))}</div><div class="crm-section"><div class="crm-section-head"><div><h5>${(globalThis.PlatformLanguage?.htmlText("settings","m_3ec5146f36a2bd","Automatic contact cadence") ?? "Automatic contact cadence")}</h5><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_9d3e92d14f6c83","Create the next tagged follow-up automatically when a call does not connect.") ?? "Create the next tagged follow-up automatically when a call does not connect.")}</span></div><label class="crm-check"><input type="checkbox" data-follow-policy-enabled ${String(policy.enabled!==false?'checked':'')}>${(globalThis.PlatformLanguage?.htmlText("settings","m_0f5e8a42aaeb75"," Enabled") ?? " Enabled")}</label></div><div class="crm-grid-2"><div class="crm-field"><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_27fcba5b5597e9","Apply when") ?? "Apply when")}</span><div class="crm-grid-3"><label class="crm-check"><input type="checkbox" data-follow-policy-trigger="voicemail" ${String((policy.triggers||[]).includes('voicemail')?'checked':'')}>${(globalThis.PlatformLanguage?.htmlText("settings","m_4c0eae2f5118f3"," Voicemail") ?? " Voicemail")}</label><label class="crm-check"><input type="checkbox" data-follow-policy-trigger="no_answer" ${String((policy.triggers||[]).includes('no_answer')?'checked':'')}>${(globalThis.PlatformLanguage?.htmlText("settings","m_734124f0fbebaf"," No answer") ?? " No answer")}</label><label class="crm-check"><input type="checkbox" data-follow-policy-trigger="manual_follow_up" ${String((policy.triggers||[]).includes('manual_follow_up')?'checked':'')}>${(globalThis.PlatformLanguage?.htmlText("settings","m_ea7a68e0598230"," Follow-up button") ?? " Follow-up button")}</label></div></div>${String(field('After the final step',`<select class="crm-select" data-follow-policy-after><option value="repeat_last" ${policy.after_last==='repeat_last'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_6c8ff1f276e6ec","Keep repeating the final interval") ?? "Keep repeating the final interval")}</option><option value="stop" ${policy.after_last==='stop'?'selected':''}>${(globalThis.PlatformLanguage?.htmlText("settings","m_1ae07271ca1ccb","Stop automatic attempts") ?? "Stop automatic attempts")}</option></select>`))}</div><div class="crm-section-head"><div><h5>${(globalThis.PlatformLanguage?.htmlText("settings","m_1f1d499c46cd5a","Attempt sequence") ?? "Attempt sequence")}</h5><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_3414daa9d67f5e","Each wait starts when the previous call is completed.") ?? "Each wait starts when the previous call is completed.")}</span></div><button class="crm-btn" type="button" data-follow-policy-add><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_7c34adf4b56d61"," Add attempt") ?? " Add attempt")}</button></div>${String(policySteps||`<div class="crm-empty">${(globalThis.PlatformLanguage?.htmlText("settings","m_978ac8f171e6cb","Add at least one attempt interval.") ?? "Add at least one attempt interval.")}</div>`)}</div><div class="crm-section"><div class="crm-section-head"><div><h5>${(globalThis.PlatformLanguage?.htmlText("settings","m_7ce767912e8be5","Quick reschedule options") ?? "Quick reschedule options")}</h5><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_a2c722d8f538a1","Manual overrides shown alongside the suggested policy date.") ?? "Manual overrides shown alongside the suggested policy date.")}</span></div><button class="crm-btn" type="button" data-follow-add-quick><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_e7713012500e3b"," Add option") ?? " Add option")}</button></div>${String(quick||`<div class="crm-empty">${(globalThis.PlatformLanguage?.htmlText("settings","m_fb1fb4c99cf558","Add at least one quick option.") ?? "Add at least one quick option.")}</div>`)}</div><div class="crm-section"><div class="crm-section-head"><div><h5>${(globalThis.PlatformLanguage?.htmlText("settings","m_a7635200c63f1a","Completion outcomes") ?? "Completion outcomes")}</h5><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_cfad848ad13467","Customize the wording while keeping the three standard behaviors. Pipeline movement is handled by the sales pipeline scope automatically.") ?? "Customize the wording while keeping the three standard behaviors. Pipeline movement is handled by the sales pipeline scope automatically.")}</span></div></div>${String(outcomes)}</div></section>`;
    }
    function renderCallWorkspace(){
      ...
      const listMarkup = filtered.map((list) => `<div class="crm-call-list-item ${state.callSelectedKey === list.key ? 'active' : ''}" data-call-list="${esc(list.key)}" data-tone="${esc(list.tone)}" role="button" tabindex="0"><span class="crm-call-list-icon"><i class="fas ${esc(list.icon || 'fa-phone')}"></i></span><span class="crm-call-list-copy"><strong>${esc(list.title)}</strong><span>${esc(callKindLabel(list.kind))} · ${list.status === 'active' ? esc(callAssignmentLabel(list)) : 'Inactive'}</span></span><span class="crm-call-count ${Number(list.pending_count || 0) ? 'has-calls' : ''}">${Number(list.pending_count || 0)}</span></div>`).join('') || `<div class="crm-call-empty"><div><i class="fas fa-magnifying-glass"></i><strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_35c4be17ab5530","No matching lists") ?? "No matching lists")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("settings","m_c8284dfd7645f8","Try a different search or create a new list.") ?? "Try a different search or create a new list.")}</span></div></div>`;
      host.querySelector('.crm-settings').innerHTML = (String(subTabs()) + "<div class=\"crm-call-page\"><header class=\"crm-call-top\"><div><div class=\"crm-call-kicker\">" + (globalThis.PlatformLanguage?.htmlText("settings","m_68f1423b4d6f83","CRM · Calls") ?? "CRM · Calls") + "</div><h3>" + (globalThis.PlatformLanguage?.htmlText("settings","m_45e0ea42bd7f61","Call workflows") ?? "Call workflows") + "</h3><p>" + (globalThis.PlatformLanguage?.htmlText("settings","m_0a72968c54b3ad","Organize calls into real lists, decide which team handles each one, and connect project scopes to the right queue.") ?? "Organize calls into real lists, decide which team handles each one, and connect project scopes to the right queue.") + "</p></div><div class=\"crm-call-actions\"><button class=\"crm-btn\" type=\"button\" data-call-refresh><i class=\"fas fa-rotate\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_4f524800833039"," Refresh") ?? " Refresh") + "</button><button class=\"crm-btn\" type=\"button\" data-call-open><i class=\"fas fa-phone\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_07e64910ede841"," Open Calls") ?? " Open Calls") + "</button><button class=\"crm-btn primary\" type=\"button\" data-call-new><i class=\"fas fa-plus\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_9307f6f8b8e9f1"," New call list") ?? " New call list") + "</button></div></header>\n        <div class=\"crm-call-stats\"><div class=\"crm-call-stat\"><span>" + (globalThis.PlatformLanguage?.htmlText("settings","m_cdce2be16f3cc8","Total lists") ?? "Total lists") + "</span><strong>" + String(lists.length) + "</strong></div><div class=\"crm-call-stat active\"><span>" + (globalThis.PlatformLanguage?.htmlText("settings","m_3689c34b89cd39","Active lists") ?? "Active lists") + "</span><strong>" + String(activeCount) + "</strong></div><div class=\"crm-call-stat waiting\"><span>" + (globalThis.PlatformLanguage?.htmlText("settings","m_dad4a2bcff697f","Calls waiting") ?? "Calls waiting") + "</span><strong>" + String(pendingCount) + "</strong></div><div class=\"crm-call-stat routed\"><span>" + (globalThis.PlatformLanguage?.htmlText("settings","m_f6bf476d818cc0","Team-routed lists") ?? "Team-routed lists") + "</span><strong>" + String(routedCount) + "</strong></div></div>\n        " + String(followUpSettingsMarkup()) + "\n        <div class=\"crm-call-workspace\"><aside class=\"crm-call-rail\"><div class=\"crm-call-rail-head\"><div class=\"crm-call-rail-title\"><strong>" + (globalThis.PlatformLanguage?.htmlText("settings","m_81df0d1019af98","Call lists") ?? "Call lists") + "</strong><span>" + ((v6) => globalThis.PlatformLanguage?.htmlText("settings","m_2594ac3351cedd",`${v6} configured`,{v6}) ?? `${v6} configured`)(lists.length) + "</span></div><label class=\"crm-call-search\"><i class=\"fas fa-magnifying-glass\"></i><input value=\"" + String(esc(state.callSearch)) + "\" placeholder=\"" + (globalThis.PlatformLanguage?.htmlText("settings","m_c28f246d7741d6","Find a list") ?? "Find a list") + "\" data-call-search></label></div><div class=\"crm-call-list\">" + String(listMarkup) + "</div></aside><main class=\"crm-call-editor\">" + String(callEditorMarkup()) + "</main></div></div>");
      bindCallWorkspace();
    }
    ...
    async function renderCallWorkflows(options = {}){
      if (!state.callLoaded || options.refresh) {
        host.querySelector('.crm-settings').innerHTML = (String(subTabs()) + "<div class=\"crm-call-loading\"><span><i class=\"fas fa-spinner fa-spin\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_74c4de2aa80926"," Loading call lists…") ?? " Loading call lists…") + "</span></div>");
        bindSubTabs();
        try { await loadCallLists({refresh:options.refresh === true}); }
        catch(error) { if(state.subtab==='calls')host.querySelector('.crm-settings').innerHTML=(String(subTabs()) + "<div class=\"crm-call-error\"><strong>" + (globalThis.PlatformLanguage?.htmlText("settings","m_56fd06d812e322","Call workflows unavailable.") ?? "Call workflows unavailable.") + "</strong><br>" + String(esc(error?.message || 'Could not load call lists.')) + "</div>");return; }
      }
      renderCallWorkspace();
    }
    ...
    function renderMisc(){
      ...
      host.querySelector('.crm-settings').innerHTML = (String(subTabs()) + "\n        <div class=\"crm-head\"><div><h3>" + (globalThis.PlatformLanguage?.htmlText("settings","m_ef207345c8196c","Celebrations") ?? "Celebrations") + "</h3><p>" + (globalThis.PlatformLanguage?.htmlText("settings","m_510e3636b7f8f9","Control celebratory sounds and effects for this branch.") ?? "Control celebratory sounds and effects for this branch.") + "</p></div><div class=\"crm-head-actions\"><span class=\"crm-status\" id=\"crmStatus\"></span><button class=\"crm-btn primary\" id=\"crmSaveProjectConfig\" type=\"button\"><i class=\"fas fa-floppy-disk\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_bfcbd339764266"," Save changes") ?? " Save changes") + "</button></div></div>\n        <section class=\"crm-settings-block\"><h4>" + (globalThis.PlatformLanguage?.htmlText("settings","m_ef207345c8196c","Celebrations") ?? "Celebrations") + "</h4><p>" + (globalThis.PlatformLanguage?.htmlText("settings","m_510e3636b7f8f9","Control celebratory sounds and effects for this branch.") ?? "Control celebratory sounds and effects for this branch.") + "</p><div class=\"crm-option-grid\">" + String(options.map(([id,label,description]) => `<button class="crm-option ${config.celebrations_mode===id?'active':''}" data-celebrations-mode="${id}" type="button"><strong>${label}</strong><span>${description}</span></button>`).join('')) + "</div><div class=\"crm-head-actions\" style=\"margin-top:14px;justify-content:flex-start\"><button class=\"crm-btn\" id=\"crmTestSmall\" type=\"button\"><i class=\"fas fa-music\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_4a1f95fa474201"," Test small") ?? " Test small") + "</button><button class=\"crm-btn\" id=\"crmTestLarge\" type=\"button\"><i class=\"fas fa-wand-magic-sparkles\"></i>" + (globalThis.PlatformLanguage?.htmlText("settings","m_b6658bbe067b5f"," Test large") ?? " Test large") + "</button></div></section>");
      bindSubTabs();
    ...
    function renderPipeline(){
      host.querySelector('.crm-settings').innerHTML = `
        ${String(subTabs())}
        <div class="crm-head"><div><h3>${(globalThis.PlatformLanguage?.htmlText("settings","m_01fbc71cf58163","Sales pipeline") ?? "Sales pipeline")}</h3><p>${(globalThis.PlatformLanguage?.htmlText("settings","m_50c607375fb3cd","The pipeline your team works before a project moves into production.") ?? "The pipeline your team works before a project moves into production.")}</p></div></div>
        <section class="crm-settings-block">
          <h4>${(globalThis.PlatformLanguage?.htmlText("settings","m_3fd795c25a0287","The sales pipeline is now a scope template") ?? "The sales pipeline is now a scope template")}</h4>
          <p>${(globalThis.PlatformLanguage?.htmlText("settings","m_7f55880e2b0579","Stages, to-dos, and automations for the sales pipeline are managed in the ") ?? "Stages, to-dos, and automations for the sales pipeline are managed in the ")}<strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_e57c54f72522ff","Sales Pipeline") ?? "Sales Pipeline")}</strong>${(globalThis.PlatformLanguage?.htmlText("settings","m_7ba4aeda049d30"," scope template — the same editor used for production scope templates. Changes there flow to the sales board, project stage pills, and pipeline automations. New projects are routed into a pipeline template by your intake routing rules.") ?? " scope template — the same editor used for production scope templates. Changes there flow to the sales board, project stage pills, and pipeline automations. New projects are routed into a pipeline template by your intake routing rules.")}</p>
          <div class="crm-head-actions" style="justify-content:flex-start">
            <button class="crm-btn primary" type="button" data-open-pipeline-template><i class="fas fa-layer-group"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_eea7cb32c145a8"," Edit the Sales Pipeline template") ?? " Edit the Sales Pipeline template")}</button>
            <button class="crm-btn" type="button" data-open-scope-templates><i class="fas fa-list"></i>${(globalThis.PlatformLanguage?.htmlText("settings","m_8ea067347f2e26"," Browse all scope templates") ?? " Browse all scope templates")}</button>
          </div>
        </section>`;
      bindSubTabs();

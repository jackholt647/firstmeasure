(function(){
  ...
  function render(){
    ...
    if(state.loadingCall){el.innerHTML=`<header class="fmcp-header"><strong style="flex:1" role="status">${(globalThis.PlatformLanguage?.text("comms","m_8f38504a814a9d","Opening call…") ?? "Opening call…")}</strong><button data-phone="close" aria-label="${(globalThis.PlatformLanguage?.text("comms","m_314202e0941af2","Close call workspace") ?? "Close call workspace")}">`+icon('xmark')+'</button></header>';return;}
    ...
    el.innerHTML=`<header class="fmcp-header"><div class="fmcm-avatar">${String(icon('phone'))}</div><div><strong>${String(esc(title))}</strong><small>${String(esc(number))}${String(number?' · ':'')}<span data-call-status>${String(esc(c?`${label(c.state)}${c.mode==='external'?' · External phone':''}`:'Ready when you are'))}</span></small></div><button data-phone="minimize" aria-label="${((v5) => globalThis.PlatformLanguage?.text("comms","m_21406d49c2fdb5",`${v5} call`,{v5}) ?? `${v5} call`)(state.minimized?'Expand':'Minimize')}">${String(icon(state.minimized?'expand':'minus'))}</button><button data-phone="close" aria-label="${(globalThis.PlatformLanguage?.text("comms","m_314202e0941af2","Close call workspace") ?? "Close call workspace")}">${String(icon('xmark'))}</button></header>
      <div class="fmcp-body">${String(state.error?`<div class="fmcm-error" role="alert">${esc(state.error)}</div>`:'')}
      ${String(c?.metadata?.provider_error?`<p class="fmcm-error">${esc(c.metadata.provider_error.message)} <button data-phone="reconcile">Check provider status</button></p>`:'')}
      ${String((state.detail?.operations||[]).filter(job=>job.state==='failed'&&['wrap_up','recording_ingest','missed_callback'].includes(job.kind)).map(job=>`<p class="fmcm-error">${esc(label(job.kind))} could not finish. Your call record is saved. <button data-phone="retry-work" data-job="${esc(job.id)}">Retry saved work</button></p>`).join(''))}
      ${String(c?`<div class="fmcp-controls">${incoming?`<button class="fmcm-primary" data-phone="answer">${icon('phone')} Answer</button><button data-phone="decline">Decline</button>`:''}
      ${consultation?'<p class="fmcm-help">Consultation in progress. The customer is on hold with your teammate.</p><button data-phone="decline">Leave consultation</button>':active?`<button data-phone="mute" ${!state.sdkCall?'disabled':''}>${icon(state.muted?'microphone-slash':'microphone')} ${state.muted?'Unmute':'Mute'}</button><button data-phone="${c.state==='held'?'resume':'hold'}" ${!connected?'disabled':''}>${icon(c.state==='held'?'play':'pause')} ${c.state==='held'?'Resume':'Hold'}</button><button data-phone="keypad" ${!connected?'disabled':''}>${icon('grip')} Keypad</button><button data-phone="transfer" ${!connected?'disabled':''}>${icon('arrow-right-arrow-left')} Transfer</button><button class="fmcm-danger" data-phone="hangup">${icon('phone-slash')} End call</button>`:''}
      ${c.project_id?`<button data-phone="project">${icon('folder-open')} Project</button><button data-phone="schedule">${icon('calendar-plus')} Schedule</button><button data-phone="message">${icon('comment')} Message</button>`:`<button data-phone="link">${icon('link')} Link project</button>`}
      ${connected&&(state.status?.settings?.recording_enabled||['recording','paused'].includes(capture))?`<button data-phone="record">${icon('circle-dot')} ${capture==='recording'?'Pause recording':capture==='paused'?'Resume recording':'Record with consent'}</button>${['recording','paused'].includes(capture)?'<button data-phone="record_stop">Stop recording</button>':''}`:''}</div>`:
      ...
      <p class="fmcm-help">External phone keeps your lists, scripts, notes, and follow-ups together. It does not record audio or measure call duration.</p><button data-phone="start" class="fmcm-primary">${icon('phone')} ${state.registered?'Start call':'Start call notes'}</button>`)}
      ...
      ${String(c?.mode==='external'?`<p class="fmcm-help"><a href="tel:${esc(number)}">Open ${esc(number)} in your phone app</a></p>`:'')}
      ${String(c?`<label class="fmcm-field">Call notes <textarea class="fmcp-notes" name="notes" placeholder="What happened? Capture decisions, concerns, and next steps.">${esc(state.notes)}</textarea></label><p class="fmcm-help" data-save-state>${state.dirty?'Saving notes…':'Notes saved'}</p>`:'')}
      ...
      <div data-appointments ${values.next_action==='scheduled'?'':'hidden'}>${select('Booked appointment','appointment_id',[['','Choose an appointment'],...(state.appointments||[]).map(a=>[a.id,`${a.title||'Appointment'} · ${a.start||a.id}`])],values.appointment_id||'')}<button data-phone="refresh-context">Refresh appointments</button></div>
      ${(c.metadata?.source_node_ids||[]).length?`<details class="fmcp-script" open><summary>Linked work (${c.metadata.source_node_ids.length})</summary><p>Select only the obligations this call resolves.</p>${c.metadata.source_node_ids.map(id=>`<label class="fmcm-check"><input type="checkbox" data-source="${esc(id)}">${esc(state.detail?.work?.find(node=>node.id===id)?.title||state.entry?.title||'Linked follow-up')}</label>`).join('')}</details>`:''}
      <div data-follow-up ${values.next_action==='follow_up'?'':'hidden'}>${field('Follow-up title','title',values.title||`Follow up with ${c.customer_name}`)}<div class="fmcm-actions" style="margin-bottom:12px">${suggestion?`<button data-phone="policy-date" data-due="${esc(suggestion.due_at)}" data-policy="true">${esc(suggestion.policy_step_label||'Use suggested date')}</button>`:''}${(state.followupOptions?.quick_options||[]).map(q=>`<button data-phone="policy-date" data-due="${esc(q.due_at)}">${esc(q.label)}</button>`).join('')}</div><div class="fmcm-form-grid">${field('Due at (your local time)','due_at',values.due_at||'','datetime-local')}${select('Channel','channel',['call','email','sms'],values.channel||'call')}</div>${state.policyDate?`<p class="fmcm-help">Follow-up: ${esc(state.policyDate)} · ${esc(state.followupOptions?.timezone||'')}</p>`:''}</div>`:'')}
      ...
      ${String(processing?'<p class="fmcm-help" role="status">Saving the outcome and updating linked work…</p>':'')}
      ${String(c?`<div data-artifacts>${mediaMarkup()}</div>`:'')}</div>
      <footer class="fmcp-footer"><span class="fmcm-help">${String(c?`${c.direction==='inbound'?'Incoming':'Outgoing'} call`:state.entry?'Call list workspace':'Communications')}</span><div class="fmcm-actions">${String(!c&&state.entry?'<button data-phone="skip">Skip for now</button>':'')}${String(c&&!active&&!saved&&!processing?'<button class="fmcm-primary" data-phone="wrap">Save outcome</button>':'')}${String(saved&&state.entry?'<button class="fmcm-primary" data-phone="next">Next contact</button>':'')}${String(c&&!active?'<button data-phone="artifacts">Recording & transcript</button>':'')}</div></footer>`;
  ...
  async function transfer(){
    ...
    if(['dialing','consulting'].includes(current.state)){
      const el=dialog('Transfer in progress',`<div data-transfer-controls><p data-transfer-status></p><div class="fmcm-actions"><button type="button" data-cancel-transfer>${(globalThis.PlatformLanguage?.text("comms","m_9c4b861d7bfb10","Return to customer") ?? "Return to customer")}</button><button type="button" class="fmcm-primary" data-complete-transfer>${(globalThis.PlatformLanguage?.text("comms","m_5e11d7ecf0739b","Complete transfer") ?? "Complete transfer")}</button></div><p data-transfer-error role="alert"></p></div>`,null);
  ...
  function keypad(){
    const el=dialog('Keypad',`<p class="fmcm-help">${(globalThis.PlatformLanguage?.text("comms","m_0770dbf1ab92a1","Send digits to the connected call.") ?? "Send digits to the connected call.")}</p><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">${String('123456789*0#'.split('').map(d=>`<button type="button" data-digit="${d}">${d}</button>`).join(''))}</div><p data-keypad-status role="status"></p>`,null);
  ...
  async function record(){
    ...
    dialog('Ask for recording permission',`<p>${String(esc(state.status?.settings?.disclosure))}</p><p class="fmcm-help">${(globalThis.PlatformLanguage?.text("comms","m_1f3afb17c82258","Read this to the caller. Recording starts only after you confirm that they agreed.") ?? "Read this to the caller. Recording starts only after you confirm that they agreed.")}</p>`,async()=>{await action('consent',{consent:'granted'});await action('record_start');render();},{submit:'Caller agreed — start recording'});
  }

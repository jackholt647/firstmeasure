...
(function(){
  ...
  function render(def){
    const c = def.content;
    return `<article class="fm-tab-promo" aria-label="${((v0) => globalThis.PlatformLanguage?.text("tab-promos","m_a9cbcece18bdac",`${v0} coming soon`,{v0}) ?? `${v0} coming soon`)(escape(def.title))}"><div class="tp-wrap">
      <div class="tp-brand"><img src="/images/logo_square.png" alt="">${(globalThis.PlatformLanguage?.text("tab-promos","m_5c4fb8df281140","FirstMate") ?? "FirstMate")}<span>${(globalThis.PlatformLanguage?.text("tab-promos","m_9011dd5d1fb41c","Coming soon") ?? "Coming soon")}</span></div>
      <section class="tp-hero"><div><p class="tp-eyebrow">${String(escape(c.eyebrow))}</p><h1>${String(escape(c.headline))}<br><em>${String(escape(c.accent))}</em></h1><p>${String(escape(c.intro))}</p><span class="tp-pill">${String(escape(c.heroPill || "Built around the way you work"))}</span></div><figure><a href="${String(escape(c.images[0].src))}" target="_blank" rel="noopener noreferrer" aria-label="${(globalThis.PlatformLanguage?.text("tab-promos","m_4a4816db7c642d","Open screenshot at full size") ?? "Open screenshot at full size")}"><img src="${String(escape(c.images[0].src))}" alt="${String(escape(c.images[0].alt))}" loading="lazy" width="1280" height="720"></a><figcaption>${String(escape(c.images[0].caption))}</figcaption></figure></section>
      <section class="tp-rule"><figure><a href="${String(escape(c.images[1].src))}" target="_blank" rel="noopener noreferrer" aria-label="${(globalThis.PlatformLanguage?.text("tab-promos","m_4a4816db7c642d","Open screenshot at full size") ?? "Open screenshot at full size")}"><img src="${String(escape(c.images[1].src))}" alt="${String(escape(c.images[1].alt))}" loading="lazy" width="1280" height="720"></a><figcaption>${String(escape(c.images[1].caption))}</figcaption></figure><div><p class="tp-eyebrow">${String(escape(c.rulesEyebrow || "Make it your own"))}</p><h2>${String(escape(c.rulesTitle))}</h2><p>${String(escape(c.rulesBody))}</p><span class="tp-pill">${String(escape(c.rulesPill || "Built for your business"))}</span></div></section>
      ...
      <footer class="tp-end"><h2>${String(escape(c.footerTitle || "More possibilities. Coming soon."))}</h2><p>${(globalThis.PlatformLanguage?.text("tab-promos","m_5469b46d69ad5b","This is a preview of what’s coming to FirstMate. Availability and final features may change. ") ?? "This is a preview of what’s coming to FirstMate. Availability and final features may change. ")}<a href="https://1m8.ai/" target="_blank" rel="noopener noreferrer">${(globalThis.PlatformLanguage?.text("tab-promos","m_ba4635a7ef2d18","Explore FirstMate ↗") ?? "Explore FirstMate ↗")}</a></p></footer>
    </div></article>`;
  }

...
(function(){
  ...
  function injectEditorCss(){
    ...
    style.textContent = `
      ...
      .fm-cf-input{width:100%;min-width:0;box-sizing:border-box;border:1px solid rgba(15,23,42,.13);border-radius:9px;background:#fff;min-height:38px;padding:8px 10px;font:850 12px/1.35 inherit;color:#101828;outline:none}.fm-cf-input:focus{border-color:rgba(var(--primary-rgb,217,48,37),.5);box-shadow:0 0 0 3px rgba(var(--primary-rgb,217,48,37),.09)}textarea.fm-cf-input{min-height:70px;resize:vertical}.fm-cf-check{display:flex;align-items:center;gap:8px;min-height:38px;font-size:12px;font-weight:850;color:#344054}.fm-cf-formula{min-height:38px;border:1px solid #e4e7ec;border-radius:9px;background:#f8fafc;padding:9px 10px;font-size:12px;font-weight:950;color:#344054}
  ...
  function inputHtml(definition, value, options = {}){
    ...
    if (def.type === 'select') return `<span class="fm-cf-select-wrap"><select ${String(attrs)}><option value="">${(globalThis.PlatformLanguage?.text("custom-fields","m_fe069e3d7b9c51","Select…") ?? "Select…")}</option>${String(def.options.map((option) => `<option value="${escapeHtml(option.value)}"${cleanText(value) === option.value ? ' selected' : ''}>${escapeHtml(option.label)}</option>`).join(''))}</select><i class="fas fa-chevron-down fm-cf-select-chevron" aria-hidden="true"></i></span>`;
    ...
    if (def.type === 'key_value') {
      ...
      const rows = (entries.length ? entries : [['', '']]).map(([key, item]) => `<div class="fm-cf-kv-row"><input class="${String(escapeHtml(inputClass))}" data-fm-cf-kv-key value="${String(escapeHtml(key))}" placeholder="${(globalThis.PlatformLanguage?.text("custom-fields","m_8cf345002184e5","Name") ?? "Name")}"><input class="${String(escapeHtml(inputClass))}" data-fm-cf-kv-value value="${String(escapeHtml(typeof item === 'object' ? JSON.stringify(item) : item))}" placeholder="${(globalThis.PlatformLanguage?.text("custom-fields","m_ec6b76d100b0ec","Value") ?? "Value")}"><button type="button" class="fm-cf-kv-remove" data-fm-cf-kv-remove aria-label="${(globalThis.PlatformLanguage?.text("custom-fields","m_a916bae1ac393b","Remove row") ?? "Remove row")}"><i class="fas fa-xmark"></i></button></div>`).join('');
      return `<div class="fm-cf-kv" data-fm-cf-input="${String(escapeHtml(def.key))}" data-fm-cf-type="key_value">${String(rows)}<button type="button" class="fm-cf-kv-add" data-fm-cf-kv-add><i class="fas fa-plus"></i>${(globalThis.PlatformLanguage?.text("custom-fields","m_8e13db264d4c9f"," Add row") ?? " Add row")}</button></div>`;
    }
    if (def.type === 'json') {
  ...
  async function renderEditor(container, entity = {}, entityType = 'project', options = {}){
    ...
    if (!caches.has(cacheKey(options.orgId, options.branchId))) {
      container.innerHTML = options.flat === true ? '' : `<div class="fm-cf-panel"><div class="fm-cf-empty">${(globalThis.PlatformLanguage?.text("custom-fields","m_b7b7212c084051","Loading custom fields…") ?? "Loading custom fields…")}</div></div>`;
    ...
    if (!definitions.length) {
      container.innerHTML = options.hideWhenEmpty === false ? `<div class="fm-cf-panel"><div class="fm-cf-empty">${(globalThis.PlatformLanguage?.text("custom-fields","m_a1bfa740917210","No custom fields apply here.") ?? "No custom fields apply here.")}</div></div>` : '';
    ...
    const fieldHtml = (definition) => {
      ...
      const help = description ? `<span class="fm-cf-help-tip" tabindex="0" role="img" aria-label="${((v0) => globalThis.PlatformLanguage?.text("custom-fields","m_05919cc51cd244",`About ${v0}`,{v0}) ?? `About ${v0}`)(escapeHtml(definition.label))}" data-fm-tooltip="${String(escapeHtml(description))}"><i class="fas fa-circle-info" aria-hidden="true"></i></span>` : '';
    ...
    const groupHtml = groups.sort((a, b) => Number(a.order || 0) - Number(b.order || 0)).map((group) => {
      ...
      const sourceTip = group.sources?.length ? `<span class="fm-cf-help-tip fm-cf-source-tip" tabindex="0" role="img" aria-label="${(globalThis.PlatformLanguage?.text("custom-fields","m_c580f7786ef337","Scope-provided fields") ?? "Scope-provided fields")}" data-fm-tooltip="These fields are provided by the active project scope."><i class="fas fa-circle-info" aria-hidden="true"></i></span>` : '';
    ...
    container.querySelectorAll('[data-fm-cf-kv-add]').forEach((button) => button.addEventListener('click', () => {
      button.insertAdjacentHTML('beforebegin', `<div class="fm-cf-kv-row"><input class="${String(escapeHtml(cleanText(options.inputClass) || 'fm-cf-input'))}" data-fm-cf-kv-key placeholder="${(globalThis.PlatformLanguage?.text("custom-fields","m_8cf345002184e5","Name") ?? "Name")}"><input class="${String(escapeHtml(cleanText(options.inputClass) || 'fm-cf-input'))}" data-fm-cf-kv-value placeholder="${(globalThis.PlatformLanguage?.text("custom-fields","m_ec6b76d100b0ec","Value") ?? "Value")}"><button type="button" class="fm-cf-kv-remove" data-fm-cf-kv-remove aria-label="${(globalThis.PlatformLanguage?.text("custom-fields","m_a916bae1ac393b","Remove row") ?? "Remove row")}"><i class="fas fa-xmark"></i></button></div>`);
  ...
  async function mountSettings(container, options = {}){
    if (!container) return;
    injectSettingsCss();
    container.innerHTML = `<div class="cf-settings"><div class="cf-empty">${(globalThis.PlatformLanguage?.text("custom-fields","m_b7b7212c084051","Loading custom fields…") ?? "Loading custom fields…")}</div></div>`;
    let settings;
    ...
    const render = () => {
      ...
      container.innerHTML = `<div class="cf-settings">
        ...
        <div>${String(selected ? `<form class="cf-editor" data-cf-editor>
          ...
          <div class="cf-editor-body">
            ...
            <section class="cf-section"><div class="cf-section-title"><i class="fas fa-shapes"></i> How should people answer?</div><div class="cf-type-grid">${TYPE_CATALOG.map((type) => `<label class="cf-type-choice"><input type="radio" name="type" value="${escapeHtml(type.value)}"${selected.type === type.value ? ' checked' : ''}><span><i class="fas ${escapeHtml(type.icon)}"></i><strong>${escapeHtml(type.label)}</strong><small>${escapeHtml(type.hint)}</small></span></label>`).join('')}</div></section>
          ...
          <div class="cf-actions"><button type="submit" class="cf-btn primary"><i class="fas fa-check"></i> Save changes</button><button type="button" class="cf-btn danger" data-cf-delete><i class="fas fa-trash"></i> Delete field</button><span class="cf-help" data-cf-status></span></div>
      ...
      const addOptionRow = (option = {}) => {
        ...
        optionList.insertAdjacentHTML('beforeend', `<div class="cf-choice-row" data-cf-option-row><span class="cf-choice-grip"><i class="fas fa-grip-vertical"></i></span><input class="cf-in" data-cf-option-label value="${String(escapeHtml(label))}" placeholder="${(globalThis.PlatformLanguage?.text("custom-fields","m_13ecdbd26d94c0","Choice name") ?? "Choice name")}"><input class="cf-in" data-cf-option-value value="${String(escapeHtml(value && value !== label ? value : ''))}" placeholder="${String(escapeHtml(slug(label, 'saved_value')))}"><button type="button" class="cf-choice-remove" data-cf-option-remove aria-label="${(globalThis.PlatformLanguage?.text("custom-fields","m_ed53c026108fd8","Remove choice") ?? "Remove choice")}"><i class="fas fa-trash"></i></button></div>`);
      ...
      optionList?.addEventListener('click', (event) => {
        ...
        if (!optionRows().length) optionList.innerHTML = `<div class="cf-choice-empty" data-cf-choice-empty>${(globalThis.PlatformLanguage?.text("custom-fields","m_cfbf5fb58f8468","Add the first choice below.") ?? "Add the first choice below.")}</div>`;
        syncConditionalRows();
      });
      ...
      form?.addEventListener('submit', async (event) => {
        ...
        try { fields = (await saveDefinitions(fields, options)).fields; render(); root.Portal?.ui?.showToast?.((globalThis.PlatformLanguage?.text("custom-fields","m_4bb4688766e904","Saved") ?? "Saved"), (globalThis.PlatformLanguage?.text("custom-fields","m_d66c043b6a8789","Custom field updated.") ?? "Custom field updated."), true); }

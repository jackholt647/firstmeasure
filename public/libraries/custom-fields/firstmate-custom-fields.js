...
(function(){
  ...
  async function mountOrganizationValues(container, options) {
    ...
    injectSettingsCss();
    injectEditorCss();
    container.innerHTML = '<div data-cf-org-editor role="status">Loading value…</div>';
    ...
    try {
      ...
      if (!definitions.length) { pane.innerHTML = '<p class="cf-help">Save this field to enter its value.</p>'; return; }
      ...
      const save = pane.querySelector('[data-fm-cf-save]'); if (save) { save.textContent = 'Save value'; save.className = 'cf-btn primary'; if (!contract.value.fields.some(f => f.writable && (!options.fieldPath || f.path === options.fieldPath))) save.hidden = true; }
    } catch(error) { pane.innerHTML = `<p class="cf-help" role="alert">${escapeHtml(error.message)}</p><button type="button" class="cf-btn" data-cf-retry>Try again</button>`; pane.querySelector('[data-cf-retry]').onclick = () => mountOrganizationValues(container,options); }
  }
...
`;
  ...
  async function mountSettings(container, options = {}){
    ...
    injectSettingsCss();
    injectEditorCss();
    container.innerHTML = `<div class="cf-settings" data-settings-autosave="off"><div class="cf-empty">${(globalThis.PlatformLanguage?.text("custom-fields","m_b7b7212c084051","Loading custom fields…") ?? "Loading custom fields…")}</div></div>`;
    let settings;
    ...
    const render = () => {
      ...
      container.innerHTML = `<div class="cf-settings" data-settings-autosave="off">
        ...
        }).join('') : '<div class="cf-empty-state"><i class="fas fa-wand-magic-sparkles"></i><strong>No fields yet</strong><span>Use Add field to create your first field.</span></div>')}</div></aside>
        <div class="cf-detail">${selected?.entity === 'organization' ? `<section class="cf-value-card"><div class="cf-value-title">Current value</div><div data-cf-value-host></div></section>` : ''}${String(selected ? `${selected.entity === 'organization' ? `<details class="cf-definition"${drafts.has(selectedId) ? ' open' : ''}><summary>Field settings <span>Name, type, and validation</span></summary>` : ''}<form class="cf-editor" data-cf-editor>
          ...
          <div class="cf-editor-body">
            ...
            <section class="cf-section"><div class="cf-section-title"><i class="fas fa-shapes"></i> How should people answer?</div><select class="cf-in" name="type" aria-label="Field type">${TYPE_CATALOG.map(type => `<option value="${escapeHtml(type.value)}"${selected.type === type.value ? ' selected' : ''}>${escapeHtml(type.label)}</option>`).join('')}</select></section>
      ...
      if (valueHost) {
        if (drafts.has(selectedId)) valueHost.innerHTML = '<p class="cf-help">Save the field below, then enter its value here.</p>';

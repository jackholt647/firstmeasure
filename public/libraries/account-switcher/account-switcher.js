...
(function () {
  ...
  class AccountSwitcher {
    ...
    renderError(error) {
      if (!this.menu) return;
      this.menu.innerHTML = `<div class="fm-account-switcher-head"><strong>${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_3a204df60cc9c8","Accounts") ?? "Accounts")}</strong><button type="button" data-fm-account-close aria-label="${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button></div><p class="fm-account-switcher-error">${String(escapeHtml(error.message || 'Unable to load accounts.'))}</p>`;
    ...
    renderList() {
      ...
      const accountRows = this.accounts.map((account) => {
        ...
        return `<div class="fm-account-row${String(active ? ' is-active' : '')}">
          <button type="button" class="fm-account-select" data-fm-account-select="${String(escapeHtml(account.account_id))}" ${String(active ? 'aria-current="true"' : '')}>
            ...
            <span class="fm-account-copy"><strong>${String(escapeHtml(displayName))}</strong><small>${String(escapeHtml(account.email))}</small>${String(needsReauth ? `<small class="fm-account-reauth">${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_4e344333f91142","Sign in again") ?? "Sign in again")}</small>` : (account.organization_name ? `<small>${escapeHtml(account.organization_name)}</small>` : ''))}</span>
            ${String(active ? `<i class="fas fa-check fm-account-current" aria-label="${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_f746e9f1038aa3","Current account") ?? "Current account")}"></i>` : (needsReauth ? `<i class="fas fa-right-to-bracket fm-account-current" aria-label="${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_4e344333f91142","Sign in again") ?? "Sign in again")}"></i>` : ''))}
          </button>
          <button type="button" class="fm-account-more" data-fm-account-more="${String(escapeHtml(accountKey))}" aria-label="${((v9) => globalThis.PlatformLanguage?.htmlText("account-switcher","m_5bdb5ea8fb0e0d",`More options for ${v9}`,{v9}) ?? `More options for ${v9}`)(escapeHtml(account.email))}" aria-haspopup="menu" aria-expanded="${String(actionsOpen ? 'true' : 'false')}"><i class="fas fa-ellipsis-vertical"></i></button>
        </div>`;
      }).join('');
      const actionMenu = actionAccount ? `<div class="fm-account-row-actions" role="menu" aria-label="${((v0) => globalThis.PlatformLanguage?.htmlText("account-switcher","m_6d7389c830a774",`Actions for ${v0}`,{v0}) ?? `Actions for ${v0}`)(escapeHtml(actionAccount.email))}">
      ...
      this.menu.innerHTML = `<div class="fm-account-switcher-head"><strong>${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_b5e633539db02a","Choose an account") ?? "Choose an account")}</strong><button type="button" data-fm-account-close aria-label="${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button></div>
        <div class="fm-account-list">${String(accountRows || `<p class="fm-account-empty">${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_dcf9d5e2347ad8","No saved accounts yet.") ?? "No saved accounts yet.")}</p>`)}</div>
        <div class="fm-account-switcher-actions">
          <button type="button" data-fm-account-add><i class="fas fa-user-plus"></i><span>${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_95d01441b9f49d","Add another account") ?? "Add another account")}</span></button>
          <button type="button" data-fm-account-logout-all><i class="fas fa-right-from-bracket"></i><span>${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_41632633478452","Sign out of all accounts") ?? "Sign out of all accounts")}</span></button>
    ...
    openLogin(email = '') {
      ...
      modal.innerHTML = `<div class="fm-account-login-backdrop" data-fm-account-login-close></div><div class="fm-account-login-card"><header><div><strong>${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_db9ad85fae0711","Sign in to another account") ?? "Sign in to another account")}</strong><span>${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_8652b533cc3e38","Your account stays available on this device.") ?? "Your account stays available on this device.")}</span></div><button type="button" data-fm-account-login-close aria-label="${(globalThis.PlatformLanguage?.htmlText("account-switcher","m_3742924668fb10","Close") ?? "Close")}"><i class="fas fa-xmark"></i></button></header><div class="fm-account-login-widget" data-firstmate-signup data-mode="login" data-show-login="true" data-show-terms="false" data-referral="none" data-redirect="/portal/"></div></div>`;

...
(function(){
  ...
  async function open(key, context = {}){
    ...
    const wizard = window.FirstMateSetupWizard.open({
      ...
      railNote:`${definition.mode === 'required' ? `<strong>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_abb8ef16fd28ae","Required setup") ?? "Required setup")}</strong><br>` : `<strong>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_d172e68b960224","Optional setup") ?? "Optional setup")}</strong><br>`}${esc(definition.summary)}`,
  ...
  function renderFeedbackDeliveryStep(container, ctx, definition){
    ...
    container.innerHTML = `<div class="fm-app-setup-head"><h2>${String(esc(definition.title))}</h2><p>${String(esc(definition.description))}</p></div><div class="fm-feedback-setup">
      <section class="fm-feedback-card"><div class="fm-feedback-card-head"><strong><i class="fas fa-clock"></i>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_4dcdaed478d0bf"," When should this be sent?") ?? " When should this be sent?")}</strong><span data-feedback-setup-insight></span></div><p>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_11cfdabed0d427","These are provisional global events. Each project still receives one idempotent request.") ?? "These are provisional global events. Each project still receives one idempotent request.")}</p><div class="fm-feedback-grid">${String(choices(timings,'data-feedback-setup-trigger',[selected]))}</div><div class="fm-feedback-scope-note"><i class="fas fa-diagram-project"></i><span><strong>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_479768d58a038e","Scope-aware timing is still being connected.") ?? "Scope-aware timing is still being connected.")}</strong>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_2cd95b8b3ef3a0"," Workflows and scope sets can already call the feedback action with complete delivery parameters; those triggers belong in the workflow and do not need a button here.") ?? " Workflows and scope sets can already call the feedback action with complete delivery parameters; those triggers belong in the workflow and do not need a button here.")}</span></div></section>
      <section class="fm-feedback-card"><div class="fm-feedback-card-head"><strong><i class="fas fa-share-nodes"></i>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_9a5412305bd598"," How do you want to send the message?") ?? " How do you want to send the message?")}</strong></div><p>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_fbe82375e44216","Choose any combination. Every channel can still be previewed on the next step.") ?? "Choose any combination. Every channel can still be previewed on the next step.")}</p><div class="fm-feedback-grid">${String(choices(channelDefs,'data-feedback-setup-channel',channels))}</div></section>
    </div>`;
  ...
  function renderFeedbackMessagesStep(container, ctx, definition){
    ...
    const editor = active === 'email'
      ? `<div class="fm-feedback-input"><label>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_eb218e4bd1f2dd","Email subject") ?? "Email subject")}</label><input data-feedback-message="email_subject" value="${String(esc(values.email_subject))}"></div><div class="fm-feedback-input"><label>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_63ea7578d93f41","Email body") ?? "Email body")}</label><textarea rows="8" data-feedback-message="email_body">${String(esc(values.email_body))}</textarea></div>`
      : active === 'portal'
        ? `<div class="fm-feedback-input"><label>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_f2b1764fc05842","Headline") ?? "Headline")}</label><input data-feedback-message="portal_title" value="${String(esc(values.portal_title))}"></div><div class="fm-feedback-input"><label>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_a16cfd85cfd122","Message") ?? "Message")}</label><textarea rows="4" data-feedback-message="portal_body">${String(esc(values.portal_body))}</textarea></div><div class="fm-feedback-input"><label>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_607eea667fd938","Button") ?? "Button")}</label><input data-feedback-message="portal_cta" value="${String(esc(values.portal_cta))}"></div>`
        : `<div class="fm-feedback-input"><label>${(globalThis.PlatformLanguage?.text("app-setup-workflows","m_def279b4381c15","Text message") ?? "Text message")}</label><textarea rows="8" data-feedback-message="sms_text">${String(esc(values.sms_text))}</textarea></div>`;

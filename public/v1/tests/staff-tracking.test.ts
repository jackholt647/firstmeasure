...
test('tracking authorization, universal collection, bridge integrity, reviews and pagination', async () => {
  ...
  try {
    ...
    for(const name of ['manager','qa','trainee','customer']) for(const route of ['access','events','access-list','people','networks?email=admin@tracking.example.test']) assert.equal((await app.inject({url:endpoint+route,headers:users[name]})).statusCode,403,name+route);
    ...
    assert.equal((await app.inject({url:endpoint+'people',headers:users.manager})).statusCode,200);
    assert.equal((await app.inject({url:endpoint+'networks?email=trainee@tracking.example.test',headers:users.manager})).statusCode,200);
    ...
    assert.equal((await app.inject({url:endpoint+'people',headers:users.manager})).statusCode,403);
    assert.equal((await app.inject({url:endpoint+'networks?email=trainee@tracking.example.test',headers:users.manager})).statusCode,403);
    ...
    await saveInternalUser({email:'quiet@tracking.example.test',name:'Quiet Person',role:'user',status:'active'});
    const directory=await app.inject({url:endpoint+'people',headers:users.admin});
    assert.equal(directory.statusCode,200);assert.equal(directory.headers['cache-control'],'no-store');
    assert.equal(directory.json().people.length,25);assert.ok(directory.json().total>=405);
    const directory2=(await app.inject({url:endpoint+'people?offset=25',headers:users.admin})).json();
    assert.ok(directory2.people.every((p:any)=>!directory.json().people.some((u:any)=>u.email===p.email)));
    const quiet=(await app.inject({url:endpoint+'people?search=Quiet%20Person',headers:users.admin})).json();
    assert.equal(quiet.total,1);assert.equal(quiet.people[0].observations,0);assert.equal(quiet.people[0].last_seen,null);
    const searched=(await app.inject({url:endpoint+'people?search=trainee',headers:users.admin})).json();
    assert.equal(searched.total,1);assert.ok(searched.people[0].ip_count>=2);assert.ok(searched.people[0].training_events>=3);
    assert.equal((await app.inject({url:endpoint+'people?ip=bad-ip',headers:users.admin})).statusCode,400);
    const byIp=(await app.inject({url:endpoint+'people?ip=::ffff:8.8.8.8',headers:users.admin})).json();
    assert.ok(byIp.total>=400);assert.ok(byIp.people.every((p:any)=>p.ip_count===1));
    for(let i=0;i<30;i++)await insertEvent({...event,id:reference('networks'+i),email:'networks@example.test',ip:`9.1.0.${i+1}`});
    await insertEvent({...event,id:reference('unknown-network'),email:'networks@example.test',ip:null});
    await insertEvent({...event,id:reference('expired-network'),email:'networks@example.test',ip:'7.7.7.7',at:new Date(Date.now()-91*86400000).toISOString()});
    const networks=(await app.inject({url:endpoint+'networks?email=networks@example.test',headers:users.admin})).json();
    const networks2=(await app.inject({url:endpoint+'networks?email=networks@example.test&offset=25',headers:users.admin})).json();
    assert.equal(networks.networks.length,25);assert.equal(networks2.networks.length,6);assert.equal(networks2.next_offset,null);
    assert.equal(new Set([...networks.networks,...networks2.networks].map((n:any)=>n.ip)).size,31);
    assert.ok([...networks.networks,...networks2.networks].some((n:any)=>n.ip===null));
    assert.ok(![...networks.networks,...networks2.networks].some((n:any)=>n.ip==='7.7.7.7'));
    ...
    if(process.env.TRACKING_BROWSER_EXECUTABLE){
      ...
      try {
        ...
        await browserPage.locator('[data-person-open]').first().waitFor();
        assert.equal(await browserPage.locator('[data-person-open]').count(),25);
        assert.equal(await browserPage.locator('.tracking-event').count(),0,'homepage is people, not events');
        await mkdir('../../outputs/staff-tracking-184',{recursive:true});
        await browserPage.screenshot({path:'../../outputs/staff-tracking-184/tracking-people-desktop.png'});
        await browserPage.locator('[name=email]').fill('trainee');
        await browserPage.getByRole('button',{name:'Refresh',exact:true}).click();
        await browserPage.locator('[data-person-open="trainee@tracking.example.test"]').click();
        await browserPage.locator('.tracking-network').first().waitFor();
        await browserPage.locator('.tracking-event').first().waitFor();
        assert.equal(await browserPage.locator('[data-people-pager]').isVisible(),false);
        assert.equal(await browserPage.locator('[data-next]').isVisible(),true);
        ...
        await browserPage.getByRole('button',{name:'← All people',exact:true}).click();
        await browserPage.locator('[name=email]').fill('');
        await browserPage.getByRole('button',{name:'Refresh',exact:true}).click();
        await browserPage.locator('[data-person-open]').first().waitFor();
        await browserPage.setViewportSize({width:390,height:844});
        assert.ok(await browserPage.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),'people mobile has no horizontal overflow');
        await browserPage.screenshot({path:'../../outputs/staff-tracking-184/tracking-people-mobile.png'});
        await browserPage.setViewportSize({width:1250,height:950});

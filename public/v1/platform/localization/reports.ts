...
export async function freezeReportLanguage(input: Record<string, unknown>, preferences: ReportPreferences) {
  ...
  const mappings = orgId ? await readBranchModule(orgId, branchId, "variable_mappings").catch(() => null) : null;
  const snapshot = await snapshotLanguage(resolveContext(preferences), ["shared", "terminology", "reports"], mappings?.data || {});

...
export const SUPPORTED_LOCALES = LANGUAGE_PACKS.map(pack => pack.code) as [string, ...string[]];
export const TRANSLATION_LANGUAGES = registry.translation_targets;
export const TRANSLATION_CODES = TRANSLATION_LANGUAGES.map(language => language.code) as [string, ...string[]];

export function normalizeTranslationLanguage(value: unknown, fallback = "en-US"): string {
  const code = String(value ?? "").trim().replaceAll("_", "-").toLowerCase();
  if (code === "en") return "en-US"; // Preserve legacy explicit English choices.
...
export function sameMessageLanguage(source: string, target: string) {
...
export function resolveTranslationPreference(preferences: Record<string, unknown>, companyLocale: string) {

...
import { registerDatasetProvider } from "./datasets.js";
...
function registerFirstMeasure(){
 const access:AccessPolicy={scopes:["organization"],permissions:["view_reports"],systemKinds:["work","module"],authorize:async(ctx,t)=>{const m=await (await import("../../firstmeasure/storage.js")).readManifest(t.id||"");if(String(object(m.organization_ref).id)!==ctx.organizationId)throw forbidden("report_owner_denied","Report belongs to another organization.");}};
 registerDataProvider({id:"firstmeasure",version:"1",apps:["firstmeasure/order","measurements"],exports:{status:{description:"Report order and completion state.",schema:shape(["id","status","project_type","address","report_mode"]),schemaVersion:"1",access,read:async(_ctx,ref)=>{const m=await (await import("../../firstmeasure/storage.js")).readManifest(id(ref));return {value:pick(m,["id","status","project_type","address","report_mode"])};}},measurements:{description:"Completed Roofplan measurements with explicit units and report artifact references.",schema:{type:"object",required:["measurements","artifacts"],properties:{measurements:{type:"object"},artifacts:{type:"array"}},additionalProperties:false},schemaVersion:"1",access,read:async(_ctx,ref)=>{const s=await import("../../firstmeasure/storage.js");const m=await s.readManifest(id(ref));if(m.status!=="completed")return {status:"pending",code:"report_pending",message:"The measurement report is not complete."};const xml=await s.readStoredXml(m.id);const text=xml.content.toString("utf8");const {measurementPayloadFromRoofplan}=await import("./firstmeasure-datasets.js");return {value:measurementPayloadFromRoofplan(text,m.id),revision:`sha256:${contentHash(text)}`,provenance:{reportId:m.id,sourceFormat:"roofplan",unitsContract:"roofplan-feet"}};}}}});
}

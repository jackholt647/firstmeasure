...
export async function storeMediaUpload(orgId: string, input: MediaUploadOptions) {
  ...
  const pendingWrites: {key:string;bytes:Buffer;contentType:string}[]=[{key:objectKey(organizationId,mediaId,originalVariant.path),bytes:input.bytes,contentType}];
  ...
  for(const file of pendingWrites)await putSharedObject(file.key,file.bytes,file.contentType);

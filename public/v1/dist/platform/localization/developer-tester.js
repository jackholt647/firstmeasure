import { createLanguage } from './core.js';
...
export function createDeveloperTester(options) {
    const storageKey = 'fm:language-tester:v1';
    let session = null;
    try {
        const saved = JSON.parse(sessionStorage.getItem(storageKey) || 'null');
        if (saved?.active === true)
            session = { active: true, locale: LANGUAGE_PACKS.some(p => p.code === saved.locale) ? saved.locale : null };
    }
    catch { /* Unavailable storage leaves normal pages untouched. */ }
    ...
    let nextId = 0, observer, host;
    let select, status, bootstrap;
    ...
    const strip = (value) => value.replace(marker, '');
    function persist() { if (session)
    ...
    function valueOf(binding) { return binding.attribute ? binding.node.getAttribute(binding.attribute) : binding.node.nodeValue; }
    function write(binding, value) {
        if (binding.attribute)
            binding.node.setAttribute(binding.attribute, value);
        else
            binding.node.nodeValue = value;
        binding.last = value;
    }
    function render(parts) { return parts.map(part => typeof part === 'string' ? part : part.html ? decode(translated(part.id)) : translated(part.id)).join(''); }
    function info(message) {
        if (status)
            status.textContent = message || (capturedAtBoot ? `${bindings.size} live labels${skipped ? ` · ${skipped} complex/skipped` : ''}` : 'Reload once to enable live capture.');
        if (select) {
            select.value = session?.locale || '';
            select.disabled = !capturedAtBoot || busy;
        }
    }
    function capture(node, attribute) {
        const value = attribute ? node.getAttribute(attribute) : node.nodeValue;
        if (!value || !value.includes(`FM${nonce}:`))
            return;
        ...
        const binding = { node, attribute, parts, last: strip(value) };
        ...
        if (canBind && parts.some(part => typeof part !== 'string')) {
            slots.set(attribute || '', binding);
            byNode.set(node, slots);
            ...
            if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement)
                element.value = strip(element.value);
        }
    }
    function scan(node) {
        if (node === host)
    ...
    function repaint() {
        ...
        for (const binding of bindings) {
            if (!binding.node.isConnected || valueOf(binding) !== binding.last) {
    ...
    function processMutation(mutation) {
        if (mutation.type === 'childList')
            ...
            capture(mutation.target, mutation.attributeName);
        else
    ...
    function boot() {
        if (booted)
            return;
        booted = true;
        observer = new MutationObserver(mutations => { for (const mutation of mutations)
            processMutation(mutation); });
        ...
        for (const prototype of [HTMLInputElement.prototype, HTMLTextAreaElement.prototype, HTMLOptionElement.prototype]) {
            ...
            if (descriptor?.set && descriptor.get)
                Object.defineProperty(prototype, 'value', { ...descriptor, set(value) { descriptor.set.call(this, typeof value === 'string' ? strip(value) : value); } });
        }
        scan(document.documentElement);
    }
    async function setLocale(locale) {
        if (locale !== null && !LANGUAGE_PACKS.some(pack => pack.code === locale))
            throw Error(`Language pack is not installed: ${locale}`);
        if (!capturedAtBoot)
            throw Error('Use the tester’s Reload & start button once to enable live capture.');
    ...
    function panel() {
        if (host?.isConnected || window.top !== window)
            return;
        host = document.createElement('div');
        host.id = 'fm-language-tester';
        ...
        bootstrap = shadow.querySelector('#start');
        select.add(new Option('Account language', ''));
        for (const pack of LANGUAGE_PACKS)
            select.add(new Option(pack.label, pack.code));
        select.onchange = () => { void setLocale(select.value || null).catch(error => info(error.message)); };
        bootstrap.hidden = capturedAtBoot;
        bootstrap.onclick = () => {
            try {
                session = { active: true, locale: null };
                persist();
                location.reload();
            }
            catch {
                info('Tab storage is unavailable; live capture could not start.');
            }
        };
    ...
    function close() {
        ...
        catch { /* Restore this page even when storage is denied. */ }
        repaint();
        host?.remove();
        host = undefined;
        select = undefined;
    ...
    return {
        api, repaint,
        register(bundle) { preview.register(bundle); },
        text(namespace, key, fallback, values, ordinary) {
            if (!session || !capturedAtBoot)
                return ordinary();
            syncContext();
            const id = ++nextId;

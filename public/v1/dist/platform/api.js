...
import { signupCommercialProfile, profileFromGlobal, organizationProfile, currentProfile, customerCommercialView, creditLabel, creditMinorAmount } from "../commerce/profile.js";
...
import { firstMeasureReportAmount as sharedFirstMeasureReportAmount, firstMeasureReportCharge as sharedFirstMeasureReportCharge } from "../firstmeasure/pricing.js";
...
import { appointmentAvailability, cancelAppointmentRescheduleRequest, commitAppointmentReschedule, holdAppointmentSlot, publicSchedulingPolicy } from "../appointments/availability.js";
...
import { applyCapabilityPreset, capabilityState, createCapabilityPreset, deleteCapabilityPreset, isCapabilityEnabled, platformRolloutConfigurationKey, readPlatformRollout, rawCapabilityValues, saveCapabilityValues, setSignupCapabilityPreset, updateCapabilityPreset, validateValueSet } from "./capabilities.js";
import { authContextFromRequest, buildAuthContext, hashPassword, listRememberedPlatformAccounts, loginPlatformIdentity, logoutAllRememberedPlatformAccounts, logoutPlatformSession, platformAuthCookieNames, publicAuthContext, rememberPlatformAccount, removeRememberedPlatformAccount, requirePlatformAuth, setPlatformAuthCookies, switchRememberedPlatformAccount } from "./auth.js";
...
import { customerPortalDocumentId as portalDocumentIdFor, normalizePortalSettings, publicPortalSettings } from "./portal_settings.js";
...
import { projectTabRelevance, tabIsRelevant } from "./portal_tab_relevance.js";
...
import { assertCanAcceptPunchList, assertCanCompletePunchWork, assertCanSubmitPunchList, assertCustomerMayAddPunchItem, assertCustomerMayEditPunchItem, isPunchList, normalizePunchSignature, publicPunchView, punchConfigOf, punchTransition, resolvePunchLabels } from "../workforce/punch_lists.js";
import { assertCommentableMedia, assertCommentBody, assertUploadQuota, consumePortalWriteToken, normalizePortalComments, normalizePortalUploads, portalSettingsFor, validatePortalUpload, visiblePortalComments, visiblePortalUploads } from "./portal_writes.js";
import { registerContactImportRoutes } from "./contact_import/api.js";
import { registerAttentionRoutes } from "./attention.js";
import { addIdentityMembership, createOrganization, createIdentity, deleteAuthSession, deleteDocument, findIdentityByEmail, listBranchModules, listIdentityMemberships, listDocuments, listMedia, listOrganizations, mediaStorageUsage, normalizeMediaTags, patchIdentity, patchOrganization, readMediaMarkupLayer, readIdentity, readAuthSession, readBranchModule, readDocument, readGlobal, mutateGlobal, readMediaFile, readMediaMetadata, readOrganization, saveBranchModule, saveGlobal, saveMediaMarkupLayer, storeMediaUpload, updateMediaTags, renameMedia, upsertDocument } from "./storage.js";
...
import { addProjectChecklistItemAttachment, createProjectChecklistItem, deleteCrewChecklistItem, listProjectChecklists, normalizeChecklistCustomerAccess, patchProjectChecklist, patchProjectChecklistItem, readProjectChecklistDetail, readProjectChecklistItem } from "../workforce/crew_storage.js";
import { cancelRecurrenceSeries, createRecurrenceSeries, listRecurrenceOccurrences, listRecurrenceSeries, patchRecurrenceSeries, setRecurrenceOccurrenceStatus } from "./recurrence.js";
...
const platformSearchCache = new Map();
const contactProjectSummaryCache = new Map();
...
import { verifyGoogleCredential } from "./google_auth.js";
...
async function withNewIdentityRegistration(email, operation) {
    return await withIdentityRegistrationLock(email, async () => {
        ...
        try {
            ...
            if (identityId)
                await deleteIdentity(identityId).catch(() => undefined);
            if (organizationId)
...
const passwordResetSmsRequests = new Map();
function passwordRecoverySignature(value) {
    return createHmac("sha256", env.platformSessionSecret).update(`password-recovery:${value}`).digest("base64url");
}
function createPasswordRecoveryToken(identityId, requestId) {
    if (!identityId || !requestId)
        return "";
...
function parsePasswordRecoveryToken(tokenValue) {
...
async function resolvePasswordRecoveryIdentity(body) {
    const token = cleanText(body.recovery_token);
    if (!token)
        return await findIdentityByIdentifier(cleanText(body.identifier || body.email || body.phone));
...
function maskEmailAddress(emailValue) {
    const [local, domain] = String(emailValue || "").split("@");
    if (!local || !domain)
        return "your email";
    return `${local.slice(0, 1)}${"*".repeat(Math.min(4, Math.max(1, local.length - 1)))}@${domain}`;
}
function googleWorkspaceWebsite(hostedDomain) {
    const domain = cleanText(hostedDomain).toLowerCase();
    if (!domain || domain === "gmail.com" || domain.length > 253)
        return "";
    if (!domain.includes(".") || !/^[a-z0-9.-]+$/.test(domain) || domain.includes(".."))
        return "";
    return domain;
}
export const registerPlatformApi = async (app, options) => {
    app.post("/auth/google", async (request, reply) => {
        ...
        if (identity) {
            ...
            if (linkedSub && linkedSub !== google.sub) {
                throw new PlatformError("google_account_mismatch", 409, "This email is already linked to a different Google account.");
            }
        ...
        if (requestedPhone && !phone)
            throw badRequest("invalid_phone_number", "Enter a valid mobile phone number, including the country code outside the US and Canada.");
        ...
        if (commercialProfile.country !== "US")
            defaultAppFlags.firstmeasure = { ...asObject(defaultAppFlags.firstmeasure), report_localization: true, metric_measurements: commercialProfile.measurement_system === "metric" };
        ...
        const registered = await withNewIdentityRegistration(google.email, async (transaction) => {
            ...
            const nextIdentity = await addIdentityMembership(String(createdIdentity.id), String(organization.id), String(user.id), "owner");
    ...
    app.get("/identities/:identityId", async (request) => {
        ...
        if (identityId !== ctx.identityId)
            await requirePlatformAuth(request, { permission: "manage_company_users" });
        return {
            ok: true,
    ...
    app.get("/auth/session", async (request) => {
        ...
        if (!ctx)
            return { ok: true, authenticated: false, cookie_names: platformAuthCookieNames() };
    ...
    app.get("/statsig/bootstrap", async (request) => {
        ...
        const enabledFlagKeys = Object.entries(enabled).flatMap(([group, names]) => (Array.isArray(names) ? names.map((name) => `${group}.${name}`) : []));
    ...
    app.patch("/me/preferences", async (request) => {
        ...
        if (patch.interface_locale !== undefined && patch.interface_locale !== (current.interface_locale ?? null) && !await isAppFlagEnabled(ctx.orgId, "firstmeasure", "report_localization"))
            throw forbidden("localization_disabled", "Language customization is not enabled for this organization.");
        const preferences = {
            ...current,
    ...
    app.post("/auth/register", async (request, reply) => {
        ...
        if (!passwordHash)
            throw badRequest("password_required", "A password is required.");
    ...
    app.post("/portal-action", async (request, reply) => {
        ...
        if (!action)
            throw badRequest("missing_action", "A portal action is required.");
    ...
    app.get("/referrals/public/:code", async (request) => publicReferralLookup(getParam(request.params, "code"), publicRequestBaseUrl(request)));
    ...
    app.post("/referrals/status", async (request) => {
        ...
        if (!actor.email)
            return { success: false, ok: false, status_code: 401, error: "Authentication required." };
    ...
    app.post("/organizations/:orgId/credits/charge", async (request) => {
        ...
        if (amount <= 0)
            throw badRequest("invalid_credit_amount", "Charge amount must be greater than zero.");
        const global = await readGlobal(orgId);
        const balance = numericValue(asObject(global.data).credits_balance);
        if (amount > balance) {
            throw new PlatformError("insufficient_credits", 402, "This organization does not have enough credits for this charge.", { balance, required: amount });
        }
    ...
    app.post("/organizations/:orgId/credits/refund", async (request) => {
        ...
        const amount = Math.abs(numericValue(body.amount));
        if (amount <= 0)
            throw badRequest("invalid_credit_amount", "Refund amount must be greater than zero.");
        return {
            ok: true,
    ...
    app.post("/organizations/:orgId/credits/order-refund", async (request) => {
        ...
        if (!chargeToken)
            throw badRequest("missing_charge_token", "A charge token is required.");
        const charged = await creditChargeForToken(orgId, chargeToken);
        if (!charged)
            throw badRequest("charge_token_not_found", "No refundable charge was found for that token.");
    ...
    app.put("/organizations/:orgId/app-flags", async (request) => {
        ...
        if (!canManageTestAppFlags(operator))
            throw forbidden("app_flags_operator_only", "Only an authorized operator can change app rollout flags.");
        const body = objectBodySchema.parse(request.body ?? {});
        ...
        const patchData = {};
        if (hasExplicitFlagInput || (!hasVariantInput && !hasPlacementInput))
            patchData.app_flags = normalizeAppFlagInput(body);
        if (hasVariantInput) {
            patchData.app_variants = normalizeAppVariantInput(body);
        }
        if (hasPlacementInput)
            patchData.app_placements = normalizeAppPlacementInput(body.app_placements);
    ...
    app.get("/organizations/:orgId/platform-rollout", async (request) => {
        ...
        if (!canManageTestAppFlags(ctx))
            throw forbidden("app_flags_operator_only", "Only an authorized operator can inspect platform rollout targeting.");
    ...
    app.put("/organizations/:orgId/platform-rollout", async (request) => {
        ...
        if (!canManageTestAppFlags(ctx))
            throw forbidden("app_flags_operator_only", "Only an authorized operator can change platform rollout targeting.");
        ...
        for (const id of userIds)
            await readDocument(orgId, "users", id);
    ...
    app.get("/organizations/:orgId/media/:mediaId", async (request) => {
        ...
        if (!canReadReceiptMedia(media, ctx))
    ...
    app.patch("/organizations/:orgId/media/:mediaId", async (request) => {
        ...
        if (!canWriteReceiptMedia(existing, ctx))
            throw forbidden("receipt_media_forbidden", "This receipt media cannot be changed by this user.");
        const body = objectBodySchema.parse(request.body ?? {});
        ...
        if (!hasTags && !nameKey)
            throw badRequest("media_update_required", "A tags array or a name is required.");
        if (nameKey) {
            ...
            if (!hasTags)
                return { ok: true, media: publicMediaMetadata(renamed) };
        }
    ...
    app.get("/organizations/:orgId/media/:mediaId/markup/:layerId", async (request) => {
        ...
        return {
            ok: true,
            layer: await readMediaMarkupLayer(orgId, mediaId, getParam(request.params, "layerId"))
        };
    });
    app.put("/organizations/:orgId/media/:mediaId/markup/:layerId", async (request) => {
        ...
        const mediaId = getParam(request.params, "mediaId");
        const media = await readMediaMetadata(orgId, mediaId);
        if (!canWriteReceiptMedia(media, ctx))
            throw forbidden("receipt_media_forbidden", "This receipt media cannot be edited by this user.");
        const body = objectBodySchema.parse(request.body ?? {});
        return {
            ok: true,
            ...(await saveMediaMarkupLayer(orgId, mediaId, getParam(request.params, "layerId"), extractDataBody(body), extractMetadataBody(body)))
        };
    });
    ...
    app.put("/organizations/:orgId/global", async (request) => {
        ...
        const body = objectBodySchema.parse(request.body ?? {});
        if (containsAppFlagMutation(body))
            throw forbidden("app_flags_operator_only", "App rollout flags are operator-controlled and cannot be changed from Platform.");
        return {
            ok: true,
    ...
    app.get("/organizations/:orgId/branch/:branchId/modules", async (request) => {
        ...
        return {
            ok: true,
            modules: await listBranchModules(orgId, getParam(request.params, "branchId"))
        };
    });
    app.get("/organizations/:orgId/branch/:branchId/modules/:moduleId", async (request) => {
        ...
        return {
            ok: true,
            module: await readBranchModule(orgId, getParam(request.params, "branchId"), getParam(request.params, "moduleId"))
        };
    });
    app.put("/organizations/:orgId/branch/:branchId/modules/:moduleId", async (request) => {
        ...
        return {
            ok: true,
            module: await saveBranchModule(orgId, getParam(request.params, "branchId"), getParam(request.params, "moduleId"), objectBodySchema.parse(request.body ?? {}), { replace: true })
        };
    });
    app.patch("/organizations/:orgId/branch/:branchId/modules/:moduleId", async (request) => {
        ...
        return {
            ok: true,
            module: await saveBranchModule(orgId, getParam(request.params, "branchId"), getParam(request.params, "moduleId"), objectBodySchema.parse(request.body ?? {}), { replace: false })
        };
    });
    app.post("/organizations/:orgId/projects/:projectId/events", async (request) => {
        ...
        const event = normalizeProjectEvent({ ...existingEvent, ...eventInput }, currentData);
        ...
        if (equipmentRefs.length && cleanText(event.start_at) && cleanText(event.status).toLowerCase() !== "unscheduled"
            ...
            if (conflictMode !== "off") {
                ...
                equipmentConflicts = assessed.flat();
            ...
            if (await isCapabilityEnabled(orgId, "equipment.operators").catch(() => false)) {
                operatorWarnings = await assessOperatorRequirements(orgId, event);
        ...
        const rangeChanged = existingIndex >= 0 && (cleanText(existingEvent.start_at || existingEvent.start) !== cleanText(event.start_at)
    ...
    app.post("/organizations/:orgId/recurrence-series/:seriesId/occurrences/:occurrenceId/:action", async (request) => {
        ...
        if (!["completed", "skipped", "cancelled"].includes(action))
            throw badRequest("invalid_recurrence_occurrence_action", "Use completed, skipped, or cancelled.");
        return { ok: true, occurrence: await setRecurrenceOccurrenceStatus(orgId, getParam(request.params, "seriesId"), getParam(request.params, "occurrenceId"), action, ctx.userId) };
    });
    ...
    app.patch("/customer-portals/:portalUuid/checklists/:checklistId/items/:itemId", async (request) => {
        const item = await updateCustomerChecklistItem(getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"), getParam(request.params, "itemId"), objectBodySchema.parse(request.body ?? {}));
        return { ok: true, item };
    });
    app.post("/customer-portals/:portalUuid/checklists/:checklistId/items", async (request, reply) => {
        const item = await createCustomerChecklistItem(getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"), objectBodySchema.parse(request.body ?? {}));
        reply.code(201);
        return { ok: true, item };
    });
    app.delete("/customer-portals/:portalUuid/checklists/:checklistId/items/:itemId", async (request) => {
        const result = await deleteCustomerChecklistItem(getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"), getParam(request.params, "itemId"));
        return { ok: true, ...result };
    });
    ...
    app.delete("/customer-portals/:portalUuid/uploads/:mediaId", async (request) => {
        const result = await withdrawCustomerUpload(getParam(request.params, "portalUuid"), getParam(request.params, "mediaId"));
        return { ok: true, ...result };
    });
    app.get("/customer-portals/:portalUuid/media/:mediaId/comments", async (request) => {
        const result = await listCustomerMediaComments(getParam(request.params, "portalUuid"), getParam(request.params, "mediaId"));
        return { ok: true, ...result };
    });
    app.post("/customer-portals/:portalUuid/media/:mediaId/comments", async (request, reply) => {
        const result = await createCustomerMediaComment(getParam(request.params, "portalUuid"), getParam(request.params, "mediaId"), objectBodySchema.parse(request.body ?? {}));
        reply.code(201);
        return { ok: true, ...result };
    });
    app.delete("/customer-portals/:portalUuid/media/:mediaId/comments/:commentId", async (request) => {
        const result = await withdrawCustomerMediaComment(getParam(request.params, "portalUuid"), getParam(request.params, "mediaId"), getParam(request.params, "commentId"));
        return { ok: true, ...result };
    });
    app.post("/customer-portals/:portalUuid/punch-lists/:checklistId/submit", async (request) => {
        const result = await submitCustomerPunchList(getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"), objectBodySchema.parse(request.body ?? {}), request);
        return { ok: true, ...result };
    });
    app.post("/customer-portals/:portalUuid/punch-lists/:checklistId/accept", async (request) => {
        const result = await acceptCustomerPunchList(getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"), objectBodySchema.parse(request.body ?? {}), request);
        return { ok: true, ...result };
    });
    app.post("/customer-portals/:portalUuid/checklists/:checklistId/items/:itemId/attachments", async (request) => {
        const result = await attachCustomerChecklistEvidence(request, getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"), getParam(request.params, "itemId"));
        return { ok: true, ...result };
    });
    app.post("/customer-portals/:portalUuid/checklists/:checklistId/voice", async (request) => {
        const result = await updateCustomerChecklistByVoice(request, getParam(request.params, "portalUuid"), getParam(request.params, "checklistId"));
        return { ok: true, ...result };
    });
    ...
    app.post("/organizations/:orgId/tagging/mention-events", async (request, reply) => {
        ...
        const mentionUsers = Array.isArray(body.mention_users || body.mentionUsers)
            ? (body.mention_users || body.mentionUsers)
            : [];
        ...
        const frontendAction = projectId ? (mentionSource === "photo_comment" || cleanText(context.kind).toLowerCase() === "photo_comment"
    ...
    app.patch("/organizations/:orgId/projects/:projectId/documents/:documentId", async (request) => {
        ...
        let updated = normalizeProjectUploadedDocuments(data.documents).map((item) => {
            if (projectDocumentIdentity(item) !== documentId && cleanText(item.media_id) !== documentId)
                return item;
            matched = true;
    ...
    app.post("/organizations/:orgId/:collection", async (request, reply) => {
        ...
        if (collection === "branch")
            await assertBranchReportPreferences(orgId, String(asObject(request.params).documentId || body.id || "default"), body);
        const document = collection === "users"
            ...
            : collection === "projects"
                ...
                : await upsertDocument(orgId, collection, body, { replace: true });
        ...
        if (platformSearchCollection(collection))
            invalidatePlatformSearchCache(orgId);
    ...
    app.get("/organizations/:orgId/:collection/:documentId", async (request) => {
        ...
        return {
            ok: true,
            document: await readDocument(orgId, collection, getParam(request.params, "documentId"))
        };
    });
    app.put("/organizations/:orgId/:collection/:documentId", async (request) => {
        ...
        const document = collection === "users"
            ...
            : collection === "projects"
                ...
                : await upsertDocument(orgId, collection, { ...body, id: documentId }, { replace: true });
        if (collection === "projects") {
            await ensurePipelinePlanForProject(orgId, { id: document.id, ...asObject(document.data) });
    ...
    app.patch("/organizations/:orgId/:collection/:documentId", async (request) => {
        ...
        const document = collection === "users"
            ...
            : collection === "projects"
                ...
                : await upsertDocument(orgId, collection, { ...body, id: documentId }, { replace: false });
        if (collection === "projects") {
            await ensurePipelinePlanForProject(orgId, { id: document.id, ...asObject(document.data) });
            const projectData = asObject(document.data);
            await emitWorkEvent({
    ...
    app.delete("/organizations/:orgId/:collection/:documentId", async (request) => {
        ...
        const deleted = await deleteDocument(orgId, collection, documentId);
...
function defaultVariableMappingsData(existing = {}) {
...
async function readBranchModuleDataOrNull(orgId, branchId, moduleId) {
    try {
...
async function ensureWorkflowModules(orgId, branchId) {
...
function isLocalPortalRequest(request) {
    const candidates = [
...
function normalizeLeadContacts(input) {
...
function defaultLeadNotificationRoles(sourceKind) {
    if (sourceKind === "canvassing")
        return ["inside_sales", "sales_appointments"];
    return ["inside_sales", "sales_appointments"];
}
export async function createPlatformLead(orgId, input = {}) {
    ...
    const rawNotification = input.notification;
...
function platformSearchCollection(collection) {
    return collection === "projects" || collection === "customers";
}
function invalidatePlatformSearchCache(orgId) {
    const key = cleanText(orgId);
    platformSearchCache.delete(key);
    contactProjectSummaryCache.delete(key);
}
function queryStringList(...values) {
    return Array.from(new Set(values.flatMap((value) => {
        if (Array.isArray(value))
            return value.flatMap((entry) => cleanText(entry).split(","));
        return cleanText(value).split(",");
    }).map((value) => cleanText(value)).filter(Boolean)));
}
function isGeneratedProjectTitle(value) {
...
function firstProjectDisplayText(...values) {
    for (const value of values) {
        const text = cleanText(value);
        if (text && !isGeneratedProjectTitle(text))
            return text;
    }
    return "";
}
function projectContactIdentity(contact) {
...
function dedupeProjectContacts(contacts) {
    const byKey = new Map();
    const order = [];
    ...
    return order.map((key) => byKey.get(key)).filter((contact) => !!contact);
}
function summarizedProjectContacts(value) {
...
function projectDisplayTitle(project, fallback = "New Project") {
    ...
    return firstProjectDisplayText(project.title, project.project_title, project.project_name, project.projectName, project.name, project.address, project.project_address, project.property_address, project.customer_name, project.customerName, project.primary_contact_name, primary.name, project.resident_name, project.residentName, typeof project.resident === "string" ? project.resident : "") || fallback;
}
function summarizedMeasurementProject(value) {
...
function summarizedProjectDocument(document) {
...
function projectContactRefs(project) {
...
function projectMatchesContactSummaryRequest(document, query) {
    const data = asObject(document.data);
...
async function contactProjectSummaries(orgId, query) {
    ...
    if ((!requestedIds.length || hasContactNeedle) && !matchedFromSearchCache) {
        ...
        docs.forEach((document) => {
            if (!projectMatchesContactSummaryRequest(document, query))
                return;
...
function cachedContactProjectIdsFromSearch(orgId, query) {
    const key = cleanText(orgId);
    const cached = platformSearchCache.get(key);
    if (!cached || Date.now() - cached.builtAt >= PLATFORM_SEARCH_CACHE_TTL_MS)
        return [];
    const contactId = cleanText(query.contact_id || query.contactId || query.id);
    const email = cleanText(query.email).toLowerCase();
    const phone = phoneDigits(query.phone);
    ...
    return Array.from(new Set(cached.rows.filter((row) => {
        if (row.type !== "contact" || !row.project_id)
            return false;
        const contact = asObject(row.contact);
        if (contactId && queryStringList(row.id, contact.id, contact.contact_id).includes(contactId))
            return true;
        if (email && cleanText(contact.email).toLowerCase() === email)
            return true;
        const contactPhone = phoneDigits(contact.phone);
...
async function cachedContactProjectSummaryDocuments(orgId) {
    const key = cleanText(orgId);
    const cached = contactProjectSummaryCache.get(key);
    if (cached && Date.now() - cached.builtAt < PLATFORM_CONTACT_PROJECT_CACHE_TTL_MS)
...
function firstSearchText(...values) {
    for (const value of values) {
...
function projectSearchSubtitle(project) {
...
function projectContactNeedles(project) {
...
function projectSearchFields(projectId, project) {
    return [
        projectId,
        project.id,
...
function contactSearchSubtitle(contact, project = {}) {
    return [
...
function addProjectContactRows(rows, seenContacts, projectId, project, updatedAt) {
    const contacts = Array.isArray(project.contacts) ? project.contacts.map((entry) => asObject(entry)) : [];
    const aliases = [{
...
function addCustomerContactRows(rows, seenContacts, customerDocs) {
...
async function buildPlatformSearchIndex(orgId) {
...
async function platformSearchRows(orgId) {
    const key = cleanText(orgId);
    const cached = platformSearchCache.get(key);
    if (cached && Date.now() - cached.builtAt < PLATFORM_SEARCH_CACHE_TTL_MS)
        return cached.rows;
...
function platformSearchTypes(value) {
    const requested = new Set(value.split(",").map((part) => part.trim().toLowerCase()).filter(Boolean));
    if (!requested.size)
        return new Set(["project", "contact"]);
    return new Set([
        ...(requested.has("project") || requested.has("projects") ? ["project"] : []),
        ...(requested.has("contact") || requested.has("contacts") ? ["contact"] : [])
    ]);
}
function platformSearchLimit(value) {
    const limit = Math.floor(Number(value || 40));
    if (!Number.isFinite(limit) || limit <= 0)
        return 40;
    return Math.min(100, Math.max(1, limit));
}
function rowMatchesSearch(row, queryText, queryTokens, queryCompact, queryPhone) {
    if (!queryText)
        return true;
    if (queryPhone.length >= 3 && row.phone_digits.includes(queryPhone))
        return true;
    if (row.search_text.includes(queryText) || (queryCompact.length >= 3 && row.compact_text.includes(queryCompact)))
        return true;
    return queryTokens.length > 0 && queryTokens.every((token) => (row.tokens.some((rowToken) => rowToken === token || rowToken.startsWith(token))
        || row.search_text.includes(token)));
}
function rowSearchScore(row, queryText, queryTokens, queryCompact, queryPhone) {
    ...
    if (queryText && title === queryText)
        score += 700;
    if (queryText && title.startsWith(queryText))
        score += 420;
    if (queryText && row.search_text.startsWith(queryText))
        score += 260;
    if (queryText && row.search_text.includes(queryText))
        score += 150;
    if (queryCompact.length >= 3 && row.compact_text.includes(queryCompact))
        score += 80;
    if (queryPhone.length >= 3 && row.phone_digits.includes(queryPhone))
        score += row.phone_digits.startsWith(queryPhone) ? 420 : 260;
    queryTokens.forEach((token) => {
        if (row.tokens.includes(token))
            score += 130;
        else if (row.tokens.some((rowToken) => rowToken.startsWith(token)))
            score += 90;
        else if (row.search_text.includes(token))
            score += 35;
    });
    const updatedMs = Date.parse(row.updated_at);
    if (Number.isFinite(updatedMs))
        score += Math.min(35, Math.max(0, (updatedMs / 1000) / 100000000));
    return score;
}
export async function searchPlatformProjectsAndContacts(orgId, input) {
...
function notificationIdFromParts(...parts) {
...
function userRoleIds(user) {
    ...
    if (!roles.length && ["owner", "admin", "super_admin"].includes(role))
        return PLATFORM_STANDARD_USER_ROLES;
    return roles;
}
export async function createPlatformNotification(orgId, input) {
    ...
    try {
        ...
        if (error instanceof PlatformError && error.code === "document_exists")
            return await readDocument(orgId, NOTIFICATION_COLLECTION, notification.id);
        throw error;
    }
    if (notification.push) {
        ...
        if (log.length)
            return await upsertDocument(orgId, NOTIFICATION_COLLECTION, { id: notification.id, data: { ...notification, push_log: log }, metadata: saved.metadata }, { replace: true });
    }
    return saved;
}
async function requireVisibleNotification(orgId, userId, notificationId, branchId) {
    const result = await listVisibleNotifications(orgId, userId, { includeDismissed: true, branchId, ignorePreferences: true });
    if (!result.notifications.some((item) => item.id === notificationId))
        throw notFound("notification_not_found", "Notification not found.");
}
async function setUserNotificationState(orgId, userId, notificationId, patch) {
    ...
    const now = new Date().toISOString();
    const stateTimestamp = (flag, timestamp) => {
        ...
        if ((hasFlag && patch[flag] === false) || (hasTimestamp && !cleanText(patch[timestamp])))
            return undefined;
...
async function listVisibleNotifications(orgId, userId, options = {}) {
    if (!await isAppFlagEnabled(orgId, "apps", "notifications"))
        return { notifications: [], unread_count: 0, active_count: 0 };
    ...
    const notifications = notificationDocs
        ...
        if (!targetUserIds.length && !targetRoleIds.length)
            return true;
...
function actionItemIdFromParts(...parts) {
...
function uniqueActionItemId(kind) {
...
function normalizeActionItemStatus(value, fallback = "open") {
...
function normalizeContactRefs(value) {
...
function actionItemHistoryEntry(type, input = {}, ctx = {}) {
    return {
        type,
...
function normalizeActionItem(input, ctx = {}) {
    const now = new Date().toISOString();
    ...
    const id = cleanText(input.id) || (projectIds.length
    ...
    return {
        ...
        completion_events: Array.isArray(input.completion_events || input.completionEvents)
            ? (input.completion_events || input.completionEvents).map((entry) => asObject(entry))
            : [],
        source,
...
function userCanSeeActionItem(item, userId, roles, resourceGroupIds = new Set()) {
    ...
    if (!assignedUserIds.length && !assignedRoleIds.length && !assignedResourceGroupIds.length)
        return true;
...
function actionItemIsManagedByUser(_item, ctx) {
...
function dateMatchesFilter(value, dueAfter = "", dueBefore = "") {
    ...
    if (Number.isFinite(afterMs) && ms < afterMs)
        return false;
    if (Number.isFinite(beforeMs) && ms > beforeMs)
        return false;
    return true;
}
function canonicalActionItemStatus(status) {
    const value = cleanText(status);
    if (value === "active")
        return "claimed";
    if (["completed", "canceled"].includes(value))
        return value;
    return "open";
}
function canonicalWorkNodeStatus(status) {
    const value = cleanText(status).toLowerCase();
    if (value === "claimed" || value === "active")
        return "active";
    if (value === "completed")
        return "completed";
...
function canonicalActionItemExternalTriggers(value) {
...
function canonicalActionItemData(node, userState = {}) {
    ...
    const history = Array.isArray(asObject(node.metadata).history) ? asObject(node.metadata).history.map(asObject) : [];
...
async function canonicalActionItemForUser(orgId, nodeId, ctx) {
    const node = (await readNodeRecord(orgId, nodeId));
    if (!node)
        throw notFound("action_item_not_found", "This to-do was not found.");
...
export async function listCanonicalActionItems(orgId, ctx, options = {}) {
    const userDoc = await readDocument(orgId, "users", cleanText(ctx.userId));
    const states = asObject(asObject(userDoc.data).action_item_state);
...
export async function createCanonicalActionItem(orgId, input, ctx) {
    ...
    const roots = Array.isArray(asObject(result.tree).root_nodes) ? asObject(result.tree).root_nodes : [];
    let node = asObject(roots[0]);
    if (cleanText(item.due_at) && cleanText(node.id))
        node = asObject(await patchWorkNode(orgId, cleanText(node.id), { due_at: item.due_at }));
    return canonicalActionItemData(node);
}
async function patchCanonicalActionItem(orgId, nodeId, patch, ctx) {
...
async function setUserActionItemState(orgId, userId, actionItemId, patch) {
    const userDoc = await readDocument(orgId, "users", userId);
    const data = asObject(userDoc.data);
...
async function upsertProjectDocumentPreservingEvents(orgId, documentId, input, replace) {
    const id = cleanText(documentId || input.id);
    if (!id)
        return await upsertDocument(orgId, "projects", input, { replace });
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        await validateProjectCustomFieldValues(orgId, cleanText(mergedProjectData.branch_id || currentData.branch_id || "default") || "default", mergedProjectData, currentData);
        const nextInput = {
            ...input,
            id,
        ...
        try {
            ...
            if (explicitExpectedRevision || !(error instanceof PlatformError) || error.code !== "revision_conflict" || attempt === 2)
                throw error;
        }
    }
 ...
 */
function normalizeEventResourceRefs(eventValue) {
...
function eventEquipmentResourceRefs(eventValue) {
    return normalizeEventResourceRefs(eventValue).filter((ref) => EQUIPMENT_REF_KINDS.has(ref.kind));
}
function normalizeEventResourceRequirements(value) {
    ...
    return requirements.map(asObject).map((requirement) => ({
        ...
        if (!requirement.equipment_type_id || seen.has(requirement.equipment_type_id))
            return false;
        seen.add(requirement.equipment_type_id);
        return true;
    });
}
function eventAssignmentKeys(eventValue, options = {}) {
    const event = asObject(eventValue);
    ...
    for (const ref of normalizeEventResourceRefs(event)) {
        if (EQUIPMENT_REF_KINDS.has(ref.kind) && options.includeEquipment !== true)
            continue;
        keys.push(`${ref.kind}:${ref.id.toLowerCase()}`);
    }
    return [...new Set(keys)].sort();
}
async function assertProjectEventAssignmentsAllowed(orgId, branchId, eventValue) {
    ...
    const policyCoversEquipment = policy.rules.some((rule) => rule.subject_types.includes("equipment_unit"));
...
async function assertRecurrenceAssignmentAllowed(orgId, inputValue) {
    ...
    if (!Object.keys(asObject(configuredType)).length)
        return;
    await assertProjectEventAssignmentsAllowed(orgId, branchId, eventTemplate);
}
function normalizeProjectEvent(input, project) {
    const now = new Date().toISOString();
    ...
    const assignedUsers = Array.isArray(input.assigned_users || input.assignedUsers)
        ? (input.assigned_users || input.assignedUsers)
        : [];
...
function isAppointmentConfirmationCandidate(event) {
    if (asObject(event.confirmation).required !== undefined)
        return true;
    if (isMaterialDeliveryProjectEvent(event))
        return false;
...
function isMaterialDeliveryProjectEvent(event) {
...
function eventHasCompleted(event, now = Date.now()) {
    if (isMaterialDeliveryProjectEvent(event))
        return false;
    if (event.completed_emitted_at || event.completed_at)
        return false;
    if (["unscheduled", "canceled", "cancelled"].includes(cleanText(event.status).toLowerCase()))
        return false;
...
function eventHasStarted(event, now = Date.now()) {
    if (isMaterialDeliveryProjectEvent(event))
        return false;
    if (event.started_emitted_at || event.completed_emitted_at || event.completed_at)
        return false;
    if (["unscheduled", "canceled", "cancelled"].includes(cleanText(event.status).toLowerCase()))
        return false;
    const start = new Date(String(event.start_at || event.start || ""));
...
export async function processProjectEventLifecycleForOrg(orgId, candidates) {
    ...
    for (const document of projects) {
        ...
        for (const event of events) {
            ...
            if (!eventHasCompleted(event, now))
                continue;
...
async function runPlatformHeartbeat(app) {
    if (heartbeatRunning)
        return;
    heartbeatRunning = true;
    try {
        if (isFirstMeasurePostgresEnabled()) {
            await runPostgresPlatformHeartbeat((orgId, project) => processProjectEventLifecycleForOrg(orgId, [project]), (error, orgId) => app.log?.warn({ err: error, orgId }, "Platform heartbeat failed for organization."));
            return;
        }
...
function startPlatformHeartbeat(app) {
    if (process.env.PLATFORM_HEARTBEAT_DISABLED === "1")
        return;
    if (heartbeatStarted)
        return;
    ...
    const timer = setInterval(() => {
        ...
        void import("./worker_tasks.js").then(({ runPlatformTask }) => runPlatformTask("platform-billing", 3600000, async () => (await import("../platform-billing/metering.js")).sweepBilling()))
            .catch(error => app.log?.warn({ err: error }, "Platform billing sweep failed."));
    }, HEARTBEAT_INTERVAL_MS);
    timer.unref?.();
}
function getParam(params, key) {
    const value = (params && typeof params === "object") ? params[key] : "";
    return String(value ?? "");
}
...
function platformUserView(userDoc) {
...
async function protectCommercialProfile(orgId, body, replace) {
    ...
    if (replace && saved !== undefined)
        return { ...body, data: { ...data, commercial_profile: saved } };
    return body;
}
function safeBillingView(value) {
...
function normalizeDocumentId(value, fallback) {
...
function identityStatusForOrgUserInput(dataInput) {
    const rawStatus = cleanText(dataInput.identity_status || dataInput.identityStatus || dataInput.status).toLowerCase();
    if (["disabled", "inactive", "deleted", "suspended"].includes(rawStatus))
        return "disabled";
    return "active";
}
function shouldRepairIdentityStatus(identity, nextStatus) {
    if (nextStatus !== "active")
        return false;
    const current = cleanText(identity.status || "active").toLowerCase();
    return ["invited", "pending"].includes(current);
}
async function applyCreditDelta(orgId, body, actorEmail, requireAvailableCredits = false, freeExpediteMutation) {
    const amount = numericValue(body.amount ?? body.delta);
    if (amount === 0 && !freeExpediteMutation)
        throw badRequest("invalid_credit_amount", "Credit amount must be non-zero.");
    let freeExpedite = null;
    let outcome = { balance: 0, ledger_entry: {}, ledger_count: 0 };
    const document = await mutateGlobal(orgId, (global) => {
        ...
        if (freeExpediteMutation) {
            ...
            if (!Number.isFinite(couponDelta) || couponDelta === 0)
                throw badRequest("invalid_free_expedite_amount", "Free expedite uses amount must be non-zero.");
            const current = Math.max(0, Math.round(numericValue(data.free_expedite_uses)));
            const next = current + couponDelta;
            if (next < 0)
                throw badRequest("insufficient_free_expedite_uses", "The free expedite was used by another order. Refresh the price and try again.");
...
async function applyFreeExpediteDelta(orgId, body, actorEmail) {
    const amount = Math.round(numericValue(body.amount ?? body.delta));
    if (amount === 0)
        throw badRequest("invalid_free_expedite_amount", "Free expedite uses amount must be non-zero.");
    let outcome = { free_expedite_uses: 0, free_expedite_ledger_entry: {}, free_expedite_ledger_count: 0 };
    const document = await mutateGlobal(orgId, (global) => {
        const data = asObject(global.data);
        const current = Math.max(0, Math.round(numericValue(data.free_expedite_uses)));
        const next = current + amount;
        if (next < 0)
            throw badRequest("insufficient_free_expedite_uses", "This organization does not have enough free expedite uses.");
...
async function creditChargeForToken(orgId, chargeToken) {
    const global = await readGlobal(orgId);
    const ledger = Array.isArray(asObject(global.data).credits_ledger) ? asObject(global.data).credits_ledger : [];
    for (let index = ledger.length - 1; index >= 0; index -= 1) {
        const entry = asObject(ledger[index]);
        const meta = asObject(entry.meta);
        if (String(meta.charge_token || "") !== chargeToken)
            continue;
        const delta = numericValue(entry.delta);
        if (delta < 0)
            return { amount: Math.abs(delta), entry };
    }
    return null;
}
function withoutRemovedUserTypeFields(value) {
...
async function createPlatformOrgUser(orgId, body) {
    ...
    if (identityId)
        await addIdentityMembership(identityId, orgId, userId, role);
    return document;
}
async function upsertPlatformOrgUserDocument(orgId, documentId, body, replace) {
    const dataInput = withoutRemovedUserTypeFields(body.data && typeof body.data === "object" ? body.data : body);
    ...
    return await upsertDocument(orgId, "users", {
        id: documentId,
        data: {
            ...data,
...
function platformOrgUserPermissionState(data) {
    const orgPermissions = asObject(data.org_permissions);
    const level = cleanText(orgPermissions.level ||
        data.org_permission_level ||
        data.permission_level ||
        data.perm_level ||
        data.role ||
        "viewer").toLowerCase() || "viewer";
...
function platformOrgUserPermissionMutation(body) {
...
function customerPortalDocumentId(projectId) {
    // Shared with portal_widgets.ts via portal_settings.ts — one definition only.
    const id = portalDocumentIdFor(projectId);
    if (!id)
        throw badRequest("invalid_project_id", "Project id is required.");
    return id;
}
async function readOptionalDocument(orgId, collection, documentId) {
    try {
...
function portalDisplayName(...values) {
    for (const value of values) {
        const text = cleanText(value);
        if (text && !isGeneratedCustomerName(text))
            return text;
    }
    return "Customer Portal";
}
function mergeCustomerContact(...values) {
    const result = {};
    for (const value of values) {
...
function publicPortalUrl(uuid, baseUrl, preview) {
...
function portalGuestTokenHash(token) {
...
function encryptPortalGuestToken(token) {
    const key = portalGuestEncryptionKey();
    if (!key)
        throw conflict("portal_sharing_encryption_not_configured", "Secure portal sharing requires a platform session secret of at least 32 characters.");
...
function decryptPortalGuestToken(value) {
...
function normalizePortalGuestPreset(value) {
    const preset = cleanText(value).toLowerCase();
    return Object.prototype.hasOwnProperty.call(PORTAL_GUEST_PRESETS, preset) ? preset : "full_view";
}
function normalizePortalGuestLinks(value) {
    const seen = new Set();
    return (Array.isArray(value) ? value : [])
        ...
        const allowed = new Set(PORTAL_GUEST_PRESETS[preset]);
        ...
        .filter((item) => {
        if (!item.id || !/^[a-f0-9]{64}$/i.test(item.token_hash) || seen.has(item.id))
            return false;
        seen.add(item.id);
        return true;
    });
}
function recoverPortalGuestLinks(value) {
    const links = normalizePortalGuestLinks(value);
    let changed = false;
    for (const link of links) {
        if (link.revoked_at || portalGuestLinkIsExpired(link.expires_at) || decryptPortalGuestToken(link.token_ciphertext))
            continue;
...
function portalGuestLinkIsExpired(value) {
...
function publicPortalGuestLink(value, baseUrl = "") {
...
function publicPortalGuestLinks(value, baseUrl = "") {
    return normalizePortalGuestLinks(value).map((link) => publicPortalGuestLink(link, baseUrl));
}
function customerPortalView(document, baseUrl) {
    const data = asObject(document.data);
...
export async function ensureCustomerPortalRecord(orgId, projectId, input = {}, baseUrl = "") {
    ...
    const document = await upsertDocument(orgId, CUSTOMER_PORTAL_COLLECTION, {
        id: documentId,
        data,
        metadata: {
...
async function updateCustomerPortalRecord(orgId, projectId, input = {}, baseUrl = "") {
    ...
    for (const mediaId of shareMedia) {
        ...
        if (existing)
            existing.include_markup = includeMarkup;
        else
            sharedItems.push({ type: "media", item_id: mediaId, shared_at: now, shared_by: actorUserId, include_markup: includeMarkup });
    }
...
async function findCustomerPortalByUuid(uuid, preview) {
    const target = cleanText(uuid);
    if (!target)
        throw badRequest("invalid_portal", "A portal id is required.");
    ...
    for (const org of orgs) {
        ...
        for (const doc of portals) {
            ...
            if (match === target)
                return { orgId, document: doc, data, access_mode: "owner" };
            if (guestHash) {
    ...
    if (unavailableGuest)
        throw forbidden("portal_share_unavailable", "This shared portal link has expired or been revoked.");
    throw forbidden("portal_not_found", "The requested customer portal could not be found.");
}
function requirePortalOwner(found) {
    if (found.access_mode !== "owner")
        throw forbidden("portal_guest_readonly", "Shared portal links are read-only.");
    return found;
}
async function savePortalGuestLinks(found, guestLinks) {
...
async function ensureRecoverablePortalGuestLinks(found) {
...
async function createPortalGuestLink(foundInput, input, baseUrl, createdBy = "customer") {
    ...
    const allowedTabs = new Set(PORTAL_GUEST_PRESETS[preset]);
...
async function revokePortalGuestLink(foundInput, shareId) {
    const found = requirePortalOwner(foundInput);
    const links = normalizePortalGuestLinks(found.data.guest_links);
    const target = links.find((item) => item.id === cleanText(shareId));
    if (!target)
        throw notFound("portal_share_not_found", "The shared portal link was not found.");
    if (!target.revoked_at)
        target.revoked_at = new Date().toISOString();
    await savePortalGuestLinks(found, links);
    return publicPortalGuestLink(target);
}
function publicProjectFirstMeasureId(project) {
...
function publicProjectTopDownSource(project) {
    const measurement = asObject(project.measurement_project ?? project.measurement);
    const raw = asObject(measurement.raw);
    ...
    if (direct)
        return direct;
    if (artifacts.has_azure_image || assets.azure)
        return "azure.png";
    if (artifacts.has_apple_image || assets.apple)
        return "apple.png";
    return "google.png";
}
function publicProjectTopDownThumbnail(project, apiBaseUrl = "") {
...
function publicProjectView(project, apiBaseUrl = "") {
...
function projectScheduleEventType(event) {
...
function projectScheduleEventCategory(event) {
    ...
    if (type.includes("delivery") || resourceType === "material" || scheduleKind.includes("delivery"))
        return "delivery";
    if (type.includes("completion") || type.includes("complete") || type.includes("estimate"))
        return "completion";
    if (type.includes("appointment") || type.includes("meeting") || type.includes("inspection") || type.includes("consult"))
        return "appointment";
    if (type === "project_work" || type.includes("work") || resourceType === "labor" || scheduleKind === "labor")
        return "work";
    if (type.includes("equipment") || resourceType === "equipment")
        return "equipment";
    return "event";
}
function publicScheduleCategoryTitle(category) {
...
function publicScheduleCrewName(event) {
    if (event.customer_show_crew !== true)
        return "";
    const assignedCrew = asObject(event.assigned_crew);
    const workResource = asObject(event.work_resource_ref);
    const explicit = cleanText(event.customer_crew_name
...
function publicProjectScheduleEvent(eventValue, schedulingValue = {}, customerSchedulingEnabled = false) {
    const event = asObject(eventValue);
    if (event.customer_visible !== true)
        return null;
    const startAt = cleanText(event.start_at || event.start || event.starts_at || event.start_date);
    const parsedStart = Date.parse(startAt);
    if (!startAt || !Number.isFinite(parsedStart))
        return null;
...
async function publicProjectSchedule(project, linkedEvents = [], schedulingValue = {}, customerSchedulingEnabled = false, orgId = "") {
    ...
    const scopeDefinition = orgId && scopeTemplateId
        ? asObject((await (async () => { try {
            return (await readScopeTemplate(orgId, cleanText(project.branch_id || "default") || "default", scopeTemplateId));
        }
        catch {
            return null;
        } })())?.definition)
...
function publicProjectAppointment(scheduleEvents) {
...
function proposalPlainText(value) {
    return cleanText(value)
        .replace(/&lt;br\s*\/?&gt;/gi, "\n")
...
function publicProposalScopeView(page) {
...
function publicProposalLineItems(page, proposalValue = {}) {
...
function publicProposalBlocks(page) {
...
function publicProposalPage(pageValue, proposalValue = {}) {
...
function publicProposalView(proposalValue, index = 0) {
...
async function publicProjectProposals(project, preview = false) {
    ...
    return Promise.all(selected.map(async (proposal) => {
        ...
        const workflow = await publicProposalWorkflow(token)
            ...
            .catch((error) => {
            console.warn(`[portal] proposal workflow enrichment failed for token ${token.slice(0, 8)}…: ${String(error?.message || error)}`);
            return null;
        });
        const snapshotProposal = asObject(asObject(workflow?.workflow).proposal);
        if (!Object.keys(snapshotProposal).length)
            return proposal;
        return {
            ...proposal,
            ...snapshotProposal,
...
function absoluteApiAssetUrl(value, apiBaseUrl) {
    const text = cleanText(value);
...
function publicPortalLogoAssetUrl(value, apiBaseUrl) {
    if (value && typeof value === "object") {
    ...
    if (/^https?:\/\//i.test(logo)) {
        try {
            ...
            if (parsed.pathname.startsWith("/v1/platform/"))
    ...
    if (logo.startsWith("/v1/platform/"))
        return logo;
    return absoluteApiAssetUrl(logo, apiBaseUrl);
}
function publicPortalLogoMediaId(value) {
    if (value && typeof value === "object") {
        const item = asObject(value);
...
function publicPortalBrandingLogoUrl(branding, orgId, apiBaseUrl) {
    for (const value of [
        branding.logo,
...
function isLegacyImportedLogoUrl(value) {
    const text = cleanText(value).replace(/^\/+/, "");
    if (!text)
        return false;
    return text.startsWith("organizations/") && /\/logo(?:\.png)?(?:$|\?)/i.test(text);
}
function publicPortalBrandColor(fallback, ...values) {
    for (const value of values) {
...
function portalMediaUrl(baseUrl, uuid, mediaId, preview, variant = "original", cacheKey = "") {
...
function customerPortalContactId(data, project = {}) {
...
async function publicPortalMedia(orgId, projectData, portalData, portalUuid, preview, apiBaseUrl) {
    const sharedMedia = new Map(normalizeSharedItems(portalData.shared_items)
        .filter((item) => item.type === "media")
        .map((item) => [item.item_id, item]));
    const photos = Array.isArray(projectData.photos) ? projectData.photos.map((item) => asObject(item)) : [];
    return await Promise.all(photos
        ...
        return {
            ...
            thumb: portalMediaUrl(apiBaseUrl, portalUuid, mediaId, preview, includeMarkup ? "thumb_320_markup" : (cleanText(item.thumbnail_variant || "thumb_320") || "thumb_320"), markupRevision),
            src: portalMediaUrl(apiBaseUrl, portalUuid, mediaId, preview, "original")
        };
    }));
}
function publicChecklistItem(itemValue) {
...
function publicChecklist(checklistValue) {
...
async function publicProjectChecklists(orgId, projectId) {
    if (!projectId)
        return [];
 ...
 */
async function publicProjectPunchLists(orgId, projectId) {
    if (!projectId)
        return [];
    ...
    return (await listProjectChecklists(orgId, projectId))
        ...
        const labels = resolvePunchLabels(config, punchDefaults);
        return {
            ...publicPunchView(checklist, config, labels),
            items: Array.isArray(asObject(checklist).items)
                ? asObject(checklist).items.map(publicChecklistItem)
                : []
        };
    });
}
async function publicPortalResources(orgId, project, preview, portalCustomerSchedulingEnabled = false) {
    ...
    const calendarEvents = projectId
        ? (await listDocuments(orgId, "calendar_events").catch(() => []))
            .map((document) => ({ ...asObject(document.data), id: cleanText(document.id) }))
 ...
 */
async function publicProjectEngineDocuments(orgId, projectId, preview) {
    if (!projectId)
        return [];
    ...
    for (const record of records) {
        ...
        if (cleanText(data.project_id) !== projectId)
            continue;
        ...
        if (status === "void") {
            ...
            if (cleanText(cancellation.customer_visibility) === "hidden")
                continue;
            if (!cleanText(delivery.sent_at))
                continue;
            out.push({
        ...
        if (!preview && !PUBLIC_DOCUMENT_STATUSES.has(status))
            continue;
        if (preview && !PUBLIC_DOCUMENT_STATUSES.has(status) && status !== "issued")
            continue;
        ...
        const payments = Object.entries(outputDefs).flatMap(([outputKey, definitionValue]) => {
            ...
            if (cleanText(definition.type) !== "payment")
                return [];
...
async function publicPortalProjectBundle(orgId, document, portalData, project, preview, baseUrl, apiBaseUrl, accessUuid = "") {
 ...
 */
async function portalChatHandoff(orgId, project, customer, preview) {
    if (preview)
        return null;
    try {
        if (!(await isCapabilityEnabled(orgId, "apps.live_chat")))
            return null;
        ...
        if (typeof chat?.loadChatSettings !== "function" || typeof chat?.mintPortalGrant !== "function")
            return null;
        const settings = asObject(await chat.loadChatSettings(orgId, branchId));
        if (settings.enabled !== true)
            return null;
        if (asObject(settings.portal).enabled === false)
            return null;
 ...
 */
function blessedPortalTabs(resources, homePageId, relevance) {
...
function customerWriteTabs(settings, resources, relevance) {
    const tabs = [];
    ...
    const punchLists = Array.isArray(resources.punch_lists) ? resources.punch_lists : [];
 ...
 */
function applyPortalTabConfig(tabs, tabConfig) {
    const resolved = [];
 ...
 */
function portalTabDescriptors(resources, portalPages, tabConfig, homePageId, settings, relevance) {
    const blessed = applyPortalTabConfig([...blessedPortalTabs(resources, homePageId, relevance), ...customerWriteTabs(settings, resources, relevance)], tabConfig);
    const pages = Array.isArray(asObject(portalPages).pages) ? asObject(portalPages).pages : [];
...
async function publicCustomerPortalBranding(orgId, project, org, apiBaseUrl) {
...
function guestPortalBundle(value, allowedTabs) {
    ...
    if (allowedTabs.has("checklists"))
        resources.checklists = Array.isArray(sourceResources.checklists) ? sourceResources.checklists : [];
    if (allowedTabs.has("punch_lists"))
        resources.punch_lists = Array.isArray(sourceResources.punch_lists) ? sourceResources.punch_lists : [];
...
async function touchPortalGuestView(found) {
    if (found.access_mode !== "guest" || !found.guest)
        return;
    const now = new Date().toISOString();
    const links = normalizePortalGuestLinks(found.data.guest_links);
    ...
    if (Number.isFinite(prior) && Date.now() - prior < 5 * 60_000)
        return;
    target.last_viewed_at = now;
    await savePortalGuestLinks(found, links);
}
async function publicCustomerPortalPayload(uuid, preview, baseUrl, authOrgId = "", apiBaseUrl = baseUrl) {
    ...
    const guestAllowedTabs = new Set(found.access_mode === "guest" ? normalizeStringArray(found.guest?.tabs) : []);
    const guestProjectIds = new Set(found.access_mode === "guest" ? normalizeStringArray(found.guest?.project_ids) : []);
    ...
    if (contactId) {
        ...
        for (const portalDoc of portals) {
            const portalData = asObject(portalDoc.data);
            if (customerPortalContactId(portalData) !== contactId)
                continue;
            if (!preview && cleanText(portalData.status || "active") !== "active")
                continue;
            const siblingProjectId = cleanText(portalData.project_id);
            if (found.access_mode === "guest" && !guestProjectIds.has(siblingProjectId))
                continue;
            if (!siblingProjectId || seenProjectIds.has(siblingProjectId))
                continue;
            try {
    ...
    let portalPages = null;
    let portalSiteSettings = null;
    try {
        const websites = await import(("../websites/service.js"));
        const listed = typeof websites?.listPortalPages === "function" ? await websites.listPortalPages(found.orgId, audienceFacts) : null;
        portalPages = listed && typeof listed === "object" ? listed : null;
    ...
    const servedTabs = portalTabDescriptors(asObject(activeBundle.resources), portalPages, portalSiteSettings?.portal_tabs || {}, portalSiteSettings?.home_page_id || "", portalSettings, await projectTabRelevance(found.orgId, project.id).catch(() => null));
 ...
 */
async function customerWriteContext(portalUuid) {
...
async function customerSchedulingContext(portalUuid, spendWriteToken = false) {
    const found = requirePortalOwner(await findCustomerPortalByUuid(portalUuid, false));
    const portalData = asObject(found.document.data);
    if (cleanText(portalData.status || "active") !== "active")
        throw forbidden("portal_inactive", "This customer portal is not active.");
    if (cleanText(portalData.public_uuid) !== cleanText(portalUuid))
        throw forbidden("portal_preview_readonly", "Preview portals are read-only.");
    if (spendWriteToken && !consumePortalWriteToken(portalUuid))
        throw forbidden("portal_rate_limited", "Too many requests. Please slow down and try again shortly.");
...
async function portalOrgDefaults(orgId) {
    try {
        const websites = await import(("../websites/service.js"));
...
async function saveCustomerPortalData(orgId, document, patch) {
...
async function createCustomerUpload(request, portalUuid) {
    const context = await customerWriteContext(portalUuid);
    const typed = request;
    const parts = typed.parts?.();
    if (!parts)
        throw badRequest("multipart_required", "Uploads must be multipart/form-data.");
    let bytes = null;
    ...
    if (!bytes)
        throw badRequest("portal_upload_empty", "Choose a file to upload.");
...
async function withdrawCustomerUpload(portalUuid, mediaId) {
    const context = await customerWriteContext(portalUuid);
    ...
    if (!entry)
        throw notFound("portal_upload_not_found", "That upload was not found.");
...
async function listCustomerMediaComments(portalUuid, mediaId) {
...
async function createCustomerMediaComment(portalUuid, mediaId, input) {
    const context = await customerWriteContext(portalUuid);
...
async function withdrawCustomerMediaComment(portalUuid, mediaId, commentId) {
    const context = await customerWriteContext(portalUuid);
    ...
    if (!entry)
        throw notFound("portal_comment_not_found", "That comment was not found.");
    entry.withdrawn_at = new Date().toISOString();
    await saveCustomerPortalData(context.orgId, context.document, { media_comments: comments });
...
async function punchListContext(portalUuid, checklistId, options = {}) {
    ...
    if (!isPunchList(checklist))
        throw notFound("punch_list_not_found", "That list was not found.");
    ...
    if (!access.visible)
        throw forbidden("punch_list_hidden", "That list is not shared with you.");
...
async function savePunchConfig(orgId, projectId, checklistId, punch) {
    return (await patchProjectChecklist(orgId, projectId, checklistId, { metadata: { punch } }));
}
async function submitCustomerPunchList(portalUuid, checklistId, input, request) {
    ...
    await emitCustomerPortalWriteEvent(context.orgId, context.projectId, "punch_list.submitted", {
        ...
        item_count: Array.isArray(asObject(context.checklist).items) ? asObject(context.checklist).items.length : 0,
...
async function acceptCustomerPunchList(portalUuid, checklistId, input, request) {
    const context = await punchListContext(portalUuid, checklistId);
 ...
 */
async function completePunchListWork(orgId, projectId, checklistId, actorUserId) {
    ...
    return { punch_list: publicPunchView(updated, punchConfigOf(updated, punchDefaults), labels) };
}
/**
 ...
 */
async function reopenPunchList(orgId, projectId, checklistId) {
    const checklist = (await readProjectChecklistDetail(orgId, projectId, checklistId));
    if (!isPunchList(checklist))
        throw notFound("punch_list_not_found", "That list was not found.");
    const punchDefaults = asObject((await portalOrgDefaults(orgId)).punch_list);
 ...
 * kind of artifact.
 */
function portalSignatureEvidence(request) {
    return {
        ip: cleanText(request.ip),
        user_agent: cleanText(asObject(request.headers)["user-agent"]),
        captured_at: new Date().toISOString(),
        source: "customer_portal"
    };
}
/** Audit trail for a customer-authored write. Never throws into the request. */
async function emitCustomerPortalWriteEvent(orgId, projectId, type, payload = {}) {
    if (!projectId)
        return;
    try {
        await emitWorkEvent({
...
async function customerChecklistContext(portalUuid, checklistId) {
    const found = requirePortalOwner(await findCustomerPortalByUuid(portalUuid, false));
    const portalData = asObject(found.document.data);
    if (cleanText(portalData.status || "active") !== "active")
        throw forbidden("portal_inactive", "This customer portal is not active.");
    ...
    const access = normalizeChecklistCustomerAccess(checklist.customer_access || asObject(checklist.metadata).customer_access, isPunchList(checklist));
    if (!access.visible)
        throw forbidden("customer_checklist_hidden", "This checklist is not shared with the customer.");
    return { ...found, portalData, projectId, checklist, access };
}
function requireCustomerChecklistCompletion(access) {
...
function requireCustomerChecklistEditing(access) {
 ...
 */
async function requirePunchEditable(context, mode) {
    if (!isPunchList(context.checklist))
        return;
    const punchDefaults = asObject((await portalOrgDefaults(context.orgId)).punch_list);
    const config = punchConfigOf(context.checklist, punchDefaults);
    if (mode === "add")
        assertCustomerMayAddPunchItem(context.checklist, config);
    else
        assertCustomerMayEditPunchItem(config);
}
async function emitCustomerChecklistEvent(orgId, projectId, checklistId, itemId, type) {
    await emitWorkEvent({
        organization_id: orgId,
        branch_id: "default",
        project_id: projectId,
...
async function updateCustomerChecklistItem(portalUuid, checklistId, itemId, input) {
    ...
    if (Object.keys(input).some(field => !allowedFields.has(field)))
        throw badRequest("customer_checklist_field_forbidden", "This field cannot be changed through the customer portal.");
    const editingFields = authorFields.some(has);
    if (editingFields)
        requireCustomerChecklistEditing(context.access);
    if (completionFields.some(has) || !editingFields)
    ...
    if (cleanText(item.checklist_id) !== checklistId)
        throw notFound("checklist_item_not_found", "Checklist item was not found.");
    const updated = (await patchProjectChecklistItem(context.orgId, context.projectId, itemId, input, "customer_portal", context.access.can_edit_items));
    if (updated.completed !== item.completed) {
        await emitCustomerChecklistEvent(context.orgId, context.projectId, checklistId, itemId, updated.completed ? "customer.checklist.item_completed" : "customer.checklist.item_reopened");
    }
    return publicChecklistItem(updated);
}
async function createCustomerChecklistItem(portalUuid, checklistId, input) {
    const context = await customerChecklistContext(portalUuid, checklistId);
    requireCustomerChecklistEditing(context.access);
    ...
    if (Object.keys(input).some(field => !allowedFields.has(field)))
        throw badRequest("customer_checklist_field_forbidden", "This field cannot be set through the customer portal.");
    if (["completed", "status", "rating"].some(field => Object.prototype.hasOwnProperty.call(input, field)))
        requireCustomerChecklistCompletion(context.access);
...
async function deleteCustomerChecklistItem(portalUuid, checklistId, itemId) {
    const context = await customerChecklistContext(portalUuid, checklistId);
    requireCustomerChecklistEditing(context.access);
    await requirePunchEditable(context, "edit");
    const item = (await readProjectChecklistItem(context.orgId, context.projectId, itemId));
...
async function attachCustomerChecklistEvidence(request, portalUuid, checklistId, itemId) {
    const context = await customerChecklistContext(portalUuid, checklistId);
    ...
    const parts = typed.parts?.();
    if (!parts)
        throw badRequest("multipart_required", "Checklist evidence must be multipart/form-data.");
    let bytes = null;
    ...
    if (!bytes)
        throw badRequest("checklist_attachment_missing", "Choose a file to attach.");
    if (bytes.length > 128 * 1024 * 1024)
        throw badRequest("checklist_attachment_too_large", "Checklist evidence cannot exceed 128 MB.");
...
async function updateCustomerChecklistByVoice(request, portalUuid, checklistId) {
    const context = await customerChecklistContext(portalUuid, checklistId);
    requireCustomerChecklistCompletion(context.access);
    ...
    const parts = typed.parts?.();
    if (!parts)
        throw badRequest("multipart_required", "Checklist voice updates must be multipart/form-data.");
    let audio = null;
    ...
    if (!audio)
        throw badRequest("missing_audio", "Record a checklist update first.");
    const checklist = (await listProjectChecklists(context.orgId, context.projectId)).find((entry) => cleanText(entry.id) === checklistId);
    if (!checklist)
        throw notFound("checklist_not_found", "Checklist was not found.");
    ...
    const reconciled = reconcileChecklistAudioOperations(result.operations.filter((entry) => entry.confidence >= 0.55), (Array.isArray(checklist.items) ? checklist.items : []).map((value) => {
        const item = asObject(value);
        return {
            id: cleanText(item.id),
            title: cleanText(item.title),
    ...
    for (const operation of reconciled.operations) {
        try {
            if (operation.action === "complete") {
                ...
                if (cleanText(item.checklist_id) !== checklistId || item.completed === true)
                    continue;
                const updated = (await patchProjectChecklistItem(context.orgId, context.projectId, item.id, cleanText(item.item_type) === "rating" ? { rating: "good" } : { completed: true }, "customer_portal", context.access.can_edit_items));
                ...
                const duplicate = (Array.isArray(checklist.items) ? checklist.items : []).some((value) => cleanText(asObject(value).title).toLowerCase() === operation.title.toLowerCase());
                if (duplicate)
                    continue;
...
async function customerPortalFeedbackLink(orgId, project, preview) {
    if (preview)
        return null;
    try {
        ...
        if (!(await isCapabilityEnabled(orgId, "feedback.portal_card")))
            return null;
        ...
        if (settings.enabled === false || asObject(settings.channels).portal !== true)
            return null;
...
async function recordCustomerPortalEvent(uuid, body, request) {
    const found = await findCustomerPortalByUuid(uuid, false);
    const data = asObject(found.document.data);
    if (cleanText(data.status || "active") !== "active")
        throw forbidden("portal_inactive", "This customer portal is not active.");
    ...
    const activityDocument = await upsertDocument(found.orgId, "activity", {
        data: event,
        metadata: {
...
function sendInlineMediaFile(request, reply, file) {
    const bytes = file.bytes;
    const total = bytes.length;
    reply.header("Content-Type", file.contentType);
    reply.header("Content-Disposition", `inline; filename="${String(file.fileName).replace(/"/g, "")}"`);
    reply.header("Accept-Ranges", "bytes");
    const rangeHeader = cleanText(request.headers?.range);
...
async function listCustomerPortalActivity(orgId, projectId) {
...
async function sendCustomerPortalMedia(reply, uuid, mediaId, preview, request, authOrgId = "") {
    const found = await findCustomerPortalByUuid(uuid, preview);
    if (preview && authOrgId !== found.orgId) {
        throw forbidden("preview_requires_staff_session", "Preview links require a staff session for the same organization.");
    }
    ...
    if (found.access_mode === "guest") {
        if (!normalizeStringArray(found.guest?.tabs).includes("photos"))
            throw forbidden("portal_guest_scope", "This shared link does not include photos.");
        ...
        if (!candidate)
            throw forbidden("media_not_shared", "This media item is not available through the shared link.");
        data = candidate;
    }
    ...
    if (!shared.has(normalizedMediaId) && !preview) {
        ...
        if (!isPublishedPageAsset && !isUserProfileAsset)
            throw forbidden("media_not_shared", "This media item is not shared with the customer portal.");
    }
    const query = request.query && typeof request.query === "object" ? request.query : {};
...
function projectDocumentIdentity(item = {}) {
    return docFirstText(item.id, item.document_id, item.media_id, item.mediaId, item.url, item.href);
}
function documentTypeMeta(typeValue) {
    const type = cleanText(typeValue || "document").toLowerCase().replace(/[\s-]+/g, "_");
    if (type === "proposal")
        return { type, label: "Proposal", icon: "fa-file-signature", color: "#2563eb" };
    if (type === "change_order" || type === "changeorder")
        return { type: "change_order", label: "Change Order", icon: "fa-file-contract", color: "#7c3aed" };
    if (type === "roof_report" || type === "report" || type === "measurement_report")
        return { type: "roof_report", label: "Roof Report", icon: "fa-ruler-combined", color: "#059669" };
    if (type === "customer_report" || type === "summary")
        return { type: "customer_report", label: "Customer Report", icon: "fa-file-lines", color: "#0891b2" };
    if (type === "instant_report")
        return { type, label: "Instant Report", icon: "fa-bolt", color: "#ca8a04" };
    if (type === "weather_report")
        return { type, label: "Weather Report", icon: "fa-cloud-sun-rain", color: "#0d9488" };
    if (type === "receipt")
        return { type, label: "Receipt", icon: "fa-receipt", color: "#0f766e" };
    if (type === "required")
        return { type, label: "Required", icon: "fa-clipboard-check", color: "#dc2626" };
    return { type: type || "document", label: "Document", icon: "fa-file-lines", color: "#64748b" };
}
function documentReferenceFromMedia(media = {}, extra = {}) {
...
function normalizeProjectDocumentReference(input) {
...
function normalizeProjectUploadedDocuments(value) {
    return (Array.isArray(value) ? value : [])
...
function proposalDocumentType(proposal = {}) {
...
function proposalPdfMediaRef(proposal = {}) {
...
function addReportDocument(docs, input) {
...
function projectReportDocuments(projectData = {}) {
    const docs = [];
...
async function projectProposalDocuments(orgId, projectData = {}) {
    const projectId = docFirstText(projectData.id, projectData.platform_project_id, projectData.base_project_id);
    const candidates = [];
    for (const proposal of Array.isArray(projectData.proposals) ? projectData.proposals : [])
        candidates.push(asObject(proposal));
...
async function projectDocumentsPayload(orgId, projectDocument) {
    const project = asObject(projectDocument.data);
...
function collectionReadPermission(collection) {
    if (collection === "users")
        return "manage_company_users|manage_company_user_permissions|manage_users|manage_sales_users";
    if (collection === CUSTOMER_PORTAL_COLLECTION)
        return "view_reports";
    return undefined;
}
function assertCanonicalPlatformCollection(collection) {
...
function collectionWritePermission(collection, operation = "domain") {
    if (collection === "projects")
        return "manage_projects";
    if (collection === "customers")
        return "manage_projects";
    if (collection === "users")
        return "manage_company_users|manage_users|manage_sales_users";
    if (collection === "branch")
        return "manage_company_settings";
    if (collection === NOTIFICATION_COLLECTION)
        return "manage_company_settings";
    if (collection === "activity")
        return operation === "create" ? undefined : "manage_company_settings";
    if (collection === "calendar_events")
        return "manage_schedule";
    if (collection === CUSTOMER_PORTAL_COLLECTION)
        return "manage_projects|order_reports|view_reports";
    return undefined;
}
function branchModuleWritePermission(moduleId) {
    if (moduleId === "pricebook" || moduleId === "presentation_style")
        return "manage_company_settings";
    return undefined;
}
...
function acquisitionRequestMetadata(request) {
...
function withAcquisitionRequestMetadata(input, request) {
    if (!request)
        return input;
    const requestMetadata = acquisitionRequestMetadata(request);
    return {
...
function extractDataBody(body) {
    if (body.data && typeof body.data === "object" && !Array.isArray(body.data))
        return body.data;
    return body;
}
function extractMetadataBody(body) {
    return body.metadata && typeof body.metadata === "object" && !Array.isArray(body.metadata)
        ? body.metadata
        : {};
}
function tryParseJsonField(value, fallback = undefined) {
    if (value == null)
        return fallback;
    if (typeof value !== "string")
        return value;
    ...
    if (!trimmed)
        return fallback;
    if (!/^[\[{"]/u.test(trimmed) && !/^(true|false|null|-?\d)/u.test(trimmed))
        return value;
    try {
...
async function parsePortalActionRequest(request) {
    const typed = request;
    const contentType = String(typed.headers?.["content-type"] ?? "");
    if (!contentType.includes("multipart/form-data"))
        return objectBodySchema.parse(typed.body ?? {});
    const fields = {};
    let file = null;
    const parts = typed.parts?.();
    if (!parts)
        throw badRequest("multipart_unavailable", "Multipart uploads are unavailable on this route.");
    for await (const part of parts) {
        if (part.type === "file") {
...
async function handlePortalAction(app, action, body, request, reply) {
    ...
    if (action === "referral_public_lookup")
        return publicReferralLookup(cleanText(body.referral_code || body.code || body.ref), request ? publicRequestBaseUrl(request) : "");
    if (action === "acquisition_public_track")
        return publicAcquisitionLookup(withAcquisitionRequestMetadata(body, request), request ? publicRequestBaseUrl(request) : "");
    if (action === "admin_stop_impersonation") {
        if (!request || !reply)
            return { success: false, status_code: 500, error: "missing_response_context" };
        await requirePlatformAuth(request, { application: false });
        return await stopPortalImpersonation(app, request, reply);
    }
    if (!request)
        return { success: false, status_code: 401, error: "Authentication required." };
    if (!(await authContextFromRequest(request)))
        return { success: false, status_code: 401, error: "Authentication required." };
    ...
    switch (action) {
        ...
        case "org_users_set_perms_my":
            if (request)
                await requirePlatformAuth(request, { orgId: ctx.orgId, permission: "manage_company_user_permissions" });
        ...
        case "portal_acquisition_bonus_offer_status":
            if (isLocalPortalRequest(request) && truthy(body.bonus_test)) {
                ...
                if (campaign && token)
                    return await acquisitionBonusOfferForCampaignToken(ctx.orgId, campaign, token, true, withAcquisitionRequestMetadata(body, request));
            }
        ...
        case "exteriors_upload": {
            ...
            if (!bytes.length || bytes.length > 8 * 1024 * 1024)
                throw badRequest("invalid_reference", "Each reference must be an image up to 8 MB.");
            ...
            if (!format)
                throw badRequest("invalid_reference", "Use JPG, PNG or WebP reference photos.");
...
async function stopPortalImpersonation(app, request, reply) {
    if (!request || !reply)
        return { success: false, status_code: 500, error: "missing_response_context" };
    const ctx = await authContextFromRequest(request);
    if (!ctx)
        return { success: false, status_code: 401, error: "authentication_required" };
    ...
    if (restoreSessionId) {
        try {
            ...
            if (currentSessionId && currentSessionId !== restoreSessionId)
                await deleteAuthSession(currentSessionId);
...
async function handleAuthLegacyAction(request, reply, body) {
    ...
    if (action === "forgot_password" || action === "forgot_password_sms") {
        ...
        if (wantsSms) {
            ...
            if (cleanText(previousReset.channel).toLowerCase() === "sms"
            ...
            if (pendingSmsRequest)
                return await pendingSmsRequest;
            ...
            try {
                ...
                if (passwordResetSmsRequests.get(smsRequestKey) === smsRequest)
                    passwordResetSmsRequests.delete(smsRequestKey);
            }
        }
    ...
    if (action === "set_new_password") {
        ...
        if (password.length < 6)
            return { success: false, ok: false, status_code: 400, error: "Password too short" };
        const identity = await resolvePasswordRecoveryIdentity(body);
        const reset = asObject(asObject(identity.metadata).password_reset);
...
function authResetCodeHash(identityId, code) {
    return createHash("sha256").update(`${identityId}:${code}`).digest("hex");
}
function authResetCodeValid(identity, reset, code) {
    const identityId = String(identity.id || "");
    const expected = cleanText(reset.code_hash);
    if (!identityId || !expected || !code || Number(reset.expires_at || 0) < Date.now())
        return false;
...
function portalActivateUrl(request, email) {
...
async function sendPlatformOrgUserInvite(orgId, userDoc, request, actorEmail = "") {
    const data = asObject(userDoc.data);
...
async function portalContext(actor, body) {
    let orgId = actor.organization_id || cleanText(body.org_id || body.organization_id);
    let userDoc = null;
    if (orgId)
        userDoc = await findPortalUserByEmail(orgId, actor.email);
    if (!orgId || !userDoc) {
    ...
    if (!orgId)
        throw badRequest("missing_org", "A portal organization is required.");
    if (!userDoc)
        userDoc = await findPortalUserByEmail(orgId, actor.email);
    return { orgId, userDoc };
}
async function findPortalUserByEmail(orgId, email) {
...
function portalUserView(userDoc) {
    const doc = asObject(userDoc);
    const data = asObject(doc.data);
...
function effectivePortalPermissions(level, items = {}) {
    ...
    if (presets[normalized])
        return { ...presets[normalized], ...items };
    return Object.keys(items).length ? items : presets.viewer;
}
async function portalOrgView(orgId) {
...
async function portalCredits(orgId, userDoc) {
    const global = await readGlobal(orgId);
    const data = asObject(global.data);
...
function portalResolveLogo(logo, branding) {
    if (!logo)
        return "";
    if (/^(https?:|data:)/i.test(logo))
        return logo;
    if (logo.startsWith("/v1/"))
        return logo.replace(/\/file(\?|$)/, "/logo$1");
    const importedLogo = cleanText(branding.logo_node_url);
    if (importedLogo && (logo.startsWith("/") || logo.startsWith("organizations/")))
        return importedLogo;
    if (logo.startsWith("/"))
        return logo.startsWith("/storage/") ? logo : "";
    if (logo.startsWith("organizations/"))
        return "";
    return logo;
}
async function portalUpdateOrg(orgId, body, userDoc = null) {
    if (cleanText(body.name))
        await patchOrganization(orgId, { name: cleanText(body.name) });
    ...
    if ((fullName || phone) && userDoc) {
        ...
        if (identityId) {
            ...
            if (phone)
                body.phone = updatedIdentity.phone;
        }
        await upsertDocument(orgId, "users", {
            id: String(userDoc.id || ""),
            data: {
...
async function portalUpdateReportSettings(orgId, body) {
...
async function portalUpdateBilling(orgId, body) {
...
async function portalBillingHistory(orgId, body) {
    const global = await readGlobal(orgId);
    const data = asObject(global.data);
...
async function portalMonthlyStatement(orgId, body) {
...
async function portalOrgUsers(orgId) {
    return (await listDocuments(orgId, "users")).map(portalUserView).filter((user) => !user.deleted);
}
async function portalAddOrgUser(orgId, body, request) {
...
async function portalUpdateOrgUser(orgId, body) {
...
async function portalSetOrgUserDisabled(orgId, body) {
    const userId = cleanText(body.user_id);
    const current = await readDocument(orgId, "users", userId);
    const data = asObject(current.data);
...
async function portalSetOrgUserPermissions(orgId, body, actorUserDoc = null) {
    const userId = cleanText(body.user_id);
    const current = await readDocument(orgId, "users", userId);
...
async function portalUploadLogo(orgId, body) {
    const file = asObject(body.__file);
    if (!file.bytes_base64)
        throw badRequest("missing_logo", "A logo file is required.");
    const media = await storeMediaUpload(orgId, {
...
async function portalUploadAvatar(orgId, body) {
    const userId = cleanText(body.user_id);
    const file = asObject(body.__file);
    if (!file.bytes_base64)
        throw badRequest("missing_avatar", "An avatar file is required.");
    const media = await storeMediaUpload(orgId, {
        ownerType: "user",
        ownerId: userId,
        slot: "avatar",
...
function signupEmailVerification(identity) {
    return asObject(asObject(identity.metadata).signup_email_verification);
}
function signupEmailIsVerified(identity) {
...
function signupOtpCodeHash(identityId, code) {
    return createHash("sha256").update(`${identityId}:signup:${code}`).digest("hex");
}
function signupOtpCodeValid(identity, verification, code) {
    const identityId = String(identity.id || "");
    const expected = cleanText(verification.code_hash);
    if (!identityId || !expected || !code || Number(verification.expires_at || 0) < Date.now())
        return false;
    return expected === signupOtpCodeHash(identityId, code);
}
function onboardingCompletionData(actorEmail, body) {
    const now = new Date().toISOString();
    return {
...
async function portalOnboardingTrack(orgId, actorEmail, body, request) {
...
async function sendSignupVerificationOtpEmail(email, code) {
    ...
    <p>This code expires in 10 minutes.</p>`;
    const sent = await sendPostmarkEmail({ to: email, subject, textBody, htmlBody });
    if (!sent.ok && !env.isProduction && sent.error === "Postmark token missing")
        return { ok: true, skipped: true };
    return sent;
}
async function portalOnboardingSignupVerificationStart(orgId, userDoc, actorEmail, body) {
    const identity = await findIdentityByEmail(actorEmail);
    const targetEmail = validEmail(body.email || identity.email);
    if (!targetEmail)
        return { success: false, status_code: 400, error: "A valid email address is required." };
    ...
    return {
        ...
        ...(env.isProduction ? {} : { dev_otp: code, email_skipped: Boolean(postmark.skipped) })
    };
}
async function portalOnboardingSignupVerificationConfirm(orgId, userDoc, actorEmail, body) {
    const identity = await findIdentityByEmail(actorEmail);
    const verification = signupEmailVerification(identity);
    const code = cleanText(body.otp || body.code);
...
async function portalOnboardingComplete(orgId, actorEmail, body) {
...
function signupAttributionPayload(input) {
    const fields = [
        "referral_code",
...
async function applyAcquisitionSignup(orgId, input) {
    ...
    if (!completed.success)
        return completed;
    if (cleanText(completed.acquisition_source_type) !== "referral")
        return completed;
...
async function portalReferralStatus(orgId) {
...
function bonusTimestampMs(...values) {
    for (const value of values) {
...
function bonusProjectData(value) {
...
function bonusProjectUsageAmount(project) {
    const measurement = asObject(project.measurement_project ?? project.measurement);
    const raw = asObject(measurement.raw);
    const manifest = asObject(project.manifest);
    return firstPositiveMoney(project.amount_charged, measurement.amount_charged, raw.amount_charged, manifest.amount_charged, project.charged_amount, project.charge_amount, project.revenue);
}
function bonusProjectUsageDateMs(project) {
    const measurement = asObject(project.measurement_project ?? project.measurement);
    const raw = asObject(measurement.raw);
    const manifest = asObject(project.manifest);
    const timestamps = asObject(raw.timestamps);
    return bonusTimestampMs(measurement.submitted_at, measurement.queued_at, measurement.created_at, raw.submitted_at, raw.queued_at, raw.created_at, manifest.created_at, timestamps.created_at, timestamps.queued_at, project.submitted_at, project.queued_at, project.completed_at, project.uploaded_at, project.created_at, project.updated_at);
}
function bonusRunRateRoundingIncrement(twoMonthValue) {
...
function bonusRunRateTier(id, label, months, matchPercent, monthlyUsage, customerPays, roundingIncrement) {
...
function normalizedBonusTierSpec(value, index) {
...
function normalizedBonusOfferConfig(value) {
...
async function bonusRunRateOffer(orgId, configInput = {}) {
    ...
    const signupMs = bonusTimestampMs(organization.created_at, metadata.created_at, metadata.signup_at, metadata.signed_up_at, globalData.created_at, globalData.signup_at, legacyGlobal.created_at, legacyGlobal.signup_at, legacyMetadata.created_at, legacyMetadata.signup_at) || (Number.isFinite(firstOrderMs) ? firstOrderMs : 0);
    ...
    const tiers = roundedBaseValue > 0
        ? config.tiers.map((tier) => bonusRunRateTier(tier.id, tier.label, tier.months, tier.match_percent, monthlyUsage, Math.max(0, Math.round(roundedBaseValue * tier.multiplier)), roundingIncrement))
        : [];
    return {
        tiers,
        basis: {
...
function bonusOfferInstanceStatus(instanceInput, nowMs = Date.now()) {
    ...
    if (viewedAt && expiresAt && nowMs > expiresAt)
        return "expired";
    return viewedAt ? "viewed" : "available";
}
function bonusOfferInstanceSecondsRemaining(instanceInput, nowMs = Date.now()) {
    const instance = asObject(instanceInput);
...
function sortedBonusOfferInstances(value) {
...
function selectPortalBonusOfferInstance(globalData, nowMs = Date.now()) {
...
async function markBonusOfferInstanceViewed(orgId, instanceInput) {
    const id = cleanText(instanceInput.id);
    if (!id)
        return { instance: instanceInput, firstView: false };
    ...
    if (cleanText(current.viewed_at || current.first_shown_at))
        return { instance: current, firstView: false };
    const next = {
        ...current,
        id,
        status: "viewed",
...
function publicBonusOfferInstance(instanceInput) {
    const instance = asObject(instanceInput);
    return {
...
function apiRequestBaseUrl(request) {
...
async function portalBonusStatus(orgId) {
...
async function stripeApiRequest(method, apiPath, fields = {}, idempotencyKey = "") {
    ...
    const response = await fetch(url, init).catch((error) => ({ ok: false, status: 0, text: async () => String(error) }));
    ...
    return { success: true, http: response.status, data: data };
}
async function stripeCreateCustomer(orgId) {
    const org = await portalOrgView(orgId);
    ...
    if (!customerId)
        return { success: false, error: "Stripe customer response was missing an id.", stripe: result.data };
...
async function stripeCustomerIdForOrg(orgId) {
    const global = await readGlobal(orgId);
    ...
    if (!created.success)
        throw badRequest("stripe_customer_create_failed", cleanText(created.error) || "Could not create Stripe customer.");
    customerId = cleanText(asObject(created).customer_id);
    if (!customerId)
        throw badRequest("stripe_customer_create_failed", "Could not create Stripe customer.");
    return customerId;
}
async function stripeBonusQuote(orgId, qty, offerInstanceId = "") {
    const offerEnabled = await isAppFlagEnabled(orgId, "firstmeasure", "bonus_upfront_match").catch(() => false);
    if (!offerEnabled)
        return { valid: false, reason: "feature_disabled", tier_id: "", bonus_dollars: 0, total_account_value: qty };
    if (offerInstanceId) {
        const global = await readGlobal(orgId);
        const data = asObject(global.data);
        const instance = asObject(bonusOfferInstances(data.bonus_offer_instances)[offerInstanceId]);
        if (!cleanText(instance.id))
            return { valid: false, reason: "offer_instance_not_found", tier_id: "", bonus_dollars: 0, total_account_value: qty };
        const status = bonusOfferInstanceStatus(instance);
        if (status !== "viewed")
            return { valid: false, reason: status === "expired" ? "offer_expired" : "offer_not_viewed", tier_id: "", bonus_dollars: 0, total_account_value: qty };
        ...
        if (!tier)
            return { valid: false, reason: "invalid_amount", tier_id: "", bonus_dollars: 0, total_account_value: qty, offer_instance_id: offerInstanceId };
        return {
            valid: true,
            reason: "",
...
async function portalStripeCreateCheckout(orgId, email, body) {
    ...
    let useBonus = false;
    if (offerId) {
        if (offerId !== "bonus_upfront_match_v1" && offerId !== "acquisition_bonus_offer_v1")
            return { success: false, error: "This offer is not fulfilled through Stripe checkout." };
        ...
        if (!quote.valid)
            return { success: false, error: "This amount is not available for the bonus offer.", offer_id: offerId, reason: quote.reason, quote };
    ...
    if (useBonus) {
        ...
        if (offerToken)
            fields["metadata[offer_token]"] = offerToken;
        if (offerInstanceMetaId)
            fields["metadata[offer_instance_id]"] = offerInstanceMetaId;
        if (offerRolloutId)
            fields["metadata[bonus_rollout_id]"] = offerRolloutId;
        if (offerSetId)
            fields["metadata[offer_set_id]"] = offerSetId;
        if (offerLabel)
            fields["metadata[offer_label]"] = offerLabel;
        if (matchPercent)
            fields["metadata[match_percent]"] = matchPercent;
    }
    ...
    if (!created.success)
        return { success: false, error: created.error || "Stripe checkout failed.", stripe: created.stripe || created.data };
...
async function portalStripeStartSetup(orgId, body) {
    const customerId = await stripeCustomerIdForOrg(orgId);
    const global = await readGlobal(orgId);
    ...
    if (!created.success)
        return { success: false, error: "Setup session failed", stripe: created.stripe || created.data };
    return { success: true, url: asObject(created.data).url || null, session: created.data };
}
async function portalStripeFinishSetup(sessionId) {
    if (!sessionId)
...
async function stripeRetrieveCheckoutSession(sessionId) {
    ...
    if (!result.success)
        return { success: false, error: result.error || "Stripe retrieve error", stripe: result.stripe || result.data };
    return { success: true, session: result.data };
}
async function stripeRetrieveCheckoutSessionExpanded(sessionId) {
    if (!sessionId)
        return { success: false, error: "Missing session_id" };
...
async function portalStripeFulfillSession(sessionId, source) {
...
async function stripeFulfillFromSession(session, source) {
    const sessionId = cleanText(session.id);
    if (!sessionId)
        return { success: false, error: "Missing session id" };
    const liveMode = session.livemode === true;
    if (liveMode !== !stripeIsTestMode())
        return { success: false, error: "Livemode mismatch", livemode: liveMode, test_mode: stripeIsTestMode(), session_id: sessionId };
    const paymentStatus = cleanText(session.payment_status);
    if (paymentStatus !== "paid")
        return { success: false, error: "Not paid yet", payment_status: paymentStatus, session_id: sessionId };
    ...
    if (!orgId && email)
        orgId = await portalOrgIdForEmail(email);
    const dollars = Math.max(0, Math.round(numericValue(meta.credit_dollars || meta.credits_qty)));
    if (!email || !orgId || dollars < 1)
        return { success: false, error: "Missing metadata", user_email: email, org_id: orgId, credit_dollars: dollars, session_id: sessionId };
    const global = await readGlobal(orgId);
    const data = asObject(global.data);
    ...
    if (fulfilled[sessionId])
        return { success: true, duplicate: true, session_id: sessionId };
    ...
    const metaCapi = await metaCapiPurchaseFromStripeSession({
        ...
        source
    }).catch((error) => ({ ok: false, skipped: false, error: cleanText(error?.message || "meta_capi_error") }));
    const latest = await readGlobal(orgId);
    const latestData = asObject(latest.data);
...
function metaAttributionFields(body) {
    const fields = {};
    for (const key of ["fbc", "fbp", "_fbc", "_fbp"]) {
        const value = cleanText(body[key]);
        if (value)
            fields[key.replace(/^_/, "")] = value.slice(0, 500);
    }
    return fields;
}
...
async function metaCapiPurchaseFromStripeSession(input) {
    ...
    if (!accessToken && !env.metaCapiTestMode)
        return { ok: false, skipped: true, reason: "missing_access_token" };
    if (!input.sessionId || !input.email || !Number.isFinite(input.value) || input.value <= 0)
        return { ok: false, skipped: true, reason: "invalid_purchase_event" };
    const userData = { em: [metaHash(input.email)] };
    if (input.fbc)
    ...
    if (env.metaCapiTestMode)
        return { ok: true, skipped: false, test_mode: true, event };
    ...
    if (!response.ok)
        return { ok: false, skipped: false, http: response.status, response: data };
    return { ok: true, skipped: false, http: response.status, response: data };
}
async function stripeSavePaymentMethodFromCheckout(orgId, session) {
    ...
    if (!orgId || !customerId || !paymentIntentId)
        return;
    ...
    if (pmId)
        await stripeSaveCardToOrg(orgId, customerId, pmId, pm, "payment_method_saved_via_checkout");
}
async function stripeSavePaymentMethodFromSession(session, eventType) {
    const meta = asObject(session.metadata);
    ...
    if (!orgId || !customerId || !setupIntentId)
        return { success: false, error: "Missing setup metadata" };
    const result = await stripeApiRequest("GET", `/v1/setup_intents/${encodeURIComponent(setupIntentId)}`, { "expand[]": "payment_method" });
    if (!result.success)
        return { success: false, error: "Could not retrieve setup intent", stripe: result.stripe || result.data };
    const pm = asObject(asObject(result.data).payment_method);
    const pmId = cleanText(pm.id);
    if (!pmId)
        return { success: false, error: "Setup intent has no payment method" };
...
async function stripeSaveCardToOrg(orgId, customerId, paymentMethodId, paymentMethod, eventType) {
...
async function stripePatchBilling(orgId, patch, eventType = "", eventMeta = {}) {
    const global = await readGlobal(orgId);
    const data = asObject(global.data);
    ...
    if (eventType)
        events.push({ ts: new Date().toISOString(), type: eventType, meta: eventMeta });
    next.events = events;
    await saveGlobal(orgId, { data: { billing: next } });
    return next;
}
async function stripeMaybeAutoTopup(orgId, actorEmail, balanceAfterSpend, triggerEntry) {
    const profile = await organizationProfile(orgId);
    const global = await readGlobal(orgId);
    const data = asObject(global.data);
    const billing = asObject(data.billing);
    ...
    if (balanceAfterSpend > threshold)
        return null;
    ...
    if (status === "succeeded" && (Number(paymentIntent.amount) !== creditMinorAmount(topup, profile) || String(paymentIntent.currency).toUpperCase() !== profile.currency || paymentIntent.livemode !== !stripeIsTestMode()))
        throw conflict("payment_amount_mismatch", "Payment amount or currency mismatch.");
    if (status !== "succeeded") {
        await stripePatchBilling(orgId, {
            auto_topup: {
                enabled: false,
...
async function portalOrgIdForEmail(email) {
    try {
        const identity = await findIdentityByEmail(email);
        const memberships = await listIdentityMemberships(String(identity.id || ""));
        const first = memberships[0] ? asObject(memberships[0]) : {};
...
async function portalStripeWebhookProxy(body) {
    const signature = cleanText(body.signature);
    const payload = Buffer.from(cleanText(body.payload_base64), "base64").toString("utf8");
    if (!payload || !signature)
        return { success: false, status_code: 400, error: "Missing payload/signature" };
    if (!verifyStripeSignatureCompat(payload, signature))
        return { success: false, status_code: 400, error: "Invalid signature" };
    const event = tryParseJsonField(payload, null);
    if (!event || !cleanText(event.id) || !cleanText(event.type))
        return { success: false, status_code: 400, error: "Invalid event JSON" };
    const liveMode = event.livemode === true;
    if (liveMode !== !stripeIsTestMode())
        return { success: true, ignored: true, reason: "livemode_mismatch", livemode: liveMode, test_mode: stripeIsTestMode() };
    const eventType = cleanText(event.type);
    const object = asObject(asObject(event.data).object);
    if (await (await import("../platform-billing/subscriptions.js")).subscriptionWebhook(event))
    ...
    if (eventType === "checkout.session.completed" || eventType === "checkout.session.async_payment_succeeded") {
        ...
        if (cleanText(session.mode) === "setup")
            return await stripeSavePaymentMethodFromSession(session, "stripe_setup_webhook");
...
function verifyStripeSignatureCompat(payload, signature) {
    const secrets = stripeWebhookSecrets();
    if (!payload || !signature || !secrets.length)
        return false;
...
async function portalFetchImage(body) {
    const url = cleanText(body.url);
    if (!url)
        return { success: false, error: "Image URL is required." };
    if (url.startsWith("data:"))
        return { success: true, data_url: url };
    if (!/^https?:\/\//i.test(url))
        return { success: false, error: "Only http/https image URLs are supported." };
    const response = await fetch(url, {
    ...
    if (!contentType.startsWith("image/"))
        return { success: false, error: "Not an image URL." };
    return { success: true, data_url: `data:${contentType};base64,${bytes.toString("base64")}` };
}
async function portalScrapeLogos(body) {
    const url = cleanText(body.url);
    if (!/^https?:\/\//i.test(url))
    ...
    for (const match of html.matchAll(/<meta\s[^>]*(?:property|name)=["'](?:og:image|twitter:image)["'][^>]*content=["']([^"']+)["'][^>]*>/gi))
        add(String(match[1] || ""), 140, "meta");
    for (const match of html.matchAll(/<link\s[^>]*rel=["'][^"']*(?:icon|apple-touch-icon|preload)[^"']*["'][^>]*href=["']([^"']+)["'][^>]*>/gi))
        add(String(match[1] || ""), 120, "link");
    for (const match of html.matchAll(/<img\s[^>]*(?:src|data-src)=["']([^"']+)["'][^>]*>/gi)) {
        const tag = match[0].toLowerCase();
        const src = String(match[1] || "");
        let score = 20;
        if (tag.includes("logo"))
            score += 140;
        if (tag.includes("brand"))
            score += 70;
        if (tag.includes("header"))
            score += 30;
        if (tag.includes("avatar") || tag.includes("photo") || tag.includes("banner"))
            score -= 80;
        add(src, score, "img");
    }
    return {
...
function firstMeasureInstantAddon(projectType) {
    return projectType === "commercial" || projectType === "multifamily" ? 4 : 2;
}
function isReportExpediteKey(value) {
    return isExpeditedReportExpediteKey(value);
}
async function assertPortalFirstMeasureFlag(orgId, flag, message) {
    if (await isAppFlagEnabled(orgId, "firstmeasure", flag).catch(() => false))
        return;
    throw forbidden("feature_disabled", message);
}
async function assertPortalPlatformFlag(orgId, flag, message) {
    if (await isAppFlagEnabled(orgId, "platform", flag).catch(() => false))
        return;
    throw forbidden("feature_disabled", message);
}
...
function firstMeasureReportAmount(body, projectType, reportMode, pins) {
    return sharedFirstMeasureReportAmount({ ...body, project_type: projectType, report_mode: reportMode, pins });
}
function isPortalStructurePinLimitedType(projectType) {
...
function portalPinLimitForType(projectType) {
    const normalized = cleanText(projectType).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
    if (normalized === "residential")
        return MAX_PORTAL_RESIDENTIAL_PINS;
    if (isPortalStructurePinLimitedType(normalized))
        return MAX_PORTAL_STRUCTURE_PINS;
    return 0;
}
function portalPinLimitMessage(maxPins) {
    return `Maximum of ${maxPins} pins per report. Remove a pin to place a new one.`;
}
function firstMeasureReportExpediteDiscount(body, projectType, pins) {
    const expediteKey = cleanText(body.report_expedite_option).toLowerCase();
    if (!isExpeditedReportExpediteKey(expediteKey))
        return 0;
...
function firstMeasureReportCharge(body, projectType, reportMode, pins, freeExpediteUses) {
...
function reportExpediteWindowLabel(start, end) {
...
function reportExpediteOptionFromQuote(projectType, optionKey, structureCount = 1) {
...
function firstMeasureManifestOrderAmount(manifest, optionKey) {
...
function firstMeasureManifestExpediteUpgradeBaseline(manifest) {
    const projectType = cleanText(manifest.project_type) || "residential";
    const pins = Array.isArray(manifest.pins) ? manifest.pins : [];
    return firstMeasureReportAmount({
...
function parseFirstMeasureTimestampMs(...values) {
    for (const value of values) {
        const text = cleanText(value);
        if (!text)
            continue;
...
function firstMeasureManifestDueEnd(manifest) {
    ...
    if (!Number.isFinite(submittedAt))
        return null;
    const pins = Array.isArray(manifest.pins) ? manifest.pins : [];
    const option = reportExpediteOptionFromQuote(cleanText(manifest.project_type) || "residential", cleanText(manifest.report_expedite_option) || "standard_3_6", Math.max(1, pins.length));
    const endMinutes = numericValue(option?.end_minutes, 360);
    return addMinutes(new Date(submittedAt), endMinutes);
}
async function firstMeasureProjectDetail(app, projectId) {
...
function assertPortalOwnsFirstMeasureProject(manifest, orgId, actor) {
    ...
    if (manifestOrgId && manifestOrgId === orgId)
        return;
    if (issuerEmail && issuerEmail === actor.email.toLowerCase())
        return;
    throw forbidden("project_access_denied", "You do not have access to this report order.");
}
async function portalExpediteQueuedProject(app, orgId, actor, body) {
    ...
    if (manifest.measurement_scope === "full_house")
        throw badRequest("exterior_delivery_locked", "Full House delivery is selected when ordering and cannot be changed here.");
    ...
    if (!option || !option.expedited || option.end_minutes == null)
        throw badRequest("invalid_expedite_option", "Choose a valid expedited delivery option.");
    const now = new Date();
    ...
    if (delta <= 0)
        throw badRequest("expedite_not_billable", "This expedite option would not increase the order total.");
    ...
    const patch = {
        ...
        workflow: {
            ...
            history: [
                ...(Array.isArray(asObject(manifest.workflow).history) ? asObject(manifest.workflow).history : []),
                {
                    ts: now.toISOString(),
    ...
    if (response.statusCode >= 400 || data.ok === false || data.success === false) {
        if (chargeDelta > 0)
            await applyCreditDelta(orgId, { amount: chargeDelta, reason: "order_expedite_upgrade_refund", meta: { project_id: projectId, failed_expedite: data } }, actor.email);
...
async function portalCancelQueuedProject(app, orgId, actor, body) {
    ...
    if (response.statusCode >= 400 || data.ok === false || data.success === false) {
        if (refundAmount > 0)
            await applyCreditDelta(orgId, { amount: -refundAmount, reason: "order_cancellation_refund_reversal", meta: { project_id: projectId, failed_cancel: data } }, actor.email).catch(() => null);
...
async function portalSubmitReportReworkRequest(app, orgId, actor, body) {
    ...
    if (!notes && requestType !== "additional_structure")
        throw badRequest("missing_request_notes", "Please describe what you need.");
    if (!notes)
        notes = "Additional structure requested by pin.";
    const projectType = cleanText(manifest.project_type).toLowerCase() || "residential";
    const existingRequests = Array.isArray(manifest.report_change_requests) ? manifest.report_change_requests : [];
    const now = new Date();
    ...
    const history = Array.isArray(asObject(manifest.workflow).history) ? asObject(manifest.workflow).history : [];
    const patch = {
...
function normalizeReportReworkRequestType(value) {
    const raw = cleanText(value).toLowerCase().replace(/[\s-]+/g, "_");
    if (raw === "issue" || raw === "report_issue")
        return "report_issue";
    if (raw === "change" || raw === "correction" || raw === "request_change" || raw === "request_correction" || raw === "change_correction")
        return "change_correction";
    if (raw === "additional_structure" || raw === "add_structure" || raw === "structure")
        return "additional_structure";
    throw badRequest("invalid_rework_request_type", "Choose a valid request type.");
}
function reportReworkRequestLabel(type) {
    if (type === "report_issue")
        return "Reported issue";
    if (type === "additional_structure")
        return "Additional structure";
    return "Change or correction";
}
function normalizeReportRequestPins(value) {
...
function normalizeReportRequestPhotos(value) {
    const parsed = typeof value === "string" ? tryParseJsonField(value, []) : value;
...
function reportIssueEmailAttachments(request) {
...
async function sendReportIssueSupportEmail(input) {
    ...
    const company = cleanText(organization?.name
        ...
        || organizationRef.name) || input.orgId;
...
async function assertBranchReportPreferences(orgId, branchId, body) {
    ...
    if (payload.localization !== undefined) {
        ...
        if (!await isAppFlagEnabled(orgId, "firstmeasure", "report_localization")) {
            ...
            if ([...new Set([...Object.keys(requestedLocalization), ...Object.keys(existing)])].some(key => requestedLocalization[key] !== existing[key]))
                throw forbidden("localization_disabled", "Language customization is not enabled for this organization.");
        }
    }
...
function normalizedPortalQueuePayload(body, actor, orgId, projectType, reportMode, charge) {
...
async function portalQueueProject(app, orgId, actor, body) {
    const exterior = body.measurement_scope === "full_house";
    if (body.measurement_scope && !exterior && body.measurement_scope !== "roof")
        throw badRequest("invalid_scope", "Unknown report scope.");
    const exteriorOrder = exterior ? await validateExteriorOrder(orgId, body, normalizeReportRequestPins(body.pins).length) : null;
    if (exteriorOrder)
        body = { ...body, report_mode: "full", include_gutter_measurements: true, include_weather_report: false, is_expedited: false };
...
async function portalRefundInstant(app, orgId, actorEmail, body) {
    const amount = Math.abs(numericValue(body.refund_amount));
    if (amount > 0)
        await applyCreditDelta(orgId, { amount, reason: "instant_no_coverage_refund", meta: body }, actorEmail);
...
async function portalFirstMeasure(app, path, payload) {
    const response = await app.inject({
        method: "POST",
...
async function parseMediaUploadRequest(request) {
    const typed = request;
    const contentType = String(typed.headers?.["content-type"] ?? "");
    if (contentType.includes("multipart/form-data")) {
        let bytes = null;
        ...
        if (!bytes)
            throw badRequest("missing_media_file", "A file field is required.");
    ...
    if (!base64)
        throw badRequest("unsupported_media_upload", "Media uploads must use multipart/form-data or include base64 bytes.");
    return {
        id: String(body.id || ""),
...
async function portalGetOrg(orgId, userDoc) {
    const org = await portalOrgView(orgId);
    const user = portalUserView(userDoc);

...
import { addMembersSchema, attentionPatchSchema, collaborationPreferencesSchema, createActionItemFromMessageSchema, createChannelSchema, createHuddleSchema, draftSchema, editMessageSchema, folderSchema, huddleRecordingSchema, huddleMediaStateSchema, huddleSignalSchema, listMessagesQuerySchema, messageReminderSchema, memberPatchSchema, postMessageSchema, reactionSchema, readStateSchema, resourceRefSchema, scheduledMessageSchema, scheduledMessagePatchSchema, searchQuerySchema, sidebarSectionSchema, tabSchema, threadSubscriptionSchema, unreadStateSchema, updateChannelSchema } from "./schemas.js";
...
export const registerChannelsApi = async (app) => {
    app.setErrorHandler((error, _request, reply) => {
        if (error instanceof ZodError)
            return reply.code(400).send({ ok: false, error: "validation_error", issues: error.issues });
    ...
    if (platformBackgroundAllowed()) {
        const scheduledDeliveryTimer = setInterval(async () => {
            try {
                (await service.deliverAllDueScheduledMessages());
            }
            catch (error) {
                app.log.error(error);
            }
    ...
    function auth(request, orgId, options = {}) {
    ...
    app.put("/organizations/:orgId/threads/:rootMessageId/subscription", async (request) => {
        ...
        return {
            ok: true,
            subscription: await service.setThreadSubscription(ctx, getParam(request.params, "rootMessageId"), body.following, body.notify_level)
        };
    });
    app.post("/organizations/:orgId/threads/:rootMessageId/read", async (request) => {
        ...
        return {
            ok: true,
            subscription: await service.markThreadRead(ctx, getParam(request.params, "rootMessageId"), Math.max(0, Number(body.last_reply_seq) || 0))
        };
    });
    ...
    app.put("/organizations/:orgId/sidebar-sections/:sectionId", async (request) => {
        const orgId = getParam(request.params, "orgId");
        const ctx = await auth(request, orgId, { csrf: true, capability: "channels.attention_v2" });
        const body = sidebarSectionSchema.parse({ ...(request.body ?? {}), id: getParam(request.params, "sectionId") });
    ...
    app.patch("/organizations/:orgId/huddles/:huddleId/media-state", async (request) => {
        ...
        return {
            ok: true,
            huddle: await service.updateHuddleMediaState(ctx, getParam(request.params, "huddleId"), huddleMediaStateSchema.parse(request.body ?? {}))
        };
    });
    ...
    app.get("/organizations/:orgId/huddles/:huddleId/signals", async (request) => {
        ...
        const ctx = await auth(request, orgId, { capability: "channels.huddles" });
        const query = request.query && typeof request.query === "object" ? request.query : {};
        const signals = await service.listHuddleSignals(ctx, getParam(request.params, "huddleId"), Math.max(0, Number(query.after) || 0), getParam(query, "peer_id"));
    ...
    app.post("/organizations/:orgId/uploads", async (request, reply) => {
        ...
        if (!parts)
            throw badRequest("multipart_required", "Attachment uploads must be multipart/form-data.");
        let bytes = null;
        let fileName = "upload";
        ...
        for await (const part of parts) {
            ...
            if (part.fieldname === "channel_id")
                channelId = String(part.value ?? "").trim();
        }
        if (!bytes)
            throw badRequest("missing_file", "A file field is required.");
        ...
        if (projectId && (contentType.startsWith("image/") || contentType.startsWith("video/"))) {
            ...
            if (!currentPhotos.some((item) => item && typeof item === "object" && String(item.media_id ?? item.id ?? "") === mediaId)) {

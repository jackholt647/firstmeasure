...
import { attachToMessage, createChannelRecord, createMessageRecord, dmKeyForMembers, editMessageRecord, findChannelByDmKey, findChannelByProject, findMessageByClientId, getChannelsDatabase, listAttachmentsForMessages, listChannelMembers, listChannelRecords, listMembershipChannelIds, listMentionRowsForUser, listMessageRecords, listMessageRevisions, listMessageTranslations, listPinnedMessages, listReactionsForMessages, listReactionsToUser, listRepliesToUser, listSavedMessageIds, listSavedMessages, readChannelMember, readChannelRecord, readChannelsMeta, readAttachmentRecord, writeChannelsMeta, readMessageRecord, readMessageBySequence, readMessageTranslation, removeChannelMember, restoreMessageRecord, searchMessageRecords, setMessagePinned, setMessageLanguage, saveMessageTranslation, setSavedItem, softDeleteMessageRecord, toggleReactionRecord, unreadSummary, updateChannelRecord, upsertChannelMember, upsertReadState } from "./storage.js";
import { detectMessageLanguage, translateMessageText, translationSourceHash } from "./translation.js";
...
const AUDIENCE_GROUPS = ["office", "crew", "sales"];
...
async function viewerTranslationPreferences(ctx) {
...
async function emitChannelsEvent(type, ctx, payload, projectId) {
    try {
...
export function viewerAudienceGroups(ctx) {
    ...
    if (/sales|estim|business.develop|account.executive/.test(roleTokens))
        groups.add("sales");
    if (/crew|field|installer|technician|foreman|production|repair/.test(roleTokens) || ctx.applicationAccess.field?.enabled === true)
        groups.add("crew");
    if (/office|admin|manager|owner|dispatch|coordinator/.test(roleTokens) || (!groups.size && ctx.applicationAccess.management?.enabled === true))
        groups.add("office");
    if (!groups.size)
        groups.add("office");
    return [...groups];
}
function messageVisibleTo(message, ctx, groups) {
    if (!message.audience.length)
        return true;
    if (message.author_id === ctx.userId)
        return true;
...
export async function userDirectory(orgId) {
...
export function invalidateUserDirectory(orgId) {
    directoryCache.delete(orgId);
}
export async function listDirectoryUsers(ctx) {
...
// --- channel access -----------------------------------------------------------
function isPrivateType(channel) {
...
export async function requireChannelAccess(ctx, channelId, options = {}) {
    ...
    if (isPrivateType(channel)) {
        ...
        if (!membership)
            throw forbidden("channel_forbidden", "You are not a member of this channel.");
    }
...
async function channelAdminAllowed(ctx, channel) {
...
function inboxSeenKey(orgId, userId) {
    return `inbox_seen:${orgId}:${userId}`;
}
export async function personalInbox(ctx, options = {}) {
    ...
    const channelById = new Map(views.map((view) => [String(view.id), view]));
    const authorName = (id) => directory.get(cleanText(id))?.name || "Someone";
    const snippet = (text) => cleanText(text).replace(/\s+/g, " ").slice(0, 120);
    const entries = [];
    ...
    for (const view of views) {
        if (view.type !== "dm" && view.type !== "group_dm")
            continue;
        const unread = asObject(view.unread);
        if (Number(unread.unread_count || 0) <= 0)
            continue;
    ...
    for (const view of views) {
        ...
        if (membership?.notify_level !== "all")
            continue;
        const unread = asObject(view.unread);
        if (Number(unread.unread_count || 0) <= 0)
            continue;
...
async function publishMessageEvent(topic, channel, message, hydrated) {
    ...
    const payload = messageIsAudienceRestricted(message) || !hydrated
...
// --- hydration ----------------------------------------------------------------
export async function hydrateMessages(ctx, channel, messages) {
    for (const message of messages) {
        if (message.language_code !== "und" || message.deleted_at)
            continue;
...
function hydrateMessage(ctx, message, helpers) {
...
// --- channel views ------------------------------------------------------------
async function channelView(ctx, channel, extras = {}) {
...
export async function listChannelsForUser(ctx, options = {}) {
    ...
    const visible = all.filter((channel) => {
        if (isPrivateType(channel))
            return membershipIds.has(channel.id);
        if (channel.type === "project")
            return canSeeProjects && channel.last_message_at;
        return true;
    });
...
async function ensureFirstMateAssistantDm(ctx) {
    try {
        const { agentChannelParticipants } = await import("../agents/participants.js");
...
async function ensureGeneralChannel(ctx) {
...
export async function createChannel(ctx, input) {
    ...
    if (isDm) {
        if (memberIds.length < 2)
            throw badRequest("dm_requires_members", "Direct messages need at least one other person.");
        ...
        if (existing)
            return channelView(ctx, existing);
    ...
    if (!name)
        throw badRequest("channel_name_required", "Channels need a name.");
    const duplicate = (await listChannelRecords(ctx.orgId, { types: ["public", "private"], includeArchived: true }))
        .find((channel) => channel.name === name);
    if (duplicate)
        throw badRequest("channel_name_taken", `A channel named #${name} already exists.`);
    const channel = (await createChannelRecord({
        organization_id: ctx.orgId,
        type: input.type,
        name,
 ...
 */
export async function ensureProjectChannelRecord(orgId, projectId, title = "", createdBy = "system") {
    const existing = (await findChannelByProject(orgId, projectId));
    if (existing) {
        if (title && existing.name !== title)
            (await updateChannelRecord(orgId, existing.id, { name: title }));
...
export async function ensureProjectChannel(ctx, projectId) {
    if (!canAccessProjectChannels(ctx)) {
    ...
    if (existing) {
        if (title && existing.name !== title)
            (await updateChannelRecord(ctx.orgId, existing.id, { name: title }));
...
export async function updateChannel(ctx, channelId, patch) {
    const { channel } = await requireChannelAccess(ctx, channelId);
    if (!(await channelAdminAllowed(ctx, channel)))
        throw forbidden("permission_denied", "Only channel admins can update this channel.");
    if (channel.type === "dm" || channel.type === "group_dm")
        throw badRequest("dm_not_editable", "Direct messages cannot be renamed.");
...
export async function setChannelArchived(ctx, channelId, archived) {
    const { channel } = await requireChannelAccess(ctx, channelId);
    if (!(await channelAdminAllowed(ctx, channel)))
        throw forbidden("permission_denied", "Only channel admins can archive this channel.");
    if (channel.type === "project")
        throw badRequest("project_channel", "Project message threads cannot be archived.");
    const updated = (await updateChannelRecord(ctx.orgId, channelId, { archived_at: archived ? nowIso() : null }));
    if (archived)
        await emitChannelsEvent("channels.channel.archived", ctx, { channel_id: channelId, name: channel.name });
    (await publishRealtimeEvent({
        organization_id: ctx.orgId,
        topic: "channels.channel.updated",
        user_ids: (await realtimeTargets(channel)),
...
export async function addChannelMembers(ctx, channelId, userIds) {
    const { channel } = await requireChannelAccess(ctx, channelId);
    if (channel.type === "dm")
        throw badRequest("dm_fixed_membership", "Direct messages have fixed membership.");
    if (channel.type === "project")
        throw badRequest("project_channel", "Project message threads use project access, not membership.");
    ...
    for (const userId of userIds) {
        if (!directory.has(userId))
            throw badRequest("unknown_user", `User ${userId} is not part of this organization.`);
...
export async function removeMember(ctx, channelId, userId) {
    const { channel } = await requireChannelAccess(ctx, channelId);
    if (channel.type === "dm")
        throw badRequest("dm_fixed_membership", "Direct messages have fixed membership.");
...
// --- messages -----------------------------------------------------------------
export async function listMessages(ctx, channelId, options) {
    const { channel } = await requireChannelAccess(ctx, channelId);
...
export async function listThread(ctx, messageId, options) {
    ...
    const groups = viewerAudienceGroups(ctx);
    if (!messageVisibleTo(root, ctx, groups))
        throw notFound("message_not_found", "This message does not exist.");
...
export async function postMessage(ctx, channelId, input) {
    ...
    if (input.parent_id) {
        ...
        if (!parent || parent.channel_id !== channelId)
            throw badRequest("invalid_parent", "The thread parent does not exist in this channel.");
 ...
 */
export async function postAgentMessage(orgId, channelId, input) {
    ...
    if (parentId)
        (await collaboration.setThreadSubscriptionRecord(orgId, channelId, parentId, cleanText(input.author_id), true));
 ...
 */
export async function ensureAgentDmChannel(orgId, agentUserId, targetUserId) {
    const memberIds = [...new Set([cleanText(agentUserId), cleanText(targetUserId)])].filter(Boolean);
    if (memberIds.length < 2)
        throw badRequest("dm_requires_members", "A DM needs two participants.");
    const dmKey = dmKeyForMembers(memberIds);
...
export async function editMessage(ctx, messageId, input) {
    ...
    if (message.author_id !== ctx.userId)
        throw forbidden("not_message_author", "Only the author can edit a message.");
    if (message.deleted_at)
        throw badRequest("message_deleted", "Removed messages cannot be edited. Restore it first.");
...
export function translationPreferences(ctx) {
    return viewerTranslationPreferences(ctx);
}
export async function translateMessage(ctx, messageId) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    ...
    const preferences = await viewerTranslationPreferences(ctx);
    if (message.language_code === "und")
        throw badRequest("language_unknown", "This message is too short to identify its language reliably.");
...
export async function restoreMessage(ctx, messageId) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    const { channel } = await requireChannelAccess(ctx, message.channel_id);
    if (!message.deleted_at)
        throw badRequest("message_not_deleted", "This message is not removed.");
    if (message.author_id !== ctx.userId && !canManageChannels(ctx)) {
...
export async function messageRevisions(ctx, messageId) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    await requireChannelAccess(ctx, message.channel_id);
    const groups = viewerAudienceGroups(ctx);
    if (!messageVisibleTo(message, ctx, groups))
        throw notFound("message_not_found", "This message does not exist.");
    const directory = await userDirectory(ctx.orgId);
...
export async function toggleReaction(ctx, messageId, emoji, on) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    const { channel } = await requireChannelAccess(ctx, message.channel_id, { write: true });
    if (message.deleted_at)
        throw badRequest("message_deleted", "Removed messages cannot be reacted to.");
    ...
    if (on)
        (await collaboration.recordReactionAttention(ctx.orgId, message, ctx.userId, emoji));
...
export async function setPinned(ctx, messageId, pinned) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    const { channel } = await requireChannelAccess(ctx, message.channel_id, { write: true });
    if (message.deleted_at)
        throw badRequest("message_deleted", "Removed messages cannot be pinned.");
...
export async function pinnedMessages(ctx, channelId) {
    const { channel } = await requireChannelAccess(ctx, channelId);
    const groups = viewerAudienceGroups(ctx);
...
export async function savedMessages(ctx) {
    const groups = viewerAudienceGroups(ctx);
...
export async function setSaved(ctx, messageId, on) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    await requireChannelAccess(ctx, message.channel_id);
...
export async function markRead(ctx, channelId, lastReadSeq) {
...
export async function unreads(ctx) {
...
export async function markUnread(ctx, channelId, seq) {
    await requireChannelAccess(ctx, channelId);
    const message = (await readMessageBySequence(ctx.orgId, channelId, seq));
    if (!message || message.seq !== seq)
        throw badRequest("invalid_unread_message", "The selected message is not in this channel.");
...
export async function undoRead(ctx, operationId) {
...
export async function allUnreadMessages(ctx, options = {}) {
    const channels = await listChannelsForUser(ctx);
    const groups = viewerAudienceGroups(ctx);
    const results = [];
    ...
    for (const channelViewValue of channels) {
        ...
        const subscriptions = new Set((await collaboration.listThreadSubscriptionRows(ctx.orgId, ctx.userId))
    ...
    return { conversations: results, total_unread: results.reduce((sum, item) => sum + item.messages.length, 0) };
}
export async function activity(ctx, options = {}) {
...
export async function updateActivity(ctx, itemId, patch) {
...
export async function markAllActivityRead(ctx) {
...
export async function followedThreads(ctx) {
    ...
    for (const row of rows) {
        try {
            ...
            const lastReplySeq = Math.max(0, ...(thread.replies || []).map((reply) => Number(reply.seq) || 0));
            const lastReadReplySeq = Number(row.last_read_reply_seq) || 0;
            results.push({
                subscription: row,
                ...thread,
                last_reply_seq: lastReplySeq,
                unread_count: (thread.replies || []).filter((reply) => Number(reply.seq) > lastReadReplySeq && cleanText(reply.author_id) !== ctx.userId).length
            });
        }
        catch {
...
export async function markThreadRead(ctx, rootMessageId, lastReplySeq) {
...
export async function setThreadSubscription(ctx, rootMessageId, following, notifyLevel = "all") {
    const root = (await readMessageRecord(ctx.orgId, rootMessageId));
    if (!root)
        throw notFound("message_not_found", "This thread does not exist.");
    await requireChannelAccess(ctx, root.channel_id);
    const canonicalRoot = root.parent_id || root.id;
    const subscription = (await collaboration.setThreadSubscriptionRecord(ctx.orgId, root.channel_id, canonicalRoot, ctx.userId, following, notifyLevel));
    (await publishRealtimeEvent({
        organization_id: ctx.orgId,
...
export async function collaborationPreferences(ctx) {
    return (await collaboration.readCollaborationPreferences(ctx.orgId, ctx.userId));
}
export async function saveCollaborationPreferences(ctx, patch) {
...
export async function deliverDueScheduledMessages(ctx) {
    return deliverAllDueScheduledMessages({ orgId: ctx.orgId, userId: ctx.userId });
}
export async function deliverAllDueScheduledMessages(scope) {
    ...
    for (const candidate of due) {
        ...
        if (!await isCapabilityEnabled(orgId, "apps.channels"))
            continue;
        try {
            ...
            if (!await can(sender, "apps.channels"))
                throw new Error("The sender no longer has access to Channels.");
            const completed = await collaboration.deliverScheduledRecord(cleanText(candidate.id), async (scheduled) => {
                ...
                if (!channel || channel.archived_at)
                    throw new Error("The target channel is unavailable.");
                ...
                if (attachments.length)
                    await attachToMessage(orgId, attachments, message.id, channel.id, senderId);
                await collaboration.recordAttentionForMessage(channel, message);
                return message.id;
            });
            if (completed)
...
export async function saveMessageReminder(ctx, messageId, remindAt) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    await requireChannelAccess(ctx, message.channel_id);
...
export async function messageReminders(ctx) {
...
export async function removeMessageReminder(ctx, reminderId) {
    return (await collaboration.removeMessageReminderRecord(ctx.orgId, ctx.userId, reminderId));
}
export async function listChannelTabs(ctx, channelId) {
    await requireChannelAccess(ctx, channelId);
    return (await collaboration.listTabRecords(ctx.orgId, channelId, ctx.userId));
}
export async function createChannelTab(ctx, channelId, input) {
...
export async function listChannelResources(ctx, channelId, options = {}) {
    ...
    if (channel.project_id && !options.folderId) {
        ...
        if (!type || type === "action_item" || type === "todos") {
            ...
            const result = await listCanonicalActionItems(ctx.orgId, ctx, {
...
export async function createChannelResource(ctx, channelId, input) {
    await requireChannelAccess(ctx, channelId, { write: true });
...
export async function deleteChannelResource(ctx, channelId, refId) {
    await requireChannelAccess(ctx, channelId, { write: true });
    return (await collaboration.deleteResourceRefRecord(ctx.orgId, channelId, refId));
}
export async function createActionItemFromMessage(ctx, messageId, input) {
    const message = (await readMessageRecord(ctx.orgId, messageId));
    if (!message)
        throw notFound("message_not_found", "This message does not exist.");
    const { channel } = await requireChannelAccess(ctx, message.channel_id);
...
export async function createHuddle(ctx, channelId, settings) {
...
export async function getHuddle(ctx, huddleId) {
    const room = (await calls.getRoom(ctx, huddleId));
    if (cleanText(room.context_type) !== "channel")
        throw notFound("huddle_not_found", "This huddle does not exist.");
    await requireChannelAccess(ctx, cleanText(room.context_id));
    return huddleView(room);
}
export async function joinHuddle(ctx, huddleId) {
...
export async function updateHuddleMediaState(ctx, huddleId, state) {
...
export async function leaveHuddle(ctx, huddleId) {
    const current = await getHuddle(ctx, huddleId);
...
export async function removeHuddleParticipant(ctx, huddleId, userId) {
    const current = await getHuddle(ctx, huddleId);
...
export async function endHuddle(ctx, huddleId) {
    const current = await getHuddle(ctx, huddleId);
...
export async function saveHuddleRecording(ctx, huddleId, attachmentId) {
...
export async function postHuddleSignal(ctx, huddleId, body) {
    await getHuddle(ctx, huddleId);
    return (await calls.postSignal(ctx, huddleId, body));
}
export async function listHuddleSignals(ctx, huddleId, afterSeq, peerId) {
    await getHuddle(ctx, huddleId);
    return (await calls.listSignals(ctx, huddleId, afterSeq, peerId));
}
// --- typing -------------------------------------------------------------------
export async function noteTyping(ctx, channelId) {
    const { channel } = await requireChannelAccess(ctx, channelId, { write: true });
...
// --- search -------------------------------------------------------------------
export async function searchMessages(ctx, query, options) {
    const operators = {};
    ...
    const directory = fromFilters.length ? await userDirectory(ctx.orgId) : new Map();
    ...
    const matches = candidates.filter((message) => {
        if (!channelsById.has(message.channel_id) || !messageVisibleTo(message, ctx, groups))
            return false;
        if (authorIds && !authorIds.has(message.author_id))
            return false;
        const created = Date.parse(message.created_at);
        if (Number.isFinite(before) && created >= before)
            return false;
        if (Number.isFinite(after) && created < after)
            return false;
        if (operators.has?.includes("link") && !/https?:\/\//i.test(message.text))
            return false;
        if (operators.is?.includes("pinned") && !message.pinned_at)
            return false;
        return true;
    });
    ...
    for (const message of matches) {
        ...
        if (operators.has?.includes("file") && !attachments.length)
            continue;
        if (operators.has?.includes("reaction") && !reactions.length)
            continue;
        if (savedIds && !savedIds.has(message.id))
            continue;
        results.push({ ...hydrated, channel: { id: channel.id, name: channel.name, type: channel.type, project_id: channel.project_id } });
...
async function notifyMentions(ctx, channel, message, alreadyNotified = new Set()) {
    const mentioned = [...new Set(message.mention_users
    ...
    for (const userId of mentioned) {
        ...
        if (asObject(preferences.dnd).enabled === true)
            continue;
        const memberLevel = (await readChannelMember(channel.id, userId))?.notify_level;
        if (cleanText(memberLevel || preferences.default_notify_level) !== "muted")
            targets.push(userId);
    }
    if (!targets.length)
        return;
...
async function notifyMessageSubscribers(ctx, channel, message) {
    ...
    for (const { user_id: userId } of await listChannelMembers(channel.id)) {
        if (!userId || userId === ctx.userId || mentioned.has(userId))
            continue;
        const preferences = await collaboration.readCollaborationPreferences(ctx.orgId, userId);
        if (asObject(preferences.dnd).enabled === true)
            continue;
        ...
        if (["dm", "group_dm"].includes(channel.type) ? effective !== "muted" : effective === "all")
            targets.push(userId);
    }
    if (!targets.length)
        return;

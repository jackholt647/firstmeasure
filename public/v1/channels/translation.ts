...
const ISO3_TO_LANGUAGE: Record<string, string> = {
...
export function detectMessageLanguage(textValue: unknown): { code: string; confidence: number } {
  const text = lettersOnly(String(textValue ?? ""));
  const letterCount = (text.match(/\p{L}/gu) || []).length;
  if (letterCount < 3) return { code: "und", confidence: 0 };
  ...
  for (const [pattern, code] of scripted) {
    if (pattern.test(text)) return { code, confidence: 1 };
  }
  ...
  if (!best || best[0] === "und") return { code: "und", confidence: 0 };
  ...
  if (letterCount < 6 || (letterCount < 12 && confidence < 0.62)) return { code: "und", confidence };
...
function protectTokens(text: string, mentionUsers: Array<Record<string, unknown>>) {
  const replacements = new Map<string, string>();
  let index = 0;
  const protect = (value: string, kind: "MENTION" | "URL" | "EMAIL") => {
...
function restoreTokens(text: string, replacements: Map<string, string>) {
...
export async function translateMessageText(input: {
  ...
  if (!result.ok) throw new Error(openAIErrorMessage(result, "Translation failed."));

...
import { priceForOrganization } from "../commerce/prices.js";
...
export const registerPlatformBillingApi:FastifyPluginAsync=async app=>{
  ...
  app.post(`${base}/subscriptions`,async request=>{const {org,ctx}=await auth(request,true);const body=z.object({price_id:z.string().max(100),request_key:z.string().min(8).max(100),accept_terms:z.literal(true)}).strict().parse(request.body);const catalogPrice=await record<Price>("_platform","price",body.price_id);const price=catalogPrice?await priceForOrganization(org,catalogPrice):null;if(price && (price.monthly_cents||price.rates.some(r=>r.unit_price_micros)))throw badRequest("billing_checkout_required","Review the subscription and complete checkout before activation.");return {ok:true,subscription:await subscribe(org,body.price_id,body.request_key,ctx.identityId,price||undefined)};});

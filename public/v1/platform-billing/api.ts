...
export const registerPlatformBillingApi:FastifyPluginAsync=async app=>{
  ...
  app.post(`${base}/invoices/:period/checkout`,async request=>{const {org,ctx}=await auth(request,true);return {ok:true,...await checkout(org,String((request.params as any).period),ctx.identityId)};});
};

...
export const priceSchema = z.object({
  ...
  regional_prices:z.object({domestic:z.record(z.string().regex(/^[A-Z]{3}$/),z.object({monthly_cents:integer.max(100_000_000),rates:z.array(rateSchema).max(12)}).strict()).optional(),international:z.record(z.string().regex(/^[A-Z]{3}$/),z.object({monthly_cents:integer.max(100_000_000),rates:z.array(rateSchema).max(12)}).strict()).optional()}).strict().optional(),
  rates:z.array(rateSchema).max(12).default([]), require_subscription:z.boolean().default(true)
}).strict();
export type Price = Omit<z.infer<typeof priceSchema>, "currency"> & { currency:string; minor_digits?:number; billing_price_key?:string; id:string; version:number; published:boolean; created_at:string; actor:string };
...
export type Invoice = { id:string; period:string; currency:string; minor_digits?:number; lines:Line[]; total_cents:number; status:"open"|"paid"|"void"; created_at:string; actor:string; payment_id?:string; checkout_id?:string; checkout_url?:string; paid_at?:string; amount_paid_cents?:number; amount_remaining_cents?:number };

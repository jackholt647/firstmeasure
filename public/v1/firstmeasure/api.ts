...
import { readMediaFile as readExteriorMediaFile } from "../platform/storage.js";
...
export const registerFirstMeasureApi: FastifyPluginAsync = async (app) => {
  ...
  app.post("/projects/queue", async (request, reply) => {
    ...
    if (rawBody.measurement_scope === "full_house" && reorderProjectId) throw badRequest("exterior_reorder_requires_new_order","Place a new Full House order with current references.");
    ...
    const exterior = rawBody.measurement_scope === "full_house";

    let created;

    if (exterior) {

      if (!env.firstMeasureInternalApiSecret || request.headers["x-firstmeasure-internal"]!==env.firstMeasureInternalApiSecret) throw new FirstMeasureError("internal_required",403,"Use the customer order workflow.");

      const orgId=String(asRecord(rawBody.organization_ref).id || "");

      const checked=await validateExteriorOrder(orgId,rawBody,Array.isArray(body.pins)?body.pins.length:0);

      created=await createProject({...projectInput,id:"exteriors_"+randomBytes(16).toString("hex"),status:"awaiting_references",measurement_scope:"full_house",include_gutter_measurements:true,exterior_references:checked.references},{customerExteriors:true});
      ...
      for(const [index,ref] of checked.references.entries()) {
        ...
        elevationPhotos.push({file_name:fileName,original_name:`Structure ${ref.structure+1} · ${ref.view}`,elevation_view:ref.view,structure:ref.structure});
    ...
    if (!exterior) await maybeEvaluateAutomaticRushMode(app, created.manifest.id);
...
async function buildLegacyProjectRow(
  ...
  return {
    ...
    ...normalizeReportPreferences(legacy),
    id: String(legacy.id ?? ""),
...
async function reopenRejectedProjectForReorder(
  ...
  const patch: Record<string, unknown> = {
    ...await resolveOrderReportPreferences(input),
...
function isExpeditedReportOption(value: unknown) {
  ...
  return value === "exteriors_priority" || value === "exteriors_same_day" || isExpeditedReportExpediteKey(value);
...
function calculateExpediteRefundAmount(manifest: ProjectManifest | Record<string, unknown>) {
  if (manifest.measurement_scope === "full_house" && isCustomerExteriorId(manifest.id)) {
    const currentAmount=moneyAmount(manifest.amount_charged), standardAmount=moneyAmount(manifest.exteriors_base_amount);
    return {currentAmount,standardAmount,refundAmount:manifest.exteriors_base_amount==null?0:moneyAmount(Math.max(0,currentAmount-standardAmount))};
  }
...
function buildPdfClientRuntimeManifest(
  ...
  return {
    ...
    script_urls: {
      ...
      report_units: `${baseUrl}/assets/report-units`,
...
function buildInlineManifest(body: unknown): ProjectManifest {
  ...
  return {
    ...
    schema_version: 1,
    ...normalizeReportPreferences(source),

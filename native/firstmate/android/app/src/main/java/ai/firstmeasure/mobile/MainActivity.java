...
public class MainActivity extends ComponentActivity {
    ...
    private final ActivityResultLauncher<String> notificationPermission = registerForActivityResult(new ActivityResultContracts.RequestPermission(), granted -> {
        if (pushReply == null) return;
        JavaScriptReplyProxy reply = pushReply; String id = pushRequestId;
        pushReply = null; pushRequestId = null;
        if (granted) requestPushToken(reply, id);
        else respond(reply, id, new JSONObject(), null);
    });
    ...
    @Override public void onCreate(Bundle saved) {
        ...
        PushMessagingService.createChannels(this);
        pendingNotificationId = getIntent().getStringExtra("notification_id");
        ...
        settings.setUserAgentString(settings.getUserAgentString()+" FirstMateMobile/"+BuildConfig.VERSION_NAME);
        ...
        web.setWebViewClient(new WebViewClient(){
            ...
            @Override public void onPageFinished(WebView view,String url){ CookieManager.getInstance().flush(); deliverPendingNotification(); }
            ...
            switch(call.getString("method")){
                ...
                case "pushRegister":
                    ...
                    if (!pushPermissionGranted()) {
                        if (pushReply != null) { respond(reply,id,null,"push_in_progress"); break; }
                        pushReply=reply; pushRequestId=id; notificationPermission.launch(android.Manifest.permission.POST_NOTIFICATIONS);
                    } else requestPushToken(reply,id);
                    break;
                case "pushUnregister": respond(reply,id,true,null); break;
    ...
    private boolean pushAvailable(){ return !FirebaseApp.getApps(this).isEmpty(); }
    private boolean pushPermissionGranted(){
        return NotificationManagerCompat.from(this).areNotificationsEnabled()
            && (Build.VERSION.SDK_INT < 33 || androidx.core.content.ContextCompat.checkSelfPermission(this,android.Manifest.permission.POST_NOTIFICATIONS)==android.content.pm.PackageManager.PERMISSION_GRANTED);
    }
    private void requestPushToken(JavaScriptReplyProxy reply,String id){
        if (!pushPermissionGranted()) { respond(reply,id,new JSONObject(),null); return; }
        FirebaseMessaging.getInstance().getToken().addOnCompleteListener(task -> {
            if (!task.isSuccessful() || task.getResult()==null) { respond(reply,id,null,"push_registration_failed"); return; }
            try { respond(reply,id,new JSONObject().put("token",task.getResult()).put("platform","android").put("environment",BuildConfig.FLAVOR),null); }
            catch(Exception e){respond(reply,id,null,"push_registration_failed");}
        });
    }
    private void deliverPendingNotification(){
        if (pendingNotificationId==null || !policy.trusted(web.getUrl())) return;
        String id=pendingNotificationId;pendingNotificationId=null;
        try { web.evaluateJavascript("window.dispatchEvent(new CustomEvent('fm:native:push',{detail:"+new JSONObject().put("notification_id",id).toString()+"}))",null); }
    ...
    @Override protected void onNewIntent(Intent intent){
        super.onNewIntent(intent);setIntent(intent);acceptAuthReturn(intent);
        String notificationId=intent.getStringExtra("notification_id");
        if(notificationId!=null){pendingNotificationId=notificationId;deliverPendingNotification();}
    }

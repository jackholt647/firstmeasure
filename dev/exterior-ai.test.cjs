...
test('AI remains narrow, bounds positions, saves every view, caps refinements, restores captures and cancels stale runs',async()=>{
 ...
 await page.goto('http://localhost/');await page.evaluate(()=>{
  ...
  window.ExteriorRendered={render:()=>{window.renderedFrames=(window.renderedFrames||0)+1;return true;}};window.WallMode={enabled:true,aiGeometry:()=>g};window.ProjectResources={reportImages:async()=>[]};window.ExteriorPDF={initializePhotoSlots:s=>s.photoSlots={front:{image:{dataUrl:canvas.toDataURL('image/png')}}}};
 });
 ...
 await page.getByRole('button',{name:'Jump to front with AI'}).click();await page.getByText('Finished: three-refinement limit reached. Match remains uncertain.',{exact:true}).waitFor();assert.ok(await page.evaluate(()=>window.renderedFrames)>0);assert.equal(calls,5);assert.equal(requests[0].images.length,2);assert.equal(requests[4].images.length,3);assert.equal(requests[4].context.phase,'final review');assert.equal(requests[4].context.current.position.z,24*.1+1.8288);

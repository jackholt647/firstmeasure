...
test('WebGL exterior wire and highlights clear their surface but stay behind nearer walls',async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   window.checkWire=({perspective,highlight,occluded,mode='opaque',zoom=1,eye=[5,4,8]})=>{
    ...
    if(highlight)wallSelectedLine(group,p=>new THREE.Vector3(p.x,p.y,p.z),pair);else group.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(pair),new THREE.LineBasicMaterial({color:'#ffff00'})));
    ...
    for(let i=1;i<20;i++){const t=i/20,x=Math.round((screen[0].x*(1-t)+screen[1].x*t+1)*350),y=Math.round((screen[0].y*(1-t)+screen[1].y*t+1)*350);let visible=false;
     for(let dx=-1;dx<=1;dx++)for(let dy=-1;dy<=1;dy++){const k=((y+dy)*700+x+dx)*4;if(pixels[k]>220&&pixels[k+1]>220&&(highlight?pixels[k+2]>220:pixels[k+2]<40))visible=true;}
     samples.push(visible);
    }
    scene.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});return samples;
   };
  });
  for(const perspective of [false,true])for(const highlight of [false,true])for(const zoom of [.65,1,1.4])for(const eye of [[5,4,8],[8,7,.5],[.5,7,8]]){
   const options={perspective,highlight,zoom,eye};const visible=await page.evaluate(o=>checkWire(o),options);assert.ok(visible.every(Boolean),JSON.stringify({...options,visible}));

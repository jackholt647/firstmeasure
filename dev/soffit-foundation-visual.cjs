...
const build=compile('dev/wall-soffit-foundation.test.cjs',s=>s.replace("const test=require('node:test')","const test=()=>{}")+'\nmodule.exports=build;');
const after=build(),saved=JSON.parse(fs.readFileSync(process.argv[3],'utf8')).exteriorsWalls;
const before={state:saved,composed:A.compose(C.compose(saved.alignedWalls,saved),saved.chimneyCleanupReport)};
(async()=>{const out=path.resolve(process.argv[2]||path.join(process.env.TEMP,'soffit-overlap')),browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',args:['--use-angle=swiftshader']});fs.mkdirSync(out,{recursive:true});try{
 ...
 for(const corner of ['chimney','overlap','front'])for(const [name,data]of [['before',before],['after',after]]){await page.evaluate(({data,corner,name})=>draw(data,corner,name+' — '+corner+' / 24-inch soffit'),{data,corner,name});await page.screenshot({path:path.join(out,corner+'-'+name+'.png')});}

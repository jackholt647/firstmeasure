...
test('soffit contacts use depth testing and a one-pixel bias only in solid modes',()=>{
 ...
 for(const mode of ['opaque','textured','translucent']){ctx.exteriorSurfaceDisplay(group,mode);const line=group.children[0];assert.equal(line.material.depthTest,mode!=='translucent');assert.equal(line.material.userData.wallLineDepthBias.pixels.value,mode==='translucent'?0:10);assert.equal(line.material.depthWrite,false);assert.equal(line.material.transparent,true);assert.equal(line.material.color.getHex(),0xef633c);assert.equal(line.renderOrder,999.5);assert.equal(line.visible,true);}
});

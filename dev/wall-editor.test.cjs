...
test('soffit contact color and visibility survive opaque and textured material setup',()=>{
 ...
 for(const mode of ['opaque','textured','translucent']){ctx.exteriorSurfaceDisplay(group,mode);const line=group.children[0];assert.equal(line.material.depthTest,false);assert.equal(line.material.color.getHex(),0xef633c);assert.equal(line.renderOrder,999.5);assert.equal(line.visible,true);}
});

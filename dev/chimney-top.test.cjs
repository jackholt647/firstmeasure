...
test('new and persisted chimney tops get a dedicated default while custom finishes and trim survive',()=>{
 ...
 const saved={wallEdits:{$surfaces:[structuredClone(cap),{...structuredClone(cap),id:'trim',trim:true,material:'trim-horizontal'},{...structuredClone(cap),id:'custom',material:'stone',finishColor:'#abcd12'}],$drafts:{d:{chimney:cap.chimney,faces:[{material:'default',points:cap.points}]}}}};C.syncVolumes(saved);assert.deepEqual(saved.wallEdits.$surfaces.map(f=>f.material),['chimney-top','trim-horizontal','stone']);assert.equal(saved.wallEdits.$drafts.d.faces[0].material,'chimney-top');
...
test('raising a chimney keeps its cap finish off new sides and reports those sides',()=>{
 const source={...cap,material:'chimney-top'},neighbor={...wall,material:'brick',finishColor:'#ab7654'},result=W.extrude(source,.5,{supports:[neighbor]});
 assert.equal(result.cap.material,'chimney-top');assert.ok(result.sides.length);assert.ok(result.sides.every(f=>f.material==='brick'&&f.finishColor==='#ab7654'&&f.chimney.cap===false));
 const saved={wallEdits:{$surfaces:[result.cap,...result.sides]}};C.syncVolumes(saved);assert.ok(saved.wallEdits.$surfaces.filter(f=>f!==result.cap).every(f=>f.material==='brick'));
 const report=R.build({faces:saved.wallEdits.$surfaces});assert.ok(Math.abs(report.totals.net-2/.3048**2)<1e-7);assert.equal(report.returns.length,0);
 const legacy={...wall,material:undefined,chimney:{id:'chimney',cap:true,derived:true}};C.syncVolumes({wallEdits:{$surfaces:[legacy]}});assert.equal(legacy.material,undefined);assert.ok(R.build({faces:[legacy]}).totals.net>0);
});

...
test('Rendered builds a separate PBR scene, loads real assets, exports 4K, and restores editor rendering',async()=>{
 ...
 try{
  ...
  for(const name of ['vendor/clipper-lib-6.4.2-clipper','vendor/earcut-3.2.3-earcut.dev','exterior_geometry','ground_geometry','ground_editor','wall_solid_geometry','wall_features','exterior_finishes','exterior_scene_cache','exterior_rendered'])await page.addScriptTag({url:'http://rendered.test/editor_scripts/'+name+'.js'});
  const initial=await page.evaluate(()=>{
   ...
   face(door,'unassigned',[],{type:'door',axis:{x:1,y:0,z:0}}).name='door-backing';face(garage,'unassigned',[],{type:'garage',axis:{x:0,y:1,z:0}}).name='garage-backing';
   ...
   const renderScene=renderer.render.bind(renderer);renderer.render=(scene,camera)=>{const presentation=[];scene.traverse(o=>{if(o.isMesh&&o.name.startsWith('rendered-'))presentation.push(o);});window.demoPresentation=presentation.map(o=>({layer:o.userData.presentationLayer,geometry:o.geometry.type,positions:Array.from(o.geometry.attributes.position.array),index:o.geometry.index&&Array.from(o.geometry.index.array),offset:o.material.polygonOffsetFactor,color:o.material.color.getHexString(),map:!!o.material.map}));window.demoBackings=presentation.filter(o=>o.name.endsWith('-backing')).map(o=>{const a=o.geometry.attributes.position,idx=o.geometry.index;const v=i=>new THREE.Vector3().fromBufferAttribute(a,idx?idx.getX(i):i);const p=v(0),q=v(1),r=v(2),center=p.clone().add(q).add(r).multiplyScalar(1/3),normal=q.clone().sub(p).cross(r.clone().sub(p)).normalize();return {name:o.name,hits:[1,-1].map(sign=>new THREE.Raycaster(center.clone().addScaledVector(normal,sign),normal.clone().multiplyScalar(-sign)).intersectObject(o,false).length>0),opaque:!o.material.transparent&&o.material.opacity===1};});window.demoSides=presentation.filter(o=>o.name.startsWith('rendered-roof-')).map(o=>o.material.side);return renderScene(scene,camera);};
  ...
  const backings=await page.evaluate(()=>window.demoBackings);
  assert.equal(backings.length,2);for(const backing of backings){assert.deepEqual(backing.hits,[true,true],backing.name+' must occlude from either wall orientation');assert.equal(backing.opaque,true);}

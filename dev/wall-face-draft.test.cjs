...
test('plane spline picking selects original controls and copies an analytic curve',()=>{
 const K=require('../public/measure/internal/editor_scripts/exterior_geometry'),p=(x,z)=>({x,y:0,z}),c={...K.splineThrough([p(1,3),p(3,3)],[p(2,3.8)],{x:0,y:1,z:0}),id:'arch'},face=K.archFaces([{id:'panel',points:[p(1,1),p(3,1),p(3,3),p(1,3)],holes:[]}],c).faces[0],state={wallEdits:{$surfaces:[face]}},f=fixture({state,walls:[],selected:null,globals:{isFreeMove:true}});f.editor.togglePlane(face);const hit=K.curvePoint(c,.25);f.editor.down(f.e(hit.x,hit.z));f.listeners.pointerup(f.e(hit.x,hit.z));assert.equal(f.editor.pointSelection().length,3);f.editor.clipboardCommand('copy');assert.equal(f.clipboard().curves.length,1);assert.equal(f.clipboard().edges.length,0);assert.equal(f.clipboard().points.length,3);f.editor.clipboardCommand('paste');f.editor.down(f.e(2,2));assert.match(f.message(),/pasted/);const draft=Object.values(state.wallEdits.$drafts).find(d=>d.constructionPlane);assert.equal(draft.sketch.curves[0].controls.length,3);assert.equal(draft.sketch.nodes.filter(n=>n.userDraftPoint).length,3);
});

test('a selected spline copies and pastes outside plane mode without loose sample edges',()=>{
 const K=require('../public/measure/internal/editor_scripts/exterior_geometry'),p=(x,z)=>({x,y:0,z}),c={...K.splineThrough([p(1,3),p(3,3)],[p(2,3.8)],{x:0,y:1,z:0}),id:'arch'},face=K.archFaces([{id:'panel',points:[p(1,1),p(3,1),p(3,3),p(1,3)],holes:[]}],c).faces[0],state={wallEdits:{$surfaces:[face]}},f=fixture({state,walls:[],selected:null,globals:{isFreeMove:true},featureHost:()=>({points:[p(0,0),p(6,0),p(6,6),p(0,6)],solid:{id:'host'}})}),samples=K.curveSamples(c),pairs=samples.slice(1).map((p,i)=>[samples[i],p]);f.editor.restoreSelection({lineSelection:[{pair:pairs[0],pairs}]});f.editor.clipboardCommand('copy');assert.equal(f.clipboard().curves.length,1);assert.equal(f.clipboard().edges.length,0);f.listeners.pointermove(f.e(2,2));f.editor.clipboardCommand('paste');f.editor.key({key:'t'});f.editor.down(f.e(2,2));assert.match(f.message(),/pasted/);const d=Object.values(state.wallEdits.$drafts).find(d=>d.constructionPlane);assert.equal(d.sketch.curves[0].controls.length,3);assert.equal(state.wallEdits.$loose.edges.length,0);
});

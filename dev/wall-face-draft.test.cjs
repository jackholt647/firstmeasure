...
test('sticker shortcuts convert selected stickers; Ctrl+D preserves the door command beside Divide',()=>{const f=stickerFixture();const original=JSON.stringify(f.d().faces.find(f=>f.feature).points);for(const key of ['d','g','w']){f.editor.key({key,ctrlKey:key==='d'});assert.equal(f.editor.busy(),false);const sticker=f.d().faces.find(f=>f.feature);assert.equal(sticker.feature.type,{d:'door',g:'garage',w:'window'}[key]);assert.equal(JSON.stringify(sticker.points),original);assert.equal(sticker.feature.preset,null);}f.editor.key({key:'w'});assert.ok(f.d().faces.some(f=>f.feature?.type==='window'&&f.feature.preset===0),f.message());});
...
test('D previews and toggles without mutation, accepts exact distances, commits once and cancels cleanly',()=>{
 const f=divideFixture(),before=JSON.stringify(f.state.wallEdits);assert.equal(f.editor.key({key:'d'}),true);assert.equal(f.editor.interaction(),'Divide window / door');assert.equal(f.editor.distanceInput().label,'Divide from top');
 ...
 f.editor.key({key:'d'});assert.equal(f.editor.distanceInput().label,'Divide from left');f.editor.distanceInput().set(2*.3048);f.editor.key({key:'escape'});assert.equal(JSON.stringify(f.state.wallEdits),before);assert.equal(f.editor.selectionSnapshot().selectedSolid,'divide-window');
 f.editor.key({key:'d'});f.editor.distanceInput().set(5*.3048);f.editor.down(f.e(2,2));assert.equal(f.history.length,1);assert.equal(JSON.stringify(f.history[0]),before);assert.equal(f.state.wallEdits.$surfaces.length,2);assert.deepEqual(f.state.wallEdits.$surfaces.map(s=>Math.round((Math.max(...s.points.map(p=>p.z))-Math.min(...s.points.map(p=>p.z)))/.3048)).sort(),[2,5]);
});
...
test('dividing a drafted opening consumes its source, and undo restores the entire original window',()=>{
 const f=stickerFixture(),M=require('../public/measure/internal/editor_scripts/exterior_model.js'),before=JSON.stringify(f.state.wallEdits);f.editor.key({key:'d'});f.editor.distanceInput().set(.5);f.editor.key({key:'enter'});const features=M.collect(f.state).filter(s=>s.feature);assert.equal(features.length,2);assert.ok(f.d().faces.find(s=>s.feature).solidId);assert.equal(JSON.stringify(f.history.at(-1)),before);

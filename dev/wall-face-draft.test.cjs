...
for(const replacement of [true,false])test(`deleting a sticker chord preserves all wire and recalculates fill (${replacement?'arc':'remaining rectangle'})`,()=>{
 const K=require('../public/measure/internal/editor_scripts/exterior_geometry'),W=require('../public/measure/internal/editor_scripts/wall_solid_geometry'),S=require('../public/measure/internal/editor_scripts/base_sketch_geometry'),p=(x,z)=>({x,y:0,z}),a=p(2,4),b=p(3,3),left={...K.splineThrough([p(1,3),a],[p(1.2,3.6)],{x:0,y:1,z:0}),id:'left'},right=K.splineThrough([a,b],[p(2.8,3.6)],{x:0,y:1,z:0}),door=K.archFaces([{id:'door',feature:{type:'door'},points:[p(1,0),p(3,0),b,a,p(1,3)],holes:[]}],left).faces[0],frame=K.frame(door),d={frame,members:['source'],faces:[{...door,points:door.points.map(p=>K.local(frame,p)),curves:door.curves.map(c=>K.mapCurve(c,p=>K.local(frame,p)))}]},state={wallEdits:{$drafts:{door:d}}};S.ensure(d);
 // An authored segment within the sticker is wire, independent of its fill.
 const x=S.add(d,K.local(frame,p(1,3)),1e-6),y=S.add(d,K.local(frame,b),1e-6);d.sketch.edges.push({id:'middle',a:x,b:y,userConnection:true,fixed:false});
 const copied={constructionPlane:true,frame,members:[],faces:[],sketch:{version:2,next:0,nodes:[],edges:[],outlines:[]}};S.addCurve(copied,K.mapCurve(right,p=>K.local(frame,p)));const first=copied.sketch.edges[0],last=copied.sketch.edges.at(-1);copied.sketch.edges=[{...first,b:last.b,curveRange:[0,1]}];if(replacement)state.wallEdits.$drafts.copied=copied;
 const f=fixture({state,walls:[],selected:null});f.editor.togglePlane(door);const snapshot=f.editor.selectionSnapshot();snapshot.workingPlane.selection=[a,b];snapshot.workingPlane.selectedLines=[[a,b]];f.editor.restoreSelection(snapshot);
 const oldCopied=JSON.stringify(copied.sketch),oldNodes=JSON.stringify(d.sketch.nodes),oldEdges=JSON.parse(JSON.stringify(d.sketch.edges)),selected=oldEdges.filter(e=>W.sharedIntervals(...[e.a,e.b].map(id=>K.world(frame,d.sketch.nodes.find(n=>n.id===id))),[{points:[a,b]}]).length);assert.equal(selected.length,1);
 f.editor.key({key:'Delete'});assert.equal(f.history.length,1,f.message());assert.equal(JSON.stringify(copied.sketch),oldCopied,'the unselected spline is byte-for-byte unchanged');assert.equal(JSON.stringify(d.sketch.nodes),oldNodes,'no original nodes are moved or removed');assert.deepEqual(JSON.parse(JSON.stringify(d.sketch.edges)),oldEdges.filter(e=>e.id!==selected[0].id),'only the selected straight edge is removed');assert.ok(!d.faces[0].solidId,'rebuilding fill must not consume the original sketch wire');
 W.adoptMergedFaceSources(state.wallEdits);assert.ok(!d.faces[0].solidId,'reload reconciliation preserves independent wire');const face=state.wallEdits.$surfaces.find(f=>f.feature&&!f.deleted);assert.ok(face,JSON.stringify({surfaces:state.wallEdits.$surfaces,faces:d.faces,message:f.message()}));if(replacement)assert.ok(K.curveSamples(right).slice(1).every((q,i)=>W.sharedIntervals(K.curveSamples(right)[i],q,[face]).length),'fill follows the still-present copied arc');else assert.ok(face.points.every(p=>p.z<=3+1e-6),'only the remaining closed rectangle is filled');const surviving=[...S.curveEdges(d.sketch),...(replacement?S.curveEdges(copied.sketch):[])].map(e=>({points:[K.world(frame,e.start),K.world(frame,e.end)]}));assert.ok(face.points.every((p,i)=>W.sharedIntervals(p,face.points[(i+1)%face.points.length],surviving).reduce((s,[lo,hi])=>s+hi-lo,0)>1-1e-5),'every fill boundary is backed by surviving wire');if(replacement){const anchors=[...door.points.filter(p=>!p.curveSample),...left.controls,...right.controls];const unexpected=W.surfaceWire(state.wallEdits).nodes.filter(n=>!n.curveSample&&!anchors.some(p=>Math.hypot(n.x-p.x,n.y-p.y,n.z-p.z)<1e-5));assert.equal(unexpected.length,0,'face rebuild must not expose sampled curve vertices: '+JSON.stringify(unexpected));}
});

test('first Resoffit selection clears on an empty click while modifier misses preserve it',()=>{
 const pair=[{x:1,y:0,z:3},{x:3,y:0,z:3}],f=fixture({state:{wallEdits:{},roof:{faces:[]}},selected:null,pickLineVisible:()=>false,pickSoffitVisible:()=>true,globals:{WallResoffit:{candidates:()=>[{id:'contact',pair}]}}});f.editor.setResoffitMode(true);
 for(let i=0;i<2;i++){const hit=f.e(2,3);assert.equal(f.editor.pickLine3D(hit),true);f.listeners.pointerup(hit);assert.equal(f.editor.selectionSnapshot().lineSelection.length,1);assert.equal(f.editor.pickLine3D(f.e(20,20,true)),false);assert.equal(f.editor.selectionSnapshot().lineSelection.length,1);assert.equal(f.editor.pickLine3D(f.e(20,20)),false);assert.equal(f.editor.selectionSnapshot().lineSelection.length,0);}
 assert.equal(f.history.length,0,'selection changes do not edit geometry');
});

 test('point-only marquee updates highlights without requesting a scene rebuild',()=>{
 let redraws=0,updates=0,picks=0;const p=(x,z)=>({x,y:0,z}),f=fixture({walls:[],selected:null,state:{wallEdits:{$surfaces:[{id:'face',points:[p(0,0),p(4,0),p(4,4),p(0,4)]}]}},redraw:()=>redraws++,redrawSelection:points=>{updates++;return true;},withSelectionPicking:run=>{picks++;return run();}});
 for(const subtract of [false,true]){f.editor.startBox({...f.e(-1,-1),ctrlKey:subtract});f.listeners.pointermove(f.e(5,5));redraws=0;f.listeners.pointerup(f.e(5,5));assert.equal(redraws,0);assert.equal(f.editor.pointSelection().length,subtract?0:4);}
 assert.equal(updates,2);assert.equal(picks,2);
 });

test('rectangle preparation reuses one wall snapshot instead of regenerating walls per draft',()=>{
 let reads=0;const walls=Array.from({length:30},(_,i)=>({id:'w'+i,bottom:[{x:i*6,y:0,z:0},{x:i*6+4,y:0,z:0}],top:[{x:i*6,y:0,z:4},{x:i*6+4,y:0,z:4}]}));const f=fixture({selected:null,getWalls:()=>{reads++;return walls;},redrawSelection:()=>true});
 f.editor.startBox(f.e(-1,-1));f.listeners.pointermove(f.e(200,5));reads=0;f.listeners.pointerup(f.e(200,5));assert.ok(reads<10,'wall snapshot reads: '+reads);assert.equal(f.editor.pointSelection().length,120);
});

test('plane rectangle selection uses the point-only highlight update',()=>{

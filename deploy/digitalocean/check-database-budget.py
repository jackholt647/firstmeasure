...
def budget(max_connections, reserved, old_nodes, new_nodes, processes, pool_max, other_connections, headroom):
    values = (max_connections, reserved, old_nodes, new_nodes, processes, pool_max, other_connections, headroom)
    if any(type(value) is not int or value < 0 for value in values) or not processes or not pool_max:
        raise ValueError('Invalid capacity inputs')
    usable = max_connections - reserved
    demand = (old_nodes + new_nodes) * processes * pool_max + other_connections
